...
-- Tests are launched cheapest-first so the quick tests clear early and the
-- expensive long-tail tests run last (this shortens the makespan when several
-- tests run concurrently via --jobs N, and gives fast feedback in serial mode).
...
-- (treated as cost 0). Ties break on test.name for a deterministic order.
local function sortByCost(tests)
   table.sort(tests, function(a, b)
      local ca = testCost[a.name] or 0
      local cb = testCost[b.name] or 0
...
local function list_action(args, name)
   ...
   -- List in the same single increasing-cost order in which 'run' launches
   -- tests: Lua and C merged into one cost-sorted list.
   local allTests = {}
...
local function run_action(args, name)
   ...
   -- Launch tests in order of increasing cost (cheapest first). Costs are read
   -- from Tool/test_costs.lua; tests with no recorded cost run first. Lua and C
   -- tests are merged into ONE cost-sorted list (built below in each path) so a
   -- cheap C test never runs after an expensive Lua test: similar-cost tests
   -- land in the same batch, maximising parallelism. These per-type sorts are
   -- kept so any code reading luaTests/cTests directly still sees cost order.
   sortByCost(luaTests)
   sortByCost(cTests)
   ...
   if jobCount <= 1 then
      ...
      -- Build one global cost-sorted list so cheap and expensive tests are
      -- interleaved by cost regardless of Lua/C type, then run each in order.
      local allTests = {}
      if not args.c_only   then for _, t in ipairs(luaTests) do allTests[#allTests+1] = t end end
      if not args.lua_only then for _, t in ipairs(cTests)   do allTests[#allTests+1] = t end end
      sortByCost(allTests)
      ...
      -- ensures every batch groups tests of similar cost (cheap tests in early
      -- batches, the expensive long tail in later batches) regardless of whether
      -- a test is Lua or C, maximising parallel throughput.
      local runPreps = {}
      ...
      -- Sort by recorded cost (cheapest first); ties break on name for a
      -- deterministic order. prep.test carries .name and .testType.
      table.sort(runPreps, function(a, b)
         local ca = testCost[a.test.name] or 0
         local cb = testCost[b.test.name] or 0
         if ca ~= cb then return ca < cb end

...
-- ---- Cost-based test ordering ----------------------------------------------
-- Tests are launched most-expensive-first so the heaviest simulations start
-- immediately and the first batch is always the most expensive (this shortens
-- the makespan when several tests run concurrently via --jobs N, since the long
-- pole is scheduled up front instead of trailing at the end of the run).
-- Costs come from Tool/test_costs.lua, a table generated by parsing a previous
...
-- Sort a list of test descriptors in place by DECREASING recorded cost, so the
-- most expensive tests launch first and the first batch is always the heaviest.
-- Tests with no recorded cost (e.g. newly added tests) sort to the front
-- (treated as +infinity) so they still run in the first batch and get a chance.
-- Ties break on test.name for a deterministic order.
local function sortByCost(tests)
   table.sort(tests, function(a, b)
      local ca = testCost[a.name] or math.huge
      local cb = testCost[b.name] or math.huge
...
-- most-expensive-first, so the heaviest simulations start immediately and the
-- first batch is always the most expensive.
--
...
local function prepareCRun(test, timeoutSecs, mode, skipCompile)
   ...
   if not skipCompile then
      ...
      if not compileOk then
         log(string.format("... COMPILE FAILED in %.3f sec\n", compileSecs))
...
-- ---- Parallel compilation (for the --jobs N path) --------------------------
-- prepareCRun compiles inline, which forces the parallel path to compile every
-- C test serially before running.  These two helpers split that compile out so
-- it can be driven through executeBatch (the same shell-background-job machinery
-- used to run tests concurrently):
--
--   prepareCCompileItem(test)  → batch item {test, runDir, testname, cmd}
--       Does the cheap, conflict-prone serial setup (mkdir, copy source +
--       Makefile + rt_arg_parse.h, remove any stale binary) and returns a
--       'make <testname>' command wrapped for executeBatch.  Returns a
--       {compileFailed=true,...} item if share/Makefile is missing.
--   finalizeCCompile(item, timeoutSecs) → prep table (same shape as prepareCRun)
--       Run AFTER executeBatch has executed item.cmd.  Detects compile success
--       by the presence of the output binary, creates the layer-source symlink,
--       and builds the run command.

local function prepareCCompileItem(test)
   local testname = stripext(basename(test.src))
   local runDir   = configVals.results_dir .. "/" .. test.layer
      .. "/creg-runs/" .. testname
   ...
   mkdir(runDir)
   os.execute(string.format("rm -f '%s'/*.gkyl 2>/dev/null", runDir))
   -- Remove any stale binary so its post-compile presence reliably indicates
   -- that *this* compile succeeded.
   os.execute(string.format("rm -f '%s/%s' 2>/dev/null", runDir, testname))

   local shareMakefile = configVals.prefix .. "/gkeyll/share/Makefile"
   if not lfs.attributes(shareMakefile) then
      return {
         test = test, runDir = runDir, testname = testname,
         compileFailed = true,
   ...
   os.execute(string.format("cp -f '%s' '%s/'", test.src, runDir))
   os.execute(string.format("cp -f '%s' '%s/'", shareMakefile, runDir))
   -- Local rt_arg_parse.h takes precedence over the installed header.
   local argParseH = dirname(test.src) .. "/rt_arg_parse.h"
   if lfs.attributes(argParseH) then
      os.execute(string.format("cp -f '%s' '%s/'", argParseH, runDir))
   end

   local innerCmd = string.format("cd '%s' && make '%s' 2>&1", runDir, testname)
   -- timeout 0 => wrapWithTimeout just appends the '; echo __EXIT__:$?' marker
   -- that executeBatch expects.
   local cmd = wrapWithTimeout(innerCmd, 0, runDir)
   return { test = test, runDir = runDir, testname = testname, cmd = cmd }
end

local function finalizeCCompile(item, timeoutSecs)
   if item.compileFailed then return item end

   local test    = item.test
   local runDir  = item.runDir
   local binPath = runDir .. "/" .. item.testname

   if not lfs.attributes(binPath) then
      return {
         compileFailed = true,
         runDir        = runDir,
         test          = test,
         compileLog    = item.compileLog or "",
         compileSecs   = item.compileSecs or 0,
      }
   end
...
local function clean_action(args, name)
   ...
   if f then
      ...
      if ok and type(cfg) == "table" and cfg.results_dir then
         resultsDir = cfg.results_dir
      end
   end
   resultsDir = resultsDir or confFileResultsDir

   -- Collect the things we are about to delete so we can warn / confirm.
   local targets = {}
   if resultsDir and lfs.attributes(resultsDir) then
      targets[#targets+1] = { kind = "tree", path = resultsDir }
   end
   if lfs.attributes(confFile) then
      targets[#targets+1] = { kind = "file", path = confFile }
   end
   local legacyConf = os.getenv("HOME") .. "/runregression.config.lua"
   if legacyConf ~= confFile and lfs.attributes(legacyConf) then
      targets[#targets+1] = { kind = "file", path = legacyConf }
   end
   ...
   log("The following will be permanently removed:\n")
   for _, t in ipairs(targets) do
      log(string.format("  %s%s\n", t.path, t.kind == "tree" and "/ (entire tree)" or ""))
   end
   ...
   if not args.yes then
      ...
      if not answer or not answer:match("^%s*[yY]") then
   ...
   for _, t in ipairs(targets) do
      if t.kind == "tree" then
         os.execute(string.format("rm -rf '%s'", t.path))
      else
         os.remove(t.path)
      end
...
local function list_action(args, name)
   ...
   for _, t in ipairs(luaTests) do t.testType = "lua" end
   for _, t in ipairs(cTests)   do t.testType = "c"   end
   -- List in the same single decreasing-cost order in which 'run' launches
   -- tests: Lua and C merged into one cost-sorted list (most expensive first).
   local allTests = {}
   if not args.c_only   then for _, t in ipairs(luaTests) do allTests[#allTests+1] = t end end
   if not args.lua_only then for _, t in ipairs(cTests)   do allTests[#allTests+1] = t end end
   sortByCost(allTests)
   for _, t in ipairs(allTests) do
      local tag = (t.testType == "c") and "[c]  " or "[lua]"
      print(string.format("%s %-55s (cost %.3fs)", tag, t.name, testCost[t.name] or 0))
   end
end
...
local function run_action(args, name)
   ...
   -- correct runner/collector for each entry.
   for _, t in ipairs(luaTests) do t.testType = "lua" end
   for _, t in ipairs(cTests)   do t.testType = "c"   end

   -- Launch tests in order of decreasing cost (most expensive first). Costs are
   -- read from Tool/test_costs.lua; tests with no recorded cost run first. Lua
   -- and C tests are merged into ONE cost-sorted list (built below in each path)
   -- so an expensive C test never runs after a cheap Lua test: similar-cost
   -- tests land in the same batch, maximising parallelism. These per-type sorts
   -- are kept so any code reading luaTests/cTests directly still sees cost order.
   sortByCost(luaTests)
   sortByCost(cTests)
   ...
   -- When a timeout is given, skip tests whose recorded cost (from a previous
   -- run's log, see Tool/test_costs.lua) already exceeds the timeout: they
   -- would only time out again and waste wall-clock time. Tests with no
   -- recorded cost are kept (treated as cost 0) so newly added tests always
   -- get a chance to run.
   if timeoutSecs > 0 then
      local function dropTooSlow(tests)
         local kept = {}
         for _, t in ipairs(tests) do
            local cost = testCost[t.name]
            -- A previously timed-out test took at least its recorded cost (the
            -- old limit) and probably longer, so drop it when that limit is
            -- >= the new timeout; an ordinary test is dropped only when it ran
            -- strictly longer than the timeout.
            local tooSlow
            if cost == nil then
               tooSlow = false
            elseif testTimedOut[t.name] then
               tooSlow = cost >= timeoutSecs
            else
               tooSlow = cost > timeoutSecs
            end
            if tooSlow then
               log(string.format(
                  "**** SKIPPING %s: recorded cost %.3f sec exceeds timeout %d sec\n",
                  t.name, cost, timeoutSecs))
            else
               table.insert(kept, t)
            end
         end
      ...
      luaTests = dropTooSlow(luaTests)
      cTests   = dropTooSlow(cTests)
   end
   ...
   local function runGpuVariant(test, runDir, testType, runFn, runArgs)
      ...
      if gpuTimedOut then
         gpuStatus = -3
         log(string.format("... GPU variant TIMED OUT after %.3f sec\n", gpuTm))
   ...
   if jobCount <= 1 then
      -- ================================================================
      -- SERIAL PATH (default): one test at a time, in global cost order.
      -- The per-test Lua and C bodies are factored into closures so a single
      -- cost-sorted loop can dispatch each test by type; cheap and expensive
      -- tests are interleaved by cost regardless of Lua/C type.
      -- ================================================================

      -- ---- Lua test body ----------------------------------------------------
      local function serialRunLua(test)
      ...
      -- ---- C test body ------------------------------------------------------
      local function serialRunC(test)
            layerCounts[test.layer].total = layerCounts[test.layer].total + 1
      ...
      -- Build one global cost-sorted list (most expensive first) so cheap and
      -- expensive tests are ordered by cost regardless of Lua/C type, then run
      -- each in order.
      local allTests = {}
      if not args.c_only   then for _, t in ipairs(luaTests) do allTests[#allTests+1] = t end end
      if not args.lua_only then for _, t in ipairs(cTests)   do allTests[#allTests+1] = t end end
      ...
      for _, test in ipairs(allTests) do
         if test.testType == "c" then serialRunC(test) else serialRunLua(test) end
      end
      ...
      --
      -- Phase 1 (C only): compile all C tests in parallel (batches of jobCount)
      --   via executeBatch.  Each test compiles in its own creg-runs/ scratch
      --   dir, so concurrent 'make' invocations never share intermediate files.
      --   Compile failures are recorded immediately and excluded from the run
      --   batch.
      ...
      -- Phase 1 (C only): compile all C tests IN PARALLEL, jobCount at a time,
      -- via executeBatch (the same machinery used to run tests concurrently).
      -- The cheap, Makefile-conflict-prone setup (mkdir + copy source/Makefile)
      -- is done serially first; only the 'make' invocations run concurrently.
      -- Compile failures are recorded and excluded from the run batch (2b).
      local cPreps = {}
      if not args.lua_only then
         -- 1a: serial per-test setup -> compile batch items.
         local compileItems = {}
         for _, test in ipairs(cTests) do
            layerCounts[test.layer].total = layerCounts[test.layer].total + 1
            table.insert(compileItems, prepareCCompileItem(test))
         end

         -- 1b: run 'make' concurrently, jobCount at a time.
         for bStart = 1, #compileItems, jobCount do
            local batch = {}
            for i = bStart, math.min(bStart + jobCount - 1, #compileItems) do
               if not compileItems[i].compileFailed then
                  table.insert(batch, compileItems[i])
               end
            end
            if #batch > 0 then
               log(string.format(
               ...
               for i, item in ipairs(batch) do
                  item.compileLog  = bResults[i].runlog
                  item.compileSecs = bResults[i].runtm
               end
            end
         end

         -- 1c: finalize preps (detect compile success, build run command).
         for _, item in ipairs(compileItems) do
            local prep = finalizeCCompile(item, timeoutSecs)
            prep.doGpu = shouldDoGpu(item.test, "c")
            table.insert(cPreps, prep)
            if prep.compileFailed then
               log(string.format("\n[C] %s COMPILE FAILED\n", item.test.name))
               insertRegressionData(
                  item.test.layer, runID, item.test.name, "c", -4,
                  prep.compileSecs, "COMPILE FAILED:\n" .. prep.compileLog)
               layerCounts[item.test.layer].failed =
                  layerCounts[item.test.layer].failed + 1
            end
         end
      end
      ...
      -- Phase 2 (run): merge the ready-to-run Lua preps and the compiled-OK C
      -- preps into ONE list, sort it by cost, and launch in cost order. This
      -- ensures every batch groups tests of similar cost (the most expensive
      -- tests in the first batch, the cheap tail in later batches) regardless of
      -- whether a test is Lua or C, so the heaviest work starts immediately.
      local runPreps = {}
      for _, prep in ipairs(luaPreps) do
         runPreps[#runPreps + 1] = prep
      end
      ...
      -- Sort by recorded cost (most expensive first); unknown-cost tests sort
      -- to the front (treated as +infinity) so new tests run in the first batch.
      -- Ties break on name for a deterministic order. prep.test carries .name
      -- and .testType.
      table.sort(runPreps, function(a, b)
         local ca = testCost[a.test.name] or math.huge
         local cb = testCost[b.test.name] or math.huge
         if ca ~= cb then return ca > cb end
         return a.test.name < b.test.name
      end)

      for bStart = 1, #runPreps, jobCount do
         local batch = {}
         for i = bStart, math.min(bStart + jobCount - 1, #runPreps) do
            table.insert(batch, runPreps[i])
         end
         log(string.format("\n[Batch] Launching %d test(s) ...\n", #batch))
         local bResults = executeBatch(batch)
         for i, prep in ipairs(batch) do
            if prep.test.testType == "c" then
...
-- test_costs.lua exactly.  Costs are stored as floats with sub-second
-- (millisecond) resolution to match the timings printed in the run log.

  ...
  for (int d = 0; d < 2; d++) {
    ...
    if ((d == 0 && wall_x) || (d == 1 && wall_y)) {
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        bdata->lower_bc_elc[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler_elc, bdata->geom, d, GKYL_LOWER_EDGE, nghost,
          five_moment_wall_bc, 0
        );
        bdata->lower_bc_ion[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler_ion, bdata->geom, d, GKYL_LOWER_EDGE, nghost,
          five_moment_wall_bc, 0
        );
        bdata->lower_bc_maxwell[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->maxwell, bdata->geom, d, GKYL_LOWER_EDGE, nghost, maxwell_wall_bc, 0
        );
      }

      if (conn->connections[d][1].edge == GKYL_PHYSICAL) {
        bdata->upper_bc_elc[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler_elc, bdata->geom, d, GKYL_UPPER_EDGE, nghost,
          five_moment_wall_bc, 0
        );
        bdata->upper_bc_ion[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->euler_ion, bdata->geom, d, GKYL_UPPER_EDGE, nghost,
          five_moment_wall_bc, 0
        );
        bdata->upper_bc_maxwell[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->maxwell, bdata->geom, d, GKYL_UPPER_EDGE, nghost, maxwell_wall_bc, 0
        );
      }
      ...
      if (conn->connections[d][0].edge == GKYL_PHYSICAL) {
        ...
        bdata->lower_bc_maxwell[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->maxwell, bdata->geom, d, GKYL_LOWER_EDGE, nghost, maxwell_copy_bc, 0
        );
      }
      ...
      if (conn->connections[d][1].edge == GKYL_PHYSICAL) {
        ...
        bdata->upper_bc_maxwell[d] = gkyl_wv_apply_bc_new(
          &bdata->grid, bdata->maxwell, bdata->geom, d, GKYL_UPPER_EDGE, nghost, maxwell_copy_bc, 0
        );
      }
    }
  }
...
void five_moment_update_block_job_func(void *ctx)
{
  ...
  ub_ctx->stat_maxwell = gkyl_wave_prop_advance(
    bdata->slvr_maxwell[d], t_curr, dt, &bdata->range, NULL, bdata->f_maxwell[d],
    bdata->f_maxwell[d + 1]
  );
...
void five_moment_update_block_job_func_source(void *ctx)
{
  ...
  gkyl_moment_em_coupling_implicit_advance(
    bdata->src_slvr, t_curr, dt, &bdata->range, fluids, app_accel, rhs_source,

...
)
{
  ...
  p_tensor_new[2] =
    2.0 *
      (p_tensor_rhs[2] +
       dt1 *
         (-(Bx * p_tensor_rhs[1]) + Bz * p_tensor_rhs[4] + By * (p_tensor_rhs[0] - p_tensor_rhs[5])
         ) *
         q_over_m +
       dt1_sq *
...
)
{
  ...
  if (nfluids == 2) {
    ...
    implicit_frictional_source_update_half(
      mom_em, Z, T_elc, Lambda_ee, t_curr, dt, f_elc_old, f_ion_old, f_elc_stage1, f_ion_stage1,
      app_accel_s, em_old, em_stage1, app_current, ext_em
    );
    implicit_frictional_source_update_half(
      mom_em, Z, T_elc, Lambda_ee, t_curr + (0.5 * dt), dt, f_elc_stage1, f_ion_stage1, f_elc_new,
      f_ion_new, app_accel_s, em_stage1, em_new, app_current, ext_em
    );

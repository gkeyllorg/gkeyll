...
static void mag_var_setup(
  ...
  double kappa_perp[][2], double kappa_cross[][2], double current_par[][3],
  double current_cross[][3]
)
{
  ...
  for (int j = start; j <= end; ++j) {
    ...
    tau[j][ION] = calc_tau(
      1.0, sqrt(2.0) * bes->coll_fac, bes->epsilon0, q[ION], q[ION], m[ION], m[ION], rho[ION],
      T[j][ION]
    );
    ...
    double thermal_perp = electron_heatFlux * 1.5 * p[ELC] /
                          (omega_c[ELC] * tau[j][ELC]
                          ); // Perpendicular thermal force coefficient (same for each species)
...
)
{
  ...
  if (ndim == 1) {
    ...
    mag_var_setup(
      bes, L_1D, U_1D, fluid_d, em_tot_d, u, b, T, tau, eta_par, eta_perp, eta_cross, kappa_par,
      kappa_perp, kappa_cross, current_par, current_cross
    );
    ...
    // Compute derived quantities
    mag_var_setup(
      bes, LL_2D, UU_2D, fluid_d, em_tot_d, u, b, T, tau, eta_par, eta_perp, eta_cross, kappa_par,
      kappa_perp, kappa_cross, current_par, current_cross
    );
    ...
    for (int n = 0; n < nfluids; ++n) {
      ...
      if (bes->param[n].type_eqn == GKYL_EQN_EULER) {
        ...
        kappa_cross_avg[n] = calc_harmonic_avg_2D(
          kappa_cross[LL_2D][n], kappa_cross[LU_2D][n], kappa_cross[UL_2D][n], kappa_cross[UU_2D][n]
        );
    ...
    if (bes->param[ELC].type_eqn == GKYL_EQN_EULER) {
      for (int k = 0; k < 3; ++k) {
        // Parallel current multiplied by parallel thermal force coefficient at cell edges (using arithmetic average)
        current_par_avg[k] = calc_arithm_avg_2D(
          current_par[LL_2D][k], current_par[LU_2D][k], current_par[UL_2D][k], current_par[UU_2D][k]
        );
        // Cross current multiplied by perpendicular thermal force coefficient at cell edges (using arithmetic average)
        current_cross_avg[k] = calc_arithm_avg_2D(
          current_cross[LL_2D][k], current_cross[LU_2D][k], current_cross[UL_2D][k],
          current_cross[UU_2D][k]
        );
      }
    }
  }

...
)
{
  ...
  for (int d = 0; d < mp->num_up_dirs; ++d) {
    ...
    while (gkyl_range_iter_next(&iter)) {
      ...
      gkyl_wv_eqn_waves(
        mp->equation, GKYL_WV_HIGH_ORDER_FLUX, delta, qlocal_l, qlocal_r, phil[0], phir[0], waves,
        speeds
      );
      gkyl_wv_eqn_qfluct(
        mp->equation, GKYL_WV_HIGH_ORDER_FLUX, qlocal_l, qlocal_r, phil[0], phir[0], waves, speeds,

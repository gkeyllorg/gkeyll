...
void test_coldfluid_rotate_roundtrip()
{
  ...
  for (int i = 0; i < 4; ++i) {
    TEST_CHECK(gkyl_compare(qback[i], q[i], 1e-14));
  }
...
void test_coldfluid_riem_roundtrip()
{
  ...
  double qstate[4];
  calcq(1.0, 0.0, 0.0, 0.0, qstate);
  double qin[4];
  calcq(2.0, 1.0, -1.0, 0.5, qin);
  double w[4], qout[4];
...
void test_coldfluid_flux_jump()
{
  ...
  double ql[4];
  calcq(rl, ul, vl, wl, ql);
  double qr[4];
  calcq(rr, ur, vr, wr, qr);
...
void test_coldfluid_waves_conservation()
{
  ...
  double ql[4];
  calcq(1.0, 2.0, 0.0, 0.0, ql);
  double qr[4];
  calcq(3.0, 2.0, 1.0, -1.0, qr);

...
)
{
  ...
  if (mom_fld->app_current) {
    ...
    fld->app_current_proj = gkyl_fv_proj_new(
      &app->grid, 2, GKYL_MOM_APP_NUM_APPLIED_CURRENT, mom_fld->app_current,
      mom_fld->app_current_ctx
    );
  }
...
)
{
  ...
  gkyl_mp_scheme_advance(
    fld->mp_slvr, &app->local, fin, app->ql, app->qr, app->amdq, app->apdq, fld->cflrate,
    fld->embed_mask, rhs
  );

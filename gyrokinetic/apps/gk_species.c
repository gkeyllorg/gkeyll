...
void gk_species_gyroaverage_disabled(
...
void gk_species_gyroaverage_enabled(
  gkyl_gyrokinetic_app *app, struct gk_species *species, struct gkyl_array *field_in,
  struct gkyl_array *field_gyroavg
)
{
...
static double gk_species_rhs_dynamic(
...
static double gk_species_rhs_implicit_dynamic(
  gkyl_gyrokinetic_app *app, struct gk_species *species, const struct gkyl_array *fin,
...
static double gk_species_rhs_static(
  gkyl_gyrokinetic_app *app, struct gk_species *species, const struct gkyl_array *fin,
  struct gkyl_array *rhs, struct gkyl_array **bflux_moms
)
{
...
static double gk_species_rhs_implicit_static(
  gkyl_gyrokinetic_app *app, struct gk_species *species, const struct gkyl_array *fin,
  struct gkyl_array *rhs, struct gkyl_array **bflux_moms, double dt
)
{
...
)
{
  ...
  gkyl_comm_array_per_sync(
    species->comm, &species->local, &species->local_ext, num_periodic_dir, species->periodic_dirs, f
  );
...
static void gk_species_combine_dynamic(
...
static void gk_species_combine_static(
  struct gkyl_array *out, double c1, const struct gkyl_array *arr1, double c2,
  const struct gkyl_array *arr2, const struct gkyl_range *rng
)
{
...
static void gk_species_copy_range_dynamic(
...
static void gk_species_copy_range_static(
  struct gkyl_array *out, const struct gkyl_array *inp, const struct gkyl_range *range
)
{
  // do nothing
...
static void gk_species_write_cfl_disabled(
...
gk_species_write_mom_dynamic(gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame)
{
  ...
  for (int m = 0; m < gks->info.num_diag_moments; ++m) {
    ...
    int sz = gkyl_calc_strlen(
      fmt, app->name, gks->info.name, gkyl_distribution_moments_strs[gks->info.diag_moments[m]],
      frame
    );
    char fileNm[sz + 1]; // ensures no buffer overflow
    snprintf(
      fileNm, sizeof fileNm, fmt, app->name, gks->info.name,
...
static void gk_species_calc_int_mom_dt_disabled(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double dt, struct gkyl_array *fdot_int_mom
)
{
}
...
gk_species_calc_L2norm_dynamic(gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm)
{
  ...
  gkyl_array_integrate_advance(
    gks->integ_wfsq_op, gks->f, (2.0 * M_PI) / gks->info.mass, app->jacobtot_inv_weak, &gks->local,
    &app->local, gks->L2norm_local
  );
...
static void gk_species_init_dynamic(
  ...
  for (int d = 0; d < cdim; ++d) {
    ...
    if (gks->lower_bc[d].type == GKYL_BC_GK_SPECIES_SHEATH) {
      ...
      if (gks->lower_bc[d].type == GKYL_BC_GK_SPECIES_REFLECT) {
        assert(
          d == cdim - 1
        ); // MF 2025/09/25: Reflecting BCs only allowed in the parallel direction.
      }
...
static void gk_species_init_static(
  struct gkyl_gk *gk_app_inp, struct gkyl_gyrokinetic_app *app, struct gk_species *gks
)
{
  // Allocate distribution function arrays.
...
)
{
  ...
  with_file(fp, inp.file_name, "r")
  {
    ...
    assert(
      strcmp(
        gks->basis.id, gkyl_msgpack_map_elem_get_string(elem_list_len, elem_list, "basis_type")
      ) == 0
    );
  ...
  with_file(fp, inp.jacobtot_inv_file_name, "r")
  {
    ...
    gkyl_dg_mul_conf_phase_op_range(
      &conf_basis_do, &basis_do, fdo_host, jacobtot_inv_do_host, fdo_host, &conf_local_ext_do,
      &local_ext_do
    );
  ...
  if (inp.enforce_positivity) {
    ...
    struct gkyl_positivity_shift_gyrokinetic *pos_shift_op = gkyl_positivity_shift_gyrokinetic_new(
      ...
      &app->local_ext, shift_everywhere, app->use_gpu
    );
...
)
{
  ...
  gks->vel_map = gkyl_velocity_map_new(
    gks->info.mapc2p, gks->grid, gks->grid_vel, gks->local, gks->local_ext, gks->local_vel,
    gks->local_ext_vel, app->use_gpu
  );
  ...
  gks->io_meta_conf = gkyl_msgpack_map_elem_union(
    sizeof(io_meta_conf_union) / sizeof(io_meta_conf_union[0]), io_meta_conf_union_len,
    io_meta_conf_union, &gks->io_meta_conf_len
  );
  ...
  if (gks->info.flr.type) {
    ...
    gks->flr_op = gkyl_deflated_fem_poisson_new(
  ...
  for (int dir = 0; dir < cdim; ++dir) {
    gkyl_skin_ghost_ranges(
      &gks->local_lower_skin[dir], &gks->local_lower_ghost[dir], dir, GKYL_LOWER_EDGE,
      &gks->local_ext, ghost
    );
    gkyl_skin_ghost_ranges(
      &gks->local_upper_skin[dir], &gks->local_upper_ghost[dir], dir, GKYL_UPPER_EDGE,
      &gks->local_ext, ghost
    );
    gkyl_skin_ghost_ranges(
      &gks->global_lower_skin[dir], &gks->global_lower_ghost[dir], dir, GKYL_LOWER_EDGE,
      &gks->global_ext, ghost
    );
    gkyl_skin_ghost_ranges(
      &gks->global_upper_skin[dir], &gks->global_upper_ghost[dir], dir, GKYL_UPPER_EDGE,
      &gks->global_ext, ghost
    );
  }
...
void gk_species_apply_ic_cross(gkyl_gyrokinetic_app *app, struct gk_species *gks_self, double t0)
{
  ...
  if (app->field->init_phi_pol && gks_self->info.scale_with_polarization) {
    ...
    gkyl_dg_mul_conf_phase_op_range(
      &app->basis, &gks_self->basis, gks_self->f, den_mod, gks_self->f, &app->local_ext,
      &gks_self->local_ext
    );

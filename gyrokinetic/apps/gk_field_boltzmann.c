...
)
{
  if (s->info.charge > 0.0) {
    ...
    gkyl_dg_div_op_range(
      s->m0.mem_geo, &app->basis, 0, field->rho_c, 0, s->m0.marr, 0, app->gk_geom->geo_int.jacobgeo,
      &app->local
    );
    ...
    gk_species_bflux_get_flux_mom(
      &s->bflux, app->cdim - 1, GKYL_LOWER_EDGE, GKYL_F_MOMENT_M0, bflux, s->m0.marr,
      &app->local_lower_ghost[app->cdim - 1]
    );
    gk_species_bflux_get_flux_mom(
      &s->bflux, app->cdim - 1, GKYL_UPPER_EDGE, GKYL_F_MOMENT_M0, bflux, s->m0.marr,
...
void gk_field_fem_new_boltzmann(struct gkyl_gyrokinetic_app *app, struct gk_field *f)
{
  ...
  f->ambi_pot = gkyl_ambi_bolt_potential_new(
    &app->grid, &app->basis, f->info.electron_mass, f->info.electron_charge, f->info.electron_temp,
    app->use_gpu
  );

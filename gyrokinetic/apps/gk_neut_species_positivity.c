...
)
{
  ...
  if (pos->type) {
    ...
    if (gkns->basis.b_type == GKYL_BASIS_MODAL_HYBRID) {
      gkyl_exit(
        "gk_neut_species_positivity: positivity shift is not supported for the tensor p=1 hybrid neutral basis."
      );
    }
...
void
gk_neut_species_positivity_write_diags(
  gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns, struct gk_positivity *pos, double tm,
  int frame
)
{
...
void
gk_neut_species_positivity_reset(

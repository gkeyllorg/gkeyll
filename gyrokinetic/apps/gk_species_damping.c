...
)
{
  ...
  int io_meta_fbar_len[] = {
    gks->io_meta_basic_len, (int)(sizeof(mpe_fbar_p0) / sizeof(mpe_fbar_p0[0])),
    app->gk_geom->io_meta_basic_len
  };
  const struct gkyl_msgpack_map_elem *io_meta_fbar[] = {
    gks->io_meta_basic, mpe_fbar_p0, app->gk_geom->io_meta_basic
  };
  ...
  gkyl_comm_array_write(
    gks->comm, &gks->grid, &gks->local, mt_fbar, gks->damping.fbar_host, fileNm_fbar
  );
  app->stat.n_io += 1;
...
)
{
  ...
  int io_meta_fbar_len[] = {gks->io_meta_phase_len, app->gk_geom->io_meta_basic_len};
  const struct gkyl_msgpack_map_elem *io_meta_fbar[] = {
    gks->io_meta_phase, app->gk_geom->io_meta_basic
  };
  struct gkyl_msgpack_data *mt_fbar = gkyl_msgpack_create_union(
    sizeof(io_meta_fbar_len) / sizeof(int), io_meta_fbar_len, io_meta_fbar
  );
...
)
{
  ...
  if (gks->info.damping.rate_profile) {
    gk_species_damping_project_phase_rate(
      app, gks, num_quad, gks->info.damping.rate_profile, gks->info.damping.rate_profile_ctx,
      damp->rate_host, damp->rate
    );

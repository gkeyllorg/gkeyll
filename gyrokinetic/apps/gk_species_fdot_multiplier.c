...
)
{
  ...
  gkyl_loss_cone_mask_gyrokinetic_advance(
    fdmul->lcm_proj_op, &gks->local, &app->global, fdmul->bmag_global, fdmul->phi_global,
    fdmul->phi_wall_lo_global, fdmul->phi_wall_up_global, fdmul->buffer
  );
...
)
{
  ...
  gkyl_comm_allreduce(
    app->comm, GKYL_DOUBLE, GKYL_MAX, app->basis.num_basis, fdmul->local_max_f, fdmul->global_max_f
  );
...
)
{
  ...
  if (fdmul->type) {
    ...
    if (fdmul->type == GKYL_GK_FDOT_MULTIPLIER_USER_INPUT) {
      ...
      fdmul->buffer = mkarr(app->use_gpu, basis_mult.num_basis, gks->local_ext.volume);
      fdmul->bmag_global =
        mkarr(app->use_gpu, app->gk_geom->geo_corn.bmag->ncomp, app->global_ext.volume);
      ...
      if (inp_proj.lower_orbit == GKYL_GK_TRAP_PASS_ORBIT_TRAPPED_SHEATH) {
        fdmul->phi_wall_lo_global =
          mkarr(app->use_gpu, app->basis.num_basis, app->global_ext.volume);
      }
      if (inp_proj.upper_orbit == GKYL_GK_TRAP_PASS_ORBIT_TRAPPED_SHEATH) {
        fdmul->phi_wall_up_global =
          mkarr(app->use_gpu, app->basis.num_basis, app->global_ext.volume);
      }

      gkyl_comm_array_allgather(
        app->comm, &app->local, &app->global, app->gk_geom->geo_corn.bmag, fdmul->bmag_global
      );

...
typedef void (*write_fdot_func_t)(
  struct gkyl_gyrokinetic_multib_app *app, struct gkyl_array *fout[]
);

static void
write_fdot_enabled(struct gkyl_gyrokinetic_multib_app *app, struct gkyl_array *fout[])
{
...
static void
write_fdot_disabled(struct gkyl_gyrokinetic_multib_app *app, struct gkyl_array *fout[])
{
  // Do nothing.
}
      ...
      case RK_STAGE_3:
        ...
        if (st.dt_actual < dt) {
          ...
          case RK_STAGE_2:
            ...
            gyrokinetic_multib_forward_euler(
              app, tcurr + dt, dt, fin, fout, bflux_in, bflux_out, fin_neut, fout_neut,
              ...
              for (int b = 0; b < nblocks_local; ++b) {
                ...
                for (int i = 0; i < ns_charged; ++i) {
                  ...
                  gk_species_bflux_set(
                    sbapp, &gks->bflux, gks->bflux.f1, 1.0 / 4.0, gks->bflux.fnew
                  );
                }
                for (int i = 0; i < ns_neut; ++i) {
                  struct gk_neut_species *gkns = &sbapp->neut_species[i];
          ...
          case RK_STAGE_3:
            ...
            if (st.dt_actual < dt) {
              ...
              for (int b = 0; b < nblocks_local; ++b) {
                ...
                for (int i = 0; i < ns_neut; ++i) {
                  struct gk_neut_species *gkns = &sbapp->neut_species[i];
                  gk_neut_species_positivity_apply(
                    sbapp, gkns, &gkns->positivity, gkns->fnew, gkns->f
                  );
                }
              }

  ...
  for (int i = 0; i < rad->num_cross_collisions; ++i) {
    ...
    if (rad->is_neut_species[i]) {
      gk_neut_species_moment_calc(
        &rad->moms[i], rad->collide_with_neut[i]->local, app->local,
    ...
    gkyl_dg_div_op_range(
      rad->moms[i].mem_geo, &app->basis, 0, rad->moms[i].marr, 0, rad->moms[i].marr, 0,
      app->gk_geom->geo_int.jacobgeo, &app->local
    );

    gkyl_dg_calc_gk_rad_vars_nI_nu_advance(
      rad->calc_gk_rad_vars, &app->local, &species->local, &rad->vnu_surf[i], &rad->vnu[i],
      &rad->vsqnu_surf[i], &rad->vsqnu[i], rad->rad_fit_ne[i], rad->m0, rad->moms[i].marr,
      rad->nvnu_surf, rad->nvnu, rad->nvsqnu_surf, rad->nvsqnu, rad->vtsq_min_per_species[i],
      rad->vtsq
    );
  }
...
gks_rad_write_drag_enabled(gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame)
{
  ...
  snprintf(
    fileNm_nvsqnu_surf, sizeof fileNm_nvsqnu_surf, fmt_nvsqnu_surf, app->name, gks->info.name, frame
  );
...
)
{
  ...
  for (int i = 0; i < rad->num_cross_collisions; ++i) {
    ...
    gkyl_dg_mul_op(
      &app->basis, 0, rad->emissivity_denominator, 0, rad->emissivity_denominator, 0,
      app->gk_geom->geo_int.jacobgeo
    );
    gkyl_dg_div_op_range(
      rad->m2.mem_geo, &app->basis, 0, rad->emissivity[i], 0, rad->m2.marr, 0,
...
)
{
  ...
  for (int i = 0; i < gks->rad.num_cross_collisions; i++) {
    ...
    if (gks->rad.is_neut_species[i]) {
      int sz_emissivity = gkyl_calc_strlen(
        fmt_emissivity, app->name, gks->info.name,
      ...
      snprintf(
        fileNm_emissivity, sizeof fileNm_emissivity, fmt_emissivity, app->name, gks->info.name,
        app->neut_species[gks->rad.collide_with_idx[i]].info.name, frame
      );
...
gks_rad_calc_integrated_mom_enabled(gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm)
{
  ...
  gkyl_comm_allreduce(
    app->comm, GKYL_DOUBLE, GKYL_SUM, num_mom, gks->rad.red_integ_diag,

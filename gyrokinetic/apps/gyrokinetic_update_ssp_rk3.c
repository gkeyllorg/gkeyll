...
static void write_fdot_enabled(gkyl_gyrokinetic_app *app, struct gkyl_array *fout[])
{
...
static void write_fdot_disabled(gkyl_gyrokinetic_app *app, struct gkyl_array *fout[])
{
  // Do nothing.
}

static void gyrokinetic_forward_euler(
  ...
  while (state != RK_COMPLETE) {
    ...
    case RK_STAGE_1:
      ...
      gyrokinetic_forward_euler(
        ...
        bflux_in_neut, bflux_out_neut, write_fdot_enabled, &st
      );
      dt = st.dt_actual;
    ...
    case RK_STAGE_2:
      ...
      gyrokinetic_forward_euler(
        app, tcurr + dt, dt, fin, fout, fbar_in, fbar_out, bflux_in, bflux_out, fin_neut, fout_neut,
      ...
      if (st.dt_actual < dt) {
        ...
        for (int i = 0; i < app->num_species; ++i) {
          ...
          gk_species_damping_combine(
            gks, gks->damping.fbar1, 3.0 / 4.0, gks->damping.fbar, 1.0 / 4.0, gks->damping.fbarnew,
            &gks->local_ext
          );
    ...
    case RK_STAGE_3:
      ...
      gyrokinetic_forward_euler(
        ...
        fout_neut, bflux_in_neut, bflux_out_neut, write_fdot_disabled, &st
      );
      ...
      if (st.dt_actual < dt) {
        ...
        for (int i = 0; i < app->num_species; ++i) {
          ...
          gk_species_damping_combine(
            gks, gks->damping.fbar, 1.0 / 3.0, gks->damping.fbar1, 2.0 / 3.0, gks->damping.fbarnew,
            &gks->local_ext
          );
          gk_species_damping_copy_range(gks, gks->damping.fbar1, gks->damping.fbar, &gks->local_ext);

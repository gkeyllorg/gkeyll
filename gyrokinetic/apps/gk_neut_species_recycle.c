...
)
{
  ...
  for (int i = 0; i < recyc->num_species; ++i) {
    ...
    gkyl_dg_updater_moment_gyrokinetic_advance(
      recyc->m0op_gk[i], &recyc->impact_normal_r[i], &recyc->impact_cbuff_r[i],
      recyc->phase_flux_gk[i], recyc->m0_flux_gk[i]
    );
    ...
    snprintf(
      fileNm, sizeof fileNm, fmt, app->name, gks->info.name, vars[dir], edge[edi], gks->info.name,
      frame
    );
  ...
  gkyl_dg_vlasov_conf_flux_surf_advance(
    s->collisionless.calc_conf_flux, &app->local, &s->local, &s->local_ext, s->conf_poisson_tensor,
    s->hamil, recyc->f_diag, s->cflrate, s->collisionless.conf_flux_surf
  );
  ...
  gkyl_mom_calc_advance(
    recyc->m0op_neut, &recyc->emit_normal_r, &recyc->emit_cbuff_r, recyc->unit_phase_flux_neut,
    recyc->emit_flux
  );
...
)
{
  ...
  gkyl_dg_vlasov_conf_flux_surf_advance(
    s->collisionless.calc_conf_flux, &app->local, &s->local, &s->local_ext, s->conf_poisson_tensor,
    s->hamil, s->f1, s->cflrate, s->collisionless.conf_flux_surf
  );
...
)
{
  ...
  for (int i = 0; i < recyc->num_species; ++i) {
    ...
    gkyl_dg_div_op_range(
      recyc->mem_geo, &app->basis, 0, recyc->m0_flux_gk[i], 0, recyc->m0_flux_gk[i], 0,
      recyc->unit_m0_flux_neut, &recyc->emit_cbuff_r
    );

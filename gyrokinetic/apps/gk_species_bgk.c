...
static void gkbgk_moms_disabled(
...
static void gkbgk_moms_enabled(
  gkyl_gyrokinetic_app *app, const struct gk_species *species, struct gk_bgk_collisions *bgk,
  const struct gkyl_array *fin
)
{
...
static void gkbgk_self_nu_calc_constNu(
  gkyl_gyrokinetic_app *app, const struct gk_species *species, struct gk_bgk_collisions *bgk
)
{
  // Empty method.
...
static void gkbgk_self_nu_calc_normNu(
  gkyl_gyrokinetic_app *app, const struct gk_species *species, struct gk_bgk_collisions *bgk
)
{
  // Calculate nu_ss(x,t).
  gkyl_spitzer_coll_freq_advance_normnu(
    bgk->spitzer_calc, &app->local, species->lte.moms.marr, bgk->vtsq_min, species->lte.moms.marr,
...
static void gkbgk_cross_nu_calc_constNu(
...
static void gkbgk_cross_nu_calc_normNu(
  gkyl_gyrokinetic_app *app, const struct gk_species *s, struct gk_bgk_collisions *bgk, int coll_idx
)
{
  // Calculate nu_sr(x,t).
...
static void gkbgk_alpha_E_constNu(
  gkyl_gyrokinetic_app *app, const struct gk_species *s, struct gk_bgk_collisions *bgk, int coll_idx
)
{
...
static void gkbgk_alpha_E_normNu(
  gkyl_gyrokinetic_app *app, const struct gk_species *s, struct gk_bgk_collisions *bgk, int coll_idx
)
{
...
static void gkbgk_cross_moms_disabled(
...
static void gkbgk_cross_moms_enabled(
  gkyl_gyrokinetic_app *app, const struct gk_species *gks, struct gk_bgk_collisions *bgk,
  int coll_idx
)
{
  ...
  gkyl_gyrokinetic_cross_prim_moms_bgk_advance(
    bgk->cross_calc, &app->local, bgk->delta_sr, bgk->betaGreenep1, gks->info.mass,
    gks->lte.moms.marr, bgk->other_m[coll_idx], bgk->other_prim_moms[coll_idx], bgk->cross_prim_moms
  );

  app->stat.species_coll_mom_tm += gkyl_time_diff_now_sec(wst);
}

static void gkbgk_rhs_disabled(
...
)
{
  ...
  gkyl_dg_mul_conf_phase_op_range(
    &app->basis, &gks->basis, bgk->nu_fmax, app->gk_geom->geo_int.jacobgeo, bgk->nu_fmax,
    &app->local, &gks->local
  );

  // Apply BGK collisions.
  gkyl_bgk_collisions_advance(
    bgk->up_bgk, &app->local, &gks->local, bgk->nu_sum, bgk->nu_fmax, fin, bgk->implicit_step, dt,
    rhs, gks->cflrate
  );

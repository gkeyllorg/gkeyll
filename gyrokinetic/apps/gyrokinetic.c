...
gyrokinetic_deflate_delta_ts(struct gkyl_gyrokinetic_app *app, struct gkyl_array *delta_ts)
{
  ...
  transd_2d_1d = gkyl_translate_dim_new(
    app->cdim - 1, app->gk_geom->surf_basis, 1, app->delta_ts_x_basis, 1, GKYL_NO_EDGE, app->use_gpu
  );
...
gkyl_gyrokinetic_app_new_geom(struct gkyl_gk *gk)
{
  ...
  for (int dir = 0; dir < cdim; ++dir) {
    gkyl_skin_ghost_ranges(
      &app->local_lower_skin[dir], &app->local_lower_ghost[dir], dir, GKYL_LOWER_EDGE,
      &app->local_ext, ghost
    );
    gkyl_skin_ghost_ranges(
      &app->local_upper_skin[dir], &app->local_upper_ghost[dir], dir, GKYL_UPPER_EDGE,
      &app->local_ext, ghost
    );
    gkyl_skin_ghost_ranges(
      &app->global_lower_skin[dir], &app->global_lower_ghost[dir], dir, GKYL_LOWER_EDGE,
      &app->global_ext, ghost
    );
    gkyl_skin_ghost_ranges(
      &app->global_upper_skin[dir], &app->global_upper_ghost[dir], dir, GKYL_UPPER_EDGE,
      &app->global_ext, ghost
    );
  }
  ...
  app->position_map = gkyl_position_map_new(
    gk->geometry.position_map_info, app->grid, app->local, app->local_ext, app->global,
    app->global_ext, app->basis
  );
  ...
  app->io_meta_dg = gkyl_msgpack_map_elem_union(
    sizeof(io_meta_dg_union) / sizeof(io_meta_dg_union[0]), io_meta_dg_union_len, io_meta_dg_union,
    &app->io_meta_dg_len
  );
  ...
  gkyl_dg_mul_op_range(
    &app->basis, 0, tmp, 0, app->gk_geom->geo_int.bmag, 0, app->gk_geom->geo_int.jacobgeo,
    &app->local
  );
  ...
  if (app->cdim == 3 && (app->gk_geom->geometry_id == GKYL_GEOMETRY_TOKAMAK ||
    ...
    if (!gyrokinetic_str_ends_in_bnum(app->name)) {
      ...
      gkyl_comm_array_per_sync(
        app->comm, &app->local, &app->local_ext, shift_num_periodic_dirs, shift_periodic_dirs,
        delta_ts
      );
...
static void
gyrokinetic_calc_field_disabled(
  gkyl_gyrokinetic_app *app, double tcurr, const struct gkyl_array *fin[],
  struct gkyl_array **bflux[]
)
{
...
gkyl_gyrokinetic_app_omegaH_init(gkyl_gyrokinetic_app *app)
{
  ...
  if (!(app->field->gkfield_id == GKYL_GK_FIELD_BOLTZMANN ||
    ...
    gkyl_dg_mul_op_range(
      &app->basis, 0, parfac, 0, app->gk_geom->geo_int.cmag, 0, app->gk_geom->geo_int.jacobtot_inv,
      &app->local
    );
...
gkyl_gyrokinetic_app_new_solver(struct gkyl_gk *gk, gkyl_gyrokinetic_app *app)
{
  ...
  app->calc_field_func = app->field->update_field ? gyrokinetic_calc_field_enabled :
                                                    gyrokinetic_calc_field_disabled;
...
void
gyrokinetic_calc_field_and_apply_bc(
...
gyrokinetic_app_geometry_copy_and_write(
  gkyl_gyrokinetic_app *app, struct gkyl_array *arr, struct gkyl_array *arr_host, char *varNm,
...
)
{
  ...
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_corn.mc2p, arr_ho3, "geo_corn_mapc2p",
    "Coord map: 3x computational to physical."
  );
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_corn.mc2nu_pos, arr_ho3, "geo_corn_mc2nu_pos",
  ...
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_corn.bmag_inv, arr_ho1, "geo_corn_bmag_inv",
  ...
  if (app->cdim < 3) {
    if (geometry_inp->geometry_id == GKYL_GEOMETRY_MIRROR ||
      ...
      gyrokinetic_app_geometry_copy_and_write(
        app, app->gk_geom->geo_corn.mc2p_deflated, arr_hocdim, "geo_corn_mapc2p_deflated",
  ...
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.mc2p, arr_ho3, "geo_int_mapc2p",
  ...
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.g_ij_neut, arr_ho6, "geo_int_g_ij_neut",
    "Covariant metric tensor for neutral species."
  );
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.dxdz, arr_ho9, "geo_int_dxdz",
    "Cartesian components of tangent vectors."
  );
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.dzdx, arr_ho9, "geo_int_dzdx",
    "Cartesian components of dual vectors."
  );
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.normals, arr_ho9, "geo_int_normals",
  ...
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.jacobgeo_inv, arr_ho1, "geo_int_jacobgeo_inv",
  ...
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.gij_neut, arr_ho6, "geo_int_gij_neut",
    "Contravariant metric tensor for neutral species."
  );
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.b_i, arr_ho3, "geo_int_b_i",
    "Covariant components of the magnetic unit vector."
  );
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.bcart, arr_ho3, "geo_int_bcart",
  ...
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.jacobtot, arr_ho1, "geo_int_jacobtot",
    "Conf-space Jacobian times guiding center Jacobian."
  );
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.jacobtot_inv, arr_ho1, "geo_int_jacobtot_inv",
  ...
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.eps2, arr_ho1, "geo_int_eps2",
  ...
  gyrokinetic_app_geometry_copy_and_write(
    app, app->gk_geom->geo_int.B3, arr_ho1, "geo_int_B3",
  ...
  for (int dir = 0; dir < app->cdim; dir++) {
    gyrokinetic_app_geometry_copy_and_write_surf(
      app, app->gk_geom->geo_surf[dir].jacobgeo, arr_surf_ho1, arr_surf_ho2, "jacobgeo", dir,
      "Conf-space Jacobian."
    );
    gyrokinetic_app_geometry_copy_and_write_surf(
      app, app->gk_geom->geo_surf[dir].jacobtot_inv, arr_surf_ho1, arr_surf_ho2, "jacobtot_inv",
      dir, "Reciprocal of conf-space Jacobian."
    );
    gyrokinetic_app_geometry_copy_and_write_surf(
    ...
    gyrokinetic_app_geometry_copy_and_write_surf(
      app, app->gk_geom->geo_surf[dir].bmag, arr_surf_ho1, arr_surf_ho2, "bmag", dir,
    ...
    gyrokinetic_app_geometry_copy_and_write_surf(
      app, app->gk_geom->geo_surf[dir].normals, arr_surf_ho9, arr_surf_ho18, "normals", dir,
      "Unit normal vectors. "
    );
    gyrokinetic_app_geometry_copy_and_write_surf(
    ...
    gyrokinetic_app_geometry_copy_and_write_surf(
      app, app->gk_geom->geo_surf[dir].bimpactangle, arr_surf_ho1, arr_surf_ho2, "bimpactangle",
      dir, "Impact angle of the magnetic field."
    );
    gyrokinetic_app_geometry_copy_and_write_surf(
  ...
  if (rank == 0) {
    ...
    struct gkyl_nodal_ops *n2m = gkyl_nodal_ops_new(&app->basis, &app->grid, false);
    gkyl_nodal_ops_m2n(
      n2m, &app->basis, &app->grid, &nrange_int_global, &app->global, 3, mc2pint_nodal,
      mc2pint_global_ho, true
    );
...
//
static void
write_field_with_metadata(
  struct gkyl_gyrokinetic_app *app, int frame, const char *field_suffix, const char *description,
  struct gkyl_array *field_host
)
{
  ...
  int io_meta_len[] = {app->io_meta_dg_len, app->gk_geom->io_meta_basic_len, 1};
  const struct gkyl_msgpack_map_elem *io_meta[] = {
    app->io_meta_dg, app->gk_geom->io_meta_basic, custom_meta
  };

  struct gkyl_msgpack_data *mt_data =
    gkyl_msgpack_create_union(sizeof(io_meta_len) / sizeof(int), io_meta_len, io_meta);
...
gkyl_gyrokinetic_app_write_field(gkyl_gyrokinetic_app *app, double tm, int frame)
{
  if (app->field->update_field || frame == 0) {
    ...
    if (app->field->is_em) {
      ...
      write_field_with_metadata(
        app, frame, "apar", "Stepped parallel component of the magnetic vector potential.",
        app->field->apar_host
      );
      ...
      write_field_with_metadata(
        app, frame, "amperesol", "Solution of the Ampere's law.", app->field->amperesol_host
      );
    }
...
gyrokinetic_rhs(
  ...
  const struct gkyl_array *aparin, struct gkyl_array *aparout, struct gkyl_update_status *st
)
{
...
gkyl_gyrokinetic_app_stat(gkyl_gyrokinetic_app *app)
{
  ...
  stat->field_sum_tm = stat->field_phi_rhs_tm + stat->field_phi_solve_tm + stat->field_apar_rhs_tm +
                       stat->field_apar_solve_tm;
...
gkyl_gyrokinetic_app_print_timings(gkyl_gyrokinetic_app *app, FILE *iostream)
{
  ...
  gkyl_gyrokinetic_app_cout(
    app, iostream, "    * Forward Euler:                   %.4e sec. / %4.2f %%.\n",
  ...
  gkyl_gyrokinetic_app_cout(
    app, iostream, "      ^ Species LTE:                   %.4e sec. / %4.2f %%.\n",
  ...
  gkyl_gyrokinetic_app_cout(
    app, iostream, "      ^ Damping (charged):             %.4e sec. / %4.2f %%.\n",
    stat->species_damp_tm, ratio_to_percent(stat->species_damp_tm, stat->fwd_euler_tm, 0.0)
  );
  gkyl_gyrokinetic_app_cout(
    app, iostream, "      ^ df/dt multiplier (charged):    %.4e sec. / %4.2f %%.\n",
  ...
  gkyl_gyrokinetic_app_cout(
    app, iostream, "      ^ Radiation terms:               %.4e sec. / %4.2f %%.\n",
  ...
  gkyl_gyrokinetic_app_cout(
    app, iostream, "      ^ Boundary fluxes (charged):     %.4e sec. / %4.2f %%.\n", bflux_tm,
  ...
  gkyl_gyrokinetic_app_cout(
    app, iostream, "      ^ Sources (charged):             %.4e sec. / %4.2f %%.\n",
    stat->species_src_tm, ratio_to_percent(stat->species_src_tm, stat->fwd_euler_tm, 0.0)
  );
  gkyl_gyrokinetic_app_cout(
    app, iostream, "      ^ BGK Sources (charged):             %.4e sec. / %4.2f %%.\n",
  ...
  gkyl_gyrokinetic_app_cout(
    app, iostream, "      ^ Species LTE (neutral):         %.4e sec. / %4.2f %%.\n",
  ...
  gkyl_gyrokinetic_app_cout(
    ...
    ratio_to_percent(
      stat->neut_species_bflux_calc_tm + stat->neut_species_bflux_moms_tm, stat->fwd_euler_tm, 0.0
    )
  );
  gkyl_gyrokinetic_app_cout(
  ...
  gkyl_gyrokinetic_app_cout(
    app, iostream, "    * Boundary conditions::            %.4e sec. / %4.2f %%.\n", stat->bc_tm,
  ...
  gkyl_gyrokinetic_app_cout(
    app, iostream, "      ^ Charged species:               %.4e sec. / %4.2f %%.\n", stat->fdot_tm,
  ...
  gkyl_gyrokinetic_app_cout(
    app, iostream, "    * Positivity:                      %.4e sec. / %4.2f %%.\n",
...
comm_reduce_app_stat(
  const gkyl_gyrokinetic_app *app, const struct gkyl_gyrokinetic_stat *local,
...
)
{
  ...
  gkyl_comm_allreduce_host(
    app->comm, GKYL_INT_64, GKYL_MAX, app->num_species, l_red_n_iter_corr, l_red_global_n_iter_corr
  );
  gkyl_comm_allreduce_host(
  ...
  if (app->num_neut_species > 0) {
    ...
    gkyl_comm_allreduce_host(
      app->comm, GKYL_INT_64, GKYL_MAX, app->num_neut_species, l_red_neut_n_iter_corr,
      l_red_global_neut_n_iter_corr
    );
    gkyl_comm_allreduce_host(
      app->comm, GKYL_INT_64, GKYL_MAX, app->num_neut_species, l_red_neut_num_corr,
      l_red_global_neut_num_corr
    );
...
gkyl_gyrokinetic_app_stat_write(gkyl_gyrokinetic_app *app)
{
  ...
  gkyl_gyrokinetic_app_cout(
    app, fp, " neut_species_coll_mom_pct : %.2f,\n",
    ratio_to_percent(stat.neut_species_coll_mom_tm, stat.fwd_euler_tm, 0.0)
  );
  gkyl_gyrokinetic_app_cout(
  ...
  gkyl_gyrokinetic_app_cout(
    app, fp, " neut_species_bflux_moms_pct : %.2f,\n",
  ...
  gkyl_gyrokinetic_app_cout(
    app, fp, " neut_species_coll_pct : %.2f,\n",
    ratio_to_percent(stat.neut_species_coll_tm, stat.fwd_euler_tm, 0.0)
  );
  gkyl_gyrokinetic_app_cout(app, fp, " neut_species_react_tm : %.4e,\n", stat.neut_species_react_tm);
  gkyl_gyrokinetic_app_cout(
    app, fp, " neut_species_react_pct : %.2f,\n",
    ratio_to_percent(stat.neut_species_react_tm, stat.fwd_euler_tm, 0.0)
  );
  gkyl_gyrokinetic_app_cout(
  ...
  gkyl_gyrokinetic_app_cout(
    app, fp, " neut_species_pos_shift_pct : %.2f,\n",
    ratio_to_percent(stat.neut_species_pos_shift_tm, stat.pos_shift_tm, 0.0)
  );
  gkyl_gyrokinetic_app_cout(
  ...
  gkyl_gyrokinetic_app_cout(
    app, fp, " pos_shift_quasineut_pct : %.2f,\n",
    ratio_to_percent(stat.pos_shift_quasineut_tm, stat.pos_shift_tm, 0.0)
  );
  gkyl_gyrokinetic_app_cout(app, fp, " pos_shift_sum_tm : %.4e,\n", stat.pos_shift_sum_tm);
  gkyl_gyrokinetic_app_cout(
    app, fp, " pos_shift_accounted_pct : %.2f,\n",
    ratio_to_percent(stat.pos_shift_sum_tm, stat.pos_shift_tm, 100.0)
  );
  ...
  gkyl_gyrokinetic_app_cout(
    app, fp, " species_diag_calc_pct : %.2f,\n",
  ...
  gkyl_gyrokinetic_app_cout(
    app, fp, " neut_species_diag_calc_pct : %.2f,\n",
    ratio_to_percent(stat.neut_species_diag_calc_tm, stat.io_tm, 0.0)
  );
  gkyl_gyrokinetic_app_cout(
  ...
  gkyl_gyrokinetic_app_cout(
    app, fp, " neut_species_diag_io_pct : %.2f,\n",
...
gyrokinetic_app_geometry_read_and_copy(
  gkyl_gyrokinetic_app *app, struct gkyl_array *arr, struct gkyl_array *arr_host, char *path,
  char *varNm
)
{
  ...
  if (rstat.io_status == GKYL_ARRAY_RIO_SUCCESS) {
    ...
    gkyl_gyrokinetic_app_cout(
      app, stderr, "*** Failed to read geometry file! (%s)\n",
...
static void
gyrokinetic_app_geometry_read_and_copy_surf(
  gkyl_gyrokinetic_app *app, struct gkyl_array *arr, struct gkyl_array *arr_host,
  struct gkyl_array *arr_host_doubled, char *path, char *varNm, int dir
)
{
  ...
  if (rstat.io_status == GKYL_ARRAY_RIO_SUCCESS) {
    ...
    gkyl_array_copy_range_to_range(
      arr_host_doubled, arr_host_doubled, &app->local_upper_ghost[dir], &app->local_upper_skin[dir]
    );
...
)
{
  ...
  gyrokinetic_app_geometry_read_and_copy(
    app, app->gk_geom->geo_corn.mc2nu_pos, arr_ho3, geometry_inp->geometry_path,
    "geo_corn_mc2nu_pos"
  );
  gyrokinetic_app_geometry_read_and_copy(
  ...
  gyrokinetic_app_geometry_read_and_copy(
    app, app->gk_geom->geo_corn.bmag_inv, arr_ho1, geometry_inp->geometry_path, "geo_corn_bmag_inv"
  );
  ...
  gyrokinetic_app_geometry_read_and_copy(
    app, app->gk_geom->geo_int.jacobgeo_inv, arr_ho1, geometry_inp->geometry_path,
  ...
  gyrokinetic_app_geometry_read_and_copy(
    app, app->gk_geom->geo_int.jacobtot_inv, arr_ho1, geometry_inp->geometry_path,
  ...
  gyrokinetic_app_geometry_read_and_copy(
    app, app->gk_geom->geo_int.dualcurlbhatoverB, arr_ho3, geometry_inp->geometry_path,
    "geo_int_dualcurlbhatoverB"
  );
  gyrokinetic_app_geometry_read_and_copy(
    app, app->gk_geom->geo_int.dualcurlbhat, arr_ho3, geometry_inp->geometry_path,
  ...
  for (int dir = 0; dir < app->cdim; dir++) {
    gyrokinetic_app_geometry_read_and_copy_surf(
      app, app->gk_geom->geo_surf[dir].jacobgeo, arr_surf_ho1, arr_surf_ho2,
      geometry_inp->geometry_path, "jacobgeo", dir
    );
    gyrokinetic_app_geometry_read_and_copy_surf(
      app, app->gk_geom->geo_surf[dir].jacobtot_inv, arr_surf_ho1, arr_surf_ho2,
      geometry_inp->geometry_path, "jacobtot_inv", dir
    );
    gyrokinetic_app_geometry_read_and_copy_surf(
      app, app->gk_geom->geo_surf[dir].b_i, arr_surf_ho3, arr_surf_ho6, geometry_inp->geometry_path,
      "b_i", dir
    );
    gyrokinetic_app_geometry_read_and_copy_surf(
      app, app->gk_geom->geo_surf[dir].cmag, arr_surf_ho1, arr_surf_ho2,
      geometry_inp->geometry_path, "cmag", dir
    );
    gyrokinetic_app_geometry_read_and_copy_surf(
      app, app->gk_geom->geo_surf[dir].bmag, arr_surf_ho1, arr_surf_ho2,
      geometry_inp->geometry_path, "bmag", dir
    );
    gyrokinetic_app_geometry_read_and_copy_surf(
      app, app->gk_geom->geo_surf[dir].normcurlbhat, arr_surf_ho1, arr_surf_ho2,
      geometry_inp->geometry_path, "normcurlbhat", dir
    );
    gyrokinetic_app_geometry_read_and_copy_surf(
      app, app->gk_geom->geo_surf[dir].B3, arr_surf_ho1, arr_surf_ho2, geometry_inp->geometry_path,
      "B3", dir
    );
    gyrokinetic_app_geometry_read_and_copy_surf(
      app, app->gk_geom->geo_surf[dir].normals, arr_surf_ho9, arr_surf_ho18,
      geometry_inp->geometry_path, "normals", dir
    );
    gyrokinetic_app_geometry_read_and_copy_surf(
      app, app->gk_geom->geo_surf[dir].lenr, arr_surf_ho1, arr_surf_ho2,
      geometry_inp->geometry_path, "lenr", dir
    );
    gyrokinetic_app_geometry_read_and_copy_surf(
      app, app->gk_geom->geo_surf[dir].bimpactangle, arr_surf_ho1, arr_surf_ho2,
      geometry_inp->geometry_path, "bimpactangle", dir
    );
    gyrokinetic_app_geometry_read_and_copy_surf(
      app, app->gk_geom->geo_surf[dir].deltats, arr_surf_ho1, arr_surf_ho2,
...
struct gkyl_app_restart_status
gkyl_gyrokinetic_app_from_file_em(
  gkyl_gyrokinetic_app *app, const char *fname_apar, const char *fname_apardot
)
{
  ...
  if (rstat.io_status == GKYL_ARRAY_RIO_SUCCESS) {
    rstat.io_status = gkyl_comm_array_read(
      app->comm, &app->grid, &app->local, app->field->apardot_host, fname_apardot
    );
    if (app->use_gpu) {
...
gkyl_gyrokinetic_app_from_frame_em(gkyl_gyrokinetic_app *app, int frame)
{
  ...
  struct gkyl_app_restart_status rstat =
    gkyl_gyrokinetic_app_from_file_em(app, fileNm_apar.str, fileNm_apardot.str);
  app->field->is_first_energy_write_call = false; // Append to existing diagnostic.
  app->field->is_first_energy_dot_write_call = false; // Append to existing diagnostic.
...
void
gkyl_gyrokinetic_app_reset_species_fdot_multiplier(
...
void
gkyl_gyrokinetic_app_reset_species_positivity(
  gkyl_gyrokinetic_app *app, double tm, const char *species_name,

...
)
{
  ...
  if (gkr->react_id[ridx] == GKYL_REACT_IZ) {
    ...
    int sz = gkyl_calc_strlen(
      fmt, app->name, gkns->info.name, gkr->react_type[ridx].elc_nm, gkr->react_type[ridx].ion_nm,
      frame
    );
    char fileNm[sz + 1]; // ensures no buffer overflow
    snprintf(
      fileNm, sizeof fileNm, fmt, app->name, gkns->info.name, gkr->react_type[ridx].elc_nm,

...
)
{
  ...
  gkyl_bgk_collisions_advance(
    src->bgk_op, &app->local, &species->local, src->rate, species->lte.f_lte, fin,
    src->implicit_step, src->dt_implicit, src->Jrate_df, species->cflrate
  );
  // Add to the rhs.
...
)
{
  ...
  gkyl_dg_div_op_range(
    species->lte.moms.mem_geo, &app->basis, 0, species->lte.moms.marr, 0, species->lte.moms.marr, 0,
  ...
  gkyl_dg_mul_op_range(
    &app->basis, 0, src->Jrate_mom, 0, src->Jrate_vtsq_shape, 0, species->lte.moms.marr, &app->local
  );
  ...
  gkyl_array_set_offset_range(
    species->lte.moms.marr, src->vtsq_amplitude, src->vtsq_shape, 2 * app->basis.num_basis,
    &app->local
  );
  ...
  gkyl_dg_mul_conf_phase_op_range(
    &app->basis, &species->basis, species->lte.f_lte, src->Jrate, species->lte.f_lte, &app->local,
    &species->local
  );
...
)
{
  ...
  gkyl_dg_div_op_range(
    species->lte.moms.mem_geo, &app->basis, 0, src->Jrate_mom, 0, src->M0dot, 0, src->rate,
    &app->local
  );
  ...
  gkyl_dg_div_op_range(
    species->lte.moms.mem_geo, &app->basis, 0, src->Jrate_mom, 0, src->M1dot, 0, src->rate,
    &app->local
  );
  gkyl_array_accumulate_offset(
  ...
  gkyl_dg_div_op_range(
    species->lte.moms.mem_geo, &app->basis, 0, src->Jrate_mom, 0, src->Jrate_mom, 0,
  ...
  gkyl_dg_div_op_range(
    species->lte.moms.mem_geo, &app->basis, 0, src->Jrate_mom, 0, src->M2dot, 0, src->rate,
    &app->local
  );
  gkyl_array_accumulate_offset(
  ...
  gkyl_dg_mul_op_range(
    &app->basis, 1, src->correct_mom_op.marr, 1, species->lte.moms.marr, 1, species->lte.moms.marr,
    &app->local
  );
  gkyl_dg_mul_op_range(
    &app->basis, 1, src->correct_mom_op.marr, 1, src->correct_mom_op.marr, 0,
    species->lte.moms.marr, &app->local
  );
...
)
{
  ...
  gkyl_dg_mul_conf_phase_op_range(
    &app->basis, &species->basis, species->lte.f_lte, src->Jrate, src->Jrate_df, &app->local,
    &species->local
  );
...
)
{
  ...
  if (src->source_bgk_id) {
    ...
    if (src->source_bgk_id == GKYL_SOURCE_BGK_STATIC) {
      ...
      gkyl_proj_on_basis *proj_rate = gkyl_proj_on_basis_new(
        &app->grid, &app->basis, app->poly_order + 1, 1, gks->info.source_bgk.rate_profile,
      ...
      gkyl_dg_mul_conf_phase_op_range(
        &app->basis, &gks->basis, src->Jrate_df, app->gk_geom->geo_corn.bmag, src->Jrate_df,
        &app->local, &gks->local
      );
      gkyl_array_scale_by_cell(src->Jrate_df, gks->vel_map->jacobvel);
      gkyl_dg_mul_conf_phase_op_range(
        &app->basis, &gks->basis, src->Jrate_df, app->gk_geom->geo_int.jacobgeo, src->Jrate_df,
        &app->local, &gks->local
      );
      ...
      gkyl_dg_mul_conf_phase_op_range(
        &app->basis, &gks->basis, src->Jrate_df, src->rate, src->Jrate_df, &app->local, &gks->local
      );
    }

...
)
{
  ...
  gkyl_dg_div_op_range(
    gkns->lte.moms.mem_geo, &app->basis, 0, gkns->lte.moms.marr, 0, gkns->lte.moms.marr, 0,
...
)
{
  ...
  gkyl_dg_mul_conf_phase_op_range(
    &app->basis, &gkns->basis, bgk->nu_fmax, app->gk_geom->geo_int.jacobgeo, bgk->nu_fmax,
    &app->local, &gkns->local
  );

  // Apply BGK collisions.
  gkyl_bgk_collisions_advance(
    bgk->up_bgk, &app->local, &gkns->local, bgk->nu_sum, bgk->nu_fmax, fin, bgk->implicit_step, dt,
    rhs, gkns->cflrate
  );
...
static void gkbgk_write_mom_disabled(
...
static void gkbgk_write_mom_enabled(
  gkyl_gyrokinetic_app *app, struct gk_neut_species *gkns, double tm, int frame
)
{
  ...
  if (bgk->collision_id == GKYL_BGK_COLLISIONS) {
    ...
    if (gkns->info.collisions.self_nu) {
      ...
      gkyl_proj_on_basis *proj = gkyl_proj_on_basis_new(
        &app->grid, &app->basis, app->poly_order + 1, 1, gkns->info.collisions.self_nu,
        gkns->info.collisions.self_nu_ctx
      );

...
struct gk_collisionless {
  ...
  union {
    ...
    struct {
      ...
      struct gkyl_array
        *vel_flux_surf; // Modal expansion of surface fluxes at velocity-space surfaces.
      struct gkyl_dg_vlasov_conf_flux_surf
        *calc_conf_flux; // Updater for computing surface fluxes (conf).
      struct gkyl_dg_vlasov_vel_flux_surf
        *calc_vel_flux; // Updater for computing surface fluxes (vel).
...
struct gk_neut_species {
  ...
  union {
    ...
    struct {
      ...
      struct gkyl_array
        *conf_poisson_tensor; // Conf-space Poisson tensor from the GK geometry triads.
      struct gkyl_array
        *conf_poisson_tensor_host; // Host-side Poisson tensor (built nodally on host).

...
void gk_species_source_write_disabled(
...
void gk_species_source_write_enabled(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame
)
{
...
void gk_species_source_write_init_only(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame
)
{
...
void gk_species_source_write_mom_disabled(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame
)
{
  // Empty.
...
void gk_species_source_write_mom_enabled(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame
)
{
  struct timespec wst = gkyl_wall_clock();
...
void gk_species_source_write_mom_init_only(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm, int frame
)
{
...
void gk_species_source_calc_integrated_mom_disabled(
...
void gk_species_source_calc_integrated_mom_enabled(
  gkyl_gyrokinetic_app *app, struct gk_species *gks, double tm
)
{
  struct timespec wst = gkyl_wall_clock();
  ...
  gkyl_comm_allreduce(
    app->comm, GKYL_DOUBLE, GKYL_SUM, num_mom, gks->src.red_integ_diag,
...
void gk_species_source_write_integrated_mom_disabled(
  gkyl_gyrokinetic_app *app, struct gk_species *gks
)
{
  // empty
}

void gk_species_source_write_integrated_mom_enabled(
  gkyl_gyrokinetic_app *app, struct gk_species *gks
)
{
  struct timespec wst = gkyl_wall_clock();
...
static void gk_species_source_adapt_disabled(
  gkyl_gyrokinetic_app *app, struct gk_species *s, struct gk_source *src,
  struct gkyl_array *f_buffer, struct gkyl_array **bflux_moms[], double tm
)
{
...
static void gk_species_source_adapt_enabled(
  gkyl_gyrokinetic_app *app, struct gk_species *s, struct gk_source *src,
  struct gkyl_array *f_buffer, struct gkyl_array **bflux_moms[], double tm
)
{
  for (int k = 0; k < s->info.source.num_adapt_sources; ++k) {
    ...
    for (int j = 0; j < adapt_src->num_boundaries; ++j) {
      ...
      gk_species_bflux_get_flux_mom(
        &s_adapt->bflux, adapt_src->dir[j], adapt_src->edge[j], GKYL_F_MOMENT_M0,
        s_adapt_bflux_moms, adapt_src->bflux_m0, &adapt_src->boundaries_conf_ghost[j]
      );
      gk_species_bflux_get_flux_mom(
        &s_adapt->bflux, adapt_src->dir[j], adapt_src->edge[j], GKYL_F_MOMENT_M2,
        s_adapt_bflux_moms, adapt_src->bflux_m2, &adapt_src->boundaries_conf_ghost[j]
      );

      // Integrate the boundary flux moments to get the total loss through the j-th boundary.
      gkyl_array_integrate_advance(
        adapt_src->integrate_op, adapt_src->bflux_m0, 1.0, 0, &adapt_src->boundaries_conf_ghost[j],
        0, adapt_src->integ_m0
      );
      gkyl_array_integrate_advance(
        adapt_src->integrate_op, adapt_src->bflux_m2, 1.0, 0, &adapt_src->boundaries_conf_ghost[j],
        0, adapt_src->integ_m2
      );
      if (app->use_gpu) {
    ...
    gkyl_comm_allreduce_host(
      app->comm, GKYL_DOUBLE, GKYL_SUM, 1, &sum_particle_loss_local, &adapt_src->particle_rate_loss
    );
    gkyl_comm_allreduce_host(
    ...
    gkyl_array_set_offset(
      src->proj_source[k].prim_moms, particle_src_new, src->proj_source[k].gaussian_profile,
...
static void gk_species_source_adapt_after_first_step(
  gkyl_gyrokinetic_app *app, struct gk_species *s, struct gk_source *src,
  struct gkyl_array *f_buffer, struct gkyl_array **bflux_moms[], double tm
)
{

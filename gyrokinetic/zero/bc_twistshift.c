...
)
{
  ...
  gkyl_rect_grid_init(
    &grid_do, 1, &inp->grid->lower[shear_dir], &inp->grid->upper[shear_dir],
    &inp->grid->cells[shear_dir]
  );
  gkyl_rect_grid_init(
    &grid_tar, 1, &ts_grid->lower[shear_dir], &ts_grid->upper[shear_dir], &ts_grid->cells[shear_dir]
  );

  struct gkyl_range shear_r_do;
  gkyl_range_init(
    &shear_r_do, 1, (int[]){inp->bcdir_ext_update_r->lower[shear_dir]},
  ...
  struct gkyl_dg_interpolate *interp = gkyl_dg_interpolate_new(
    1, &shift_b, &grid_do, &grid_tar, &shear_r_do, shear_r_fine, (int[]){0}, false
  );
...
gkyl_bc_twistshift_inew(const struct gkyl_bc_twistshift_inp *inp)
{
  ...
  if (inp->edge == GKYL_LOWER_EDGE) {
    gkyl_range_shorten_from_above(
      &up->coarse_ghost_r, inp->bcdir_ext_update_r, inp->bc_dir, inp->num_ghost[inp->bc_dir]
    );
  } else {
  ...
  if (inp->edge == GKYL_LOWER_EDGE) {
    gkyl_range_shorten_from_above(
      &up->ghost_r, &up->ts_update_r, inp->bc_dir, inp->num_ghost[inp->bc_dir]
    );
  } else {
    gkyl_range_shorten_from_below(
      &up->ghost_r, &up->ts_update_r, inp->bc_dir, inp->num_ghost[inp->bc_dir]
  ...
  up->filter = gkyl_dg_lowpass_filter_new(
    inp->shear_dir, up->half_width_fine, up->filter_cutoff_wavelength, inp->basis, &up->ts_grid,
  ...
  if (up->upsample_factor > 1) {
    ...
    up->coarsen = gkyl_dg_interpolate_new(

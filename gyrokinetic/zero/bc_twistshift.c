...
double ts_grid_cell_boundary_in_dir(
...
long ts_shift_dir_idx_do_linidx(
  const int *num_do, int shear_dir_idx, int shift_dir_idx, int shift_dir_num_cells,
  int shear_r_lower
)
{
...
void ts_check_shifted_test_point(
  struct gkyl_bc_twistshift *up, const double *test_pt, const double *xc, const double *dx,
  double *shift_c, const int *idx, bool pick_lower, int *num_do_curr,
...
)
{
  ...
  double shifted_test_pt[] = {ts_wrap_to_range(
    ...
    up->ts_grid.upper[up->shift_dir_in_ts_grid], false
  )}; // Shifted test point.
  ...
  long linidx = ts_shift_dir_idx_do_linidx(
    up->num_do, shear_idx[0], shift_idx[0], up->ts_grid.cells[up->shift_dir_in_ts_grid],
    up->shear_r.lower[0]
  );
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    for (int dC = 0; dC < 2; dC++) { // dC=0: x=const, dC=1: y=const (boundaries).
      for (int xS = 0; xS < 2; xS++) { // xS=0: lower, xS=1 upper (boundary).
        ...
        // Shift the test point, find the cell that contains it, and if we
        // haven't included it yet, add it to our list of donors.
        ts_check_shifted_test_point(
          up, test_pt, xc, up->ts_grid.dx, shift_c, iter.idx, pick_lower, &num_do_curr,
          shift_dir_idx_do_buff
        );
...
struct gkyl_qr_res ts_find_intersect(
  struct gkyl_bc_twistshift *up, double shiftCoordTar, double shiftCoordDo,
...
static inline void ts_comp_to_phys(
...
void ts_integral_xlimdg(
  struct gkyl_bc_twistshift *up, double sFac, const double *xLimLo, const double *xLimUp,
  double yLimLo, double yLimUp, double dyDo, double yOff, const double *ySh, struct gkyl_mat *mat_do
)
{
...
void ts_integral_ylimdg(
  struct gkyl_bc_twistshift *up, double sFac, double xLimLo, double xLimUp, const double *yLimLo,
  const double *yLimUp, double dyDo, double yOff, const double *ySh, struct gkyl_mat *mat_do
)
{
...
void ts_integral_fullcelllimdg(
  struct gkyl_bc_twistshift *up, double dyDo, double yOff, const double *ySh,
...
void ts_subcellint_sNi_sNii(
  struct gkyl_bc_twistshift *up, struct ts_val_found *inter_pts, const double *xc_do,
  const double *xc_tar, const double *cellb_do, const double *cellb_tar,
...
)
{
  ...
  if (is_sNi) {
    ...
    xi_b[0] = ts_p2l(
      inter_pts[2].value, xc_do[up->shear_dir_in_ts_grid], up->ts_grid.dx[up->shear_dir_in_ts_grid]
    );
    xi_b[1] = ts_p2l(
    ...
    if (fabs(xi_b[1] - xi_b[0]) > tol_xi) {
      up->kernels->ylimdg(
        1.0, xi_b[0], xi_b[1], etalo_xi, etaup_xi, up->ts_grid.dx[up->shift_dir_in_ts_grid], xs_off,
        shift_c, mat_do
      );
    }
    ...
    xi_b[1] = ts_p2l(
      inter_pts[1].value, xc_do[up->shear_dir_in_ts_grid], up->ts_grid.dx[up->shear_dir_in_ts_grid]
    );
...
struct gkyl_nmat *ts_calc_mats(struct gkyl_bc_twistshift *up)
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    long linidx_do = ts_shift_dir_idx_do_linidx(
      up->num_do, iter.idx[0], shift_dir_idx_tar, up->ts_grid.cells[up->shift_dir_in_ts_grid],
      up->shear_r.lower[0]
    );
    ...
    for (int iC = 0; iC < up->num_do[iter.idx[0] - up->shear_r.lower[0]]; iC++) {
      ...
      int nP_primary = (int
      )round((S_c - (xc_tar[up->shift_dir_in_ts_grid] - xc_do[up->shift_dir_in_ts_grid])) / Ly);
      ...
      for (int i = 0; i < 2; i++) { // Loop over j_tar-/+1/2
        for (int j = 0; j < 2; j++) { // Loop over j_do-/+1/2
          ...
          struct gkyl_qr_res inter_res = ts_find_intersect(
            up, shift_dir_coord_tar, shift_dir_coord_do,
            (double[]
            ){cellb_tar[cellb_lo(up->shear_dir_in_ts_grid)],
      ...
      if (num_inter_pts_found == 4) {
        ...
        ts_subcellint_sNi_sNii(
          up, inter_pts, xc_do, xc_tar, cellb_do, cellb_tar, is_upper_shift_dir_cell, shift_c,
          &mat_do
        );
...
long *ts_calc_num_numcol_fidx_do(struct gkyl_bc_twistshift *up)
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    while (gkyl_range_iter_next(&shear_dir_iter)) {
      ...
      long linidx_do = ts_shift_dir_idx_do_linidx(
        up->num_do, shear_dir_idx, iter.idx[shift_dir_in_shearbc_perp_r],
  ...
  if (up->use_gpu) {
    ...
    gkyl_cu_memcpy(
      num_numcol_fidx_do, num_numcol_fidx_do_ho, up->fmat->num * up->fmat->nc * sizeof(long),
      GKYL_CU_MEMCPY_H2D
    );
  }
#endif
...
void gkyl_bc_twistshift_choose_kernels(
...
struct gkyl_bc_twistshift *gkyl_bc_twistshift_inew(const struct gkyl_bc_twistshift_inp *inp)
{
  ...
  gkyl_range_init(
    &up->shear_r, 1, (int[]){up->local_bcdir_ext_r.lower[inp->shear_dir]},
  ...
  gkyl_range_init(
    &up->shift_r, 1, (int[]){up->local_bcdir_ext_r.lower[inp->shift_dir]},
...
struct gkyl_array *gkyl_bc_twistshift_get_shift_objects(
  struct gkyl_bc_twistshift *up, struct gkyl_rect_grid *shear_grid, struct gkyl_range *shear_r,

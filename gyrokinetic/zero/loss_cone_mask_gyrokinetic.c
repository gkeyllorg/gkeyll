...
{
  struct gkyl_array *basis_at_corners_ho =
    gkyl_array_new(GKYL_DOUBLE, basis->num_basis, corner_range->volume);
...
gkyl_loss_cone_mask_gyrokinetic_inew(const struct gkyl_loss_cone_mask_gyrokinetic_inp *inp)
{
  ...
  assert(
    up->lower_orbit >= GKYL_GK_TRAP_PASS_ORBIT_PASSING &&
    up->lower_orbit <= GKYL_GK_TRAP_PASS_ORBIT_TRAPPED_WALL
  );
  assert(
    up->upper_orbit >= GKYL_GK_TRAP_PASS_ORBIT_PASSING &&
  ...
  gk_lcm_init_corner_values(
    &up->conf_corner_range, inp->conf_basis, &up->basis_at_corners_conf, inp->use_gpu
  );
...
)
{
  ...
  while (gkyl_range_iter_next(&phase_iter)) {
    ...
    while (cell_trapped && gkyl_range_iter_next(&corner_iter)) {
      ...
      double mu = gvm->vmap_basis->eval_expand(mu_eta, vmap_d + gvm->vmap_basis->num_basis);

      // Evaluate the Hamiltonian at this phase-space corner.
      long linidx_conf = gkyl_range_idx(conf_range, conf_idx);
      double bmag_curr = gk_lcm_field_corner_value(
        bmag, up->basis_at_corners_conf, num_basis_conf, linidx_conf, conf_corner
      );
      double phi_curr = gk_lcm_field_corner_value(
        phi, up->basis_at_corners_conf, num_basis_conf, linidx_conf, conf_corner
      );
      ...
      gk_lcm_escape_barriers(
        cdim, num_basis_conf, conf_range, &up->conf_corner_range, up->basis_at_corners_conf, phi,
        bmag, phi_wall_lo, phi_wall_up, conf_idx, conf_idx[zdim], conf_corner, mu, up->charge,
        up->lower_orbit, up->upper_orbit, &barrier_left, &barrier_right
      );

      // Equality means that the particle reaches the loss endpoint with zero
      // parallel kinetic energy, so it is not trapped.
      cell_trapped = gk_lcm_hamiltonian_below_barrier(
                       kinetic_energy, magnetic_energy, electric_energy, barrier_left
                     ) &&
                     gk_lcm_hamiltonian_below_barrier(
                       kinetic_energy, magnetic_energy, electric_energy, barrier_right
                     );
    }

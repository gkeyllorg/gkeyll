...
ts_check_shifted_test_point(
  struct gkyl_twistshift_dg *up, const double *test_pt, const double *xc, const double *dx,
...
struct gkyl_qr_res
ts_find_intersect(
  struct gkyl_twistshift_dg *up, double shiftCoordTar, double shiftCoordDo,
...
void
ts_nod2mod_proj_1d(
  struct gkyl_twistshift_dg *up, evalf_t func, void *func_ctx, const double *interval, double *out
)
{
...
void
ts_integral_xlimdg(
  struct gkyl_twistshift_dg *up, double sFac, const double *xLimLo, const double *xLimUp,
...
void
ts_integral_ylimdg(
  struct gkyl_twistshift_dg *up, double sFac, double xLimLo, double xLimUp, const double *yLimLo,
...
void
ts_integral_fullcelllimdg(
  struct gkyl_twistshift_dg *up, double dyDo, double yOff, const double *ySh,
...
ts_subcellint_sNi_sNii(
  struct gkyl_twistshift_dg *up, struct ts_val_found *inter_pts, const double *xc_do,
...
void
gkyl_twistshift_dg_choose_kernels(
  struct gkyl_basis basis, int cdim, int shift_poly_order, struct gkyl_twistshift_dg_kernels *kers
)
{
  int dim = basis.ndim;
  int vdim = dim - cdim;
  ...
  switch (basis_type) {
    ...
    case GKYL_BASIS_MODAL_SERENDIPITY:
      if (shift_poly_order == 1) {
        kers->xlimdg = vdim == 0 ?
                         ser_twistshift_xlimdg_list_0v_yShp1[cdim - 2].kernels[poly_order] :
                         ser_twistshift_xlimdg_list_2v_yShp1[cdim - 2].kernels[poly_order];
        kers->ylimdg = vdim == 0 ?
                         ser_twistshift_ylimdg_list_0v_yShp1[cdim - 2].kernels[poly_order] :
        ...
        kers->xlimdg = vdim == 0 ?
                         ser_twistshift_xlimdg_list_0v_yShp2[cdim - 2].kernels[poly_order] :
                         ser_twistshift_xlimdg_list_2v_yShp2[cdim - 2].kernels[poly_order];
        kers->ylimdg = vdim == 0 ?
                         ser_twistshift_ylimdg_list_0v_yShp2[cdim - 2].kernels[poly_order] :
...
struct gkyl_array *
gkyl_twistshift_dg_get_shift_objects(

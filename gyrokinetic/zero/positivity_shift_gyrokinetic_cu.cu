...
gkyl_pos_shift_gk_set_cu_ker_ptrs(
...
void
pos_shift_gk_choose_shift_kernel_cu(
  struct gkyl_positivity_shift_gyrokinetic_kernels *kernels, struct gkyl_basis cbasis,
  struct gkyl_basis pbasis, enum gkyl_positivity_shift_type stype
)
{
...
gkyl_positivity_shift_gyrokinetic_advance_shift_cu_ker(
  ...
  double mass, struct gkyl_positivity_shift_gyrokinetic_shift_region_idx shift_regions,
  const struct gkyl_array *GKYL_RESTRICT bmag, const struct gkyl_array *GKYL_RESTRICT jacobtot,
...
void
gkyl_positivity_shift_gyrokinetic_advance_cu(
...
)
{
  ...
  gkyl_positivity_shift_gyrokinetic_advance_shift_cu_ker<<<nblocks_phase, nthreads_phase>>>(
    ...
    up->ffloor_fac, up->cellav_fac, up->mass, up->shift_regions, up->gk_geom->geo_int.bmag->on_dev,

...
gkyl_gk_geometry_reset_io_meta(struct gk_geometry *up)
{
  ...
  if (up->geometry_id == GKYL_GEOMETRY_TOKAMAK || up->geometry_id == GKYL_GEOMETRY_MIRROR) {
    if (gkyl_msgpack_map_elem_has_key(
      ...
      up->io_meta_basic = gkyl_msgpack_map_elem_union(
        sizeof(io_meta_basic_list_len) / sizeof(int), io_meta_basic_list_len, io_meta_basic_list,
...
gkyl_gk_geometry_populate_nodal(struct gk_geometry *gk_geom)
{
  ...
  gkyl_nodal_ops_m2n(
    n2m, &gk_geom->basis, &gk_geom->grid, &gk_geom->nrange_int, &gk_geom->local, 1,
    gk_geom->geo_int.jacobgeo_nodal, gk_geom->geo_int.jacobgeo, true
  );
  gkyl_nodal_ops_m2n(
    n2m, &gk_geom->basis, &gk_geom->grid, &gk_geom->nrange_int, &gk_geom->local, 9,
  ...
  gkyl_nodal_ops_m2n(
    n2m, &gk_geom->basis, &gk_geom->grid, &gk_geom->nrange_int, &gk_geom->local, 3,
  ...
  for (int dir = 0; dir < gk_geom->grid.ndim; dir++) {
    ...
    gkyl_nodal_ops_m2n_surface(
      n2m, &gk_geom->surf_basis, &gk_geom->grid, &gk_geom->nrange_surf[dir], &local_ext_in_dir, 1,
    ...
    gkyl_nodal_ops_m2n_surface(
      n2m, &gk_geom->surf_basis, &gk_geom->grid, &gk_geom->nrange_surf[dir], &local_ext_in_dir, 3,
    ...
    gkyl_nodal_ops_m2n_surface(
      n2m, &gk_geom->surf_basis, &gk_geom->grid, &gk_geom->nrange_surf[dir], &local_ext_in_dir, 9,
...
gkyl_gk_geometry_deflate(const struct gk_geometry *up_3d, struct gkyl_gk_geometry_inp *geometry_inp)
{
  ...
  gkyl_deflate_geo_advance(
    deflator, &up_3d->local, &up->local, up_3d->geo_int.jacobgeo_ghost, up->geo_int.jacobgeo_ghost,
    1
  );
  gkyl_deflate_geo_advance(
  ...
  gkyl_deflate_geo_advance(
    deflator, &up_3d->local, &up->local, up_3d->geo_int.dualcurlbhatoverB,
  ...
  gkyl_deflate_geo_advance_nodal(
    deflator, &up_3d->nrange_int, &up->nrange_int, up_3d->geo_int.jacobgeo_nodal,
    up->geo_int.jacobgeo_nodal, 1
  );
  gkyl_deflate_geo_advance_nodal(
    deflator, &up_3d->nrange_int, &up->nrange_int, up_3d->geo_int.dxdz_nodal,
    up->geo_int.dxdz_nodal, 9
  );
  gkyl_deflate_geo_advance_nodal(
    deflator, &up_3d->nrange_int, &up->nrange_int, up_3d->geo_int.dzdx_nodal,
    up->geo_int.dzdx_nodal, 9
  );
  gkyl_deflate_geo_advance_nodal(
    deflator, &up_3d->nrange_int, &up->nrange_int, up_3d->geo_int.B3_nodal, up->geo_int.B3_nodal, 1
  );
  gkyl_deflate_geo_advance_nodal(
    deflator, &up_3d->nrange_int, &up->nrange_int, up_3d->geo_int.bmag_nodal,
    up->geo_int.bmag_nodal, 1
  );
  gkyl_deflate_geo_advance_nodal(
    deflator, &up_3d->nrange_int, &up->nrange_int, up_3d->geo_int.dualcurlbhat_nodal,
    up->geo_int.dualcurlbhat_nodal, 3
  );
  gkyl_deflate_geo_advance_nodal(
    deflator, &up_3d->nrange_int, &up->nrange_int, up_3d->geo_int.g_ij_nodal,
    up->geo_int.g_ij_nodal, 6
  );
  ...
  for (int dir = 0; dir < 3; dir++) {
    if (rem_dirs[dir] == 0) {
      ...
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].bmag,
        up->geo_surf[count].bmag, 1
      );
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].jacobgeo,
        up->geo_surf[count].jacobgeo, 1
      );
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].jacobgeo_ratio,
        up->geo_surf[count].jacobgeo_ratio, 1
      );
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].jacobtot_inv,
        up->geo_surf[count].jacobtot_inv, 1
      );
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].b_i,
        up->geo_surf[count].b_i, 3
      );
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].cmag,
        up->geo_surf[count].cmag, 1
      );
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].B3,
        up->geo_surf[count].B3, 1
      );
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].normcurlbhat,
        up->geo_surf[count].normcurlbhat, 1
      );
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].normals,
        up->geo_surf[count].normals, 9
      );
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].lenr,
        up->geo_surf[count].lenr, 1
      );
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].deltats,
        up->geo_surf[count].deltats, 1
      );
      gkyl_deflate_geo_surf_advance(
        deflator_surf, &local_ext_in_dir_3d, &local_ext_in_dir, up_3d->geo_surf[dir].bimpactangle,
        up->geo_surf[count].bimpactangle, 1
      );
      // deflate nodal quantities
      gkyl_deflate_geo_surf_advance_nodal(
        deflator_surf, &up_3d->nrange_surf[dir], &up->nrange_surf[count],
        up_3d->geo_surf[dir].bmag_nodal, up->geo_surf[count].bmag_nodal, 1
      );
      gkyl_deflate_geo_surf_advance_nodal(
        deflator_surf, &up_3d->nrange_surf[dir], &up->nrange_surf[count],
        up_3d->geo_surf[dir].jacobgeo_nodal, up->geo_surf[count].jacobgeo_nodal, 1
      );
      gkyl_deflate_geo_surf_advance_nodal(
        deflator_surf, &up_3d->nrange_surf[dir], &up->nrange_surf[count],
        up_3d->geo_surf[dir].B3_nodal, up->geo_surf[count].B3_nodal, 1
      );
      gkyl_deflate_geo_surf_advance_nodal(
        deflator_surf, &up_3d->nrange_surf[dir], &up->nrange_surf[count],
        up_3d->geo_surf[dir].normcurlbhat_nodal, up->geo_surf[count].normcurlbhat_nodal, 1
      );
      gkyl_deflate_geo_surf_advance_nodal(
        deflator_surf, &up_3d->nrange_surf[dir], &up->nrange_surf[count],
        up_3d->geo_surf[dir].b_i_nodal, up->geo_surf[count].b_i_nodal, 3
      );
      gkyl_deflate_geo_surf_advance_nodal(
        deflator_surf, &up_3d->nrange_surf[dir], &up->nrange_surf[count],
        up_3d->geo_surf[dir].normals_nodal, up->geo_surf[count].normals_nodal, 9
      );
      gkyl_deflate_geo_surf_advance_nodal(
        deflator_surf, &up_3d->nrange_surf[dir], &up->nrange_surf[count],
        up_3d->geo_surf[dir].lenr_nodal, up->geo_surf[count].lenr_nodal, 1
      );
      gkyl_deflate_geo_surf_advance_nodal(
        deflator_surf, &up_3d->nrange_surf[dir], &up->nrange_surf[count],
        up_3d->geo_surf[dir].deltats_nodal, up->geo_surf[count].deltats_nodal, 1
      );
      gkyl_deflate_geo_surf_advance_nodal(
        deflator_surf, &up_3d->nrange_surf[dir], &up->nrange_surf[count],
        up_3d->geo_surf[dir].bimpactangle_nodal, up->geo_surf[count].bimpactangle_nodal, 1
      );
      count += 1;
...
void
gkyl_gk_geometry_write_efit(
  struct gkyl_gk_geometry_inp *geometry_inp, struct gkyl_msgpack_map_elem *io_meta_basic_basic,

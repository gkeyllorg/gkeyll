...
typedef double (*dg_gyrokinetic_vol_es_t)(
  const double *w, const double *dxv, const double *vmap, const double *vmapSq, const double q_,
  const double m_, const double *bmag, const double *phi, const double *dualcurlbhatoverB,
  const double *rtg33inv, const double *bioverJB, const double *fin, double *GKYL_RESTRICT out
);

typedef double (*dg_gyrokinetic_vol_add_apar_t)(
  const double *w, const double *dxv, const double *vmap, const double *vmapSq, const double q_,
  const double m_, const double *bmag, const double *jacobtot_inv, const double *dualcurlbhatoverB,
  const double *bioverJB, const double *b_i, const double *phi, const double *apar,
  const double *fin, double *GKYL_RESTRICT out
);

typedef double (*dg_gyrokinetic_vol_add_apardot_t)(
  const double *vmap, const double q_, const double m_, const double *apardot, const double *fin,
  double *GKYL_RESTRICT out
);

typedef double (*dg_gyrokinetic_surf_t)(
...
typedef double (*dg_gyrokinetic_boundary_surf_t)(
  const double *w, const double *dxv, const double *vmap_prime_edge, const double *vmap_prime_skin,
  const double *flux_surf_edge, const double *flux_surf_skin, const int edge,
  double *GKYL_RESTRICT out
);
...
typedef struct {
  dg_gyrokinetic_vol_es_t kernels[3];
} gkyl_dg_gyrokinetic_vol_es_kern_list;
typedef struct {
  dg_gyrokinetic_vol_add_apar_t kernels[3];
} gkyl_dg_gyrokinetic_vol_add_apar_kern_list;
typedef struct {
  dg_gyrokinetic_vol_add_apardot_t kernels[3];
} gkyl_dg_gyrokinetic_vol_add_apardot_kern_list;
typedef struct {
...
struct dg_gyrokinetic {
  ...
  dg_gyrokinetic_vol_add_apardot_t
    vol_add_apardot_kernel; // Additional volume term for Apar contribution.
...
GKYL_CU_DH static double
dg_gyrokinetic_add_apar_vol_none(
  const double *w, const double *dxv, const double *vmap, const double *vmapSq, const double q_,
  const double m_, const double *bmag, const double *jacobtot_inv, const double *dualcurlbhatoverB,
  const double *bioverJB, const double *b_i, const double *phi, const double *apar,
  const double *fin, double *GKYL_RESTRICT out
...
GKYL_CU_DH static double
dg_gyrokinetic_add_apardot_vol_none(
  const double *vmap, const double q_, const double m_, const double *apardot, const double *fin,
  double *GKYL_RESTRICT out
)
{
...
GKYL_CU_D static const gkyl_dg_gyrokinetic_vol_es_kern_list ser_vol_es_kernels[] = {
  // 1x kernels
...
// Need to be separated like this for GPU build
//
GKYL_CU_D static const gkyl_dg_gyrokinetic_vol_es_kern_list ser_no_by_vol_es_kernels[] = {
  // 1x kernels
  {NULL, dg_gyrokinetic_vol_1x1v_ser_p1, NULL}, // 0
  {NULL, dg_gyrokinetic_vol_1x2v_ser_p1, NULL}, // 1
  // 2x kernels
...
GKYL_CU_D static const gkyl_dg_gyrokinetic_vol_add_apar_kern_list ser_add_apar_vol_kernels[] = {
  // 1x kernels
...
GKYL_CU_D static const gkyl_dg_gyrokinetic_vol_add_apardot_kern_list ser_add_apardot_vol_kernels[] =
  {
    // 1x kernels
...
struct gkyl_dg_eqn *gkyl_dg_gyrokinetic_cu_dev_new(
  ...
  const bool complete_em, const struct gk_geometry *gk_geom, const struct gkyl_velocity_map *vel_map
);

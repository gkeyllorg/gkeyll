...
)
{
  ...
  if (use_gpu) {
    return gkyl_gk_collisionless_flux_cu_dev_new(
      phase_grid, conf_basis, phase_basis, charge, mass, type, gk_geom, dg_geom, gk_dg_geom,
      vel_map, bctype_conf
    );
  }
#endif
    ...
    for (int dir = 0; dir < cdim; ++dir) {
      ...
      if (idx[dir] == phase_range->lower[dir]) {
        ...
        cflrate_d[0] += up->flux_surf_edge_lo[dir](
          ...
          jacgeo_rat_surfL_d, jacgeo_rat_surfR_d, phi_d, fL, fR, flux_surf_d
        );
      } else {

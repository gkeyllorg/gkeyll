...
)
{
  ...
  if (use_gpu) {
    return gkyl_gk_collisionless_flux_cu_dev_new(
      phase_grid, conf_basis, phase_basis, charge, mass, type, no_by, complete_em, gk_geom, dg_geom,
      gk_dg_geom, vel_map, bctype_conf
    );
  }
#endif
...
)
{
  ...
  if (gkyl_array_is_cu_dev(flux_surf)) {
    return gkyl_gk_collisionless_flux_surf_cu(
      up, conf_range, phase_range, conf_ext_range, phase_ext_range, phi, apar, apardot, fin,
      flux_surf, cflrate
    );
  }
#endif

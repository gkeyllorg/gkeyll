...
)
{
  ...
  for (int zidx = up->local.lower[up->cdim - 1]; zidx <= up->local.upper[up->cdim - 1]; zidx++) {
    ...
    gkyl_deflate_zsurf_advance(
      up->deflator_lo, zidx, &up->local, &up->deflated_local, lop, up->d_bop_data[ctr].deflated_lop,
      lop_ncomp
    );
    gkyl_deflate_zsurf_advance(
      up->deflator_lo, zidx, &up->local, &up->deflated_local, rop, up->d_bop_data[ctr].deflated_rop,
      rop_ncomp
    );
    ...
    if (op_type == GKYL_DEFLATED_DIV) {
      gkyl_dg_div_op_range(
        up->mem, &up->deflated_basis, c_oop, up->d_bop_data[ctr].deflated_out, c_lop,
        up->d_bop_data[ctr].deflated_lop, c_rop, up->d_bop_data[ctr].deflated_rop,
        &up->deflated_local
      );
    } else if (op_type == GKYL_DEFLATED_MUL) {
      gkyl_dg_mul_op_range(
        &up->deflated_basis, c_oop, up->d_bop_data[ctr].deflated_out, c_lop,
        up->d_bop_data[ctr].deflated_lop, c_rop, up->d_bop_data[ctr].deflated_rop,
        &up->deflated_local
      );
    }
    ...
    gkyl_nodal_ops_m2n_deflated(
      ...
      &up->deflated_nrange, &up->deflated_local, 1, up->nodal_fld, up->d_bop_data[ctr].deflated_out,
      ctr
    );
    ctr += 1;
    if (zidx == up->local.upper[up->cdim - 1]) {
      // Deflate lop and rop
      gkyl_deflate_zsurf_advance(
        up->deflator_up, zidx, &up->local, &up->deflated_local, lop,
        up->d_bop_data[ctr].deflated_lop, lop_ncomp
      );
      gkyl_deflate_zsurf_advance(
        up->deflator_up, zidx, &up->local, &up->deflated_local, rop,

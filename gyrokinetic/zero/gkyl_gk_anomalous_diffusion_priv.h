...
typedef double (*gk_anom_diff_surf_t)(
  const double *wc, const double *dxc, const double *nul, const double *nuc, const double *nur,
...
typedef double (*gk_anom_diff_boundary_surf_t)(
  const double *wSkin, const double *dxSkin, const double *nuEdge, const double *nuSkin,
  const double *jacobgeo_invEdge, const double *jacobgeo_invSkin, int edge, const double *JfEdge,
  const double *JfSkin, double *GKYL_RESTRICT out
);
...
)
{
  ...
  if (dir == 0) {
    gkad->surf(
      xcC, dxC, _cfnu(idxL), _cfnu(idxC), _cfnu(idxR), _cfJacInv(idxL), _cfJacInv(idxC),
...
)
{
  ...
  if (dir == 0) {
    if (edge == -1) {
      gkad->boundary_surf[0](
        xcSkin, dxSkin, _cfnu(idxEdge), _cfnu(idxSkin), _cfJacInv(idxEdge), _cfJacInv(idxSkin),
        edge, qInEdge, qInSkin, qRhsOut
      );
    } else {
      gkad->boundary_surf[1](
        xcSkin, dxSkin, _cfnu(idxEdge), _cfnu(idxSkin), _cfJacInv(idxEdge), _cfJacInv(idxSkin),
        edge, qInEdge, qInSkin, qRhsOut
      );
    }
...
)
{
  ...
  if (dir == 0) {
    if (edge == -1) {
      gkad->boundary_diag[0](
        xcSkin, dxSkin, _cfnu(idxSkin), _cfnu(idxGhost), _cfJacInv(idxSkin), _cfJacInv(idxGhost),
        edge, qInSkin, qInGhost, qRhsGhost
      );
    } else {
      gkad->boundary_diag[1](
        xcSkin, dxSkin, _cfnu(idxSkin), _cfnu(idxGhost), _cfJacInv(idxSkin), _cfJacInv(idxGhost),
        edge, qInSkin, qInGhost, qRhsGhost
      );
    }

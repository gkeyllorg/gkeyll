...
__global__ static void gkyl_loss_cone_mask_gyrokinetic_advance_cu_ker(
  ...
  const struct gkyl_array *bmag, const struct gkyl_array *phi, const struct gkyl_array *phi_wall_lo,
  const struct gkyl_array *phi_wall_up, enum gkyl_gk_trapped_passing_orbit_type lower_orbit,
  enum gkyl_gk_trapped_passing_orbit_type upper_orbit, const struct gkyl_velocity_map *gvm,
  ...
  for (unsigned long tid = threadIdx.x + blockIdx.x * blockDim.x; tid < phase_range.volume;
    ...
    for (long corner = 0; corner < phase_corner_range.volume && cell_trapped; ++corner) {
      ...
      double bmag_curr = gk_lcm_field_corner_value(
        bmag, basis_at_corners_conf, num_basis_conf, linidx_conf, conf_corner
      );
      double phi_curr = gk_lcm_field_corner_value(
        phi, basis_at_corners_conf, num_basis_conf, linidx_conf, conf_corner
      );
      ...
      gk_lcm_escape_barriers(
        cdim, num_basis_conf, &conf_range, &conf_corner_range, basis_at_corners_conf, phi, bmag,
        phi_wall_lo, phi_wall_up, conf_idx, conf_idx[zdim], conf_corner, mu, charge, lower_orbit,
        upper_orbit, &barrier_left, &barrier_right
      );

...
gk_lcm_conf_corner_z_side(const struct gkyl_range *conf_corner_range, int conf_corner, int zdim)
{
...
gk_lcm_escape_barriers(
  ...
  const struct gkyl_array *phi_wall_up, const int *base_idx, int z_cell, int anchor_conf_corner,
  double mu, double charge, enum gkyl_gk_trapped_passing_orbit_type lower_orbit,
  enum gkyl_gk_trapped_passing_orbit_type upper_orbit, double *barrier_left, double *barrier_right
)
{
  int zdim = cdim - 1;
  ...
  int z_upper_corner =
    gk_lcm_conf_corner_with_z_side(conf_corner_range, anchor_conf_corner, zdim, 1);
  int z_lower_corner =
    gk_lcm_conf_corner_with_z_side(conf_corner_range, anchor_conf_corner, zdim, 0);
  ...
  *barrier_right = -DBL_MAX;
  ...
  for (int iz = conf_range->lower[zdim]; iz <= conf_range->upper[zdim]; ++iz) {
    ...
    double phi_lower =
      gk_lcm_field_corner_value(phi, basis_at_corners_conf, num_basis_conf, linidx, z_lower_corner);
    double phi_upper =
      gk_lcm_field_corner_value(phi, basis_at_corners_conf, num_basis_conf, linidx, z_upper_corner);
    double bmag_lower = gk_lcm_field_corner_value(
      bmag, basis_at_corners_conf, num_basis_conf, linidx, z_lower_corner
    );
    double bmag_upper = gk_lcm_field_corner_value(
      bmag, basis_at_corners_conf, num_basis_conf, linidx, z_upper_corner
    );
  ...
  if (lower_orbit == GKYL_GK_TRAP_PASS_ORBIT_TRAPPED_WALL) {
    *barrier_left = DBL_MAX;
    ...
    long linidx = gkyl_range_idx(conf_range, scan_idx);
    double bmag_wall = gk_lcm_field_corner_value(
      bmag, basis_at_corners_conf, num_basis_conf, linidx, z_lower_corner
    );
    double phi_wall = gk_lcm_field_corner_value(
      phi_wall_lo, basis_at_corners_conf, num_basis_conf, linidx, z_lower_corner
    );
  ...
  if (upper_orbit == GKYL_GK_TRAP_PASS_ORBIT_TRAPPED_WALL) {
    *barrier_right = DBL_MAX;
    ...
    double phi_wall = gk_lcm_field_corner_value(
      phi_wall_up, basis_at_corners_conf, num_basis_conf, linidx, z_upper_corner
    );

...
)
{
  ...
  if (use_gpu) {
    return gkyl_gk_collisionless_passive_flux_cu_dev_new(
      phase_grid, conf_basis, phase_basis, passive_speeds, charge, mass, gk_geom, dg_geom,
    ...
    for (int dir = 0; dir < cdim; dir++) {
      ...
      if (idx[dir] == phase_range->lower[dir]) {
        ...
        cflrate_d[0] += up->flux_surf_edge_lo[dir](
          ...
          jacgeo_rat_surfL_d, jacgeo_rat_surfR_d, speeds_L, speeds_R, fL, fR, flux_surf_d
        );
      } else {

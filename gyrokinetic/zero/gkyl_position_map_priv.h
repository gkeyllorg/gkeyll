...
position_map_constB_z_numeric(double t, const double *xn, double *fout, void *ctx)
{
  ...
  while (outside_region)
  {
    ...
    else if (fabs(interval_lower_eval) < 16.0*DBL_EPSILON*bscale || fabs(interval_upper_eval) < 16.0*DBL_EPSILON*bscale) {
      // If either evaluation is very close to zero, we're at or very near the solution
      // Just use the corresponding endpoint
      Theta = fabs(interval_lower_eval) < fabs(interval_upper_eval) ? interval_lower : interval_upper;
      goto map_value;
    }
    else {
  ...
  struct gkyl_qr_res res = gkyl_ridders(position_map_numeric_optimization_function, &ridders_ctx,
    interval_lower, interval_upper, interval_lower_eval, interval_upper_eval, 100, 1e-12);
  Theta = res.res;
map_value:
...
position_map_xpt_compression(double t, const double *xn, double *fout, void *ctx)
{
  ...
  double nonuniform_coordinate = zshift + (1.0-app->compression_factor)*sin(M_PI*zshift/zcut)*zcut/M_PI + app->zcenter;
  fout[0] = nonuniform_coordinate;
}
...
position_map_sep_compression(double t, const double *xn, double *fout, void *ctx)
{
  ...
  double nonuniform_coordinate = xshift - (1.0-app->radial_compression_factor)*sin(M_PI*xshift/w)*w/M_PI + app->psisep;
  fout[0] = nonuniform_coordinate;
}

...
)
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    up->rad_nu_mu(
      vmap_d, vmapSq_d, up->charge, up->mass, a, alpha, beta, gamma, v0, bmag_d, vsqnu_surf_d,
      vsqnu_d
    );
  }
}
...
)
{
  ...
  if (gkyl_array_is_cu_dev(nI)) {
    return gkyl_dg_calc_gk_rad_vars_nI_nu_advance_cu(
      up, conf_range, phase_range, vnu_surf, vnu, vsqnu_surf, vsqnu, n_elc_rad, n_elc, nI,
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    if (vtsq_d[0] > vtsq_min_d[0]) {
      ...
      up->rad_nI_nu(
        vnu_surf_d, vnu_d, vsqnu_surf_d, vsqnu_d, nI_d, nvnu_surf_d, nvnu_d, nvsqnu_surf_d, nvsqnu_d
      );
    }
  }
}

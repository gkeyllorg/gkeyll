...
check_diffusion(int cdim, int vdim, int poly_order, bool const_coeff, const bool *diff_in_dir)
{
  ...
  int diff_order = 2;
  struct gkyl_dg_eqn *eqn = gkyl_dg_diffusion_gyrokinetic_new(
    &basis, &cbasis, const_coeff, diff_in_dir, diff_order, &diff_range, false
  );
...
test_diff_ctor_acquire()
{
  ...
  struct gkyl_dg_eqn *eqn =
    gkyl_dg_diffusion_gyrokinetic_new(&basis, &cbasis, true, dir, 2, &diff_range, false);

...
static void test_L_domain_send_connections_dir0_cuts1()
{
...
static void test_L_domain_send_connections_dir0_cuts2()
{
  int num_blocks = 3; // L-shaped example.
  int ndim = 2;
  int cuts_flat1[] = {
...
static void test_L_domain_send_connections_dir0_cuts2_par()
{
  printf("\n");
  // Create world comm.
  bool use_mpi = true;
...
static void test_L_domain_recv_connections_dir0_cuts2_par()
{
  printf("\n");
  // Create world comm.
  bool use_mpi = true;
...
static void test_L_domain_allgather_dir0_cuts2_par()
{
  printf("\n");
  // Create world comm.
  bool use_mpi = true;
  ...
  for (int bI = 0; bI < num_local_blocks; bI++) {
    ...
    gkyl_multib_comm_conn_create_multib_ranges_in_dir(
      global_ranges_ext[bI], global_ranges[bI], nghost, nconnected[bid], block_list[bid], dir,
      decomp
    );
  ...
  for (int bI = 0; bI < num_local_blocks; bI++) {
    ...
    for (int nr = 0; nr < mbcc_recv[bI]->num_comm_conn; ++nr) {
      ...
      // Make range a subrange
      gkyl_sub_range_init(
        &mbcc_recv[bI]->comm_conn[nr].range, global_ranges_ext[bI],

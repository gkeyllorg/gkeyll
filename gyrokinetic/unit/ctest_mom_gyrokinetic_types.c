...
void test_mom_types_1x2v()
{
  ...
  struct {
    enum gkyl_distribution_moments mt;
    int nm;
  } cases[] = {{GKYL_F_MOMENT_M0, 1},
               {GKYL_F_MOMENT_M1, 1},
               {GKYL_F_MOMENT_M2, 1},
...
void test_int_mom_types_1x2v()
{
  ...
  for (int i = 0; i < ncase; i++) {
    struct gkyl_mom_type *m = gkyl_int_mom_gyrokinetic_new(
      &fx.confBasis, &fx.basis, &fx.confLocal, 1.0, 1.0, fx.gvm, fx.gk_geom, NULL, cases[i].mt,
      false
    );
    TEST_CHECK(m != NULL);
...
void test_mom_types_1x1v()
{
  ...
  struct {
    enum gkyl_distribution_moments mt;
    int nm;
  } cases[] = {{GKYL_F_MOMENT_M0, 1},    {GKYL_F_MOMENT_M1, 1},    {GKYL_F_MOMENT_M2, 1},
               {GKYL_F_MOMENT_M2PAR, 1}, {GKYL_F_MOMENT_M3PAR, 1}, {GKYL_F_MOMENT_M0M1M2, 3}};
  int ncase = sizeof(cases) / sizeof(cases[0]);

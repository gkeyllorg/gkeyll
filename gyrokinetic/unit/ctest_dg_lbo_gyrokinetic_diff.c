// Test construction of the gyrokinetic LBO diff DG equation object
// (gkyl_dg_lbo_gyrokinetic_diff_new). Verifies the base num_equations, the
...
lbo_env_init(struct lbo_env *e)
{
  ...
  e->gvm = gkyl_velocity_map_new(
    c2p_in, e->phaseGrid, velGrid, e->phaseRange, e->phaseRange_ext, velLocal, velLocal_ext, false
  );
}
...
test_lbo_diff_ctor()
{
  ...
  double mass = 2.5;
  struct gkyl_dg_eqn *eqn = gkyl_dg_lbo_gyrokinetic_diff_new(
    &e.confBasis, &e.basis, &e.confRange, &e.phaseGrid, mass, e.gk_geom, e.gvm, false
  );
  ...
  struct dg_lbo_gyrokinetic_diff *diff = container_of(eqn, struct dg_lbo_gyrokinetic_diff, eqn);

  TEST_CHECK(diff->cdim == 3);
  TEST_CHECK(diff->pdim == 5);
  TEST_CHECK(diff->mass == mass);
  TEST_CHECK(diff->num_cbasis == e.confBasis.num_basis);
  TEST_CHECK(diff->conf_range.volume == 512);
  TEST_CHECK(diff->gk_geom != NULL);
  TEST_CHECK(diff->vel_map != NULL);
  TEST_CHECK(diff->surf[0] != NULL);
  TEST_CHECK(diff->boundary_surf[0] != NULL);
...
test_lbo_diff_acquire()
{
  ...
  struct gkyl_dg_eqn *eqn = gkyl_dg_lbo_gyrokinetic_diff_new(
    &e.confBasis, &e.basis, &e.confRange, &e.phaseGrid, 1.0, e.gk_geom, e.gvm, false
  );

...
struct quadratic_map_ctx { double shift, scale, curvature; };
...
{
  return lower + dx*(quadrature ? node/2 + 0.5*(1.0 + (node%2 ? 1.0 : -1.0)/sqrt(3.0)) : node);
}

static void
check_mirror_mapping(bool analytic, enum gkyl_mirror_grid_gen_field_line_coord coord,
  struct quadratic_map_ctx map, bool map_all, bool include_axis)
{
  struct gkyl_rect_grid grid;
  gkyl_rect_grid_init(&grid, 3, (double[]) {0.02, -0.4, -0.75},
    (double[]) {0.2, 0.4, 0.75}, (int[]) {8, 2, 16});
  ...
  double slope_error = 0.0, tangent_error = 0.0, jacobian_error = 0.0, partition_error = 0.0, curl_error = 0.0, normal_field_error = 0.0;
      ...
      for (int cuts=1; cuts<=8 && cuts<=grid.cells[split_dim]; cuts*=2) {
        for (int rank=0; rank<cuts; ++rank) {
          ...
          for (int dim=0; dim<3; ++dim) {
            lower[dim] = global.lower[dim]; upper[dim] = global.upper[dim];
            if (dim==split_dim) {
              lower[dim] += rank*grid.cells[dim]/cuts;
              upper[dim] = lower[dim] + grid.cells[dim]/cuts - 1;
            }
            quad[dim] = kind==-2 || (kind>=0 && dim!=kind);
            shape[dim] = quad[dim] ? 2*(upper[dim]-lower[dim]+1) : upper[dim]-lower[dim]+2;
          }
          ...
          struct gkyl_mirror_grid_gen_inp inp = {
            .comp_grid = &grid, .local = local, .global = global, .nrange = nodes,
            .R = {0.0, 1.0}, .Z = {-1.0, 1.0}, .nrcells = 8, .nzcells = 16,
            .psiRZ = psi, .position_map = pmap, .fl_coord = coord, .dir = kind, .include_axis = include_axis,
          };
          struct gkyl_mirror_grid_gen *geom = kind==-2 ? gkyl_mirror_grid_gen_int_inew(&inp)
            : kind==-1 ? gkyl_mirror_grid_gen_inew(&inp) : gkyl_mirror_grid_gen_surf_inew(&inp);
          TEST_ASSERT(geom != 0);
          if (cuts==1) { full = geom; full_nodes = nodes; }
          ...
          while (gkyl_range_iter_next(&iter)) {
            ...
            long loc = gkyl_range_idx(&nodes, iter.idx), full_loc = gkyl_range_idx(&full_nodes, index);
            const struct gkyl_mirror_grid_gen_geom *geo = gkyl_array_cfetch(geom->nodes_geom, loc);
            const struct gkyl_mirror_grid_gen_geom *ref = gkyl_array_cfetch(full->nodes_geom, full_loc);
            ...
            double radius = rz[0], height = rz[1], bz = 1.0+height*height, br = -radius*height;
            ...
            double tangent_len = sqrt(geo->tang[2].x[0]*geo->tang[2].x[0]+geo->tang[2].x[2]*geo->tang[2].x[2]);
            ...
            for (int idir=0; idir<3; ++idir) {
              for (int jdir=0; jdir<3; ++jdir) {
                ...
                if (radius>0.0) tangent_error = fmax(tangent_error, fabs(dot-(idir==jdir)));
              }
            }
          }
          if (cuts!=1) gkyl_mirror_grid_gen_release(geom);
        }
      }
      gkyl_mirror_grid_gen_release(full);
    }
  }
  TEST_CHECK(slope_error < 2e-11); TEST_MSG("mapping slope error %.17g", slope_error);
  TEST_CHECK(tangent_error < 2e-13); TEST_MSG("reciprocal basis error %.17g", tangent_error);
  TEST_CHECK(jacobian_error < 2e-10); TEST_MSG("Jacobian error %.17g", jacobian_error);
  TEST_CHECK(curl_error < 2e-10); TEST_MSG("curl error %.17g", curl_error);
  TEST_CHECK(normal_field_error < 2e-10); TEST_MSG("normal B error %.17g", normal_field_error);
  TEST_CHECK(partition_error < 2e-12); TEST_MSG("partition error %.17g", partition_error);
  gkyl_position_map_release(pmap);
  gkyl_array_release(psi);
}

static void test_mirror_identity(void) {
  check_mirror_mapping(false, GKYL_GEOMETRY_MIRROR_GRID_GEN_PSI_CART_Z, (struct quadratic_map_ctx) {0.0, 1.0, 0.0}, false, false);
}
static void test_mirror_affine(void) {
  check_mirror_mapping(true, GKYL_GEOMETRY_MIRROR_GRID_GEN_PSI_CART_Z, (struct quadratic_map_ctx) {0.05, 0.75, 0.0}, false, false);
}
static void test_mirror_nonlinear_numeric(void) {
  check_mirror_mapping(false, GKYL_GEOMETRY_MIRROR_GRID_GEN_PSI_CART_Z, (struct quadratic_map_ctx) {0.05, 0.75, 0.12}, false, false);
}
static void test_mirror_nonlinear_analytic(void) {
  check_mirror_mapping(true, GKYL_GEOMETRY_MIRROR_GRID_GEN_PSI_CART_Z, (struct quadratic_map_ctx) {0.05, 0.75, 0.12}, false, false);
}
static void test_mirror_sqrt_psi(void) {
  check_mirror_mapping(false, GKYL_GEOMETRY_MIRROR_GRID_GEN_SQRT_PSI_CART_Z, (struct quadratic_map_ctx) {0.05, 0.75, 0.12}, false, false);
}

static void
curved_cartesian_map(double t, const double *xn, double *out, void *ctx)
{
  for (int dim=0; dim<3; ++dim) out[dim] = xn[dim];
  out[0] += 0.1*xn[2]*xn[2];
}
...
{
  out[0] = 0.4*xn[2]; out[1] = 0.0; out[2] = 2.0;
}

static void
compare_nodes(const struct gkyl_array *actual, const struct gkyl_array *expected,
  const struct gkyl_range *nodes, const struct gkyl_range *full_nodes,
  int split_dim, int node_offset, double *error, const char *name)
{
  struct gkyl_range_iter iter;
  gkyl_range_iter_init(&iter, nodes);
  while (gkyl_range_iter_next(&iter)) {
    ...
    for (int comp=0; comp<actual->ncomp; ++comp) {
      union { double value; uint64_t bits; } actual_bits = {.value = value[comp]}, ref_bits = {.value = ref[comp]};
      bool finite = (actual_bits.bits & UINT64_C(0x7ff0000000000000)) != UINT64_C(0x7ff0000000000000)
        && (ref_bits.bits & UINT64_C(0x7ff0000000000000)) != UINT64_C(0x7ff0000000000000);
      TEST_CHECK(finite);
      TEST_MSG("%s at %d %d %d component %d: actual %g reference %g", name,
        iter.idx[0], iter.idx[1], iter.idx[2], comp, value[comp], ref[comp]);
      if (!finite) return;
...
static void
check_geometry_partitions(bool tokamak, bool analytic)
{
  struct gkyl_rect_grid grid;
  double pi = M_PI;
  gkyl_rect_grid_init(&grid, 3, (double[]) {0.1, -0.4, -pi+1e-14},
    (double[]) {0.2, 0.4, pi-1e-14}, (int[]) {4, 2, 16});
  ...
  struct gkyl_position_map *pmap = gkyl_position_map_new(map_inp,
    grid, global, global_ext, global, global_ext, basis);
  struct gkyl_gk_geometry_inp inp = {
    .geometry_id = tokamak ? GKYL_GEOMETRY_TOKAMAK : GKYL_GEOMETRY_MAPC2P,
    .mapc2p = curved_cartesian_map, .bfield_func = curved_field, .position_map = pmap,
    .efit_info = {.filepath = "gyrokinetic/data/eqdsk/straight_cylinder.geqdsk",
      .rz_poly_order = 2, .flux_poly_order = 1, .reflect = true},
    .tok_grid_info = {.rclose = 0.5, .zmin = -1.0, .zmax = 1.0,
      .rleft = 0.001, .rmax = 1.0, .rright = 1.0},
    .grid = grid, .basis = basis, .global = global, .global_ext = global_ext,
    .geo_grid = grid, .geo_basis = basis, .geo_global = global, .geo_global_ext = global_ext,
    .local = global, .local_ext = global_ext, .geo_local = global, .geo_local_ext = global_ext,
  };
  struct gk_geometry *full = tokamak ? gkyl_gk_geometry_tok_new(&inp) : gkyl_gk_geometry_mapc2p_new(&inp);
    ...
    for (int cuts=2; cuts<=grid.cells[split_dim] && cuts<=8; cuts*=2) {
      for (int rank=0; rank<cuts; ++rank) {
        ...
        for (int dim=0; dim<3; ++dim) {
          ...
          elo[dim] = lower[dim]-1; eup[dim] = upper[dim]+1;
        }
        ...
        struct gk_geometry *geom = tokamak ? gkyl_gk_geometry_tok_new(&inp) : gkyl_gk_geometry_mapc2p_new(&inp);
        int offset = rank*grid.cells[split_dim]/cuts;
        compare_nodes(geom->geo_corn.mc2p_nodal, full->geo_corn.mc2p_nodal,
          &geom->nrange_corn, &full->nrange_corn, split_dim, offset, &error, "corners");
        compare_nodes(geom->geo_corn.mc2nu_pos_nodal, full->geo_corn.mc2nu_pos_nodal,
          &geom->nrange_corn, &full->nrange_corn, split_dim, offset, &error, "corners");
#define COMPARE_INT(field) compare_nodes(geom->geo_int.field, full->geo_int.field, \
  &geom->nrange_int, &full->nrange_int, split_dim, 2*offset, &error, "interior " #field)
        COMPARE_INT(mc2p_nodal); COMPARE_INT(jacobgeo_nodal); COMPARE_INT(g_ij_nodal);
        COMPARE_INT(dxdz_nodal); COMPARE_INT(dzdx_nodal); COMPARE_INT(bmag_nodal);
        COMPARE_INT(curlbhat_nodal); COMPARE_INT(B3_nodal);
#undef COMPARE_INT
        compare_nodes(geom->geo_int.bcart, full->geo_int.bcart,
          &geom->local, &full->local, split_dim, 0, &error, "modal Cartesian b");
        for (int dir=0; dir<3; ++dir) {
#define COMPARE_SURF(field) compare_nodes(geom->geo_surf[dir].field, full->geo_surf[dir].field, \
  &geom->nrange_surf[dir], &full->nrange_surf[dir], split_dim, (dir==split_dim ? 1 : 2)*offset, &error, "surface " #field)
          COMPARE_SURF(jacobgeo_nodal); COMPARE_SURF(g_ij_nodal);
          COMPARE_SURF(dxdz_nodal); COMPARE_SURF(dzdx_nodal); COMPARE_SURF(bmag_nodal);
          COMPARE_SURF(curlbhat_nodal); COMPARE_SURF(B3_nodal); COMPARE_SURF(lenr_nodal);
#undef COMPARE_SURF
          ...
          while (gkyl_range_iter_next(&iter)) {
            ...
            const double *tangent = gkyl_array_cfetch(geom->geo_surf[dir].dxdz_nodal,
              gkyl_range_idx(&geom->nrange_surf[dir], iter.idx));
            slope_error = fmax(slope_error, fabs(tangent[8]-slope/(tokamak ? pi : 1.0)));
            const double *curl = gkyl_array_cfetch(geom->geo_surf[dir].curlbhat_nodal,
              gkyl_range_idx(&geom->nrange_surf[dir], iter.idx));
            double zphys;
  ...
  TEST_CHECK(curl_error < 2e-6); TEST_MSG("analytic curl error %.17g", curl_error);
  TEST_CHECK(error < 2e-10); TEST_MSG("partition error %.17g", error);
  TEST_CHECK(slope_error < 2e-10); TEST_MSG("axial tangent error %.17g", slope_error);
  gkyl_gk_geometry_release(full);
  gkyl_position_map_release(pmap);
}

static void test_mapc2p_partitions(void) { check_geometry_partitions(false, false); }
static void test_tokamak_partitions_numeric(void) { check_geometry_partitions(true, false); }
static void test_tokamak_partitions_analytic(void) { check_geometry_partitions(true, true); }

static void test_mirror_all_maps(void) {
  check_mirror_mapping(false, GKYL_GEOMETRY_MIRROR_GRID_GEN_PSI_CART_Z,
    (struct quadratic_map_ctx) {0.05, 0.75, 0.12}, true, false);
}
static void test_mirror_nonuniform_axis(void) {
  check_mirror_mapping(false, GKYL_GEOMETRY_MIRROR_GRID_GEN_SQRT_PSI_CART_Z,

...
static double
analytic_phi(const struct loss_cone_mask_test_ctx *ctx, double z, bool use_nonzero_phi)
{
...
{
  return mu * analytic_bmag(ctx, z) + ctx->charge * analytic_phi(ctx, z, use_nonzero_phi);
}
...
)
{
  ...
  while (gkyl_range_iter_next(&phase_iter)) {
    ...
    while (cell_trapped && gkyl_range_iter_next(&corner_iter)) {
      ...
      double vpar = grid->lower[1] + (phase_iter.idx[1] - 1 + corner_iter.idx[1]) * grid->dx[1];
      double mu = grid->lower[2] + (phase_iter.idx[2] - 1 + corner_iter.idx[2]) * grid->dx[2];
      ...
      for (int vertex = 0; vertex <= ctx->Nz; ++vertex) {
        ...
        double u_scan = analytic_effective_potential(ctx, z_scan, mu, use_nonzero_phi);
        if (vertex <= z_vertex) {
...
run_case_1x2v(int poly_order, bool use_gpu, bool use_nonzero_phi)
{
  ...
  struct gkyl_velocity_map *gvm_ho =
    use_gpu ? gkyl_velocity_map_new(
  ...
  gkyl_eval_on_nodes *evbmag =
    gkyl_eval_on_nodes_new(&grid_conf, &basis_conf, 1, bmag_func_1x, &ctx);
  ...
  gkyl_loss_cone_mask_gyrokinetic_advance(proj_mask, &local, &local_conf, bmag, phi, 0, 0, mask);
  gkyl_array_copy(mask_ho, mask);

  gkyl_array_clear(mask_ref, 0.0);
  build_analytic_reference_mask_1x2v(&grid, &local, &ctx, use_nonzero_phi, mask_ref);
...
)
{
  ...
  return orbit == GKYL_GK_TRAP_PASS_ORBIT_TRAPPED_SHEATH && use_wall ?
           GKYL_MAX2(0.0, charge * (phi_wall - phi_plasma)) :
           0.0;
}
...
run_constant_boundary_cases(bool use_gpu)
{
  ...
  for (int c = 0; c < sizeof(cases) / sizeof(cases[0]); ++c) {
    ...
    double barrier_lo = constant_boundary_energy(
      test->charge, test->phi_plasma, test->phi_wall_lo, test->use_wall_lo, test->lower_orbit
    );
    double barrier_up = constant_boundary_energy(
      test->charge, test->phi_plasma, test->phi_wall_up, test->use_wall_up, test->upper_orbit
    );
...
TEST_LIST = {
  ...
  {"test_loss_cone_mask_1x2v_p1_nonzero_phi_gk_ho", test_loss_cone_mask_1x2v_p1_nonzero_phi_gk_ho},
  {"test_loss_cone_mask_1x2v_p1_constant_boundaries_gk_ho",
   test_loss_cone_mask_1x2v_p1_constant_boundaries_gk_ho},
#ifdef GKYL_HAVE_CUDA
  {"test_loss_cone_mask_1x2v_p1_gk_dev", test_loss_cone_mask_1x2v_p1_gk_dev},
  {"test_loss_cone_mask_1x2v_p1_nonzero_phi_gk_dev", test_loss_cone_mask_1x2v_p1_nonzero_phi_gk_dev
  },
  {"test_loss_cone_mask_1x2v_p1_constant_boundaries_gk_dev",
   test_loss_cone_mask_1x2v_p1_constant_boundaries_gk_dev},
#endif
  {NULL, NULL}
};

...
make_identity_vmap(int cdim, int vdim, double *plower, double *pupper, int *pcells)
{
  ...
  return gkyl_velocity_map_new(c2p_in, grid, vgrid, local, local_ext, vlocal, vlocal_ext, false);
}
...
check_bcorr(int cdim, int vdim, int poly_order, double *plower, double *pupper, int *pcells)
{
  ...
  double mass = 1.25;
  struct gkyl_mom_type *bcorr =
    gkyl_mom_bcorr_lbo_gyrokinetic_new(&cbasis, &pbasis, mass, gvm, false);
...
test_bcorr_ctor_acquire()
{
  ...
  struct gkyl_mom_type *bcorr =
    gkyl_mom_bcorr_lbo_gyrokinetic_new(&cbasis, &pbasis, 1.0, gvm, false);
...
TEST_LIST = {{"bcorr_ctor_1x1v_p1", test_bcorr_ctor_1x1v_p1},

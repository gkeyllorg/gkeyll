...
static void
calc_moms_vlasov(
  struct gkyl_rect_grid *grid, struct gkyl_rect_grid *velGrid, struct gkyl_basis *confBasis,
  struct gkyl_basis *basis, struct gkyl_range *confLocal, struct gkyl_range *local,
  struct gkyl_range *velLocal, bool use_gpu, struct gkyl_array *distf, struct gkyl_array *moms
)
{
  ...
  struct gkyl_vlasov_velocity_map *vvm =
    gkyl_vlasov_velocity_map_new(velGrid, velLocal, &vbasis, inp_vmap, false, use_gpu);
  ...
  gkyl_dg_vlasov_calc_hamil(
    velGrid, &vbasis, velLocal, GKYL_MODEL_DEFAULT, vvm, hamil, gamma_inv, use_gpu
  );
...
test_1x1v_vlasov(const int *cells, const int *cells_tar, int poly_order, bool use_gpu)
{
  ...
  calc_moms_vlasov(
    &grid, &velGrid, &confBasis, &basis, &confLocal, &local, &velLocal, use_gpu, distf, moms
  );
  ...
  calc_moms_vlasov(
    &grid_tar, &velGrid_tar, &confBasis, &basis, &confLocal_tar, &local_tar, &velLocal_tar, use_gpu,
    distf_tar, moms_tar
  );
...
)
{
  ...
  if (cdim < 3) {
    ...
    gkyl_create_grid_ranges(
      &geometry_input.geo_grid, geo_ghost, &geometry_input.geo_global_ext,
...
{
  struct gkyl_dg_updater_moment *mom_op = gkyl_dg_updater_moment_gyrokinetic_new(
    grid, confBasis, basis, confLocal, mass, charge, gvm, gk_geom, 0, GKYL_F_MOMENT_M0M1M2, false,
    use_gpu
  );
...
test_1x1v_gk(const int *cells, const int *cells_tar, int poly_order, bool use_gpu)
{
  ...
  struct gkyl_array *moms_ho = use_gpu ? mkarr(false, moms->ncomp, moms->size) :
                                         gkyl_array_acquire(moms);
  calc_moms_gk(
    &grid, &confBasis, &basis, &confLocal, &local, mass, charge, gvm, gk_geom, use_gpu, distf, moms
  );
  gkyl_array_copy(moms_ho, moms);
  ...
  struct gkyl_velocity_map *gvm_tar = gkyl_velocity_map_new(
    c2p_in, grid_tar, velGrid_tar, local_tar, local_tar_ext, velLocal_tar, velLocal_tar_ext, use_gpu
  );
  ...
  struct gkyl_array *moms_tar_ho = use_gpu ? mkarr(false, moms_tar->ncomp, moms_tar->size) :
                                             gkyl_array_acquire(moms_tar);
  calc_moms_gk(
    &grid_tar, &confBasis, &basis, &confLocal_tar, &local_tar, mass, charge, gvm_tar, gk_geom_tar,
    use_gpu, distf_tar, moms_tar
  );
  gkyl_array_copy(moms_tar_ho, moms_tar);

...
)
{
  ...
  if (fabs(z) <= app->z_m) {
    fout[0] = fmax(
      NSrcFloor, (NSrc / sqrt(2.0 * M_PI * pow(sigSrc, 2.))) *
...
static void run_2x_sim(
  struct gk_mirror_ctx ctx, const struct gkyl_app_args app_args, int *cells_x, int *cells_v,

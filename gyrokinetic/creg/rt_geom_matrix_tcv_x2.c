...
create_asdex_lsn_gk_block_geom(void *ctx)
{
  ...
  gkyl_gk_block_geom_set_block(bgeom, 0, &(struct gkyl_gk_block_geom_info) {
      ...
      .connections[0] = { // x-direction.
        { .bid = 1, .dir = 0, .edge = GKYL_UPPER_POSITIVE },  // Separatrix: b1 upper.
  ...
  gkyl_gk_block_geom_set_block(bgeom, 1, &(struct gkyl_gk_block_geom_info) {
      ...
      .connections[0] = { // x-direction.
        ...
        { .bid = 0, .dir = 0, .edge = GKYL_LOWER_POSITIVE },  // Separatrix: b0 lower.
      },
      .connections[1] = { // z-direction.
  ...
  gkyl_gk_block_geom_set_block(bgeom, 2, &(struct gkyl_gk_block_geom_info) {
      ...
      .connections[0] = { // x-direction.
        ...
        { .bid = 5, .dir = 0, .edge = GKYL_UPPER_POSITIVE },  // Separatrix: b5 upper.
      },
      .connections[1] = { // z-direction.
  ...
  gkyl_gk_block_geom_set_block(bgeom, 3, &(struct gkyl_gk_block_geom_info) {
      ...
      .connections[0] = { // x-direction.
        ...
        { .bid = 4, .dir = 0, .edge = GKYL_LOWER_POSITIVE },  // Separatrix: b4 lower.
      },
      .connections[1] = { // z-direction.
  ...
  gkyl_gk_block_geom_set_block(bgeom, 4, &(struct gkyl_gk_block_geom_info) {
      ...
      .connections[0] = { // x-direction.
        { .bid = 3, .dir = 0, .edge = GKYL_UPPER_POSITIVE },  // Separatrix: b3 upper.
  ...
  gkyl_gk_block_geom_set_block(bgeom, 5, &(struct gkyl_gk_block_geom_info) {
      ...
      .connections[0] = { // x-direction.
        ...
        { .bid = 2, .dir = 0, .edge = GKYL_UPPER_POSITIVE },  // Separatrix: b2 upper.
      },
      .connections[1] = { // z-direction.
...
create_ctx(void)
{
  ...
  int Npsi_pf = 8;
  int Npsi_core = 16;
  int Ntheta_divertor_lfs = 16;
  int Ntheta_divertor_hfs = 8;
  int Ntheta_sol = 48;
...
main(int argc, char **argv)
{
  ...
  // GEOMETRY ONLY: build the multiblock geometry, write it (including the
  // geo_corn_nodes files the quality check reads) and stop before any species,
  // field or time stepping exists.
  gkyl_gyrokinetic_multib_app *geom_app =
    gkyl_gyrokinetic_multib_app_new_geom(app_inp);
  if (!geom_app) {
    fprintf(stderr, "GEOM_FAIL: could not construct multiblock geometry app\n");
    return 1;
  }
  gkyl_gyrokinetic_multib_app_write_geometry(geom_app);
  gkyl_gyrokinetic_multib_app_release_geom(geom_app);
  fprintf(stderr, "GEOM_OK: multiblock geometry constructed and written\n");

...
void eval_density_ion(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  struct gk_app_ctx *app = ctx;
  double x = xn[0];
  // Gaussian envelope keeps the perturbation away from the x walls.
  double env = exp(-pow(x/app->pert_width, 2));
  fout[0] = app->n0*(1.0 + app->delta_n*env*sin(app->kx*(x - app->x_min)));
}

void eval_density_bc(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  struct gk_app_ctx *app = ctx;
  fout[0] = app->n0;
}
...
struct gk_app_ctx create_ctx(void)
{
  ...
  double Lx = 48*rho_s;   // Domain size along x (as in Goerler et al. 2011).
  ...
  // Zonal density perturbation.
  double delta_n = 1.0e-3; // e*phi0/Te ~ delta_n/(kx*rho_s)^2 ~ 0.06.
  double kx = 2.0*M_PI/Lx;
  double pert_width = Lx/4.0;

  // GAM frequency (Sugama & Watanabe, JPP 2006) and RH residual. Both use
  ...
  // printf("q0 = %g, eps = %g, RH residual = %g, t_gam = %g s\n", q0, inv_asp_ratio, residual_rh, t_gam);
...
main(int argc, char **argv)
{
  ...
  struct gkyl_gyrokinetic_species ion = {
    ...
    // Moment-corrected Maxwellian so the zonal charge is only the imposed perturbation.
    .projection = {
      .proj_id = GKYL_PROJ_MAXWELLIAN_PRIM,

...
struct gk_app_ctx {
  ...
  double a_shift, Z_axis, R_axis, R0, a_mid, x_inner, r0, B0, kappa, delta, q0, Cy, Bref;
  // Plasma parameters
  int num_species;
...
{
  // x is offset by the radial extent of the core region in the IWL test (0.04 m), so
  // that the profile is the same in physical space as in rt_gk_tcv_iwl_adapt_source_3x2v_p1.
  double x = xn[0] + 0.04, z = xn[2];
  struct gk_app_ctx *app = ctx;
  double n0 = 5e19;
  double x0 = -0.03;
  double c1 = 0.5;
...
struct gk_app_ctx create_ctx(void)
{
  ...
  // SOL-only domain: the inner radial boundary is the LCFS (complement of rt_gk_tcv_core_3x2v_p1).
  double x_inner = 0.00; // Radial extent inside LCFS
  ...
  // Core source (models the outflow from the core, placed next to the LCFS = inner radial boundary):
  // - Injects energy (P_inj per species).
  ...
  double center_srcRECY[3] = {x_min, 0.0, M_PI}; // Next to the LCFS, at the limiter.
  double sigma_srcRECY[3] = {0.01, 0.0, 0.05*Lz}; // Same radial width [m] as in the IWL test.
  ...
  int Nx = 16; // (16) Same radial cell size as the SOL part of the IWL test.
  int Ny = 4; // (16)
  int Nz = 8; // (12)
  int Nvpar = 8; // (12)
  int Nmu = 4; // (8)

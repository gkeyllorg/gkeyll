...
void
density_init_ion(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
...
void
density_init_elc(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  struct gk_app_ctx *app = ctx;
  double z = xn[1];
...
create_ctx(void)
{
  ...
  double omega =
    sqrt(pow(kz_min * va, 2) / (1 + beta_hat * pow(kperp_rhos, 2))); // KAW frequency [rad/s].
...
main(int argc, char **argv)
{
  ...
  struct gkyl_gyrokinetic_species elc = {
    ...
    .collisionless = {.type = GKYL_GK_COLLISIONLESS_EM},

...
test_em_coupling_static_Epush_1x_p1()
{
  ...
  struct gkyl_dg_calc_pkpm_em_coupling *up = gkyl_dg_calc_pkpm_em_coupling_new(
    &cbasis, &local, num_species, qbym, epsilon0, field_static, false
  );
  TEST_CHECK(up != NULL);
  ...
  gkyl_dg_calc_pkpm_em_coupling_advance(up, dt, app_accel, ext_em, app_current, moms, u, euler, em);

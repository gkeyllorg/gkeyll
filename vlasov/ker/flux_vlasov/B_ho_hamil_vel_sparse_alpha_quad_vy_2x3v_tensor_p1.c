...
GKYL_CU_DH double B_ho_hamil_vel_sparse_alpha_quad_vy_2x3v_tensor_p1_node(int i, int j, const double *dxv, const double *jacob_vel_surf,
...
GKYL_CU_DH void B_ho_hamil_vel_sparse_alpha_quad_vy_2x3v_tensor_p1(const double *dxv, const double *jacob_vel_surf,
  const double *hamil, const double *qmem, double* GKYL_RESTRICT alpha_quad) 
{ 
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 16; ++j) alpha_quad[i*16 + j] += B_ho_hamil_vel_sparse_alpha_quad_vy_2x3v_tensor_p1_node(i, j, dxv, jacob_vel_surf, hamil, qmem); 
  } 
} 

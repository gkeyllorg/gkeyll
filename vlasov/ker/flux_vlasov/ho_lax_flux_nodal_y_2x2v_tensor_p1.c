...
{ 
  ...
  for (int k = 0; k < 36; ++k) { 
    const int a = vst_2x2v_tensor_p1_ho_ph_x1_cmap[k]; 
    G_l[a] += vst_2x2v_tensor_p1_ho_ph_x1_V[j*18 + vst_2x2v_tensor_p1_ho_ph_x1_vlmap[k]]*(vst_2x2v_tensor_p1_ho_ph_x1_coefl[k]*f_l[k]); 
    G_r[a] += vst_2x2v_tensor_p1_ho_ph_x1_V[j*18 + vst_2x2v_tensor_p1_ho_ph_x1_vrmap[k]]*(vst_2x2v_tensor_p1_ho_ph_x1_coefr[k]*f_r[k]); 
  } 
  double f_l_quad = 0.0; 
  double f_r_quad = 0.0; 
  for (int a = 0; a < 2; ++a) { 
    f_l_quad += vst_2x2v_tensor_p1_ho_ph_x1_Cm[i*2 + a]*G_l[a]; 
    f_r_quad += vst_2x2v_tensor_p1_ho_ph_x1_Cm[i*2 + a]*G_r[a]; 
  } 
...
{ 
  ...
  for (int i = 0; i < 2; ++i) { 
    for (int j = 0; j < 16; ++j) { 
      alpha_max = fmax(alpha_max, ho_lax_flux_nodal_y_2x2v_tensor_p1_node(i, j, jacob_pos_l, jacob_pos_r, alpha_quad[i*16 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 

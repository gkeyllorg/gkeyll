...
GKYL_CU_DH double hamil_vel_sparse_alpha_quad_x_2x3v_tensor_p1_node(int i, int m, int hamil_pt_edge, const double *w, const double *dxv,
...
{ 
  for (int i = 0; i < 2; ++i) { 
    for (int m = 0; m < 27; ++m) alpha_quad[i*27 + m] += hamil_vel_sparse_alpha_quad_x_2x3v_tensor_p1_node(i, m, hamil_pt_edge, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 

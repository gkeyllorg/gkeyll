...
GKYL_CU_DH double hamil_vel_dense_ho_alpha_quad_x_2x2v_tensor_p1_node(int i, int m, int hamil_pt_edge, const double *w, const double *dxv,
...
{ 
  ...
  double dH_dv0 = 0.0; 
  for (int b = 0; b < 9; ++b) dH_dv0 += vst_2x2v_tensor_p1_ho_vel_vol_dv0[m*9 + b]*hamil[b]; 
  double dH_dv1 = 0.0; 
  for (int b = 0; b < 9; ++b) dH_dv1 += vst_2x2v_tensor_p1_ho_vel_vol_dv1[m*9 + b]*hamil[b]; 
  if (hamil_pt_edge == -1) { 
    double P0 = 0.0; 
    ...
    return P0*dH_dv0*dv10*(1.0/jacob_vel_surf[0 + m/4]) + P1*dH_dv1*dv11*(1.0/jacob_vel_surf[4 + m%4]); 
  } 
...
{ 
  for (int i = 0; i < 2; ++i) { 
    for (int m = 0; m < 16; ++m) alpha_quad[i*16 + m] += hamil_vel_dense_ho_alpha_quad_x_2x2v_tensor_p1_node(i, m, hamil_pt_edge, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 

...
GKYL_CU_DH double hamil_vel_dense_ho_alpha_quad_x_1x3v_tensor_p1_node(int i, int m, int hamil_pt_edge, const double *w, const double *dxv,
...
{ 
  ...
  double dH_dv0 = 0.0; 
  for (int b = 0; b < 27; ++b) dH_dv0 += vst_1x3v_tensor_p1_ho_vel_vol_dv0[m*27 + b]*hamil[b]; 
  double dH_dv1 = 0.0; 
  for (int b = 0; b < 27; ++b) dH_dv1 += vst_1x3v_tensor_p1_ho_vel_vol_dv1[m*27 + b]*hamil[b]; 
  double dH_dv2 = 0.0; 
  for (int b = 0; b < 27; ++b) dH_dv2 += vst_1x3v_tensor_p1_ho_vel_vol_dv2[m*27 + b]*hamil[b]; 
  if (hamil_pt_edge == -1) { 
    double P0 = 0.0; 
...
{ 
  for (int i = 0; i < 1; ++i) { 
    for (int m = 0; m < 64; ++m) alpha_quad[i*64 + m] += hamil_vel_dense_ho_alpha_quad_x_1x3v_tensor_p1_node(i, m, hamil_pt_edge, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 

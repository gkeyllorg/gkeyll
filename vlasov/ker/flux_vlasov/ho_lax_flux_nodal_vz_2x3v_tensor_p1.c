...
{ 
  ...
  for (int k = 0; k < 108; ++k) { 
    ...
    G_l[a] += vst_2x3v_tensor_p1_ho_ph_v2_V[j*27 + vst_2x3v_tensor_p1_ho_ph_v2_vlmap[k]]*(vst_2x3v_tensor_p1_ho_ph_v2_coefl[k]*f_l[k]); 
    G_r[a] += vst_2x3v_tensor_p1_ho_ph_v2_V[j*27 + vst_2x3v_tensor_p1_ho_ph_v2_vrmap[k]]*(vst_2x3v_tensor_p1_ho_ph_v2_coefr[k]*f_r[k]); 
  } 
  double f_l_quad = 0.0; 
  double f_r_quad = 0.0; 
  for (int a = 0; a < 4; ++a) { 
    f_l_quad += vst_2x3v_tensor_p1_ho_ph_v2_Cm[i*4 + a]*G_l[a]; 
    f_r_quad += vst_2x3v_tensor_p1_ho_ph_v2_Cm[i*4 + a]*G_r[a]; 
  } 
  const int n = i*16 + j; 
...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 16; ++j) { 
      alpha_max = fmax(alpha_max, ho_lax_flux_nodal_vz_2x3v_tensor_p1_node(i, j, jacob_vel_surf_r, alpha_quad[i*16 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 

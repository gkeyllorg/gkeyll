...
GKYL_CU_DH double nc_hamil_phase_ho_alpha_quad_vy_3x3v_tensor_p1_node(int i, int j, const double *w, const double *dxv,
...
{ 
  ...
  for (int k = 0; k < 216; ++k) { 
    const int a = vst_3x3v_tensor_p1_ho_ph_v1_cmap[k]; 
    G[a] += vst_3x3v_tensor_p1_ho_ph_v1_V[j*27 + vst_3x3v_tensor_p1_ho_ph_v1_vrmap[k]]*(vst_3x3v_tensor_p1_ho_ph_v1_coefr[k]*hamil[k]); 
    Gd0[a] += vst_3x3v_tensor_p1_ho_ph_v1_Vd0[j*19 + vst_3x3v_tensor_p1_ho_ph_v1_vrd0map[k]]*(vst_3x3v_tensor_p1_ho_ph_v1_dcoefr0[k]*hamil[k]); 
    Gd2[a] += vst_3x3v_tensor_p1_ho_ph_v1_Vd2[j*19 + vst_3x3v_tensor_p1_ho_ph_v1_vrd2map[k]]*(vst_3x3v_tensor_p1_ho_ph_v1_dcoefr2[k]*hamil[k]); 
  } 
  double dH_dv0 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dv0 += vst_3x3v_tensor_p1_ho_ph_v1_Cm[i*8 + a]*Gd0[a]; 
  double dH_dv2 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dv2 += vst_3x3v_tensor_p1_ho_ph_v1_Cm[i*8 + a]*Gd2[a]; 
  ...
  double px2_q = 0.0; 
  for (int a = 0; a < 8; ++a) { 
    px0_q += vst_3x3v_tensor_p1_ho_conf_ev[i*8 + a]*poisson_tensor_conf_x1[a]; 
    px1_q += vst_3x3v_tensor_p1_ho_conf_ev[i*8 + a]*poisson_tensor_conf_x4[a]; 
    px2_q += vst_3x3v_tensor_p1_ho_conf_ev[i*8 + a]*poisson_tensor_conf_x7[a]; 
  } 
  double dH_dx0 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dx0 += vst_3x3v_tensor_p1_ho_ph_v1_CmDx0[i*8 + a]*G[a]; 
  double dH_dx1 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dx1 += vst_3x3v_tensor_p1_ho_ph_v1_CmDx1[i*8 + a]*G[a]; 
  double dH_dx2 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dx2 += vst_3x3v_tensor_p1_ho_ph_v1_CmDx2[i*8 + a]*G[a]; 
  ...
  const double xn0 = vst_3x3v_tensor_p1_ho_vel_nodes_v1[j*2 + 0]; 
  ...
  const double xn2 = vst_3x3v_tensor_p1_ho_vel_nodes_v1[j*2 + 1]; 
...
GKYL_CU_DH void nc_hamil_phase_ho_alpha_quad_vy_3x3v_tensor_p1(const double *w, const double *dxv, const double *vmap,
  const double *jacob_pos, const double *jacob_vel_surf, const double *poisson_tensor_conf,
  const double *hamil, double* GKYL_RESTRICT alpha_quad) 
{ 
  for (int i = 0; i < 8; ++i) { 
    for (int j = 0; j < 16; ++j) alpha_quad[i*16 + j] += nc_hamil_phase_ho_alpha_quad_vy_3x3v_tensor_p1_node(i, j, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 

...
{ 
  ...
  for (int k = 0; k < 216; ++k) { 
    const int a = vst_3x3v_tensor_p1_ph_x0_cmap[k]; 
    G_l[a] += vst_3x3v_tensor_p1_ph_x0_V[j*54 + vst_3x3v_tensor_p1_ph_x0_vlmap[k]]*(vst_3x3v_tensor_p1_ph_x0_coefl[k]*f_l[k]); 
    G_r[a] += vst_3x3v_tensor_p1_ph_x0_V[j*54 + vst_3x3v_tensor_p1_ph_x0_vrmap[k]]*(vst_3x3v_tensor_p1_ph_x0_coefr[k]*f_r[k]); 
  } 
  double f_l_quad = 0.0; 
  double f_r_quad = 0.0; 
  for (int a = 0; a < 4; ++a) { 
    f_l_quad += vst_3x3v_tensor_p1_ph_x0_Cm[i*4 + a]*G_l[a]; 
    f_r_quad += vst_3x3v_tensor_p1_ph_x0_Cm[i*4 + a]*G_r[a]; 
  } 
...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 27; ++j) { 
      alpha_max = fmax(alpha_max, lax_flux_nodal_x_3x3v_tensor_p1_node(i, j, jacob_pos_l, jacob_pos_r, alpha_quad[i*27 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 

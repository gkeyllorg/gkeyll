...
GKYL_CU_DH double nc_hamil_vel_sparse_alpha_quad_vx_2x2v_tensor_p1_node(int i, int j, const double *w, const double *dxv,
...
{ 
  ...
  for (int s = 0; s < 5; ++s) { 
    ...
    dH_dv1 += vst_2x2v_tensor_p1_vel_dv1_v0[j*9 + b]*hamil[b]; 
  } 
...
GKYL_CU_DH void nc_hamil_vel_sparse_alpha_quad_vx_2x2v_tensor_p1(const double *w, const double *dxv, const double *vmap,
  const double *jacob_pos, const double *jacob_vel_surf, const double *poisson_tensor_conf,
  const double *hamil, double* GKYL_RESTRICT alpha_quad) 
{ 
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 3; ++j) alpha_quad[i*3 + j] += nc_hamil_vel_sparse_alpha_quad_vx_2x2v_tensor_p1_node(i, j, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 

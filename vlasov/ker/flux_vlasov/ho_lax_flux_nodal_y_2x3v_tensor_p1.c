...
{ 
  ...
  for (int k = 0; k < 108; ++k) { 
    const int a = vst_2x3v_tensor_p1_ho_ph_x1_cmap[k]; 
    G_l[a] += vst_2x3v_tensor_p1_ho_ph_x1_V[j*54 + vst_2x3v_tensor_p1_ho_ph_x1_vlmap[k]]*(vst_2x3v_tensor_p1_ho_ph_x1_coefl[k]*f_l[k]); 
    G_r[a] += vst_2x3v_tensor_p1_ho_ph_x1_V[j*54 + vst_2x3v_tensor_p1_ho_ph_x1_vrmap[k]]*(vst_2x3v_tensor_p1_ho_ph_x1_coefr[k]*f_r[k]); 
  } 
  double f_l_quad = 0.0; 
  double f_r_quad = 0.0; 
  for (int a = 0; a < 2; ++a) { 
    f_l_quad += vst_2x3v_tensor_p1_ho_ph_x1_Cm[i*2 + a]*G_l[a]; 
    f_r_quad += vst_2x3v_tensor_p1_ho_ph_x1_Cm[i*2 + a]*G_r[a]; 
  } 
  f_l_quad *= 1.0/jacob_pos_l[2]; 
  f_r_quad *= 1.0/jacob_pos_r[2]; 
  const int n = i*64 + j; 
  Fhat_nodal[128 + n] = 0.5*(alpha*(f_r_quad + f_l_quad) - fabs(alpha)*(f_r_quad - f_l_quad)); 
  return fabs(alpha); 
} 
...
{ 
  ...
  for (int i = 0; i < 2; ++i) { 
    for (int j = 0; j < 64; ++j) { 
      alpha_max = fmax(alpha_max, ho_lax_flux_nodal_y_2x3v_tensor_p1_node(i, j, jacob_pos_l, jacob_pos_r, alpha_quad[i*64 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 

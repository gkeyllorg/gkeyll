...
{ 
  ...
  for (int k = 0; k < 12; ++k) { 
    const int a = vst_2x1v_tensor_p1_ho_ph_v0_cmap[k]; 
    G_l[a] += vst_2x1v_tensor_p1_ho_ph_v0_V[j*3 + vst_2x1v_tensor_p1_ho_ph_v0_vlmap[k]]*(vst_2x1v_tensor_p1_ho_ph_v0_coefl[k]*f_l[k]); 
    G_r[a] += vst_2x1v_tensor_p1_ho_ph_v0_V[j*3 + vst_2x1v_tensor_p1_ho_ph_v0_vrmap[k]]*(vst_2x1v_tensor_p1_ho_ph_v0_coefr[k]*f_r[k]); 
  } 
  double f_l_quad = 0.0; 
  double f_r_quad = 0.0; 
  for (int a = 0; a < 4; ++a) { 
    f_l_quad += vst_2x1v_tensor_p1_ho_ph_v0_Cm[i*4 + a]*G_l[a]; 
    f_r_quad += vst_2x1v_tensor_p1_ho_ph_v0_Cm[i*4 + a]*G_r[a]; 
  } 
  const int n = i*1 + j; 
...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 1; ++j) { 
      alpha_max = fmax(alpha_max, ho_lax_flux_nodal_vx_2x1v_tensor_p1_node(i, j, jacob_vel_surf_r, alpha_quad[i*1 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 

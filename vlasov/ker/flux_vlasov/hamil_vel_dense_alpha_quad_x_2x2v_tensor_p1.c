...
GKYL_CU_DH double hamil_vel_dense_alpha_quad_x_2x2v_tensor_p1_node(int i, int m, int hamil_pt_edge, const double *w, const double *dxv,
...
{ 
  for (int i = 0; i < 2; ++i) { 
    for (int m = 0; m < 9; ++m) alpha_quad[i*9 + m] += hamil_vel_dense_alpha_quad_x_2x2v_tensor_p1_node(i, m, hamil_pt_edge, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 

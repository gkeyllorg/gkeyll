...
{ 
  ...
  for (int k = 0; k < 216; ++k) { 
    const int a = vst_3x3v_tensor_p1_ho_ph_x2_cmap[k]; 
    G_l[a] += vst_3x3v_tensor_p1_ho_ph_x2_V[j*54 + vst_3x3v_tensor_p1_ho_ph_x2_vlmap[k]]*(vst_3x3v_tensor_p1_ho_ph_x2_coefl[k]*f_l[k]); 
    G_r[a] += vst_3x3v_tensor_p1_ho_ph_x2_V[j*54 + vst_3x3v_tensor_p1_ho_ph_x2_vrmap[k]]*(vst_3x3v_tensor_p1_ho_ph_x2_coefr[k]*f_r[k]); 
  } 
  double f_l_quad = 0.0; 
  double f_r_quad = 0.0; 
  for (int a = 0; a < 4; ++a) { 
    f_l_quad += vst_3x3v_tensor_p1_ho_ph_x2_Cm[i*4 + a]*G_l[a]; 
    f_r_quad += vst_3x3v_tensor_p1_ho_ph_x2_Cm[i*4 + a]*G_r[a]; 
  } 
  f_l_quad *= 1.0/jacob_pos_l[4]; 
  f_r_quad *= 1.0/jacob_pos_r[4]; 
  const int n = i*64 + j; 
  Fhat_nodal[512 + n] = 0.5*(alpha*(f_r_quad + f_l_quad) - fabs(alpha)*(f_r_quad - f_l_quad)); 
  return fabs(alpha); 
} 
...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 64; ++j) { 
      alpha_max = fmax(alpha_max, ho_lax_flux_nodal_z_3x3v_tensor_p1_node(i, j, jacob_pos_l, jacob_pos_r, alpha_quad[i*64 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 

...
{ 
  ...
  for (int k = 0; k < 108; ++k) { 
    ...
    G_l[a] += vst_2x3v_tensor_p1_ph_v0_V[j*27 + vst_2x3v_tensor_p1_ph_v0_vlmap[k]]*(vst_2x3v_tensor_p1_ph_v0_coefl[k]*f_l[k]); 
    G_r[a] += vst_2x3v_tensor_p1_ph_v0_V[j*27 + vst_2x3v_tensor_p1_ph_v0_vrmap[k]]*(vst_2x3v_tensor_p1_ph_v0_coefr[k]*f_r[k]); 
  } 
  double f_l_quad = 0.0; 
  double f_r_quad = 0.0; 
  for (int a = 0; a < 4; ++a) { 
    f_l_quad += vst_2x3v_tensor_p1_ph_v0_Cm[i*4 + a]*G_l[a]; 
    f_r_quad += vst_2x3v_tensor_p1_ph_v0_Cm[i*4 + a]*G_r[a]; 
  } 
  const int n = i*9 + j; 
...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 9; ++j) { 
      alpha_max = fmax(alpha_max, lax_flux_nodal_vx_2x3v_tensor_p1_node(i, j, jacob_vel_surf_r, alpha_quad[i*9 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 

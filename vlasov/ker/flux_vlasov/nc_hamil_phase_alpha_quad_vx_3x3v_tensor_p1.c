...
GKYL_CU_DH double nc_hamil_phase_alpha_quad_vx_3x3v_tensor_p1_node(int i, int j, const double *w, const double *dxv,
...
{ 
  ...
  for (int k = 0; k < 216; ++k) { 
    ...
    G[a] += vst_3x3v_tensor_p1_ph_v0_V[j*27 + vst_3x3v_tensor_p1_ph_v0_vrmap[k]]*(vst_3x3v_tensor_p1_ph_v0_coefr[k]*hamil[k]); 
    Gd1[a] += vst_3x3v_tensor_p1_ph_v0_Vd1[j*19 + vst_3x3v_tensor_p1_ph_v0_vrd1map[k]]*(vst_3x3v_tensor_p1_ph_v0_dcoefr1[k]*hamil[k]); 
    Gd2[a] += vst_3x3v_tensor_p1_ph_v0_Vd2[j*19 + vst_3x3v_tensor_p1_ph_v0_vrd2map[k]]*(vst_3x3v_tensor_p1_ph_v0_dcoefr2[k]*hamil[k]); 
  } 
  double dH_dv1 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dv1 += vst_3x3v_tensor_p1_ph_v0_Cm[i*8 + a]*Gd1[a]; 
  double dH_dv2 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dv2 += vst_3x3v_tensor_p1_ph_v0_Cm[i*8 + a]*Gd2[a]; 
  ...
  double px2_q = 0.0; 
  for (int a = 0; a < 8; ++a) { 
    px0_q += vst_3x3v_tensor_p1_conf_ev[i*8 + a]*poisson_tensor_conf_x0[a]; 
    px1_q += vst_3x3v_tensor_p1_conf_ev[i*8 + a]*poisson_tensor_conf_x3[a]; 
    px2_q += vst_3x3v_tensor_p1_conf_ev[i*8 + a]*poisson_tensor_conf_x6[a]; 
  } 
  double dH_dx0 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dx0 += vst_3x3v_tensor_p1_ph_v0_CmDx0[i*8 + a]*G[a]; 
  double dH_dx1 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dx1 += vst_3x3v_tensor_p1_ph_v0_CmDx1[i*8 + a]*G[a]; 
  double dH_dx2 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dx2 += vst_3x3v_tensor_p1_ph_v0_CmDx2[i*8 + a]*G[a]; 
  ...
  const double xn1 = vst_3x3v_tensor_p1_vel_nodes_v0[j*2 + 0]; 
  ...
  const double xn2 = vst_3x3v_tensor_p1_vel_nodes_v0[j*2 + 1]; 
...
GKYL_CU_DH void nc_hamil_phase_alpha_quad_vx_3x3v_tensor_p1(const double *w, const double *dxv, const double *vmap,
  const double *jacob_pos, const double *jacob_vel_surf, const double *poisson_tensor_conf,
  const double *hamil, double* GKYL_RESTRICT alpha_quad) 
{ 
  for (int i = 0; i < 8; ++i) { 
    for (int j = 0; j < 9; ++j) alpha_quad[i*9 + j] += nc_hamil_phase_alpha_quad_vx_3x3v_tensor_p1_node(i, j, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 

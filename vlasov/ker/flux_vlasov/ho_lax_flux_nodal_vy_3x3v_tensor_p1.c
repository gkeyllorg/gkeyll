...
{ 
  ...
  for (int k = 0; k < 216; ++k) { 
    ...
    G_l[a] += vst_3x3v_tensor_p1_ho_ph_v1_V[j*27 + vst_3x3v_tensor_p1_ho_ph_v1_vlmap[k]]*(vst_3x3v_tensor_p1_ho_ph_v1_coefl[k]*f_l[k]); 
    G_r[a] += vst_3x3v_tensor_p1_ho_ph_v1_V[j*27 + vst_3x3v_tensor_p1_ho_ph_v1_vrmap[k]]*(vst_3x3v_tensor_p1_ho_ph_v1_coefr[k]*f_r[k]); 
  } 
  double f_l_quad = 0.0; 
  double f_r_quad = 0.0; 
  for (int a = 0; a < 8; ++a) { 
    f_l_quad += vst_3x3v_tensor_p1_ho_ph_v1_Cm[i*8 + a]*G_l[a]; 
    f_r_quad += vst_3x3v_tensor_p1_ho_ph_v1_Cm[i*8 + a]*G_r[a]; 
  } 
  const int n = i*16 + j; 
...
{ 
  ...
  for (int i = 0; i < 8; ++i) { 
    for (int j = 0; j < 16; ++j) { 
      alpha_max = fmax(alpha_max, ho_lax_flux_nodal_vy_3x3v_tensor_p1_node(i, j, jacob_vel_surf_r, alpha_quad[i*16 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 

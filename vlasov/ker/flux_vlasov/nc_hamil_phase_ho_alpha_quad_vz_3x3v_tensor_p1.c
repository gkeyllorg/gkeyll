...
GKYL_CU_DH double nc_hamil_phase_ho_alpha_quad_vz_3x3v_tensor_p1_node(int i, int j, const double *w, const double *dxv,
...
{ 
  ...
  for (int k = 0; k < 216; ++k) { 
    const int a = vst_3x3v_tensor_p1_ho_ph_v2_cmap[k]; 
    G[a] += vst_3x3v_tensor_p1_ho_ph_v2_V[j*27 + vst_3x3v_tensor_p1_ho_ph_v2_vrmap[k]]*(vst_3x3v_tensor_p1_ho_ph_v2_coefr[k]*hamil[k]); 
    Gd0[a] += vst_3x3v_tensor_p1_ho_ph_v2_Vd0[j*19 + vst_3x3v_tensor_p1_ho_ph_v2_vrd0map[k]]*(vst_3x3v_tensor_p1_ho_ph_v2_dcoefr0[k]*hamil[k]); 
    Gd1[a] += vst_3x3v_tensor_p1_ho_ph_v2_Vd1[j*19 + vst_3x3v_tensor_p1_ho_ph_v2_vrd1map[k]]*(vst_3x3v_tensor_p1_ho_ph_v2_dcoefr1[k]*hamil[k]); 
  } 
  double dH_dv0 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dv0 += vst_3x3v_tensor_p1_ho_ph_v2_Cm[i*8 + a]*Gd0[a]; 
  double dH_dv1 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dv1 += vst_3x3v_tensor_p1_ho_ph_v2_Cm[i*8 + a]*Gd1[a]; 
  ...
  double px2_q = 0.0; 
  for (int a = 0; a < 8; ++a) { 
    px0_q += vst_3x3v_tensor_p1_ho_conf_ev[i*8 + a]*poisson_tensor_conf_x2[a]; 
    px1_q += vst_3x3v_tensor_p1_ho_conf_ev[i*8 + a]*poisson_tensor_conf_x5[a]; 
    px2_q += vst_3x3v_tensor_p1_ho_conf_ev[i*8 + a]*poisson_tensor_conf_x8[a]; 
  } 
  double dH_dx0 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dx0 += vst_3x3v_tensor_p1_ho_ph_v2_CmDx0[i*8 + a]*G[a]; 
  double dH_dx1 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dx1 += vst_3x3v_tensor_p1_ho_ph_v2_CmDx1[i*8 + a]*G[a]; 
  double dH_dx2 = 0.0; 
  for (int a = 0; a < 8; ++a) dH_dx2 += vst_3x3v_tensor_p1_ho_ph_v2_CmDx2[i*8 + a]*G[a]; 
...
GKYL_CU_DH void nc_hamil_phase_ho_alpha_quad_vz_3x3v_tensor_p1(const double *w, const double *dxv, const double *vmap,
  const double *jacob_pos, const double *jacob_vel_surf, const double *poisson_tensor_conf,
  const double *hamil, double* GKYL_RESTRICT alpha_quad) 
{ 
  for (int i = 0; i < 8; ++i) { 
    for (int j = 0; j < 16; ++j) alpha_quad[i*16 + j] += nc_hamil_phase_ho_alpha_quad_vz_3x3v_tensor_p1_node(i, j, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 

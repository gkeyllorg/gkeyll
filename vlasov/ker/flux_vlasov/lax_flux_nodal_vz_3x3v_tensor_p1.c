...
{ 
  ...
  for (int k = 0; k < 216; ++k) { 
    ...
    G_l[a] += vst_3x3v_tensor_p1_ph_v2_V[j*27 + vst_3x3v_tensor_p1_ph_v2_vlmap[k]]*(vst_3x3v_tensor_p1_ph_v2_coefl[k]*f_l[k]); 
    G_r[a] += vst_3x3v_tensor_p1_ph_v2_V[j*27 + vst_3x3v_tensor_p1_ph_v2_vrmap[k]]*(vst_3x3v_tensor_p1_ph_v2_coefr[k]*f_r[k]); 
  } 
  double f_l_quad = 0.0; 
  double f_r_quad = 0.0; 
  for (int a = 0; a < 8; ++a) { 
    f_l_quad += vst_3x3v_tensor_p1_ph_v2_Cm[i*8 + a]*G_l[a]; 
    f_r_quad += vst_3x3v_tensor_p1_ph_v2_Cm[i*8 + a]*G_r[a]; 
  } 
  const int n = i*9 + j; 
  const double jac = jacob_vel_surf_r[0 + j/3]*jacob_vel_surf_r[3 + j%3]; 
  Fhat_nodal[144 + n] = 0.5*jac*(alpha*(f_r_quad + f_l_quad) - fabs(alpha)*(f_r_quad - f_l_quad)); 
  return fabs(alpha); 
} 
...
{ 
  ...
  for (int i = 0; i < 8; ++i) { 
    for (int j = 0; j < 9; ++j) { 
      alpha_max = fmax(alpha_max, lax_flux_nodal_vz_3x3v_tensor_p1_node(i, j, jacob_vel_surf_r, alpha_quad[i*9 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 

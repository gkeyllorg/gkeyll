...
GKYL_CU_DH double nc_hamil_phase_alpha_quad_vy_1x3v_tensor_p1_node(int i, int j, const double *w, const double *dxv,
...
{ 
  ...
  double dH_dv0 = 0.0; 
  for (int a = 0; a < 2; ++a) dH_dv0 += vst_1x3v_tensor_p1_ph_v1_Cm[i*2 + a]*Gd0[a]; 
  double dH_dv2 = 0.0; 
  for (int a = 0; a < 2; ++a) dH_dv2 += vst_1x3v_tensor_p1_ph_v1_Cm[i*2 + a]*Gd2[a]; 
  ...
  double dH_dx0 = 0.0; 
  for (int a = 0; a < 2; ++a) dH_dx0 += vst_1x3v_tensor_p1_ph_v1_CmDx0[i*2 + a]*G[a]; 
...
GKYL_CU_DH void nc_hamil_phase_alpha_quad_vy_1x3v_tensor_p1(const double *w, const double *dxv, const double *vmap,
  const double *jacob_pos, const double *jacob_vel_surf, const double *poisson_tensor_conf,
  const double *hamil, double* GKYL_RESTRICT alpha_quad) 
{ 
  for (int i = 0; i < 2; ++i) { 
    for (int j = 0; j < 9; ++j) alpha_quad[i*9 + j] += nc_hamil_phase_alpha_quad_vy_1x3v_tensor_p1_node(i, j, w, dxv, vmap, jacob_pos, jacob_vel_surf, poisson_tensor_conf, hamil); 
  } 
} 

...
{ 
  ...
  for (int k = 0; k < 36; ++k) { 
    ...
    G_l[a] += vst_2x2v_tensor_p1_ph_v1_V[j*9 + vst_2x2v_tensor_p1_ph_v1_vlmap[k]]*(vst_2x2v_tensor_p1_ph_v1_coefl[k]*f_l[k]); 
    G_r[a] += vst_2x2v_tensor_p1_ph_v1_V[j*9 + vst_2x2v_tensor_p1_ph_v1_vrmap[k]]*(vst_2x2v_tensor_p1_ph_v1_coefr[k]*f_r[k]); 
  } 
  double f_l_quad = 0.0; 
  double f_r_quad = 0.0; 
  for (int a = 0; a < 4; ++a) { 
    f_l_quad += vst_2x2v_tensor_p1_ph_v1_Cm[i*4 + a]*G_l[a]; 
    f_r_quad += vst_2x2v_tensor_p1_ph_v1_Cm[i*4 + a]*G_r[a]; 
  } 
  const int n = i*3 + j; 
  const double jac = jacob_vel_surf_r[0 + j]; 
  Fhat_nodal[12 + n] = 0.5*jac*(alpha*(f_r_quad + f_l_quad) - fabs(alpha)*(f_r_quad - f_l_quad)); 
  return fabs(alpha); 
} 
...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    for (int j = 0; j < 3; ++j) { 
      alpha_max = fmax(alpha_max, lax_flux_nodal_vy_2x2v_tensor_p1_node(i, j, jacob_vel_surf_r, alpha_quad[i*3 + j], f_l, f_r, Fhat_nodal)); 
    } 
  } 

...
GKYL_CU_DH double fpo_vlasov_diff_boundary_surfvyvy_1x3v_ser_p1_upvy(const double *dxv, const double *diff_coeff_C,
  ...
  const double *DC = &diff_coeff_C[64]; 
  ...
  f_rec_up[0] = 1.224744871391589*fC[3]+0.7071067811865475*fC[0]; 
  f_rec_up[1] = 1.224744871391589*fC[6]+0.7071067811865475*fC[1]; 
  f_rec_up[2] = 1.224744871391589*fC[7]+0.7071067811865475*fC[2]; 
  f_rec_up[3] = 1.224744871391589*fC[10]+0.7071067811865475*fC[4]; 
  f_rec_up[4] = 1.224744871391589*fC[11]+0.7071067811865475*fC[5]; 
  f_rec_up[5] = 1.224744871391589*fC[13]+0.7071067811865475*fC[8]; 
  f_rec_up[6] = 1.224744871391589*fC[14]+0.7071067811865475*fC[9]; 
  f_rec_up[7] = 1.224744871391589*fC[15]+0.7071067811865475*fC[12]; 
  ...
  surft2_up[0] = 0.4330127018922193*(f_rec_up[7]*DC[15]+f_rec_up[6]*DC[14]+f_rec_up[5]*DC[13]+f_rec_up[4]*DC[11]+f_rec_up[3]*DC[10]+f_rec_up[2]*DC[7]+f_rec_up[1]*DC[6]+f_rec_up[0]*DC[3])+0.25*(f_rec_up[7]*DC[12]+f_rec_up[6]*DC[9]+f_rec_up[5]*DC[8]+f_rec_up[4]*DC[5]+f_rec_up[3]*DC[4]+DC[2]*f_rec_up[2]+DC[1]*f_rec_up[1]+DC[0]*f_rec_up[0]); 
  surft2_up[1] = 0.4330127018922193*(f_rec_up[6]*DC[15]+f_rec_up[7]*DC[14]+f_rec_up[3]*DC[13]+f_rec_up[2]*DC[11]+f_rec_up[5]*DC[10]+f_rec_up[4]*DC[7]+f_rec_up[0]*DC[6]+f_rec_up[1]*DC[3])+0.25*(f_rec_up[6]*DC[12]+f_rec_up[7]*DC[9]+f_rec_up[3]*DC[8]+DC[4]*f_rec_up[5]+f_rec_up[2]*DC[5]+DC[2]*f_rec_up[4]+DC[0]*f_rec_up[1]+f_rec_up[0]*DC[1]); 
  surft2_up[2] = 0.4330127018922193*(f_rec_up[5]*DC[15]+f_rec_up[3]*DC[14]+f_rec_up[7]*DC[13]+f_rec_up[1]*DC[11]+f_rec_up[6]*DC[10]+f_rec_up[0]*DC[7]+f_rec_up[4]*DC[6]+f_rec_up[2]*DC[3])+0.25*(f_rec_up[5]*DC[12]+f_rec_up[3]*DC[9]+f_rec_up[7]*DC[8]+DC[4]*f_rec_up[6]+f_rec_up[1]*DC[5]+DC[1]*f_rec_up[4]+DC[0]*f_rec_up[2]+f_rec_up[0]*DC[2]); 
  surft2_up[3] = 0.4330127018922193*(f_rec_up[4]*DC[15]+f_rec_up[2]*DC[14]+f_rec_up[1]*DC[13]+f_rec_up[7]*DC[11]+f_rec_up[0]*DC[10]+f_rec_up[6]*DC[7]+f_rec_up[5]*DC[6]+DC[3]*f_rec_up[3])+0.25*(f_rec_up[4]*DC[12]+f_rec_up[2]*DC[9]+f_rec_up[1]*DC[8]+DC[5]*f_rec_up[7]+DC[2]*f_rec_up[6]+DC[1]*f_rec_up[5]+f_rec_up[0]*DC[4]+DC[0]*f_rec_up[3]); 
  surft2_up[4] = 0.4330127018922193*(f_rec_up[3]*DC[15]+f_rec_up[5]*DC[14]+f_rec_up[6]*DC[13]+f_rec_up[0]*DC[11]+f_rec_up[7]*DC[10]+f_rec_up[1]*DC[7]+f_rec_up[2]*DC[6]+DC[3]*f_rec_up[4])+0.25*(f_rec_up[3]*DC[12]+f_rec_up[5]*DC[9]+f_rec_up[6]*DC[8]+DC[4]*f_rec_up[7]+f_rec_up[0]*DC[5]+DC[0]*f_rec_up[4]+DC[1]*f_rec_up[2]+f_rec_up[1]*DC[2]); 
  surft2_up[5] = 0.4330127018922193*(f_rec_up[2]*DC[15]+f_rec_up[4]*DC[14]+f_rec_up[0]*DC[13]+f_rec_up[6]*DC[11]+f_rec_up[1]*DC[10]+DC[7]*f_rec_up[7]+f_rec_up[3]*DC[6]+DC[3]*f_rec_up[5])+0.25*(f_rec_up[2]*DC[12]+f_rec_up[4]*DC[9]+f_rec_up[0]*DC[8]+DC[2]*f_rec_up[7]+DC[5]*f_rec_up[6]+DC[0]*f_rec_up[5]+f_rec_up[1]*DC[4]+DC[1]*f_rec_up[3]); 
  surft2_up[6] = 0.4330127018922193*(f_rec_up[1]*DC[15]+f_rec_up[0]*DC[14]+f_rec_up[4]*DC[13]+f_rec_up[5]*DC[11]+f_rec_up[2]*DC[10]+DC[6]*f_rec_up[7]+f_rec_up[3]*DC[7]+DC[3]*f_rec_up[6])+0.25*(f_rec_up[1]*DC[12]+f_rec_up[0]*DC[9]+f_rec_up[4]*DC[8]+DC[1]*f_rec_up[7]+DC[0]*f_rec_up[6]+DC[5]*f_rec_up[5]+f_rec_up[2]*DC[4]+DC[2]*f_rec_up[3]); 
  surft2_up[7] = 0.4330127018922193*(f_rec_up[0]*DC[15]+f_rec_up[1]*DC[14]+f_rec_up[2]*DC[13]+f_rec_up[3]*DC[11]+f_rec_up[4]*DC[10]+DC[3]*f_rec_up[7]+f_rec_up[5]*DC[7]+DC[6]*f_rec_up[6])+0.25*(f_rec_up[0]*DC[12]+f_rec_up[1]*DC[9]+f_rec_up[2]*DC[8]+DC[0]*f_rec_up[7]+DC[1]*f_rec_up[6]+DC[2]*f_rec_up[5]+f_rec_up[3]*DC[5]+DC[4]*f_rec_up[4]); 

  vol[3] = 0.75*(fC[12]*DC[15]+fC[9]*DC[14]+fC[8]*DC[13]+fC[5]*DC[11]+fC[4]*DC[10]+fC[2]*DC[7]+fC[1]*DC[6]+fC[0]*DC[3]); 
  vol[6] = 0.75*(fC[9]*DC[15]+fC[12]*DC[14]+fC[4]*DC[13]+fC[2]*DC[11]+fC[8]*DC[10]+fC[5]*DC[7]+fC[0]*DC[6]+fC[1]*DC[3]); 
  vol[7] = 0.75*(fC[8]*DC[15]+fC[4]*DC[14]+fC[12]*DC[13]+fC[1]*DC[11]+fC[9]*DC[10]+fC[0]*DC[7]+fC[5]*DC[6]+fC[2]*DC[3]); 
  vol[10] = 0.75*(fC[5]*DC[15]+fC[2]*DC[14]+fC[1]*DC[13]+DC[11]*fC[12]+fC[0]*DC[10]+DC[7]*fC[9]+DC[6]*fC[8]+DC[3]*fC[4]); 
  vol[11] = 0.75*(fC[4]*DC[15]+fC[8]*DC[14]+fC[9]*DC[13]+DC[10]*fC[12]+fC[0]*DC[11]+fC[1]*DC[7]+fC[2]*DC[6]+DC[3]*fC[5]); 
  vol[13] = 0.75*(fC[2]*DC[15]+fC[5]*DC[14]+fC[0]*DC[13]+DC[7]*fC[12]+fC[9]*DC[11]+fC[1]*DC[10]+DC[3]*fC[8]+fC[4]*DC[6]); 
  vol[14] = 0.75*(fC[1]*DC[15]+fC[0]*DC[14]+fC[5]*DC[13]+DC[6]*fC[12]+fC[8]*DC[11]+fC[2]*DC[10]+DC[3]*fC[9]+fC[4]*DC[7]); 
  vol[15] = 0.75*(fC[0]*DC[15]+fC[1]*DC[14]+fC[2]*DC[13]+DC[3]*fC[12]+fC[4]*DC[11]+fC[5]*DC[10]+DC[6]*fC[9]+DC[7]*fC[8]); 

  out[0] += (0.5*vol[0]-0.35355339059327373*surft1_lo[0])*dv_inv_sq; 
  out[1] += (0.5*vol[1]-0.35355339059327373*surft1_lo[1])*dv_inv_sq; 
  out[2] += (0.5*vol[2]-0.35355339059327373*surft1_lo[2])*dv_inv_sq; 
  out[3] += (0.5*vol[3]-0.6123724356957945*surft2_up[0]+0.6123724356957945*(surft2_lo[0]+surft1_lo[0]))*dv_inv_sq; 
  out[4] += (0.5*vol[4]-0.35355339059327373*surft1_lo[3])*dv_inv_sq; 
  out[5] += (0.5*vol[5]-0.35355339059327373*surft1_lo[4])*dv_inv_sq; 
  out[6] += (0.5*vol[6]-0.6123724356957945*surft2_up[1]+0.6123724356957945*(surft2_lo[1]+surft1_lo[1]))*dv_inv_sq; 
  ...
  out[9] += (0.5*vol[9]-0.35355339059327373*surft1_lo[6])*dv_inv_sq; 
  out[10] += (0.5*vol[10]-0.6123724356957945*surft2_up[3]+0.6123724356957945*(surft2_lo[3]+surft1_lo[3]))*dv_inv_sq; 
  out[11] += (0.5*vol[11]-0.6123724356957945*surft2_up[4]+0.6123724356957945*(surft2_lo[4]+surft1_lo[4]))*dv_inv_sq; 
  out[12] += (0.5*vol[12]-0.35355339059327373*surft1_lo[7])*dv_inv_sq; 
  out[13] += (0.5*vol[13]-0.6123724356957945*surft2_up[5]+0.6123724356957945*(surft2_lo[5]+surft1_lo[5]))*dv_inv_sq; 

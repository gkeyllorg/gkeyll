...
GKYL_CU_DH void vlasov_self_prim_moments_2x2v_tensor_p1(struct gkyl_mat *A, struct gkyl_mat *rhs, const double *moms, const double *boundary_corrections, const double *nu) 
{ 
  ...
  if (notCellAvg) { 
    ...
    gkyl_mat_set(rhs,8,0,2.0*moms[12]); 
    gkyl_mat_set(rhs,9,0,2.0*moms[13]); 
    gkyl_mat_set(rhs,10,0,2.0*moms[14]); 
    gkyl_mat_set(rhs,11,0,2.0*moms[15]); 
  } else { 
    m0r[0] = moms[0]; 
    m0r[1] = 0.0; 
    m0r[2] = 0.0; 

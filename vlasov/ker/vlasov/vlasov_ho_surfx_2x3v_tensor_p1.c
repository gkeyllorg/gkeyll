...
{ 
  ...
  for (int i = 0; i < 2; ++i) { 
    ...
    double t_r = 0.0; 
    for (int j = 0; j < 64; ++j) { 
      t_l += vst_2x3v_tensor_p1_ho_prj_x0_Vw[j*27 + b]*Fhat_l_nodal[0 + i*64 + j]; 
      t_r += vst_2x3v_tensor_p1_ho_prj_x0_Vw[j*27 + b]*Fhat_r_nodal[0 + i*64 + j]; 
    } 
    g_l += vst_2x3v_tensor_p1_ho_prj_x0_Cw[i*2 + a]*t_l; 
    g_r += vst_2x3v_tensor_p1_ho_prj_x0_Cw[i*2 + a]*t_r; 
  } 

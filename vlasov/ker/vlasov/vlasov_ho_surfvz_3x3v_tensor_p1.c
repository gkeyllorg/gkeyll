...
{ 
  ...
  for (int i = 0; i < 8; ++i) { 
    ...
    double t_r = 0.0; 
    for (int j = 0; j < 16; ++j) { 
      t_l += vst_3x3v_tensor_p1_ho_prj_v2_Vw[j*9 + b]*Fhat_l_nodal[256 + i*16 + j]; 
      t_r += vst_3x3v_tensor_p1_ho_prj_v2_Vw[j*9 + b]*Fhat_r_nodal[256 + i*16 + j]; 
    } 
    g_l += vst_3x3v_tensor_p1_ho_prj_v2_Cw[i*8 + a]*t_l; 
    g_r += vst_3x3v_tensor_p1_ho_prj_v2_Cw[i*8 + a]*t_r; 
  } 

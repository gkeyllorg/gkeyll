...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    ...
    double t_r = 0.0; 
    for (int j = 0; j < 16; ++j) { 
      t_l += vst_2x3v_tensor_p1_ho_prj_v1_Vw[j*9 + b]*Fhat_l_nodal[64 + i*16 + j]; 
      t_r += vst_2x3v_tensor_p1_ho_prj_v1_Vw[j*9 + b]*Fhat_r_nodal[64 + i*16 + j]; 
    } 
    g_l += vst_2x3v_tensor_p1_ho_prj_v1_Cw[i*4 + a]*t_l; 
    g_r += vst_2x3v_tensor_p1_ho_prj_v1_Cw[i*4 + a]*t_r; 
  } 
  for (int q = vst_2x3v_tensor_p1_ho_prj_v1_out_off[k]; q < vst_2x3v_tensor_p1_ho_prj_v1_out_off[k+1]; ++q) { 
    out[vst_2x3v_tensor_p1_ho_prj_v1_out_mode[q]] += dv11*(vst_2x3v_tensor_p1_ho_prj_v1_out_cl[q]*g_l + vst_2x3v_tensor_p1_ho_prj_v1_out_cr[q]*g_r); 
  } 
  } 
  return 0.0;
} 

...
{ 
  ...
  for (int i = 0; i < 8; ++i) { 
    double t = 0.0; 
    for (int j = 0; j < 9; ++j) { 
      t += vst_3x3v_tensor_p1_prj_v2_Vw[j*9 + b]*flux[144 + i*9 + j]; 
    } 
    g += vst_3x3v_tensor_p1_prj_v2_Cw[i*8 + a]*t; 
  } 
  if (edge == -1) { 
    for (int q = vst_3x3v_tensor_p1_prj_v2_out_off[k]; q < vst_3x3v_tensor_p1_prj_v2_out_off[k+1]; ++q) { 
      out[vst_3x3v_tensor_p1_prj_v2_out_mode[q]] += dv12*vst_3x3v_tensor_p1_prj_v2_out_cr[q]*g; 
    } 
  } else { 
    for (int q = vst_3x3v_tensor_p1_prj_v2_out_off[k]; q < vst_3x3v_tensor_p1_prj_v2_out_off[k+1]; ++q) { 
      out[vst_3x3v_tensor_p1_prj_v2_out_mode[q]] += dv12*vst_3x3v_tensor_p1_prj_v2_out_cl[q]*g; 
    } 
  } 
  } 
  return 0.0;

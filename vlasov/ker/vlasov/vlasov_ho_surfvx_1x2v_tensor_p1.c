...
{ 
  ...
  for (int i = 0; i < 2; ++i) { 
    ...
    double t_r = 0.0; 
    for (int j = 0; j < 4; ++j) { 
      t_l += vst_1x2v_tensor_p1_ho_prj_v0_Vw[j*3 + b]*Fhat_l_nodal[0 + i*4 + j]; 
      t_r += vst_1x2v_tensor_p1_ho_prj_v0_Vw[j*3 + b]*Fhat_r_nodal[0 + i*4 + j]; 
    } 
    g_l += vst_1x2v_tensor_p1_ho_prj_v0_Cw[i*2 + a]*t_l; 
    g_r += vst_1x2v_tensor_p1_ho_prj_v0_Cw[i*2 + a]*t_r; 
  } 

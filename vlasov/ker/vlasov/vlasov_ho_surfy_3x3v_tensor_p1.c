...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    ...
    double t_r = 0.0; 
    for (int j = 0; j < 64; ++j) { 
      t_l += vst_3x3v_tensor_p1_ho_prj_x1_Vw[j*27 + b]*Fhat_l_nodal[256 + i*64 + j]; 
      t_r += vst_3x3v_tensor_p1_ho_prj_x1_Vw[j*27 + b]*Fhat_r_nodal[256 + i*64 + j]; 
    } 
    g_l += vst_3x3v_tensor_p1_ho_prj_x1_Cw[i*4 + a]*t_l; 
    g_r += vst_3x3v_tensor_p1_ho_prj_x1_Cw[i*4 + a]*t_r; 
  } 
  for (int q = vst_3x3v_tensor_p1_ho_prj_x1_out_off[k]; q < vst_3x3v_tensor_p1_ho_prj_x1_out_off[k+1]; ++q) { 
    out[vst_3x3v_tensor_p1_ho_prj_x1_out_mode[q]] += dx11*(vst_3x3v_tensor_p1_ho_prj_x1_out_cl[q]*g_l + vst_3x3v_tensor_p1_ho_prj_x1_out_cr[q]*g_r); 
  } 
  } 
  return 0.0;
} 

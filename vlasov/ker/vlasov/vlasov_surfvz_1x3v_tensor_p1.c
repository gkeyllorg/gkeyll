...
{ 
  ...
  for (int i = 0; i < 2; ++i) { 
    ...
    double t_r = 0.0; 
    for (int j = 0; j < 9; ++j) { 
      t_l += vst_1x3v_tensor_p1_prj_v2_Vw[j*9 + b]*Fhat_l_nodal[36 + i*9 + j]; 
      t_r += vst_1x3v_tensor_p1_prj_v2_Vw[j*9 + b]*Fhat_r_nodal[36 + i*9 + j]; 
    } 
    g_l += vst_1x3v_tensor_p1_prj_v2_Cw[i*2 + a]*t_l; 
    g_r += vst_1x3v_tensor_p1_prj_v2_Cw[i*2 + a]*t_r; 
  } 

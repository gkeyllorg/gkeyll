...
{ 
  ...
  for (int i = 0; i < 4; ++i) { 
    ...
    double t_r = 0.0; 
    for (int j = 0; j < 27; ++j) { 
      t_l += vst_3x3v_tensor_p1_prj_x2_Vw[j*27 + b]*Fhat_l_nodal[216 + i*27 + j]; 
      t_r += vst_3x3v_tensor_p1_prj_x2_Vw[j*27 + b]*Fhat_r_nodal[216 + i*27 + j]; 
    } 
    g_l += vst_3x3v_tensor_p1_prj_x2_Cw[i*4 + a]*t_l; 
    g_r += vst_3x3v_tensor_p1_prj_x2_Cw[i*4 + a]*t_r; 
  } 

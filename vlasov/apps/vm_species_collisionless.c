  ...
  if (vms->field_id == GKYL_FIELD_E_B) {
    ...
    if (cls->em_no_J) {
      gkyl_vlasov_position_map_divide_jacobpos_conf(
        vms->pos_map, &app->local, cls->em_no_J->ncomp, em, cls->em_no_J
      );
      em_force = cls->em_no_J;
    }
    gkyl_array_accumulate_range(cls->qmem, cls->qbym, em_force, &app->local);
  } else if (cls->has_gr_em_triad_coupling) {
    gkyl_dg_gr_maxwell_lorentz_conf_advance(
      cls->calc_lorentz, &app->local, app->vm_geom->lapse, app->vm_geom->shift, app->vm_geom->h_ij,
      app->vm_geom->h_ij_inv, app->vm_geom->det_h, app->vm_geom->vierb_cov, app->vm_geom->vierb_con,
      em, cls->qmem
    );
...
static void
vm_species_collisionless_rhs_enabled(
  gkyl_vlasov_app *app, struct vm_species *vms, struct vm_collisionless *cls,
...
)
{
  ...
  gkyl_vlasov_velocity_map_divide_jacobvel(
    vms->vel_map, &app->basis, &vms->basis, &vms->local, fin, vms->f_no_J
  );
  ...
  if (vms->model_id == GKYL_MODEL_TRIAD || vms->hamil_id == GKYL_HAMIL_PHASE) {
    gkyl_dg_vlasov_conf_flux_surf_advance(
      cls->calc_conf_flux, &app->local, &vms->local, &vms->local_ext, vms->conf_poisson_tensor,
      vms->hamil, fin, vms->cflrate, cls->conf_flux_surf
    );
  }

  // Compute the surface expansion of the phase space flux in velocity space.
  gkyl_dg_vlasov_vel_flux_surf_advance(
    cls->calc_vel_flux, &app->local, &vms->local, vms->conf_poisson_tensor, vms->hamil, cls->qmem,
    cls->pot_tot, vms->rad, vms->f_no_J, vms->cflrate, cls->vel_flux_surf
  );
...
static void
vm_species_collisionless_rhs_disabled(
  gkyl_vlasov_app *app, struct vm_species *vms, struct vm_collisionless *cls,
  const struct gkyl_array *fin, const struct gkyl_array *em, struct gkyl_array *rhs
)
{
...
bool
vm_species_has_gr_em_triad_coupling(
...
)
{
  ...
  cls->em_no_J = ((vms->field_id == GKYL_FIELD_E_B) && (!vms->pos_map->is_identity)) ?
                   mkarr(app->use_gpu, 8 * app->basis.num_basis, app->local_ext.volume) :
                   0;
  ...
  if (vms->info.app_accel) {
    ...
    cls->app_accel_proj = gkyl_proj_on_basis_new(
      &app->grid, &app->basis, app->basis.poly_order + 1, 3, vms->info.app_accel,
      vms->info.app_accel_ctx
    );
  }
  ...
  cls->slvr = gkyl_hyper_dg_new(
    &vms->grid, &vms->basis, cls->eqn, num_up_dirs, up_dirs, zero_flux_flags, 1, app->use_gpu
  );
...
void
vm_species_collisionless_release(
...
)
{
  ...
  if (cls->em_no_J) {
    gkyl_array_release(cls->em_no_J);
  }

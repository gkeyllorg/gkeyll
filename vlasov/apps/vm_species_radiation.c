...
)
{
  ...
  gkyl_proj_on_basis *proj_nu = gkyl_proj_on_basis_new(
    &app->grid, &app->confBasis, app->poly_order + 1, 1, s->info.radiation.nu,
  ...
  gkyl_proj_on_basis *proj_rad_drag = gkyl_proj_on_basis_new(
    &app->grid, &app->confBasis, app->poly_order + 1, vdim + 1, s->info.radiation.nu_rad_drag,
    s->info.radiation.ctx_nu_rad_drag
  );

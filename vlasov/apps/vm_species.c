...
vm_species_new_hamil(struct gkyl_vm *vm_app_inp, struct gkyl_vlasov_app *app, struct vm_species *vms)
{
  ...
  vms->conf_poisson_tensor =
    mkarr(app->use_gpu, app->basis.num_basis * num_pt_indices[vdim - 1], app->local_ext.volume);
  vms->conf_poisson_tensor_host = vms->conf_poisson_tensor;
  if (app->use_gpu) {
    vms->conf_poisson_tensor_host =
      mkarr(false, app->basis.num_basis * num_pt_indices[vdim - 1], app->local_ext.volume);
  }
  ...
  if (vms->model_id == GKYL_MODEL_DEFAULT || vms->model_id == GKYL_MODEL_SR) {
    ...
    gkyl_dg_vlasov_calc_hamil(
      &vms->grid_vel, &vms->basis_vel, &vms->local_vel, vms->model_id, vms->vel_map, vms->mom_hamil,
      vms->gamma_inv, app->use_gpu
    );
    ...
    if (vms->model_id == GKYL_MODEL_SR) {
      vms->sr_vars = gkyl_dg_calc_sr_vars_new(
        &vms->grid, &vms->grid_vel, &app->basis, &vms->basis_vel, &app->local, &vms->local_vel,
    ...
    vms->gamma_inv = mkarr(app->use_gpu, vms->basis_vel.num_basis, vms->local_vel.volume);
    gkyl_dg_vlasov_calc_hamil(
      &vms->grid_vel, &vms->basis_vel, &vms->local_vel, GKYL_MODEL_DEFAULT, vms->vel_map,
      vms->mom_hamil, vms->gamma_inv, app->use_gpu
    );
    ...
    } else if ((vms->info.triad_basis) && (vms->info.triad_basis_gradient) &&
    ...
    gkyl_vlasov_triad_geom_new(
      &app->grid, &app->local, app->basis, &vms->grid, &vms->local, vms->basis, inp_triad_geom,
      vms->conf_poisson_tensor_host
    );
    ...
    vms->gamma_inv = mkarr(app->use_gpu, vms->basis_vel.num_basis, vms->local_vel.volume);
    gkyl_dg_vlasov_calc_hamil(
      &vms->grid_vel, &vms->basis_vel, &vms->local_vel, GKYL_MODEL_SR, vms->vel_map, vms->mom_hamil,
      vms->gamma_inv, app->use_gpu
    );
    ...
    for (int d = 0; d < app->cdim; ++d) {
      gkyl_array_shiftc(
        vms->conf_poisson_tensor, pow(sqrt(2.0), app->cdim), app->basis.num_basis * (d * vdim + d)
      );
    }
    ...
    vms->h_ij =
      mkarr(app->use_gpu, app->basis.num_basis * vdim * (vdim + 1) / 2, app->local_ext.volume);
    vms->h_ij_host = vms->h_ij;
    if (app->use_gpu) {
      vms->h_ij_host =
        mkarr(false, app->basis.num_basis * vdim * (vdim + 1) / 2, app->local_ext.volume);
    }
    ...
    if (vms->info.use_extended_hamil_def) {
      ...
      vms->background_flows =
        mkarr(app->use_gpu, app->basis.num_basis * vdim, app->local_ext.volume);
      vms->background_flows_host = vms->background_flows;
      if (app->use_gpu) {
        vms->background_flows_host =
          mkarr(false, app->basis.num_basis * vdim, app->local_ext.volume);
      }
    ...
    struct gkyl_eval_on_nodes *h_ij_proj = gkyl_eval_on_nodes_inew(&(struct gkyl_eval_on_nodes_inp){
      .grid = &app->grid,
      .basis = &app->basis,
      .num_ret_vals = vdim * (vdim + 1) / 2,
      .eval = vms->info.h_ij,
      .ctx = vms->info.h_ij_ctx,
      .c2p_func = vm_species_c2p_conf,
      .c2p_func_ctx = &c2p_ctx,
    });
    ...
    struct gkyl_eval_on_nodes *h_ij_inv_proj =
      gkyl_eval_on_nodes_inew(&(struct gkyl_eval_on_nodes_inp){
        ...
        .basis = &app->basis,
        .num_ret_vals = vdim * (vdim + 1) / 2,
        .eval = vms->info.h_ij_inv,
        .ctx = vms->info.h_ij_inv_ctx,
        .c2p_func = vm_species_c2p_conf,
        .c2p_func_ctx = &c2p_ctx,
      });
    ...
    if (vms->info.use_extended_hamil_def) {
      struct gkyl_eval_on_nodes *background_flows_proj =
        gkyl_eval_on_nodes_inew(&(struct gkyl_eval_on_nodes_inp){
          .grid = &app->grid,
          .basis = &app->basis,
          .num_ret_vals = vdim,
          .eval = vms->info.background_flows,
          .ctx = vms->info.background_flows_ctx,
          .c2p_func = vm_species_c2p_conf,
          .c2p_func_ctx = &c2p_ctx,
        });
      gkyl_eval_on_nodes_advance(
        background_flows_proj, 0.0, &app->local, vms->background_flows_host
      );
      if (app->use_gpu) {
      ...
      struct gkyl_eval_on_nodes *effective_potential_proj =
        gkyl_eval_on_nodes_inew(&(struct gkyl_eval_on_nodes_inp){
          .grid = &app->grid,
          .basis = &app->basis,
          .num_ret_vals = 1,
          .eval = vms->info.effective_potential,
          .ctx = vms->info.effective_potential_ctx,
          .c2p_func = vm_species_c2p_conf,
          .c2p_func_ctx = &c2p_ctx,
        });
      gkyl_eval_on_nodes_advance(
        effective_potential_proj, 0.0, &app->local, vms->effective_potential_host
      );
      if (app->use_gpu) {
...
static void
vm_species_new_radiation(
  struct gkyl_vm *vm_app_inp, struct gkyl_vlasov_app *app, struct vm_species *vms
)
{
  int vdim = app->vdim;
  vms->has_rad = false;
  ...
  if (radiation_id == GKYL_VM_COMPTON_RADIATION || radiation_id == GKYL_VM_CURVATURE_RADIATION) {
    ...
    gkyl_dg_vlasov_calc_radiation(
      &vms->grid_vel, &vms->basis_vel, &vms->local_vel, radiation_id, vms->vel_map,
      vms->info.radiation.t_cool, vms->info.radiation.p0, vms->rad, app->use_gpu
    );
  }
}
...
double
vm_species_rhs_implicit_dynamic(
  gkyl_vlasov_app *app, struct vm_species *vms, const struct gkyl_array *fin,
...
static double
vm_species_rhs_implicit_static(
  gkyl_vlasov_app *app, struct vm_species *vms, const struct gkyl_array *fin,
  struct gkyl_array *rhs, double dt
)
{
...
)
{
  ...
  gkyl_comm_array_per_sync(
    vms->comm, &vms->local, &vms->local_ext, num_periodic_dir, app->periodic_dirs, f
  );
...
static void
vm_species_step_f_dynamic(struct gkyl_array *out, double dt, const struct gkyl_array *inp)
{
  gkyl_array_accumulate(gkyl_array_scale(out, dt), 1.0, inp);
}

static void
vm_species_step_f_static(struct gkyl_array *out, double dt, const struct gkyl_array *inp)
{
  // do nothing
}

static void
vm_species_combine_dynamic(
...
vm_species_write_dynamic(gkyl_vlasov_app *app, struct vm_species *vms, double tm, int frame)
{
  ...
  gkyl_vlasov_velocity_map_divide_jacobvel(
    vms->vel_map, &app->basis, &vms->basis, &vms->local, vms->f, vms->f_no_J
  );
...
vm_species_write_cfl_enabled(gkyl_vlasov_app *app, struct vm_species *vms, double tm, int frame)
{
  ...
  gkyl_comm_array_write(vms->comm, &vms->grid, &vms->local, mt, vms->cflrate_host, fileNm);
...
static void
vm_species_write_cell_avg_disabled(
  gkyl_vlasov_app *app, struct vm_species *vms, double tm, int frame
)
{
  // do nothing
}
...
vm_species_write_lte_enabled(gkyl_vlasov_app *app, struct vm_species *vms, double tm, int frame)
{
  ...
  // the velocity-space Jacobian at specific quadrature points.
  gkyl_vlasov_velocity_map_divide_jacobvel(
    vms->vel_map, &app->basis, &vms->basis, &vms->local, vms->lte.f_lte, vms->f_no_J
  );
  gkyl_vlasov_position_map_divide_jacobpos(
    vms->pos_map, &vms->basis, &vms->local, vms->f_no_J, vms->f_no_J
  );
...
vm_species_write_mom_dynamic(gkyl_vlasov_app *app, struct vm_species *vms, double tm, int frame)
{
  ...
  for (int m = 0; m < vms->info.num_diag_moments; ++m) {
    ...
    if (!vms->pos_map->is_identity) {
      int num_coeff_divide = vms->moms[m].is_vlasov_lte_moms ? app->basis.num_basis :
                                                               vms->moms[m].marr_host->ncomp;
      gkyl_vlasov_position_map_divide_jacobpos_conf(
        vms->pos_map, &app->local, num_coeff_divide, vms->moms[m].marr_host, vms->moms[m].marr_host
      );
    }
    ...
    gkyl_comm_array_write(app->comm, &app->grid, &app->local, mt, vms->moms[m].marr_host, fileNm);
    app->stat.species_diag_io_tm += gkyl_time_diff_now_sec(wst);
  ...
  for (int d = 0; d < cdim; ++d) {
    ...
    if (vms->lower_bc[d].type == GKYL_SPECIES_EMISSION) {
      ...
      vm_species_emission_init(
        app, &vms->bc_emission_lo, d, GKYL_LOWER_EDGE, vms->lower_bc[d].aux_ctx
      );
    } else {
      ...
      vms->bc_lo[d] = gkyl_bc_basic_new(
        d, GKYL_LOWER_EDGE, bctype, vms->basis_on_dev, &vms->lower_skin[d], &vms->lower_ghost[d],
        vms->f->ncomp, cdim, app->use_gpu
      );
    }
    ...
    if (vms->upper_bc[d].type == GKYL_SPECIES_EMISSION) {
      ...
      vms->calc_bflux = true;
      vm_species_emission_init(
        app, &vms->bc_emission_up, d, GKYL_UPPER_EDGE, vms->upper_bc[d].aux_ctx
      );
    } else {
      ...
      vms->bc_up[d] = gkyl_bc_basic_new(
        d, GKYL_UPPER_EDGE, bctype, vms->basis_on_dev, &vms->upper_skin[d], &vms->upper_ghost[d],
        vms->f->ncomp, cdim, app->use_gpu
      );
    }
  }
...
vm_species_check_hyb_build(struct gkyl_vlasov_app *app, struct vm_species *vms)
{
  ...
  if (cdim == 3 && vdim == 3) {
    gkyl_exit(
      "vm_species: the 3x3v tensor p=1 hybrid kernels were not built. Reconfigure with --build-vlasov-hyb-3x3v=yes."
    );
  }
#endif
  ...
  if (cdim == 3 && vdim == 3 && vms->hamil_id == GKYL_HAMIL_PHASE) {
    gkyl_exit(
      "vm_species: the 3x3v tensor p=1 hybrid phase-space Hamiltonian kernels were not built. Reconfigure with --build-vlasov-hyb-3x3v-phase=yes (and --build-vlasov-hyb-3x3v=yes)."
    );
  }
#endif
}
...
vm_species_init(struct gkyl_vm *vm_app_inp, struct gkyl_vlasov_app *app, struct vm_species *vms)
{
  ...
  vms->vel_map = gkyl_vlasov_velocity_map_new(
    &vms->grid_vel, &vms->local_vel, &vms->basis_vel, inp_vmap, vms->info.use_lo, app->use_gpu
  );
  ...
  gkyl_vlasov_velocity_map_write(vms->vel_map, vms->comm, app->name, vms->name);
  ...
  for (int dir = 0; dir < cdim; ++dir) {
    ...
    gkyl_skin_ghost_ranges(
      &vms->lower_skin[dir], &vms->lower_ghost[dir], dir, GKYL_LOWER_EDGE, &vms->local_ext, ghost
    );
    // Create local upper skin and ghost ranges for distribution function
    gkyl_skin_ghost_ranges(
      &vms->upper_skin[dir], &vms->upper_ghost[dir], dir, GKYL_UPPER_EDGE, &vms->local_ext, ghost
    );
  }
  ...
  for (int dir = 0; dir < cdim; ++dir) {
    gkyl_skin_ghost_ranges(
      &vms->global_lower_skin[dir], &vms->global_lower_ghost[dir], dir, GKYL_LOWER_EDGE,
      &vms->global_ext, ghost
    );
    gkyl_skin_ghost_ranges(
      &vms->global_upper_skin[dir], &vms->global_upper_ghost[dir], dir, GKYL_UPPER_EDGE,
      &vms->global_ext, ghost
    );

    gkyl_sub_range_intersect(
      &vms->global_lower_skin[dir], &vms->local_ext, &vms->global_lower_skin[dir]
    );
    gkyl_sub_range_intersect(
      &vms->global_upper_skin[dir], &vms->local_ext, &vms->global_upper_skin[dir]
    );

    gkyl_sub_range_intersect(
      &vms->global_lower_ghost[dir], &vms->local_ext, &vms->global_lower_ghost[dir]
    );
    gkyl_sub_range_intersect(
      &vms->global_upper_ghost[dir], &vms->local_ext, &vms->global_upper_ghost[dir]
    );
  }

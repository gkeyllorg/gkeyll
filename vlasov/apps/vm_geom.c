...
vm_dg_maxwell_geom_new(struct gkyl_vm *vm_app_inp, struct gkyl_vlasov_app *app, struct vm_geom *vmg)
{
  ...
  struct gkyl_dg_gr_maxwell_geom_ctx ctx = {.mass_bh = vmg->mass_bh, .spin_bh = vmg->spin_bh};
  ...
  for (int i = 0; i < app->cdim; ++i) {
    const enum gkyl_field_bc_type *bc = i == 0 ? vm_app_inp->field.bcx :
                                        i == 1 ? vm_app_inp->field.bcy :
                                                 vm_app_inp->field.bcz;
  ...
  struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *lapse_proj =
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_inew(&(struct gkyl_dg_gr_maxwell_surf_and_vol_nodes_inp){
      .grid = &app->grid,
      .basis = &app->basis,
      .num_ret_vals = 1,
      .polyorder = vm_app_inp->poly_order,
      .eval = gkyl_dg_gr_maxwell_preset_lapse(vmg->triad_preset_geom_type),
      .ctx = &ctx,
      // Sample the GR geometry at physical conf coordinates on mapped meshes.
      .c2p_func = vm_geom_pos_c2p,
      .c2p_func_ctx = app->pos_map,
    });
  vmg->lapse_init =
    gkyl_surf_and_vol_node_arrays_new(lapse_proj, app->local_ext.volume, app->use_gpu);
  ...
  struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *shift_proj =
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_inew(&(struct gkyl_dg_gr_maxwell_surf_and_vol_nodes_inp){
      ...
      .num_ret_vals = 3,
      .polyorder = vm_app_inp->poly_order,
      .eval = gkyl_dg_gr_maxwell_preset_shift(vmg->triad_preset_geom_type),
      .ctx = &ctx,
      // Sample the GR geometry at physical conf coordinates on mapped meshes.
      .c2p_func = vm_geom_pos_c2p,
      .c2p_func_ctx = app->pos_map,
  ...
  vmg->shift_init =
    gkyl_surf_and_vol_node_arrays_new(shift_proj, app->local_ext.volume, app->use_gpu);
  ...
  struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *geom_factor_con_proj =
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_inew(&(struct gkyl_dg_gr_maxwell_surf_and_vol_nodes_inp){
      .grid = &app->grid,
      .basis = &app->basis,
      .num_ret_vals = 3,
      .polyorder = vm_app_inp->poly_order,
      .eval = gkyl_dg_gr_maxwell_preset_geom_factor_con(vmg->triad_preset_geom_type),
      .ctx = &ctx,
      // Sample the GR geometry at physical conf coordinates on mapped meshes.
      .c2p_func = vm_geom_pos_c2p,
      .c2p_func_ctx = app->pos_map,
    });
  vmg->geom_factor_con_init =
    gkyl_surf_and_vol_node_arrays_new(geom_factor_con_proj, app->local_ext.volume, app->use_gpu);
  gkyl_dg_gr_maxwell_surf_and_vol_nodes_advance(
    geom_factor_con_proj, 0.0, &app->local_ext, vmg->geom_factor_con_init
  );
  if (app->use_gpu) {
    vmg->geom_factor_con =
      gkyl_surf_and_vol_node_copy_to_device(vmg->geom_factor_con_init, app->cdim);
  } else {
  ...
  struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *h_ij_proj =
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_inew(&(struct gkyl_dg_gr_maxwell_surf_and_vol_nodes_inp){
      ...
      .num_ret_vals = 6,
      .polyorder = vm_app_inp->poly_order,
      .eval = gkyl_dg_gr_maxwell_preset_h_ij(vmg->triad_preset_geom_type),
      .ctx = &ctx,
      // Sample the GR geometry at physical conf coordinates on mapped meshes.
      .c2p_func = vm_geom_pos_c2p,
      .c2p_func_ctx = app->pos_map,
  ...
  vmg->h_ij_init =
    gkyl_surf_and_vol_node_arrays_new(h_ij_proj, app->local_ext.volume, app->use_gpu);
  ...
  struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *h_ij_inv_proj =
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_inew(&(struct gkyl_dg_gr_maxwell_surf_and_vol_nodes_inp){
      ...
      .num_ret_vals = 6,
      .polyorder = vm_app_inp->poly_order,
      .eval = gkyl_dg_gr_maxwell_preset_h_ij_inv(vmg->triad_preset_geom_type),
      .ctx = &ctx,
      // Sample the GR geometry at physical conf coordinates on mapped meshes.
      .c2p_func = vm_geom_pos_c2p,
      .c2p_func_ctx = app->pos_map,
  ...
  vmg->h_ij_inv_init =
    gkyl_surf_and_vol_node_arrays_new(h_ij_inv_proj, app->local_ext.volume, app->use_gpu);
  gkyl_dg_gr_maxwell_surf_and_vol_nodes_advance(
    h_ij_inv_proj, 0.0, &app->local_ext, vmg->h_ij_inv_init
  );
  if (app->use_gpu) {
  ...
  struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *det_h_proj =
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_inew(&(struct gkyl_dg_gr_maxwell_surf_and_vol_nodes_inp){
      ...
      .num_ret_vals = 1,
      .polyorder = vm_app_inp->poly_order,
      .eval = gkyl_dg_gr_maxwell_preset_det_h(vmg->triad_preset_geom_type),
      .ctx = &ctx,
      // Sample the GR geometry at physical conf coordinates on mapped meshes.
      .c2p_func = vm_geom_pos_c2p,
      .c2p_func_ctx = app->pos_map,
  ...
  vmg->det_h_init =
    gkyl_surf_and_vol_node_arrays_new(det_h_proj, app->local_ext.volume, app->use_gpu);
  ...
  if (vmg->has_gr_em_triad_coupling) {
    ...
    int vdim = app->vdim;
    struct gkyl_triad_geom_ctx triad_ctx = {.mass_bh = vmg->mass_bh, .spin_bh = vmg->spin_bh};

    struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *vierb_cov_proj =
      gkyl_dg_gr_maxwell_surf_and_vol_nodes_inew(&(struct gkyl_dg_gr_maxwell_surf_and_vol_nodes_inp
      ){
        .grid = &app->grid,
        .basis = &app->basis,
        .num_ret_vals = vdim * vdim,
        .polyorder = vm_app_inp->poly_order,
        .eval = gkyl_vlasov_triad_preset_vierbein(vdim, vmg->triad_preset_geom_type),
        .ctx = &triad_ctx,
        // Sample the GR geometry at physical conf coordinates on mapped meshes.
        .c2p_func = vm_geom_pos_c2p,
        .c2p_func_ctx = app->pos_map,
    ...
    vmg->vierb_cov_init =
      gkyl_surf_and_vol_node_arrays_new(vierb_cov_proj, app->local_ext.volume, app->use_gpu);
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_advance(
      vierb_cov_proj, 0.0, &app->local_ext, vmg->vierb_cov_init
    );
    if (app->use_gpu) {
    ...
    struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *vierb_con_proj =
      gkyl_dg_gr_maxwell_surf_and_vol_nodes_inew(&(struct gkyl_dg_gr_maxwell_surf_and_vol_nodes_inp
      ){
        .grid = &app->grid,
        .basis = &app->basis,
        .num_ret_vals = vdim * vdim,
        .polyorder = vm_app_inp->poly_order,
        .eval = gkyl_vlasov_triad_preset_vierbein_inv(vdim, vmg->triad_preset_geom_type),
        .ctx = &triad_ctx,
        // Sample the GR geometry at physical conf coordinates on mapped meshes.
        .c2p_func = vm_geom_pos_c2p,
        .c2p_func_ctx = app->pos_map,
    ...
    vmg->vierb_con_init =
      gkyl_surf_and_vol_node_arrays_new(vierb_con_proj, app->local_ext.volume, app->use_gpu);
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_advance(
      vierb_con_proj, 0.0, &app->local_ext, vmg->vierb_con_init
    );
    if (app->use_gpu) {
...
vm_geom_init(struct gkyl_vm *vm_app_inp, struct gkyl_vlasov_app *app, struct vm_geom *vmg)
{
  *vmg = (struct vm_geom){0};
  ...
  for (int i = 0; i < vm_app_inp->num_species; ++i) {
    if (vm_app_inp->species[i].type == GKYL_SPECIES_FLUID) {
      continue;
    }

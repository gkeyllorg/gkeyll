...
)
{
  ...
  if (bgk->fixed_temp_relax) {
    ...
    gkyl_array_set_offset_range(
      bgk->lte.moms.marr, 1.0, bgk->fixed_temp, (app->vdim + 1) * app->confBasis.num_basis,
      &app->local
    );
  }
...
)
{
  ...
  gkyl_array_set_offset_range(
    bgk->fixed_temp, 1.0, bgk->lte.moms.marr, (app->vdim + 1) * app->confBasis.num_basis,
    &app->local
  );
}
...
)
{
  ...
  gkyl_dg_mul_conf_phase_op_range(
    &app->confBasis, &app->basis, bgk->lte.f_lte, bgk->self_nu, bgk->lte.f_lte, &app->local,
    &species->local
  );
  gkyl_array_accumulate(bgk->nu_f_lte, 1.0, bgk->lte.f_lte);

  gkyl_bgk_collisions_advance(
    bgk->up_bgk, &app->local, &species->local, bgk->nu_sum, bgk->nu_f_lte, fin, bgk->implicit_step,
    bgk->dt_implicit, rhs, species->cflrate
  );

...
static void
vmbgk_self_nu_calc_constNu(
  gkyl_vlasov_app *app, const struct vm_species *vms, struct vm_bgk_collisions *bgk
)
{
  // Zero out collision frequency in cells with n, T < 0.
  gkyl_bgk_collisions_correct_nu(
    bgk->up_bgk, &app->local, vms->lte.moms.marr, bgk->ref_self_nu, bgk->self_nu
  );
...
static void
vmbgk_self_nu_calc_normNu(
  gkyl_vlasov_app *app, const struct vm_species *vms, struct vm_bgk_collisions *bgk
)
{
  // Calculate nu_ss(x,t). Automatically zeros out nu_ss if n, T < 0.
  gkyl_spitzer_coll_freq_advance_normnu(
    bgk->spitzer_calc, &app->local, vms->lte.moms.marr, bgk->vtsq_min, vms->lte.moms.marr,
    bgk->vtsq_min, bgk->norm_nu_fac_self, bgk->self_nu
  );
...
)
{
  ...
  gkyl_bgk_collisions_correct_nu(
    bgk->up_bgk, &app->local, bgk->other_prim_moms[coll_idx], bgk->ref_cross_nu[coll_idx],
...
static void
vmbgk_cross_nu_calc_normNu(
  gkyl_vlasov_app *app, const struct vm_species *vms, struct vm_bgk_collisions *bgk, int coll_idx
)
{
  // Calculate nu_sr(x,t). Automatically zeros out nu_sr if n, T < 0.
  gkyl_spitzer_coll_freq_advance_normnu(
    bgk->spitzer_calc, &app->local, vms->lte.moms.marr, bgk->vtsq_min,
...
static void
vmbgk_alpha_E_constNu(
  gkyl_vlasov_app *app, const struct vm_species *s, struct vm_bgk_collisions *bgk, int coll_idx
)
{
...
)
{
  ...
  gkyl_vlasov_cross_prim_moms_bgk_advance(
    bgk->cross_calc, &app->local, bgk->delta_sr, bgk->betaGreenep1, vms->mass, vms->lte.moms.marr,
    bgk->other_m[coll_idx], bgk->other_prim_moms[coll_idx], bgk->cross_prim_moms
  );
...
)
{
  ...
  if (bgk->fixed_temp_relax) {
    ...
    gkyl_array_set_offset_range(
      lte->moms.marr, 1.0, bgk->fixed_temp, (app->vdim + 1) * app->basis.num_basis, &app->local
    );
  }
  ...
  gkyl_dg_mul_conf_phase_op_range(
    &app->basis, &vms->basis, bgk->nu_f_lte, bgk->self_nu, lte->f_lte, &app->local, &vms->local
  );
  ...
  for (int i = 0; i < bgk->num_cross_collisions; ++i) {
    ...
    gkyl_dg_mul_conf_phase_op_range(
      &app->basis, &vms->basis, lte->f_lte, bgk->cross_nu[i], lte->f_lte, &app->local, &vms->local
    );
  ...
  gkyl_bgk_collisions_advance(
    bgk->up_bgk, &app->local, &vms->local, bgk->nu_sum, bgk->nu_f_lte, fin, bgk->implicit_step, dt,
    rhs, vms->cflrate
  );
...
)
{
  ...
  gkyl_array_set_offset_range(
    bgk->fixed_temp, 1.0, vms->lte.moms.marr, (app->vdim + 1) * app->basis.num_basis, &app->local
  );
}
...
void
vm_species_bgk_init(
  struct gkyl_vlasov_app *app, struct vm_species *vms, struct vm_bgk_collisions *bgk
)
{
...
)
{
  ...
  if (bgk->collision_id == GKYL_BGK_COLLISIONS) {
    if (vms->bgk.num_cross_collisions) {
      ...
      bgk->cross_prim_moms =
        mkarr(app->use_gpu, (app->vdim + 2) * app->basis.num_basis, app->local_ext.volume);
      for (int i = 0; i < bgk->num_cross_collisions; ++i) {

...
vm_species_moment_init(
  struct gkyl_vlasov_app *app, struct vm_species *vms, struct vm_species_moment *sm,
...
)
{
  ...
  if (sm->is_vlasov_lte_moms) {
    ...
    num_mom = app->vdim + 2;
    sm->marr = mkarr(app->use_gpu, num_mom * app->basis.num_basis, app->local_ext.volume);
  } else {

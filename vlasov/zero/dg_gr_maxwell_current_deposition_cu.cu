...
)
{
  ...
  for (unsigned long linc = threadIdx.x + blockIdx.x * blockDim.x; linc < conf_range.volume;
    ...
    up->current_deposition(
      q_over_eps0, (const double *)gkyl_array_cfetch(lapse->nodal_arr_vol, cidx),
      (const double *)gkyl_array_cfetch(shift->nodal_arr_vol, cidx),
      (const double *)gkyl_array_cfetch(vierb_con->nodal_arr_vol, cidx),
      (const double *)gkyl_array_cfetch(m0, cidx), (const double *)gkyl_array_cfetch(m1i, cidx),
...
)
{
  ...
  gkyl_dg_gr_maxwell_current_deposition_advance_cu_kernel<<<nblocks, nthreads>>>(
    up->on_dev, *conf_range, q_over_eps0, lapse->on_dev, shift->on_dev, vierb_con->on_dev,
    m0->on_dev, m1i->on_dev, rhs->on_dev
  );
}

__global__ static void
gkyl_dg_gr_maxwell_current_deposition_set_cu_dev_ptrs(
  struct gkyl_dg_gr_maxwell_current_deposition *up, enum gkyl_basis_type b_type, int cdim, int vdim,
  int poly_order
)
{
...
)
{
  ...
  gkyl_dg_gr_maxwell_current_deposition_set_cu_dev_ptrs<<<1, 1>>>(

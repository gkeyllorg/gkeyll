...
gkyl_vlasov_lte_moments_inew(const struct gkyl_vlasov_lte_moments_inp *inp)
{
  ...
  if (up->model_id == GKYL_MODEL_SR) {
    ...
    up->sr_vars = gkyl_dg_calc_sr_vars_new(
      inp->phase_grid, inp->vel_grid, inp->conf_basis, inp->vel_basis, inp->conf_range,
      inp->vel_range, inp->use_gpu
    );
    ...
    up->M0_calc = gkyl_dg_updater_moment_new(
      inp->phase_grid, inp->conf_basis, inp->phase_basis, inp->conf_range, inp->vel_range,
      inp->phase_range, up->model_id, &sr_inp, GKYL_F_MOMENT_M0, false, inp->use_gpu
    );
    up->M1i_calc = gkyl_dg_updater_moment_new(
      inp->phase_grid, inp->conf_basis, inp->phase_basis, inp->conf_range, inp->vel_range,
      inp->phase_range, up->model_id, &sr_inp, GKYL_F_MOMENT_M1, false, inp->use_gpu
    );
    ...
    up->M0_calc = gkyl_dg_updater_moment_new(
      inp->phase_grid, inp->conf_basis, inp->phase_basis, inp->conf_range, inp->vel_range,
      inp->phase_range, up->model_id, 0, GKYL_F_MOMENT_M0, false, inp->use_gpu
    );
    up->M1i_calc = gkyl_dg_updater_moment_new(
      inp->phase_grid, inp->conf_basis, inp->phase_basis, inp->conf_range, inp->vel_range,
      inp->phase_range, up->model_id, &can_pb_inp, GKYL_F_MOMENT_M1_FROM_H, false, inp->use_gpu
    );
    up->Pcalc = gkyl_dg_updater_moment_new(
      inp->phase_grid, inp->conf_basis, inp->phase_basis, inp->conf_range, inp->vel_range,
      inp->phase_range, up->model_id, &can_pb_inp, GKYL_F_MOMENT_ENERGY, false, inp->use_gpu
    );
  } else {
    ...
    up->M1i_calc = gkyl_dg_updater_moment_new(
      inp->phase_grid, inp->conf_basis, inp->phase_basis, inp->conf_range, inp->vel_range,
      inp->phase_range, up->model_id, 0, GKYL_F_MOMENT_M1, false, inp->use_gpu
    );
    up->Pcalc = gkyl_dg_updater_moment_new(
      inp->phase_grid, inp->conf_basis, inp->phase_basis, inp->conf_range, inp->vel_range,
      inp->phase_range, up->model_id, 0, GKYL_F_MOMENT_M2, false, inp->use_gpu
    );
  }
  return up;
}
...
)
{
  ...
  if (lte_moms->model_id == GKYL_MODEL_SR) {
    ...
    for (int d = 0; d < vdim; ++d) {
      gkyl_dg_div_op_range(
        lte_moms->mem, &lte_moms->conf_basis, d, lte_moms->V_drift, d, lte_moms->M1i, 0, moms_out,
        conf_local
      );
    }
    ...
    gkyl_dg_calc_sr_vars_GammaV(
      lte_moms->sr_vars, conf_local, lte_moms->V_drift, lte_moms->V_drift_sq, lte_moms->GammaV,
      lte_moms->GammaV_sq
    );
    ...
    gkyl_dg_calc_sr_vars_pressure(
      lte_moms->sr_vars, conf_local, phase_local, lte_moms->gamma, lte_moms->gamma_inv,
      lte_moms->V_drift, lte_moms->V_drift_sq, lte_moms->GammaV, lte_moms->GammaV_sq, fin,
      lte_moms->pressure
    );
  } else {
    ...
    for (int d = 0; d < vdim; ++d) {
      gkyl_dg_div_op_range(
        lte_moms->mem, &lte_moms->conf_basis, d, lte_moms->V_drift, d, lte_moms->M1i, 0,
        lte_moms->M0, conf_local
      );
    }
    ...
    if (lte_moms->model_id == GKYL_MODEL_CANONICAL_PB) {
      ...
      gkyl_canonical_pb_contra_to_covariant_m1i(
        lte_moms->can_pb_vars, conf_local, lte_moms->h_ij, lte_moms->V_drift, lte_moms->M1i,
      ...
      gkyl_canonical_pb_pressure(
        lte_moms->can_pb_vars, conf_local, lte_moms->h_ij_inv, lte_moms->energy_moment,
      ...
      gkyl_dg_dot_product_op_range(
        &lte_moms->conf_basis, lte_moms->V_drift_dot_M1i, lte_moms->V_drift, lte_moms->M1i,
        conf_local
      );
  ...
  // ( T/m = P/(mn) )
  gkyl_dg_div_op_range(
    lte_moms->mem, &lte_moms->conf_basis, 0, lte_moms->temperature, 0, lte_moms->pressure, 0,
    moms_out, conf_local
  );

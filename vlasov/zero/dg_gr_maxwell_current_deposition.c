...
)
{
  ...
  if (gkyl_array_is_cu_dev(rhs)) {
    return gkyl_dg_gr_maxwell_current_deposition_advance_cu(
      up, conf_range, q_over_eps0, lapse, shift, vierb_con, m0, m1i, rhs
    );
  }
#endif
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    up->current_deposition(
      q_over_eps0, gkyl_array_cfetch(lapse->nodal_arr_vol, cidx),
      gkyl_array_cfetch(shift->nodal_arr_vol, cidx),
      gkyl_array_cfetch(vierb_con->nodal_arr_vol, cidx), gkyl_array_cfetch(m0, cidx),
      gkyl_array_cfetch(m1i, cidx), gkyl_array_fetch(rhs, cidx)
    );
  }
}

void
gkyl_dg_gr_maxwell_current_deposition_release(struct gkyl_dg_gr_maxwell_current_deposition *up)
{

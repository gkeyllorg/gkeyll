...
typedef int (*canonical_pb_fluid_alpha_surf_t)(
  const double *w, const double *dxv, const double *phi, double *GKYL_RESTRICT alpha_surf,
  double *GKYL_RESTRICT sgn_alpha_surf
);
typedef void (*canonical_pb_fluid_source_t)(
  const double *dxv, double alpha, const double *phi, const double *n0,
...
GKYL_CU_D static const gkyl_dg_canonical_pb_fluid_subtract_zonal_kern_list
  ser_canonical_pb_fluid_subtract_zonal_kernels[] = {
    ...
    {NULL, NULL, NULL}, // 0
    // 2x kernels
    {NULL, canonical_pb_fluid_subtract_zonal_2x_ser_p1, canonical_pb_fluid_subtract_zonal_2x_ser_p2
    }, // 1
    // 3x kernels
    {NULL, NULL, NULL} // 2
};
...
)
{
  switch (b_type) {
    case GKYL_BASIS_MODAL_SERENDIPITY:
      ...
      break;
    case GKYL_BASIS_MODAL_TENSOR:
      return tensor_canonical_pb_fluid_hasegawa_wakatani_source_kernels[cdim - 1]
        .kernels[poly_order];
      break;
    default:
      assert(false);

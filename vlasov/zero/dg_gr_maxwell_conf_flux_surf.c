...
)
{
  ...
  if (gkyl_array_is_cu_dev(conf_flux_surf)) {
    return gkyl_dg_gr_maxwell_conf_flux_surf_advance_cu(
      up, conf_range, conf_range_ext, lapse, shift, h_ij, h_ij_inv, det_h, field_con,
      field_no_J_con, cflrate, conf_flux_surf
    );
  }
#endif
  int cdim = up->cdim;

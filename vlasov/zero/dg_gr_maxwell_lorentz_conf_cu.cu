...
gkyl_dg_gr_maxwell_lorentz_conf_advance_cu_kernel(
  ...
  const struct gkyl_array *jacob_pos, const struct gkyl_surf_and_vol_node_arrays *lapse,
...
)
{
  ...
  for (unsigned long linc = threadIdx.x + blockIdx.x * blockDim.x; linc < conf_range.volume;
    ...
    for (int k = 0; k < 3 * num_basis; ++k) {
      E_conf_for_force[k] = 0.0;
    }
    for (int k = 0; k < 3 * num_basis; ++k) {
      B_conf_for_force[k] = 0.0;
    }

    up->lorentz_conf(
      &up->gr_maxwell_data, up->conf_grid.dx, (const double *)gkyl_array_cfetch(jacob_pos, cidx),
      ...
      (const double *)gkyl_array_cfetch(vierb_con->nodal_arr_vol, cidx), E_conf_for_force,
      B_conf_for_force
    );
...
void
gkyl_dg_gr_maxwell_lorentz_conf_advance_cu(
  struct gkyl_dg_gr_maxwell_lorentz_conf *up, const struct gkyl_range *conf_range,
...
)
{
  ...
  gkyl_dg_gr_maxwell_lorentz_conf_advance_cu_kernel<<<nblocks, nthreads>>>(
    ...
    h_ij_inv->on_dev, det_h->on_dev, vierb_cov->on_dev, vierb_con->on_dev, field_con->on_dev,
    qmem->on_dev
  );
}
...
gkyl_dg_gr_maxwell_lorentz_conf_set_cu_dev_ptrs(
  struct gkyl_dg_gr_maxwell_lorentz_conf *up, enum gkyl_basis_type b_type, int cdim, int vdim,
  int poly_order
)
{
...
gkyl_dg_gr_maxwell_lorentz_conf_cu_dev_inew(const struct gkyl_dg_gr_maxwell_lorentz_conf_inp *inp)
{
  ...
  gkyl_dg_gr_maxwell_lorentz_conf_set_cu_dev_ptrs<<<1, 1>>>(
    up_cu, inp->conf_basis->b_type, cdim, vdim, poly_order
  );

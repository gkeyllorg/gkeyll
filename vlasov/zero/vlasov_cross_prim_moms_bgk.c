...
gkyl_vlasov_cross_prim_moms_bgk_new(
  const struct gkyl_basis *pbasis, const struct gkyl_basis *cbasis, bool use_gpu
)
{
...
#ifdef GKYL_HAVE_CUDA
  if (up->use_gpu) {
    return gkyl_vlasov_cross_prim_moms_bgk_advance_cu(
...
void
gkyl_vlasov_cross_prim_moms_bgk_release(gkyl_vlasov_cross_prim_moms_bgk *up)
{
  if (up->use_gpu) {

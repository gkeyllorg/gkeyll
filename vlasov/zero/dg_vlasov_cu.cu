...
dg_vlasov_set_cu_dev_ptrs(
  struct dg_vlasov *vlasov, enum gkyl_basis_type b_type, int cdim, int vdim, int poly_order,
  enum gkyl_model_id model_id, enum gkyl_hamil_id hamil_id, bool has_E, bool has_phi, bool has_B,
  bool has_rad, bool use_lo
)
{
...
gkyl_dg_vlasov_cu_dev_inew(const struct gkyl_dg_vlasov_inp *inp)
{
  ...
  dg_vlasov_set_cu_dev_ptrs<<<1, 1>>>(
    vlasov_cu, gkyl_basis_phase_kernel_type(inp->conf_basis, inp->phase_basis), cdim, vdim,
    poly_order, inp->model_id, inp->hamil_id, inp->has_E, inp->has_phi, inp->has_B, inp->has_rad,
    inp->use_lo
  );

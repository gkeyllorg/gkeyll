...
typedef void (*gr_maxwell_geom_source_t)(
  const gkyl_dg_gr_maxwell_inp *meq, const double *w, const double *dx,
  const double *geom_factor_con_nodal, const double *Jc_fields, double *GKYL_RESTRICT out
);

// for use in kernel tables
typedef struct {
  gr_maxwell_geom_source_t kernels[4];
} gkyl_dg_gr_maxwell_geom_source_kern_list;
...
GKYL_CU_D static const gkyl_dg_gr_maxwell_geom_source_kern_list ser_geom_source_kernels[] = {
  // 1x kernels
...
GKYL_CU_D static const gkyl_dg_gr_maxwell_geom_source_kern_list ten_geom_source_kernels[] = {
  // 1x kernels

...
__global__ void static dg_maxwell_set_cu_dev_ptrs(
  struct dg_maxwell *maxwell, enum gkyl_basis_type b_type, int cdim, bool use_conf_flux_surf,
  int poly_order
)
{
  ...
  const gkyl_dg_maxwell_surf_from_flux_kern_list *surf_from_flux_x_kernels,
    *surf_from_flux_y_kernels, *surf_from_flux_z_kernels;
...
gkyl_dg_maxwell_cu_dev_inew(const struct gkyl_dg_maxwell_inp *inp)
{
  ...
  if (inp->field_id == GKYL_FIELD_GR_D_B) {
    assert(fabs(inp->lightSpeed - 1.0) < 1e-12);
  }
  ...
  dg_maxwell_set_cu_dev_ptrs<<<1, 1>>>(
    maxwell_cu, inp->cbasis->b_type, inp->cbasis->ndim, maxwell->use_conf_flux_surf,

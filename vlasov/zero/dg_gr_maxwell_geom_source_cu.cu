...
__global__ void
gkyl_dg_gr_maxwell_geom_source_advance_cu_kernel(
...
)
{
  ...
  for (unsigned long linc = threadIdx.x + blockIdx.x * blockDim.x; linc < conf_range.volume;
    ...
    const double *geom_factor_con_d =
      (const double *)gkyl_array_cfetch(geom_factor_con->nodal_arr_vol, cidx);
...
void
gkyl_dg_gr_maxwell_geom_source_advance_cu(
  struct gkyl_dg_gr_maxwell_geom_source *up, const struct gkyl_range *conf_range,
  const struct gkyl_surf_and_vol_node_arrays *geom_factor_con, const struct gkyl_array *field_con,
  struct gkyl_array *rhs
)
{
  ...
  gkyl_dg_gr_maxwell_geom_source_advance_cu_kernel<<<nblocks, nthreads>>>(
    up->on_dev, *conf_range, geom_factor_con->on_dev, field_con->on_dev, rhs->on_dev
  );
}
...
gkyl_dg_gr_maxwell_geom_source_set_cu_dev_ptrs(
  struct gkyl_dg_gr_maxwell_geom_source *up, enum gkyl_basis_type b_type, int cdim, int poly_order,
...
gkyl_dg_gr_maxwell_geom_source_cu_dev_inew(const struct gkyl_dg_gr_maxwell_geom_source_inp *inp)
{
  ...
  gkyl_dg_gr_maxwell_geom_source_set_cu_dev_ptrs<<<1, 1>>>(

...
struct gkyl_dg_gr_maxwell_conf_flux_surf_inp {
  ...
  const struct gkyl_vlasov_position_map *
    pos_map; // Configuration-space mapping object (REQUIRED; provides the cell-constant jacob_pos).
  enum gkyl_field_id
    field_id; // enum to determine what type of Maxwell model (e.g., curved space non-relativistic vs. relativistic).
  const int
    *theta_pole_lo; // (lower edge) Default zeros, but 1 if any directions use theta-pole BC's.
  const int
    *theta_pole_up; // (upper edge) Default zeros, but 1 if any directions use theta-pole BC's.
 ...
 */
struct gkyl_dg_gr_maxwell_conf_flux_surf *gkyl_dg_gr_maxwell_conf_flux_surf_inew(
  const struct gkyl_dg_gr_maxwell_conf_flux_surf_inp *inp
);
 ...
 */
struct gkyl_dg_gr_maxwell_conf_flux_surf *gkyl_dg_gr_maxwell_conf_flux_surf_cu_dev_inew(
  const struct gkyl_dg_gr_maxwell_conf_flux_surf_inp *inp
);
 ...
 */
void gkyl_dg_gr_maxwell_conf_flux_surf_advance(
 ...
 */
void gkyl_dg_gr_maxwell_conf_flux_surf_advance_cu(
  struct gkyl_dg_gr_maxwell_conf_flux_surf *up, const struct gkyl_range *conf_range,
  const struct gkyl_range *conf_range_ext, const struct gkyl_surf_and_vol_node_arrays *lapse,
  const struct gkyl_surf_and_vol_node_arrays *shift,
  const struct gkyl_surf_and_vol_node_arrays *h_ij,
...
void gkyl_dg_gr_maxwell_conf_flux_surf_release(struct gkyl_dg_gr_maxwell_conf_flux_surf *up);

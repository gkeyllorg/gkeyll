...
__global__ void static dg_diffusion_vlasov_set_cu_dev_ptrs(
  struct dg_diffusion_vlasov *diffusion, enum gkyl_basis_type b_type, int cdim, int vdim,
...
)
{
  ...
  dg_diffusion_vlasov_set_cu_dev_ptrs<<<1, 1>>>(
    diffusion_cu, gkyl_basis_phase_kernel_type(cbasis, basis), cdim, vdim, poly_order, diff_order,
    dirs_linidx
  );

...
__global__ void
gkyl_dg_gr_maxwell_conf_flux_surf_advance_cu_kernel(
  struct gkyl_dg_gr_maxwell_conf_flux_surf *up, const struct gkyl_range conf_range,
  const struct gkyl_range conf_range_ext, const struct gkyl_array *jacob_pos,
  ...
  const struct gkyl_surf_and_vol_node_arrays *det_h, const struct gkyl_array *field_con,
  ...
  for (unsigned long linc1 = threadIdx.x + blockIdx.x * blockDim.x; linc1 < conf_range.volume;
    ...
    for (int dir = 0; dir < cdim; ++dir) {
      ...
      cflrate_d[0] += up->conf_flux_surf(
        up, dir, xcC, up->conf_grid.dx, theta_pole, jacob_pos_l, jacob_pos_c, lapse_d, shift_d,
        h_ij_d, h_ij_inv_d, det_h_d, field_con_l, field_con_c, field_no_J_con_l, field_no_J_con_c,
        flux
      );
      ...
      if (idx[dir] == conf_range.upper[dir]) {
        ...
        cflrate_d_r[0] += up->conf_flux_surf(
          up, dir, xcR, up->conf_grid.dx, theta_pole, jacob_pos_c, jacob_pos_r, lapse_d, shift_d,
          h_ij_d, h_ij_inv_d, det_h_d, field_con_c, field_con_r, field_no_J_con_c, field_no_J_con_r,
          flux_r
        );
      }
    }
...
void
gkyl_dg_gr_maxwell_conf_flux_surf_advance_cu(
  struct gkyl_dg_gr_maxwell_conf_flux_surf *up, const struct gkyl_range *conf_range,
  const struct gkyl_range *conf_range_ext, const struct gkyl_surf_and_vol_node_arrays *lapse,
  const struct gkyl_surf_and_vol_node_arrays *shift,
  const struct gkyl_surf_and_vol_node_arrays *h_ij,
  const struct gkyl_surf_and_vol_node_arrays *h_ij_inv,
  const struct gkyl_surf_and_vol_node_arrays *det_h, const struct gkyl_array *field_con,
...
)
{
  ...
  gkyl_dg_gr_maxwell_conf_flux_surf_advance_cu_kernel<<<nblocks, nthreads>>>(
    up->on_dev, *conf_range, *conf_range_ext, up->jacob_pos->on_dev, lapse->on_dev, shift->on_dev,
    h_ij->on_dev, h_ij_inv->on_dev, det_h->on_dev, field_con->on_dev, field_no_J_con->on_dev,
    cflrate->on_dev, conf_flux_surf->on_dev
  );
}
...
gkyl_dg_gr_maxwell_conf_flux_surf_set_cu_dev_ptrs(
  struct gkyl_dg_gr_maxwell_conf_flux_surf *up, enum gkyl_basis_type b_type, int cdim,
...
)
{
  switch (b_type) {
    case GKYL_BASIS_MODAL_SERENDIPITY:
      ...
      up->dg_gr_maxwell_alpha_quad[0] =
        ser_dg_gr_maxwell_alpha_quad_x_kernels[cdim - 1].kernels[poly_order];
      up->dg_gr_maxwell_alpha_quad[1] =
        ser_dg_gr_maxwell_alpha_quad_y_kernels[cdim - 1].kernels[poly_order];
      up->dg_gr_maxwell_alpha_quad[2] =
        ser_dg_gr_maxwell_alpha_quad_z_kernels[cdim - 1].kernels[poly_order];
    ...
    case GKYL_BASIS_MODAL_TENSOR:
      ...
      // Kernels to compute the maximum of the eigenvalues and isolate the fluxes E^i, H^i.
      up->dg_gr_maxwell_alpha_quad[0] =
        ten_dg_gr_maxwell_alpha_quad_x_kernels[cdim - 1].kernels[poly_order];
      up->dg_gr_maxwell_alpha_quad[1] =
        ten_dg_gr_maxwell_alpha_quad_y_kernels[cdim - 1].kernels[poly_order];
      up->dg_gr_maxwell_alpha_quad[2] =
        ten_dg_gr_maxwell_alpha_quad_z_kernels[cdim - 1].kernels[poly_order];
...
{
  struct gkyl_dg_gr_maxwell_conf_flux_surf *up =
    (struct gkyl_dg_gr_maxwell_conf_flux_surf *)gkyl_malloc(sizeof(*up));
  ...
  struct gkyl_dg_gr_maxwell_conf_flux_surf *up_cu =
    (struct gkyl_dg_gr_maxwell_conf_flux_surf *)gkyl_cu_malloc(sizeof(*up_cu));
  gkyl_cu_memcpy(up_cu, up, sizeof(gkyl_dg_gr_maxwell_conf_flux_surf), GKYL_CU_MEMCPY_H2D);

  gkyl_dg_gr_maxwell_conf_flux_surf_set_cu_dev_ptrs<<<1, 1>>>(
    up_cu, inp->conf_basis->b_type, cdim, poly_order, inp->field_id
  );

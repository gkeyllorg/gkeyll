...
struct gkyl_vlasov_dist_proj_on_basis {
  struct gkyl_dist_proj_type *dist_proj;
  ...
  int num_comp_mom; // Number of output moment components
  ...
  struct gkyl_array *fun_at_ords; // function distribution evaluated at
                                  // ordinates in a cell.

  int *p2c_qidx;  // Mapping between Configuration-space and Phase-space ordinates.
  struct gkyl_array *f_dist_quad; // Array keeping f_dist at phase-space quadrature nodes
  struct gkyl_array *moms_dist_quad; // Array keeping moms_dist  
...
//   struct gkyl_vlasov_dist_moments *moments_up; // LTE moment calculation routine for computing density

...
)
{
  ...
  if (gkyl_array_is_cu_dev(qmem)) {
    return gkyl_dg_gr_maxwell_lorentz_conf_advance_cu(
      up, conf_range, lapse, shift, h_ij, h_ij_inv, det_h, vierb_cov, vierb_con, field_con, qmem
    );
  }
#endif
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    up->lorentz_conf(
      &up->gr_maxwell_data, up->conf_grid.dx, gkyl_array_cfetch(up->jacob_pos, cidx),
      gkyl_array_cfetch(lapse->nodal_arr_vol, cidx), gkyl_array_cfetch(shift->nodal_arr_vol, cidx),
      gkyl_array_cfetch(h_ij->nodal_arr_vol, cidx),
      gkyl_array_cfetch(h_ij_inv->nodal_arr_vol, cidx),
      gkyl_array_cfetch(det_h->nodal_arr_vol, cidx), gkyl_array_cfetch(field_con, cidx),
      gkyl_array_cfetch(vierb_cov->nodal_arr_vol, cidx),
      gkyl_array_cfetch(vierb_con->nodal_arr_vol, cidx), E_conf_for_force, B_conf_for_force
    );

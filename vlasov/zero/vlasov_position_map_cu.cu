...
__global__ void
gkyl_vlasov_position_map_scale_jacobpos_conf_cu_kernel(struct gkyl_range jac_range,
  struct gkyl_range conf_range, int ncomp, int num_coeff_scale, bool divide,
  const struct gkyl_array *jacob_pos_gauss, const struct gkyl_array *in, struct gkyl_array *out)
{
...
void
gkyl_vlasov_position_map_divide_jacobpos_conf_cu(const struct gkyl_range *jac_range,
...
{
  gkyl_vlasov_position_map_scale_jacobpos_conf_cu_kernel<<<conf_range->nblocks, conf_range->nthreads>>>(
    *jac_range, *conf_range, Jmom->ncomp, num_coeff_divide, true,
    jacob_pos_gauss->on_dev, Jmom->on_dev, mom_no_J->on_dev);
}

void
gkyl_vlasov_position_map_rescale_jacobpos_conf_cu(const struct gkyl_range *jac_range,
  const struct gkyl_range *conf_range,
  const struct gkyl_array *jacob_pos_gauss, const struct gkyl_array *a_no_J, struct gkyl_array *Ja)
{
  gkyl_vlasov_position_map_scale_jacobpos_conf_cu_kernel<<<conf_range->nblocks, conf_range->nthreads>>>(
    *jac_range, *conf_range, a_no_J->ncomp, a_no_J->ncomp, false,
    jacob_pos_gauss->on_dev, a_no_J->on_dev, Ja->on_dev);
}

...
typedef void (*hamil_vol_t)(
  const double *w, const double *dxv, const double *vmap, const double *jacob_pos,
  const double *jacob_vel, const double *poisson_tensor_conf, const double *hamil, const double *f,
  const double *f_no_J, double *GKYL_RESTRICT out
);

typedef void (*E_vol_t)(
  const double *w, const double *dxv, const double *jacob_vel, const double *qmem, const double *f,
  double *GKYL_RESTRICT out
);

typedef void (*phi_vol_t)(
  const double *w, const double *dxv, const double *jacob_pos, const double *jacob_vel,
  const double *phi, const double *f, double *GKYL_RESTRICT out
);

typedef void (*B_vol_t)(
  const double *w, const double *dxv, const double *jacob_vel, const double *hamil,
  const double *qmem, const double *f, double *GKYL_RESTRICT out
);

typedef void (*rad_vol_t)(
  const double *w, const double *dxv, const double *jacob_vel, const double *rad, const double *f,
  double *GKYL_RESTRICT out
);

typedef double (*vlasov_stream_surf_t)(
  const double *w, const double *dxv, const double *jacob_pos_l, const double *jacob_pos_c,
  const double *jacob_pos_r, const double *jacob_vel, const double *poisson_tensor_conf,
  const double *hamil, const double *fl, const double *fc, const double *fr,
  double *GKYL_RESTRICT out
);

typedef double (*vlasov_stream_boundary_surf_t)(
  const double *w, const double *dxv, const double *jacob_pos_edge, const double *jacob_pos_skin,
  const double *jacob_vel, const double *poisson_tensor_conf, const double *hamil, const int edge,
  const double *fedge, const double *fskin, double *GKYL_RESTRICT out
);

typedef double (*vlasov_stream_surf_from_flux_t)(
  const double *w, const double *dxv, const double *flux_l, const double *flux_r,
  double *GKYL_RESTRICT out
);

typedef double (*vlasov_stream_boundary_surf_from_flux_t)(
  const double *w, const double *dxv, const int edge, const double *flux, double *GKYL_RESTRICT out
);

typedef double (*vlasov_accel_surf_t)(
  const double *w, const double *dxv, const double *flux_l, const double *flux_r,
  double *GKYL_RESTRICT out
);

typedef double (*vlasov_accel_boundary_surf_t)(
  const double *w, const double *dxv, const int edge, const double *flux, double *GKYL_RESTRICT out
);
...
typedef struct {
  hamil_vol_t kernels[4];
} gkyl_dg_vlasov_hamil_vol_kern_list;
typedef struct {
  E_vol_t kernels[4];
} gkyl_dg_vlasov_E_vol_kern_list;
typedef struct {
  phi_vol_t kernels[4];
} gkyl_dg_vlasov_phi_vol_kern_list;
typedef struct {
  B_vol_t kernels[4];
} gkyl_dg_vlasov_B_vol_kern_list;
typedef struct {
  rad_vol_t kernels[4];
} gkyl_dg_vlasov_rad_vol_kern_list;

// Surface kernel type tables.
typedef struct {
  vlasov_stream_surf_t kernels[4];
} gkyl_dg_vlasov_stream_surf_kern_list;
typedef struct {
  vlasov_stream_boundary_surf_t kernels[4];
} gkyl_dg_vlasov_stream_boundary_surf_kern_list;
typedef struct {
  vlasov_stream_surf_from_flux_t kernels[4];
} gkyl_dg_vlasov_stream_surf_from_flux_kern_list;
typedef struct {
  vlasov_stream_boundary_surf_from_flux_t kernels[4];
} gkyl_dg_vlasov_stream_boundary_surf_from_flux_kern_list;
typedef struct {
  vlasov_accel_surf_t kernels[4];
} gkyl_dg_vlasov_accel_surf_kern_list;
typedef struct {
  vlasov_accel_boundary_surf_t kernels[4];
...
struct dg_vlasov {
  ...
  struct gkyl_range
    conf_range; // Configuration-space range for use in indexing forces (EM fields, potentials, etc.)
  ...
  const struct gkyl_vlasov_velocity_map *
    vel_map; // Velocity-space mapping object (acquired host-side for lifetime safety; 0 if not given).
  const struct gkyl_array
    *jacob_vel; // Velocity-space Jacobian (borrowed from vel_map; device pointer on GPUs).
  const struct gkyl_array *
    vmap; // Velocity map (4-slot cubic rep per direction; borrowed from vel_map; device pointer on GPUs).
  const struct gkyl_vlasov_position_map *
    pos_map; // Configuration-space mapping object (acquired host-side for lifetime safety; 0 if not given).
  const struct gkyl_array *
    jacob_pos; // Configuration-space (position-map) Jacobian, per-conf-cell constant (borrowed from pos_map; device pointer on GPUs; defined on the extended conf range).
  const struct gkyl_array *
    poisson_tensor_conf; // Hamiltonian utilized to compute advection in configuration and velocity space.
  const struct gkyl_array
  ...
  const struct gkyl_array
    *conf_flux_surf; // Nodal expansion of fluxes at configuration space surfaces.
  const struct gkyl_array *vel_flux_surf; // Nodal expansion of fluxes at velocity space surfaces.
  const struct gkyl_array *
    f_no_J; // Distribution function without velocity-space Jacobian. Used by magnetic field updates.
  ...
  vlasov_stream_boundary_surf_t
    stream_boundary_surf[3]; // Boundary surface terms for advection in configuration space.
  vlasov_stream_surf_from_flux_t
    stream_surf_from_flux[3]; // Surface terms for streaming using fluxes.
  vlasov_stream_boundary_surf_from_flux_t
    stream_boundary_surf_from_flux[3]; // Surface terms for streaming using fluxes.
...
GKYL_CU_DH static void
no_rad_vol(
  const double *w, const double *dxv, const double *jacob_vel, const double *phi, const double *f,
  double *GKYL_RESTRICT out
)
{
}
...
)
{
  ...
  vlasov->hamil_vol(
    xc, dx, vlasov->vmap ? (const double *)gkyl_array_cfetch(vlasov->vmap, vidx) : 0,
    ...
    (const double *)gkyl_array_cfetch(vlasov->hamil, hidx), qIn,
    (const double *)gkyl_array_cfetch(vlasov->f_no_J, pidx), qRhsOut
  );
  vlasov->E_vol(
    xc, dx, vlasov->jacob_vel ? (const double *)gkyl_array_cfetch(vlasov->jacob_vel, vidx) : 0,
    vlasov->qmem ? (const double *)gkyl_array_cfetch(vlasov->qmem, cidx) : 0, qIn, qRhsOut
  );
  vlasov->phi_vol(
    xc, dx, vlasov->jacob_pos ? (const double *)gkyl_array_cfetch(vlasov->jacob_pos, cidx) : 0,
    vlasov->jacob_vel ? (const double *)gkyl_array_cfetch(vlasov->jacob_vel, vidx) : 0,
    vlasov->pot_tot ? (const double *)gkyl_array_cfetch(vlasov->pot_tot, cidx) : 0, qIn, qRhsOut
  );
  ...
  vlasov->rad_vol(
    xc, dx, vlasov->jacob_vel ? (const double *)gkyl_array_cfetch(vlasov->jacob_vel, vidx) : 0,
    vlasov->rad ? (const double *)gkyl_array_cfetch(vlasov->rad, vidx) : 0, qIn, qRhsOut
  );
...
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list ser_hamil_vel_dense_vol_kernels[] = {
  // 1x kernels
  {NULL, vlasov_hamil_vel_dense_vol_1x1v_ser_p1, vlasov_hamil_vel_dense_vol_1x1v_ser_p2,
   vlasov_hamil_vel_dense_vol_1x1v_ser_p3}, // 0
  ...
  // 2x kernels
  {NULL, vlasov_hamil_vel_dense_vol_2x1v_ser_p1, vlasov_hamil_vel_dense_vol_2x1v_ser_p2,
   vlasov_hamil_vel_dense_vol_2x1v_ser_p3}, // 3
...
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list ser_hamil_vel_sparse_vol_kernels[] = {
  // 1x kernels
  {NULL, vlasov_hamil_vel_dense_vol_1x1v_ser_p1, vlasov_hamil_vel_dense_vol_1x1v_ser_p2,
   vlasov_hamil_vel_dense_vol_1x1v_ser_p3}, // 0
  {NULL, vlasov_hamil_vel_sparse_vol_1x2v_ser_p1, vlasov_hamil_vel_sparse_vol_1x2v_ser_p2, NULL
  }, // 1
  {NULL, vlasov_hamil_vel_sparse_vol_1x3v_ser_p1, vlasov_hamil_vel_sparse_vol_1x3v_ser_p2, NULL
  }, // 2
  // 2x kernels
  {NULL, vlasov_hamil_vel_dense_vol_2x1v_ser_p1, vlasov_hamil_vel_dense_vol_2x1v_ser_p2,
   vlasov_hamil_vel_dense_vol_2x1v_ser_p3}, // 3
  {NULL, vlasov_hamil_vel_sparse_vol_2x2v_ser_p1, vlasov_hamil_vel_sparse_vol_2x2v_ser_p2, NULL
  }, // 4
  {NULL, vlasov_hamil_vel_sparse_vol_2x3v_ser_p1, vlasov_hamil_vel_sparse_vol_2x3v_ser_p2, NULL
  }, // 5
  // 3x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list ser_hamil_phase_vol_kernels[] = {
  // 1x kernels
  {NULL, vlasov_hamil_phase_vol_1x1v_ser_p1, vlasov_hamil_phase_vol_1x1v_ser_p2,
   vlasov_hamil_phase_vol_1x1v_ser_p3}, // 0
  ...
  // 2x kernels
  {NULL, vlasov_hamil_phase_vol_2x1v_ser_p1, vlasov_hamil_phase_vol_2x1v_ser_p2,
   vlasov_hamil_phase_vol_2x1v_ser_p3}, // 3
...
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list tensor_hamil_phase_vol_kernels[] = {
  // 1x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list ser_nc_hamil_vel_dense_vol_kernels[] = {
  // 1x kernels
  {NULL, vlasov_nc_hamil_vel_dense_vol_1x1v_ser_p1, vlasov_nc_hamil_vel_dense_vol_1x1v_ser_p2, NULL
  }, // 0
  {NULL, vlasov_nc_hamil_vel_dense_vol_1x2v_ser_p1, vlasov_nc_hamil_vel_dense_vol_1x2v_ser_p2, NULL
  }, // 1
  {NULL, vlasov_nc_hamil_vel_dense_vol_1x3v_ser_p1, vlasov_nc_hamil_vel_dense_vol_1x3v_ser_p2, NULL
  }, // 2
  // 2x kernels
  {NULL, NULL, NULL, NULL}, // 3
  {NULL, vlasov_nc_hamil_vel_dense_vol_2x2v_ser_p1, vlasov_nc_hamil_vel_dense_vol_2x2v_ser_p2, NULL
  }, // 4
  {NULL, vlasov_nc_hamil_vel_dense_vol_2x3v_ser_p1, vlasov_nc_hamil_vel_dense_vol_2x3v_ser_p2, NULL
  }, // 5
  // 3x kernels
...
// Sparse-Hamiltonian variant; currently identical to the dense table
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list ser_nc_hamil_vel_sparse_vol_kernels[] = {
  // 1x kernels
  {NULL, vlasov_nc_hamil_vel_dense_vol_1x1v_ser_p1, vlasov_nc_hamil_vel_dense_vol_1x1v_ser_p2, NULL
  }, // 0
  {NULL, vlasov_nc_hamil_vel_sparse_vol_1x2v_ser_p1, vlasov_nc_hamil_vel_sparse_vol_1x2v_ser_p2,
   NULL}, // 1
  {NULL, vlasov_nc_hamil_vel_sparse_vol_1x3v_ser_p1, vlasov_nc_hamil_vel_sparse_vol_1x3v_ser_p2,
   NULL}, // 2
  // 2x kernels
  {NULL, NULL, NULL, NULL}, // 3
  {NULL, vlasov_nc_hamil_vel_sparse_vol_2x2v_ser_p1, vlasov_nc_hamil_vel_sparse_vol_2x2v_ser_p2,
   NULL}, // 4
  {NULL, vlasov_nc_hamil_vel_sparse_vol_2x3v_ser_p1, vlasov_nc_hamil_vel_sparse_vol_2x3v_ser_p2,
   NULL}, // 5
  // 3x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list tensor_nc_hamil_vel_sparse_vol_kernels[] =
  {
    // 1x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list tensor_nc_hamil_vel_dense_vol_kernels[] = {
  // 1x kernels
  {NULL, vlasov_nc_hamil_vel_dense_vol_1x1v_tensor_p1, NULL, NULL}, // 0
...
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list tensor_nc_hamil_phase_vol_kernels[] = {
  // 1x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list ser_nc_hamil_phase_vol_kernels[] = {
  // 1x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list tensor_hamil_vel_dense_vol_kernels[] = {
  // 1x kernels
  {NULL, vlasov_hamil_vel_dense_vol_1x1v_tensor_p1, vlasov_hamil_vel_dense_vol_1x1v_tensor_p2,
   vlasov_hamil_vel_dense_vol_1x1v_tensor_p3}, // 0
  {NULL, vlasov_hamil_vel_dense_vol_1x2v_tensor_p1, vlasov_hamil_vel_dense_vol_1x2v_tensor_p2, NULL
  }, // 1
  {NULL, vlasov_hamil_vel_dense_vol_1x3v_tensor_p1, vlasov_hamil_vel_dense_vol_1x3v_tensor_p2, NULL
  }, // 2
  // 2x kernels
  {NULL, vlasov_hamil_vel_dense_vol_2x1v_tensor_p1, vlasov_hamil_vel_dense_vol_2x1v_tensor_p2,
...
// Sparse-Hamiltonian variant; currently identical to the dense table
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_hamil_vol_kern_list tensor_hamil_vel_sparse_vol_kernels[] = {
  // 1x kernels
  {NULL, vlasov_hamil_vel_dense_vol_1x1v_tensor_p1, vlasov_hamil_vel_dense_vol_1x1v_tensor_p2,
   vlasov_hamil_vel_dense_vol_1x1v_tensor_p3}, // 0
  {NULL, vlasov_hamil_vel_sparse_vol_1x2v_tensor_p1, vlasov_hamil_vel_sparse_vol_1x2v_tensor_p2,
   NULL}, // 1
  {NULL, vlasov_hamil_vel_sparse_vol_1x3v_tensor_p1, vlasov_hamil_vel_sparse_vol_1x3v_tensor_p2,
   NULL}, // 2
  // 2x kernels
  {NULL, vlasov_hamil_vel_dense_vol_2x1v_tensor_p1, vlasov_hamil_vel_dense_vol_2x1v_tensor_p2,
   vlasov_hamil_vel_dense_vol_2x1v_tensor_p3}, // 3
...
GKYL_CU_D static const gkyl_dg_vlasov_E_vol_kern_list ser_E_vol_kernels[] = {
  // 1x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_E_vol_kern_list tensor_E_vol_kernels[] = {
  // 1x kernels
  {NULL, vlasov_E_vol_1x1v_tensor_p1, vlasov_E_vol_1x1v_tensor_p2, vlasov_E_vol_1x1v_tensor_p3
  }, // 0
...
GKYL_CU_D static const gkyl_dg_vlasov_phi_vol_kern_list ser_phi_vol_kernels[] = {
  // 1x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_phi_vol_kern_list tensor_phi_vol_kernels[] = {
  // 1x kernels
  {NULL, vlasov_phi_vol_1x1v_tensor_p1, vlasov_phi_vol_1x1v_tensor_p2, vlasov_phi_vol_1x1v_tensor_p3
  }, // 0
...
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list ser_Bx_hamil_vel_dense_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, no_B_vol, no_B_vol, NULL}, // 1
  {NULL, vlasov_Bx_hamil_vel_dense_vol_1x3v_ser_p1, vlasov_Bx_hamil_vel_dense_vol_1x3v_ser_p2, NULL
  }, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
  {NULL, no_B_vol, no_B_vol, NULL}, // 4
  {NULL, vlasov_Bx_hamil_vel_dense_vol_2x3v_ser_p1, vlasov_Bx_hamil_vel_dense_vol_2x3v_ser_p2, NULL
  }, // 5
  // 3x kernels
...
// Sparse-Hamiltonian variant; currently identical to the dense table
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list ser_Bx_hamil_vel_sparse_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, no_B_vol, no_B_vol, NULL}, // 1
  {NULL, vlasov_Bx_hamil_vel_sparse_vol_1x3v_ser_p1, vlasov_Bx_hamil_vel_sparse_vol_1x3v_ser_p2,
   NULL}, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
  {NULL, no_B_vol, no_B_vol, NULL}, // 4
  {NULL, vlasov_Bx_hamil_vel_sparse_vol_2x3v_ser_p1, vlasov_Bx_hamil_vel_sparse_vol_2x3v_ser_p2,
   NULL}, // 5
  // 3x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list tensor_Bx_hamil_vel_dense_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, no_B_vol, no_B_vol, NULL}, // 1
  {NULL, vlasov_Bx_hamil_vel_dense_vol_1x3v_tensor_p1, vlasov_Bx_hamil_vel_dense_vol_1x3v_tensor_p2,
   NULL}, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
  {NULL, no_B_vol, no_B_vol, NULL}, // 4
...
// Sparse-Hamiltonian variant; currently identical to the dense table
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list tensor_Bx_hamil_vel_sparse_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, no_B_vol, no_B_vol, NULL}, // 1
  {NULL, vlasov_Bx_hamil_vel_sparse_vol_1x3v_tensor_p1,
   vlasov_Bx_hamil_vel_sparse_vol_1x3v_tensor_p2, NULL}, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
  {NULL, no_B_vol, no_B_vol, NULL}, // 4
...
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list ser_Bx_hamil_phase_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, no_B_vol, no_B_vol, NULL}, // 1
...
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list ser_By_hamil_vel_dense_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, no_B_vol, no_B_vol, NULL}, // 1
  {NULL, vlasov_By_hamil_vel_dense_vol_1x3v_ser_p1, vlasov_By_hamil_vel_dense_vol_1x3v_ser_p2, NULL
  }, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
  {NULL, no_B_vol, no_B_vol, NULL}, // 4
  {NULL, vlasov_By_hamil_vel_dense_vol_2x3v_ser_p1, vlasov_By_hamil_vel_dense_vol_2x3v_ser_p2, NULL
  }, // 5
  // 3x kernels
...
// Sparse-Hamiltonian variant; currently identical to the dense table
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list ser_By_hamil_vel_sparse_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, no_B_vol, no_B_vol, NULL}, // 1
  {NULL, vlasov_By_hamil_vel_sparse_vol_1x3v_ser_p1, vlasov_By_hamil_vel_sparse_vol_1x3v_ser_p2,
   NULL}, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
  {NULL, no_B_vol, no_B_vol, NULL}, // 4
  {NULL, vlasov_By_hamil_vel_sparse_vol_2x3v_ser_p1, vlasov_By_hamil_vel_sparse_vol_2x3v_ser_p2,
   NULL}, // 5
  // 3x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list tensor_By_hamil_vel_dense_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, no_B_vol, no_B_vol, NULL}, // 1
  {NULL, vlasov_By_hamil_vel_dense_vol_1x3v_tensor_p1, vlasov_By_hamil_vel_dense_vol_1x3v_tensor_p2,
   NULL}, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
  {NULL, no_B_vol, no_B_vol, NULL}, // 4
...
// Sparse-Hamiltonian variant; currently identical to the dense table
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list tensor_By_hamil_vel_sparse_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, no_B_vol, no_B_vol, NULL}, // 1
  {NULL, vlasov_By_hamil_vel_sparse_vol_1x3v_tensor_p1,
   vlasov_By_hamil_vel_sparse_vol_1x3v_tensor_p2, NULL}, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
  {NULL, no_B_vol, no_B_vol, NULL}, // 4
...
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list ser_By_hamil_phase_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, no_B_vol, no_B_vol, NULL}, // 1
...
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list ser_Bz_hamil_vel_dense_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, vlasov_Bz_hamil_vel_dense_vol_1x2v_ser_p1, vlasov_Bz_hamil_vel_dense_vol_1x2v_ser_p2, NULL
  }, // 1
  {NULL, vlasov_Bz_hamil_vel_dense_vol_1x3v_ser_p1, vlasov_Bz_hamil_vel_dense_vol_1x3v_ser_p2, NULL
  }, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
  {NULL, vlasov_Bz_hamil_vel_dense_vol_2x2v_ser_p1, vlasov_Bz_hamil_vel_dense_vol_2x2v_ser_p2, NULL
  }, // 4
  {NULL, vlasov_Bz_hamil_vel_dense_vol_2x3v_ser_p1, vlasov_Bz_hamil_vel_dense_vol_2x3v_ser_p2, NULL
  }, // 5
  // 3x kernels
...
// Sparse-Hamiltonian variant; currently identical to the dense table
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list ser_Bz_hamil_vel_sparse_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, vlasov_Bz_hamil_vel_sparse_vol_1x2v_ser_p1, vlasov_Bz_hamil_vel_sparse_vol_1x2v_ser_p2,
   NULL}, // 1
  {NULL, vlasov_Bz_hamil_vel_sparse_vol_1x3v_ser_p1, vlasov_Bz_hamil_vel_sparse_vol_1x3v_ser_p2,
   NULL}, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
  {NULL, vlasov_Bz_hamil_vel_sparse_vol_2x2v_ser_p1, vlasov_Bz_hamil_vel_sparse_vol_2x2v_ser_p2,
   NULL}, // 4
  {NULL, vlasov_Bz_hamil_vel_sparse_vol_2x3v_ser_p1, vlasov_Bz_hamil_vel_sparse_vol_2x3v_ser_p2,
   NULL}, // 5
  // 3x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list tensor_Bz_hamil_vel_dense_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, vlasov_Bz_hamil_vel_dense_vol_1x2v_tensor_p1, vlasov_Bz_hamil_vel_dense_vol_1x2v_tensor_p2,
   NULL}, // 1
  {NULL, vlasov_Bz_hamil_vel_dense_vol_1x3v_tensor_p1, vlasov_Bz_hamil_vel_dense_vol_1x3v_tensor_p2,
   NULL}, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
...
// Sparse-Hamiltonian variant; currently identical to the dense table
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list tensor_Bz_hamil_vel_sparse_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
  {NULL, vlasov_Bz_hamil_vel_sparse_vol_1x2v_tensor_p1,
   vlasov_Bz_hamil_vel_sparse_vol_1x2v_tensor_p2, NULL}, // 1
  {NULL, vlasov_Bz_hamil_vel_sparse_vol_1x3v_tensor_p1,
   vlasov_Bz_hamil_vel_sparse_vol_1x3v_tensor_p2, NULL}, // 2
  // 2x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 3
...
GKYL_CU_D static const gkyl_dg_vlasov_B_vol_kern_list ser_Bz_hamil_phase_vol_kernels[] = {
  // 1x kernels
  {NULL, no_B_vol, no_B_vol, no_B_vol}, // 0
...
GKYL_CU_D static const gkyl_dg_vlasov_rad_vol_kern_list ser_rad_vol_kernels[] = {
  // 1x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_rad_vol_kern_list tensor_rad_vol_kernels[] = {
  // 1x kernels
  {NULL, vlasov_rad_vol_1x1v_tensor_p1, vlasov_rad_vol_1x1v_tensor_p2, vlasov_rad_vol_1x1v_tensor_p3
  }, // 0
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  ser_stream_hamil_vel_dense_surf_x_kernels[] = {
    // 1x kernels
    {NULL, vlasov_hamil_vel_dense_surfx_1x1v_ser_p1, vlasov_hamil_vel_dense_surfx_1x1v_ser_p2,
     vlasov_hamil_vel_dense_surfx_1x1v_ser_p3}, // 0
    {NULL, vlasov_hamil_vel_dense_surfx_1x2v_ser_p1, vlasov_hamil_vel_dense_surfx_1x2v_ser_p2, NULL
    }, // 1
    {NULL, vlasov_hamil_vel_dense_surfx_1x3v_ser_p1, vlasov_hamil_vel_dense_surfx_1x3v_ser_p2, NULL
    }, // 2
    // 2x kernels
    {NULL, vlasov_hamil_vel_dense_surfx_2x1v_ser_p1, vlasov_hamil_vel_dense_surfx_2x1v_ser_p2,
     vlasov_hamil_vel_dense_surfx_2x1v_ser_p3}, // 3
    {NULL, vlasov_hamil_vel_dense_surfx_2x2v_ser_p1, vlasov_hamil_vel_dense_surfx_2x2v_ser_p2, NULL
    }, // 4
    {NULL, vlasov_hamil_vel_dense_surfx_2x3v_ser_p1, vlasov_hamil_vel_dense_surfx_2x3v_ser_p2, NULL
    }, // 5
    // 3x kernels
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  ser_stream_hamil_vel_sparse_surf_x_kernels[] = {
    // 1x kernels
    {NULL, vlasov_hamil_vel_dense_surfx_1x1v_ser_p1, vlasov_hamil_vel_dense_surfx_1x1v_ser_p2,
     vlasov_hamil_vel_dense_surfx_1x1v_ser_p3}, // 0
    {NULL, vlasov_hamil_vel_sparse_surfx_1x2v_ser_p1, vlasov_hamil_vel_sparse_surfx_1x2v_ser_p2,
     NULL}, // 1
    {NULL, vlasov_hamil_vel_sparse_surfx_1x3v_ser_p1, vlasov_hamil_vel_sparse_surfx_1x3v_ser_p2,
     NULL}, // 2
    // 2x kernels
    {NULL, vlasov_hamil_vel_dense_surfx_2x1v_ser_p1, vlasov_hamil_vel_dense_surfx_2x1v_ser_p2,
     vlasov_hamil_vel_dense_surfx_2x1v_ser_p3}, // 3
    {NULL, vlasov_hamil_vel_sparse_surfx_2x2v_ser_p1, vlasov_hamil_vel_sparse_surfx_2x2v_ser_p2,
     NULL}, // 4
    {NULL, vlasov_hamil_vel_sparse_surfx_2x3v_ser_p1, vlasov_hamil_vel_sparse_surfx_2x3v_ser_p2,
     NULL}, // 5
    // 3x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  tensor_stream_hamil_vel_dense_surf_x_kernels[] = {
    // 1x kernels
    {NULL, vlasov_hamil_vel_dense_surfx_1x1v_tensor_p1, vlasov_hamil_vel_dense_surfx_1x1v_tensor_p2,
     vlasov_hamil_vel_dense_surfx_1x1v_tensor_p3}, // 0
    {NULL, vlasov_hamil_vel_dense_surfx_1x2v_tensor_p1, vlasov_hamil_vel_dense_surfx_1x2v_tensor_p2,
     NULL}, // 1
    {NULL, vlasov_hamil_vel_dense_surfx_1x3v_tensor_p1, vlasov_hamil_vel_dense_surfx_1x3v_tensor_p2,
     NULL}, // 2
    // 2x kernels
    {NULL, vlasov_hamil_vel_dense_surfx_2x1v_tensor_p1, vlasov_hamil_vel_dense_surfx_2x1v_tensor_p2,
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  tensor_stream_hamil_vel_sparse_surf_x_kernels[] = {
    // 1x kernels
    {NULL, vlasov_hamil_vel_dense_surfx_1x1v_tensor_p1, vlasov_hamil_vel_dense_surfx_1x1v_tensor_p2,
     vlasov_hamil_vel_dense_surfx_1x1v_tensor_p3}, // 0
    {NULL, vlasov_hamil_vel_sparse_surfx_1x2v_tensor_p1,
     vlasov_hamil_vel_sparse_surfx_1x2v_tensor_p2, NULL}, // 1
    {NULL, vlasov_hamil_vel_sparse_surfx_1x3v_tensor_p1,
     vlasov_hamil_vel_sparse_surfx_1x3v_tensor_p2, NULL}, // 2
    // 2x kernels
    {NULL, vlasov_hamil_vel_dense_surfx_2x1v_tensor_p1, vlasov_hamil_vel_dense_surfx_2x1v_tensor_p2,
     vlasov_hamil_vel_dense_surfx_2x1v_tensor_p3}, // 3
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  ser_stream_hamil_vel_dense_surf_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    ...
    {NULL, vlasov_hamil_vel_dense_surfy_2x2v_ser_p1, vlasov_hamil_vel_dense_surfy_2x2v_ser_p2, NULL
    }, // 4
    {NULL, vlasov_hamil_vel_dense_surfy_2x3v_ser_p1, vlasov_hamil_vel_dense_surfy_2x3v_ser_p2, NULL
    }, // 5
    // 3x kernels
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  ser_stream_hamil_vel_sparse_surf_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
    // 2x kernels
    {NULL, no_stream_surf, no_stream_surf, no_stream_surf}, // 3
    {NULL, vlasov_hamil_vel_sparse_surfy_2x2v_ser_p1, vlasov_hamil_vel_sparse_surfy_2x2v_ser_p2,
     NULL}, // 4
    {NULL, vlasov_hamil_vel_sparse_surfy_2x3v_ser_p1, vlasov_hamil_vel_sparse_surfy_2x3v_ser_p2,
     NULL}, // 5
    // 3x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  tensor_stream_hamil_vel_dense_surf_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  tensor_stream_hamil_vel_sparse_surf_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  ser_stream_hamil_vel_dense_surf_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  ser_stream_hamil_vel_sparse_surf_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  tensor_stream_hamil_vel_dense_surf_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_kern_list
  tensor_stream_hamil_vel_sparse_surf_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_from_flux_kern_list ser_stream_surf_x_kernels[] = {
  // 1x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_from_flux_kern_list ser_stream_surf_y_kernels[] = {
  // 1x kernels
  {NULL, NULL, NULL, NULL}, // 0
  {NULL, NULL, NULL, NULL}, // 1
  {NULL, NULL, NULL, NULL}, // 2
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_from_flux_kern_list ser_stream_surf_z_kernels[] = {
  // 1x kernels
  {NULL, NULL, NULL, NULL}, // 0
  {NULL, NULL, NULL, NULL}, // 1
  {NULL, NULL, NULL, NULL}, // 2
...
// Streaming general (NC) Hamiltonian surface kernel list: x-direction (Serendipity basis)
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_from_flux_kern_list
  ser_stream_ho_surf_x_kernels[] = {
    // 1x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_from_flux_kern_list
  tensor_stream_ho_surf_x_kernels[] = {
    // 1x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_from_flux_kern_list
  tensor_stream_ho_surf_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_surf_from_flux_kern_list
  tensor_stream_ho_surf_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_from_flux_kern_list
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_from_flux_kern_list
  tensor_stream_boundary_ho_surf_x_kernels[] = {
    // 1x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_from_flux_kern_list
  tensor_stream_boundary_ho_surf_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_from_flux_kern_list
  tensor_stream_boundary_ho_surf_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
GKYL_CU_D static const gkyl_dg_vlasov_accel_surf_kern_list tensor_accel_surf_vx_kernels[] = {
  // 1x kernels
  {NULL, vlasov_surfvx_1x1v_tensor_p1, vlasov_surfvx_1x1v_tensor_p2, vlasov_surfvx_1x1v_tensor_p3
  }, // 0
...
// Acceleration surface kernel list: vx-direction (Serendipity basis)
GKYL_CU_D static const gkyl_dg_vlasov_accel_surf_kern_list ser_accel_ho_surf_vx_kernels[] = {
  // 1x kernels
  {NULL, vlasov_surfvx_1x1v_ser_p1, vlasov_ho_surfvx_1x1v_ser_p2, vlasov_ho_surfvx_1x1v_ser_p3
  }, // 0
  ...
  // 2x kernels
  {NULL, vlasov_surfvx_2x1v_ser_p1, vlasov_ho_surfvx_2x1v_ser_p2, vlasov_ho_surfvx_2x1v_ser_p3
  }, // 3
...
// Acceleration surface kernel list: vy-direction (Serendipity basis)
GKYL_CU_D static const gkyl_dg_vlasov_accel_surf_kern_list ser_accel_ho_surf_vy_kernels[] = {
  // 1x kernels
  {NULL, NULL, NULL, NULL}, // 0
...
// Acceleration surface kernel list: vz-direction (Serendipity basis)
GKYL_CU_D static const gkyl_dg_vlasov_accel_surf_kern_list ser_accel_ho_surf_vz_kernels[] = {
  // 1x kernels
  {NULL, NULL, NULL, NULL}, // 0
  {NULL, NULL, NULL, NULL}, // 1
...
GKYL_CU_D static const gkyl_dg_vlasov_accel_surf_kern_list tensor_accel_ho_surf_vx_kernels[] = {
  // 1x kernels
  {NULL, vlasov_ho_surfvx_1x1v_tensor_p1, vlasov_surfvx_1x1v_tensor_p2, vlasov_surfvx_1x1v_tensor_p3
  }, // 0
...
// variants) this list holds the ho_ kernels; tensor p>1 has a single
// (high-order by design) variant shared with the plain tensor list.
GKYL_CU_D static const gkyl_dg_vlasov_accel_surf_kern_list tensor_accel_ho_surf_vy_kernels[] = {
  // 1x kernels
  {NULL, NULL, NULL, NULL}, // 0
...
// variants) this list holds the ho_ kernels; tensor p>1 has a single
// (high-order by design) variant shared with the plain tensor list.
GKYL_CU_D static const gkyl_dg_vlasov_accel_surf_kern_list tensor_accel_ho_surf_vz_kernels[] = {
  // 1x kernels
  {NULL, NULL, NULL, NULL}, // 0
  {NULL, NULL, NULL, NULL}, // 1
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  ser_stream_hamil_vel_dense_boundary_surf_x_kernels[] = {
    // 1x kernels
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_1x1v_ser_p1,
     vlasov_hamil_vel_dense_boundary_surfx_1x1v_ser_p2,
     vlasov_hamil_vel_dense_boundary_surfx_1x1v_ser_p3}, // 0
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_1x2v_ser_p1,
     vlasov_hamil_vel_dense_boundary_surfx_1x2v_ser_p2, NULL}, // 1
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_1x3v_ser_p1,
     vlasov_hamil_vel_dense_boundary_surfx_1x3v_ser_p2, NULL}, // 2
    // 2x kernels
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_2x1v_ser_p1,
     vlasov_hamil_vel_dense_boundary_surfx_2x1v_ser_p2,
     vlasov_hamil_vel_dense_boundary_surfx_2x1v_ser_p3}, // 3
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_2x2v_ser_p1,
     vlasov_hamil_vel_dense_boundary_surfx_2x2v_ser_p2, NULL}, // 4
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_2x3v_ser_p1,
     vlasov_hamil_vel_dense_boundary_surfx_2x3v_ser_p2, NULL}, // 5
    // 3x kernels
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  ser_stream_hamil_vel_sparse_boundary_surf_x_kernels[] = {
    // 1x kernels
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_1x1v_ser_p1,
     vlasov_hamil_vel_dense_boundary_surfx_1x1v_ser_p2,
     vlasov_hamil_vel_dense_boundary_surfx_1x1v_ser_p3}, // 0
    {NULL, vlasov_hamil_vel_sparse_boundary_surfx_1x2v_ser_p1,
     vlasov_hamil_vel_sparse_boundary_surfx_1x2v_ser_p2, NULL}, // 1
    {NULL, vlasov_hamil_vel_sparse_boundary_surfx_1x3v_ser_p1,
     vlasov_hamil_vel_sparse_boundary_surfx_1x3v_ser_p2, NULL}, // 2
    // 2x kernels
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_2x1v_ser_p1,
     vlasov_hamil_vel_dense_boundary_surfx_2x1v_ser_p2,
     vlasov_hamil_vel_dense_boundary_surfx_2x1v_ser_p3}, // 3
    {NULL, vlasov_hamil_vel_sparse_boundary_surfx_2x2v_ser_p1,
     vlasov_hamil_vel_sparse_boundary_surfx_2x2v_ser_p2, NULL}, // 4
    {NULL, vlasov_hamil_vel_sparse_boundary_surfx_2x3v_ser_p1,
     vlasov_hamil_vel_sparse_boundary_surfx_2x3v_ser_p2, NULL}, // 5
    // 3x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  tensor_stream_hamil_vel_dense_boundary_surf_x_kernels[] = {
    // 1x kernels
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_1x1v_tensor_p1,
     vlasov_hamil_vel_dense_boundary_surfx_1x1v_tensor_p2,
     vlasov_hamil_vel_dense_boundary_surfx_1x1v_tensor_p3}, // 0
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_1x2v_tensor_p1,
     vlasov_hamil_vel_dense_boundary_surfx_1x2v_tensor_p2, NULL}, // 1
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_1x3v_tensor_p1,
     vlasov_hamil_vel_dense_boundary_surfx_1x3v_tensor_p2, NULL}, // 2
    // 2x kernels
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_2x1v_tensor_p1,
     vlasov_hamil_vel_dense_boundary_surfx_2x1v_tensor_p2,
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  tensor_stream_hamil_vel_sparse_boundary_surf_x_kernels[] = {
    // 1x kernels
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_1x1v_tensor_p1,
     vlasov_hamil_vel_dense_boundary_surfx_1x1v_tensor_p2,
     vlasov_hamil_vel_dense_boundary_surfx_1x1v_tensor_p3}, // 0
    {NULL, vlasov_hamil_vel_sparse_boundary_surfx_1x2v_tensor_p1,
     vlasov_hamil_vel_sparse_boundary_surfx_1x2v_tensor_p2, NULL}, // 1
    {NULL, vlasov_hamil_vel_sparse_boundary_surfx_1x3v_tensor_p1,
     vlasov_hamil_vel_sparse_boundary_surfx_1x3v_tensor_p2, NULL}, // 2
    // 2x kernels
    {NULL, vlasov_hamil_vel_dense_boundary_surfx_2x1v_tensor_p1,
     vlasov_hamil_vel_dense_boundary_surfx_2x1v_tensor_p2,
     vlasov_hamil_vel_dense_boundary_surfx_2x1v_tensor_p3}, // 3
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  ser_stream_hamil_vel_dense_boundary_surf_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
    ...
    {NULL, vlasov_hamil_vel_dense_boundary_surfy_2x2v_ser_p1,
     vlasov_hamil_vel_dense_boundary_surfy_2x2v_ser_p2, NULL}, // 4
    {NULL, vlasov_hamil_vel_dense_boundary_surfy_2x3v_ser_p1,
     vlasov_hamil_vel_dense_boundary_surfy_2x3v_ser_p2, NULL}, // 5
    // 3x kernels
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  ser_stream_hamil_vel_sparse_boundary_surf_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
    // 2x kernels
    {NULL, no_stream_boundary_surf, no_stream_boundary_surf, no_stream_boundary_surf}, // 3
    {NULL, vlasov_hamil_vel_sparse_boundary_surfy_2x2v_ser_p1,
     vlasov_hamil_vel_sparse_boundary_surfy_2x2v_ser_p2, NULL}, // 4
    {NULL, vlasov_hamil_vel_sparse_boundary_surfy_2x3v_ser_p1,
     vlasov_hamil_vel_sparse_boundary_surfy_2x3v_ser_p2, NULL}, // 5
    // 3x kernels
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  tensor_stream_hamil_vel_dense_boundary_surf_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  tensor_stream_hamil_vel_sparse_boundary_surf_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  ser_stream_hamil_vel_dense_boundary_surf_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  ser_stream_hamil_vel_sparse_boundary_surf_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  tensor_stream_hamil_vel_dense_boundary_surf_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
// (points at the same kernels) until the sparse kernels land.
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_kern_list
  tensor_stream_hamil_vel_sparse_boundary_surf_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
// Streaming general (NC) Hamiltonian boundary surface kernel list: x-direction (Serendipity basis)
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_from_flux_kern_list
  ser_stream_boundary_ho_surf_x_kernels[] = {
    // 1x kernels
...
// Streaming general (NC) Hamiltonian boundary surface kernel list: y-direction (Serendipity basis)
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_from_flux_kern_list
  ser_stream_boundary_ho_surf_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
// Streaming general (NC) Hamiltonian boundary surface kernel list: z-direction (Serendipity basis)
GKYL_CU_D static const gkyl_dg_vlasov_stream_boundary_surf_from_flux_kern_list
  ser_stream_boundary_ho_surf_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, NULL, NULL, NULL}, // 2
...
GKYL_CU_D static const gkyl_dg_vlasov_accel_boundary_surf_kern_list
  ser_accel_boundary_surf_vx_kernels[] = {
    // 1x kernels
    {NULL, vlasov_boundary_surfvx_1x1v_ser_p1, vlasov_boundary_surfvx_1x1v_ser_p2,
     vlasov_boundary_surfvx_1x1v_ser_p3}, // 0
    ...
    // 2x kernels
    {NULL, vlasov_boundary_surfvx_2x1v_ser_p1, vlasov_boundary_surfvx_2x1v_ser_p2,
     vlasov_boundary_surfvx_2x1v_ser_p3}, // 3
...
GKYL_CU_D static const gkyl_dg_vlasov_accel_boundary_surf_kern_list
  tensor_accel_boundary_surf_vx_kernels[] = {
    // 1x kernels
    {NULL, vlasov_boundary_surfvx_1x1v_tensor_p1, vlasov_boundary_surfvx_1x1v_tensor_p2,
     vlasov_boundary_surfvx_1x1v_tensor_p3}, // 0
...
// Acceleration boundary surface kernel (zero-flux BCs) list: vx-direction (Serendipity basis)
GKYL_CU_D static const gkyl_dg_vlasov_accel_boundary_surf_kern_list
  ser_accel_boundary_ho_surf_vx_kernels[] = {
    // 1x kernels
    {NULL, vlasov_boundary_surfvx_1x1v_ser_p1, vlasov_boundary_ho_surfvx_1x1v_ser_p2,
     vlasov_boundary_ho_surfvx_1x1v_ser_p3}, // 0
    ...
    // 2x kernels
    {NULL, vlasov_boundary_surfvx_2x1v_ser_p1, vlasov_boundary_ho_surfvx_2x1v_ser_p2,
     vlasov_boundary_ho_surfvx_2x1v_ser_p3}, // 3
...
// Acceleration boundary surface kernel (zero-flux BCs) list: vy-direction (Serendipity basis)
GKYL_CU_D static const gkyl_dg_vlasov_accel_boundary_surf_kern_list
  ser_accel_boundary_ho_surf_vy_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
...
// Acceleration boundary surface kernel (zero-flux BCs) list: vz-direction (Serendipity basis)
GKYL_CU_D static const gkyl_dg_vlasov_accel_boundary_surf_kern_list
  ser_accel_boundary_ho_surf_vz_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
...
// (high-order by design) variant shared with the plain tensor list.
GKYL_CU_D static const gkyl_dg_vlasov_accel_boundary_surf_kern_list
  tensor_accel_boundary_ho_surf_vx_kernels[] = {
    // 1x kernels
    {NULL, vlasov_boundary_ho_surfvx_1x1v_tensor_p1, vlasov_boundary_surfvx_1x1v_tensor_p2,
     vlasov_boundary_surfvx_1x1v_tensor_p3}, // 0
    {NULL, vlasov_boundary_ho_surfvx_1x2v_tensor_p1, vlasov_boundary_surfvx_1x2v_tensor_p2, NULL
    }, // 1
    {NULL, vlasov_boundary_ho_surfvx_1x3v_tensor_p1, vlasov_boundary_surfvx_1x3v_tensor_p2, NULL
    }, // 2
    // 2x kernels
    {NULL, vlasov_boundary_ho_surfvx_2x1v_tensor_p1, vlasov_boundary_surfvx_2x1v_tensor_p2,
     vlasov_boundary_surfvx_2x1v_tensor_p3}, // 3
...
// (high-order by design) variant shared with the plain tensor list.
GKYL_CU_D static const gkyl_dg_vlasov_accel_boundary_surf_kern_list
  tensor_accel_boundary_ho_surf_vy_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, vlasov_boundary_ho_surfvy_1x2v_tensor_p1, vlasov_boundary_surfvy_1x2v_tensor_p2, NULL
    }, // 1
    {NULL, vlasov_boundary_ho_surfvy_1x3v_tensor_p1, vlasov_boundary_surfvy_1x3v_tensor_p2, NULL
    }, // 2
    // 2x kernels
    {NULL, NULL, NULL, NULL}, // 3
...
// (high-order by design) variant shared with the plain tensor list.
GKYL_CU_D static const gkyl_dg_vlasov_accel_boundary_surf_kern_list
  tensor_accel_boundary_ho_surf_vz_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    {NULL, NULL, NULL, NULL}, // 1
    {NULL, vlasov_boundary_ho_surfvz_1x3v_tensor_p1, vlasov_boundary_surfvz_1x3v_tensor_p2, NULL
    }, // 2
    // 2x kernels
    {NULL, NULL, NULL, NULL}, // 3
    {NULL, NULL, NULL, NULL}, // 4
...
)
{
  ...
  if (fabs(qInL[0]) < vlasov->skip_cell_thresh && fabs(qInC[0]) < vlasov->skip_cell_thresh &&
  ...
  if (dir < vlasov->cdim) {
    ...
    if (vlasov->use_conf_flux_surf) {
      ...
      const double *conf_flux_surf_l =
        (const double *)gkyl_array_cfetch(vlasov->conf_flux_surf, pidxC);
      const double *conf_flux_surf_r =
        (const double *)gkyl_array_cfetch(vlasov->conf_flux_surf, pidxR);

      return vlasov->stream_surf_from_flux[dir](
        xcC, dxC, conf_flux_surf_l, conf_flux_surf_r, qRhsOut
      );
    } else {
    ...
    return vlasov->accel_surf[dir - vlasov->cdim](
      xcC, dxC, vel_flux_surf_l, vel_flux_surf_r, qRhsOut
    );
  }
}
...
)
{
  ...
  if (fabs(qInEdge[0]) < vlasov->skip_cell_thresh && fabs(qInSkin[0]) < vlasov->skip_cell_thresh) {
    return 0.0; // Immediately return if phase-space density is below threshold.
  }
  ...
  if (dir < vlasov->cdim) {
    ...
    if (vlasov->use_conf_flux_surf) {
      ...
      return vlasov->stream_boundary_surf_from_flux[dir](
        xcSkin, dxSkin, edge, conf_flux_surf, qRhsOut
      );
    } else {
    ...
    return vlasov->accel_boundary_surf[dir - vlasov->cdim](
      xcSkin, dxSkin, edge, vel_flux_surf, qRhsOut
    );
  }
  return 0.;
}

...
)
{
  ...
  if (use_gpu) {
    ...
    assert(
      NULL != ((gkyl_basis_phase_kernel_type(cbasis, pbasis) == GKYL_BASIS_MODAL_TENSOR) ?
                 ten_vol_kernels :
                 ser_vol_kernels)[cv_index[cdim_h].vdim[vdim_h]]
                .kernels[po_h]
    );
    assert(
      NULL != ((gkyl_basis_phase_kernel_type(cbasis, pbasis) == GKYL_BASIS_MODAL_TENSOR) ?
                 ten_surf_vx_kernels :
                 ser_surf_vx_kernels)[cv_index[cdim_h].vdim[vdim_h]]
                .kernels[po_h]
    );

  ...
  for (unsigned long linc1 = threadIdx.x + blockIdx.x * blockDim.x; linc1 < conf_range.volume;
    ...
    for (int dir = 0; dir < cdim; ++dir) {
      const_sgn_alpha_d[dir] = up->alpha_surf[dir](
        xc, up->conf_grid.dx, (const double *)gkyl_array_cfetch(phi, loc), alpha_surf_d,
        sgn_alpha_surf_d
      );
      ...
      if (idx[dir] == conf_range.upper[dir]) {
        ...
        const_sgn_alpha_ext_d[dir] = up->alpha_edge_surf[dir](
          xc, up->conf_grid.dx, (const double *)gkyl_array_fetch(phi, loc), alpha_surf_ext_d,
...
{
  struct gkyl_dg_calc_canonical_pb_fluid_vars *up = (struct gkyl_dg_calc_canonical_pb_fluid_vars *)

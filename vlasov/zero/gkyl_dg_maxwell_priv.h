...
typedef double (*maxwell_surf_t)(
  const gkyl_maxwell_inp *meq, const double *w, const double *dx, const double *jacob_pos_l,
  const double *jacob_pos_c, const double *jacob_pos_r, const double *ql, const double *qc,
  const double *qr, double *GKYL_RESTRICT out
);

typedef double (*maxwell_surf_from_flux_t)(
  const double *w, const double *dx, const double *flux_l, const double *flux_r,
  double *GKYL_RESTRICT out
);

typedef void (*maxwell_gr_maxwell_vol_t)(
  const gkyl_dg_gr_maxwell_inp *meq, const double *w, const double *dx, const double *jacob_pos,
  const double *lapse_nodal, const double *shift_nodal, const double *h_ij_nodal,
  const double *h_ij_inv_nodal, const double *det_h_nodal, const double *fields_no_J,
  double *GKYL_RESTRICT out
);
...
typedef struct {
  maxwell_gr_maxwell_vol_t kernels[4];
} gkyl_dg_maxwell_gr_maxwell_vol_kern_list;
typedef struct {
  maxwell_surf_from_flux_t kernels[4];
} gkyl_dg_maxwell_surf_from_flux_kern_list;
...
struct dg_maxwell {
  ...
  const struct gkyl_array
    *jacob_pos; // Configuration-space position-map Jacobian (borrowed; never NULL:
...
GKYL_CU_DH static double
kernel_maxwell_vol_2x_tensor_p3(
  const struct gkyl_dg_eqn *eqn, const double *xc, const double *dx, const int *idx,
  const double *qIn, double *GKYL_RESTRICT qRhsOut
)
{
...
GKYL_CU_D static const gkyl_dg_maxwell_vol_kern_list ser_vol_kernels[] = {
  {NULL, kernel_maxwell_vol_1x_ser_p1, kernel_maxwell_vol_1x_ser_p2, kernel_maxwell_vol_1x_ser_p3
  }, // 0
  {NULL, kernel_maxwell_vol_2x_ser_p1, kernel_maxwell_vol_2x_ser_p2, kernel_maxwell_vol_2x_ser_p3
  }, // 1
...
GKYL_CU_D static const gkyl_dg_maxwell_vol_kern_list ten_vol_kernels[] = {
  ...
  {NULL, kernel_maxwell_vol_2x_ser_p1, kernel_maxwell_vol_2x_tensor_p2,
   kernel_maxwell_vol_2x_tensor_p3}, // 1
  {NULL, kernel_maxwell_vol_3x_ser_p1, kernel_maxwell_vol_3x_tensor_p2, NULL} // 2
};

// Volume kernel list (Serendipity basis)
GKYL_CU_D static const gkyl_dg_maxwell_gr_maxwell_vol_kern_list ser_gr_maxwell_vol_kernels[] = {
...
// Volume kernel list (Tensor basis)
GKYL_CU_D static const gkyl_dg_maxwell_gr_maxwell_vol_kern_list ten_gr_maxwell_vol_kernels[] = {
...
GKYL_CU_D static const gkyl_dg_maxwell_surf_from_flux_kern_list
  ser_gr_maxwell_surf_from_flux_x_kernels[] = {
    // 1x kernels
...
GKYL_CU_D static const gkyl_dg_maxwell_surf_from_flux_kern_list
  ser_gr_maxwell_surf_from_flux_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    // 2x kernels
...
GKYL_CU_D static const gkyl_dg_maxwell_surf_from_flux_kern_list
  ser_gr_maxwell_surf_from_flux_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    // 2x kernels
    {NULL, NULL, NULL, NULL}, // 1
...
GKYL_CU_D static const gkyl_dg_maxwell_surf_from_flux_kern_list
  tensor_gr_maxwell_surf_from_flux_x_kernels[] = {
    // 1x kernels
...
GKYL_CU_D static const gkyl_dg_maxwell_surf_from_flux_kern_list
  tensor_gr_maxwell_surf_from_flux_y_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    // 2x kernels
...
GKYL_CU_D static const gkyl_dg_maxwell_surf_from_flux_kern_list
  tensor_gr_maxwell_surf_from_flux_z_kernels[] = {
    // 1x kernels
    {NULL, NULL, NULL, NULL}, // 0
    // 2x kernels
    {NULL, NULL, NULL, NULL}, // 1
...
)
{
  ...
  if (maxwell->use_conf_flux_surf) {
    ...
    const double *h_ij_inv =
      (const double *)gkyl_array_cfetch(maxwell->h_ij_inv->nodal_arr_vol, cidx);
    ...
    maxwell->vol(
      &maxwell->gr_maxwell_data, xc, dx, jacob_pos_d, lapse, shift, h_ij, h_ij_inv, det_h, qIn,
      qRhsOut
    );
...
)
{
  ...
  if (maxwell->use_conf_flux_surf) {
    ...
    const double *conf_flux_surf_l =
      (const double *)gkyl_array_cfetch(maxwell->conf_flux_surf, cidxC);
    const double *conf_flux_surf_r =
      (const double *)gkyl_array_cfetch(maxwell->conf_flux_surf, cidxR);

    return maxwell->surf_from_flux[dir](xcC, dxC, conf_flux_surf_l, conf_flux_surf_r, qRhsOut);
  } else {
    ...
    return maxwell->surf[dir](
      &maxwell->maxwell_data, xcC, dxC, jpl, jpc, jpr, qInL, qInC, qInR, qRhsOut
    );
  }
}

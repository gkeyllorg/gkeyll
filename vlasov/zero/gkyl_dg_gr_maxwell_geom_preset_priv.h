...
void
eval_spherical_h_ij_inv_rtheta(
  double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx
)
{
...
eval_ks_h_ij_inv_r(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double Jc_sq = h_pp * h_rt * h_rt - 2 * h_rt * h_rp * h_tp + h_tt * h_rp * h_rp +
                 h_rr * h_tp * h_tp - h_rr * h_tt * h_pp;
...
)
{
  ...
  fout[0] = (-2.0 * M * M * a * a - M * a * a * r + 5.0 * M * r * r * r + 2.0 * r * r * r * r) /
            ((r * r + 2.0 * M * r) * (r * r + 2.0 * M * r) * r);
  fout[1] = 0.0;
...
)
{
  ...
  fout[0] = (4 * a * a * r + 4 * r * r * r + 2 * M * a * a * sin(theta) * sin(theta) -
             2 * a * a * r * sin(theta) * sin(theta)) /
              (rho_sq_M * rho_sq) -
            (term1 * ((M + r) / rho_sq_M + r / rho_sq)) / (rho_sq_M * rho_sq);
  fout[1] = (cos(theta) * (rho_sq_M * rho_sq + 2 * M * a * a * r * sin(theta) * sin(theta))) /
            (rho_sq_M * rho_sq * rho_sq * sin(theta));

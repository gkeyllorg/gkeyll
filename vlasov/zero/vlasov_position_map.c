...
static void
vlasov_position_map_c0_linear(
  const struct gkyl_rect_grid *cgrid, const struct gkyl_range *crange_ext, int c_poly_order,
  struct gkyl_vlasov_position_map_inp inp_pmap[GKYL_MAX_CDIM], struct gkyl_array *pmap,
  struct gkyl_array *jacob_pos, struct gkyl_array *jacob_pos_surf, struct gkyl_array *pmap_pgkyl,
  struct gkyl_array *pmap_avg_pgkyl, struct gkyl_array *jacob_pos_gauss
)
{
  int cdim = cgrid->ndim;
  ...
  for (int i = 0; i < cdim; ++i) {
    ...
    gkyl_eval_on_nodes *ev_map =
      gkyl_eval_on_nodes_new(&grid_1d, &basis, 1, inp_pmap[i].eval_pmap, inp_pmap[i].ctx);
...
{
  struct gkyl_vlasov_position_map *vpm =
    container_of(ref, struct gkyl_vlasov_position_map, ref_count);
...
)
{
  ...
  if (use_gpu) {
    ...
    vpm->jacob_pos_gauss_host =
      mkarr(false, vpm->jacob_pos_gauss->ncomp, vpm->jacob_pos_gauss->size);
  } else {
  ...
  vlasov_position_map_c0_linear(
    cgrid, crange_ext, poly_order, inp, vpm->pmap_host, vpm->jacob_pos_host,
    vpm->jacob_pos_surf_host, vpm->pmap_pgkyl_host, vpm->pmap_avg_pgkyl_host,
    vpm->jacob_pos_gauss_host
  );
...
void
gkyl_vlasov_position_map_eval_mc2p(
...
)
{
  ...
  for (int d = 0; d < cdim; ++d) {
    int idx =
      vpm->local_pos.lower[d] + (int)floor((xc[d] - vpm->grid_pos.lower[d]) / vpm->grid_pos.dx[d]);
...
void
gkyl_vlasov_position_map_write(
...
)
{
  ...
  struct gkyl_msgpack_data *mt_pmap = gkyl_msgpack_create(
    4,
  ...
  gkyl_comm_array_write(
    comm, &vpm->grid_pos, &vpm->local_pos, mt_pmap, vpm->pmap_pgkyl_host, fileNm_pmap
  );
  ...
  struct gkyl_msgpack_data *mt_pmap_avg = gkyl_msgpack_create(
    4,
    (struct gkyl_msgpack_map_elem[]){
      {.key = "time", .elem_type = GKYL_MP_DOUBLE, .dval = 0.0},
      {.key = "frame", .elem_type = GKYL_MP_INT, .ival = 0},
  ...
  gkyl_comm_array_write(
    comm, &vpm->grid_pos, &vpm->local_pos, mt_pmap_avg, vpm->pmap_avg_pgkyl_host, fileNm_pmap_avg
  );
...
)
{
  ...
  if (gkyl_vlasov_position_map_is_cu_dev(vpm)) {
    gkyl_vlasov_position_map_divide_jacobpos_cu(
      cdim, phase_basis, &vpm->local_pos, phase_range, vpm->jacob_pos_gauss, Jf, f_no_J
    );
    return;
  }
#endif
...
)
{
  ...
  if (gkyl_vlasov_position_map_is_cu_dev(vpm)) {
    gkyl_vlasov_position_map_rescale_jacobpos_cu(
      cdim, phase_basis, &vpm->local_pos, phase_range, vpm->jacob_pos_gauss, f_no_J, Jf
    );
    return;
  }
#endif
...
)
{
  ...
  if (gkyl_array_is_cu_dev(mom_no_J)) {
    ...
    gkyl_vlasov_position_map_divide_jacobpos_conf_cu(
      &vpm->local_pos, conf_range, num_coeff_divide, vpm->jacob_pos_gauss, Jmom, mom_no_J
    );
    return;
  }
#endif
...
)
{
  ...
  if (gkyl_array_is_cu_dev(Ja)) {
    ...
    gkyl_vlasov_position_map_rescale_jacobpos_conf_cu(
      &vpm->local_pos, conf_range, vpm->jacob_pos_gauss, a_no_J, Ja
    );
    return;
  }
#endif

...
vlasovApp = Vlasov.App.new {
  ...
  mapc2pPos = {
    -- x mapping: sinusoidal stretching; Jacobian matches at the two periodic
    -- endpoints (J = 1 + A at both), so the map is periodicity-consistent.
    {
      pmap = function (t, xn)
        local xc = xn[1]
        local A = 0.3
        return xc - (A * Lx / (2.0 * pi)) * math.sin(2.0 * pi * xc / Lx)
      end
    },
  },

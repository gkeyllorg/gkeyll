...
-- Create the equation + fluid species with no other Lua references, then
-- force full collection: only the species' anchor keeps the equation alive.
local fluid_species = Vlasov.FluidSpecies.new {
...
collectgarbage("collect")
-- (Two sequential collections are deliberate: a full cycle only RUNS the
-- __gc finalizers of newly unreachable objects -- e.g. the equation
-- userdata's C-side release -- while their memory is reclaimed on the NEXT
-- cycle. Collecting twice forces the heap to quiescence regardless of which
-- metatables carry finalizers.)
...
-- Run application.
-- The species local above still references the species userdata here, but the
-- equation userdata inside it has no Lua reference except the species' anchor.
fluid_species = nil
collectgarbage("collect")
collectgarbage("collect")

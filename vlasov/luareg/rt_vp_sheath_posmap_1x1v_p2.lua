...
vlasovApp = Vlasov.App.new {
  ...
  -- Configuration-space position map: asymmetric sheath problem (absorbing wall at
  -- x=0, reflecting/symmetry boundary at x=Lx), so resolution is clustered at the
  -- wall. x(xi) = (1-A)*xi + A*xi^2/Lx with A=0.4 -> cell Jacobian
  -- dx/dxi = (1-A) + 2*A*xi/Lx increases monotonically from 1-A (fine, wall) to
  -- 1+A (coarse, symmetry boundary); endpoints fixed so the physical domain is [0,Lx].
  mapc2pPos = {
    -- x mapping
    {
      pmap = function (t, xn)

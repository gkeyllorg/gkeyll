...
vlasovApp = Vlasov.App.new {
  ...
  -- Configuration-space position map. This VM sheath has the absorbing wall at the
  -- UPPER boundary (x=Lx; the lower boundary is reflecting), so resolution is
  -- clustered there: x(xi) = (1+A)*xi - A*xi^2/Lx with A=0.4 -> cell Jacobian
  -- dx/dxi = (1+A) - 2*A*xi/Lx decreases from 1+A (coarse) to 1-A (fine, wall).
  mapc2pPos = {
    -- x mapping
    {
      pmap = function (t, xn)

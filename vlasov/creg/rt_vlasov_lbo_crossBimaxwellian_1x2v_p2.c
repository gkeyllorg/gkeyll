...
create_ctx(void)
{
  ...
  double nu_elc = log_lambda_elc * pow(charge_ion, 4.0) * n0 /
                  (12.0 * pow(GKYL_PI, 3.0 / 2.0) * pow(epsilon0, 2.0) * sqrt(mass_elc) *
                   pow(T_par_elc, 3.0 / 2.0)); // Electron collision frequency.
  double nu_ion = log_lambda_ion * pow(charge_ion, 4.0) * n0 /
                  (12.0 * pow(GKYL_PI, 3.0 / 2.0) * pow(epsilon0, 2.0) * sqrt(mass_ion) *
  ...
  double vx_max_elc =
    6.0 * vte_par; // First electron domain boundary (velocity space: vx-direction).
  double vx_max_ion = 6.0 * vti_par; // Second ion domain boundary (velocity space: vx-direction).
  double vy_max_elc =
    6.0 * vte_par; // First electron domain boundary (velocity space: vy-direction).
...
evalElcInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double n = (n0 / (2.0 * pow(pi, 3.0 / 2.0) * vte_par * pow(vte_perp, 2.0))) *
             exp(
               (-vsq_par / pow(vte_par, 2.0)) + (-vsq_perp / pow(vte_perp, 2.0))
             ); // Bimaxwellian distribution function.
...
evalIonInit(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double n = (n0 / (2.0 * pow(pi, 3.0 / 2.0) * vti_par * pow(vti_perp, 2.0))) *
             exp(
               (-vsq_par / pow(vti_par, 2.0)) + (-vsq_perp / pow(vti_perp, 2.0))
             ); // Bimaxwellian distribution function.

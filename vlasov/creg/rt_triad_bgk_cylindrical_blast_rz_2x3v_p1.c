...
)
{
  ...
  double d_sigma_tx_dtheta =
    -cos(q_theta); // Triad Basis Gradient Coefficients (theta-x coefficient).
  double d_sigma_ty_dtheta =
    -sin(q_theta); // Triad Basis Gradient Coefficients (theta-y coefficient).
...
main(int argc, char **argv)
{
  ...
  struct gkyl_vlasov_kinetic_species neut = {
    ...
    .correct =
      {.correct_all_moms = true, .iter_eps = 1.0e-12, .max_iter = 18, .use_last_converged = true},

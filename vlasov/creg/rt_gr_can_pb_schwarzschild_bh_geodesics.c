...
evalInitialf(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double angular_momentum = sqrt(
    (bh_mass * latus_rectum * latus_rectum) / (latus_rectum - bh_mass * (3 + pow(eccentricity, 2)))
  );
  double energy = sqrt(
    1 - (pow(angular_momentum, 2) / pow(latus_rectum, 3)) * (latus_rectum - 4 * bh_mass) *
...
evalHamiltonian(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double gamma = sqrt(
    1.0 + inv_metric_r_r * p_r_dot * p_r_dot + 2.0 * inv_metric_r_theta * p_r_dot * p_theta_dot +

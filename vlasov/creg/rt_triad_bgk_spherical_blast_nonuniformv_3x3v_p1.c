...
evalCovTangentBasis(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double e_rx =
    cos(q_phi) * sin(q_theta); // Covariant Tangent Basis Coefficients (r-x coefficient).
  double e_ry =
    sin(q_phi) * sin(q_theta); // Covariant Tangent Basis Coefficients (r-y coefficient).
  double e_rz = cos(q_theta); // Covariant Tangent Basis Coefficients (r-z coefficient).
  double e_tx =
    q_r * cos(q_phi) * cos(q_theta); // Covariant Tangent Basis Coefficients (theta-x coefficient).
  double e_ty =
    q_r * sin(q_phi) * cos(q_theta); // Covariant Tangent Basis Coefficients (theta-y coefficient).
  double e_tz = -q_r * sin(q_theta); // Covariant Tangent Basis Coefficients (theta-z coefficient).
  double e_px =
    -q_r * sin(q_phi) * sin(q_theta); // Covariant Tangent Basis Coefficients (phi-x coefficient).
  double e_py =
    q_r * cos(q_phi) * sin(q_theta); // Covariant Tangent Basis Coefficients (phi-y coefficient).
...
)
{
  ...
  // d/dtheta components
  double d_sigma_rx_dtheta =
    cos(q_phi) * cos(q_theta); // Triad Basis Gradient Coefficients (r-x coefficient).
  double d_sigma_ry_dtheta =
    sin(q_phi) * cos(q_theta); // Triad Basis Gradient Coefficients (r-y coefficient).
  double d_sigma_rz_dtheta = -sin(q_theta); // Triad Basis Gradient Coefficients (r-z coefficient).
  double d_sigma_tx_dtheta =
    -cos(q_phi) * sin(q_theta); // Triad Basis Gradient Coefficients (theta-x coefficient).
  double d_sigma_ty_dtheta =
    -sin(q_phi) * sin(q_theta); // Triad Basis Gradient Coefficients (theta-y coefficient).
  double d_sigma_tz_dtheta =
    -cos(q_theta); // Triad Basis Gradient Coefficients (theta-z coefficient).
  ...
  // d/dphi components
  double d_sigma_rx_dp =
    -sin(q_phi) * sin(q_theta); // Triad Basis Gradient Coefficients (r-x coefficient).
  double d_sigma_ry_dp =
    cos(q_phi) * sin(q_theta); // Triad Basis Gradient Coefficients (r-y coefficient).
  double d_sigma_rz_dp = 0.0; // Triad Basis Gradient Coefficients (r-z coefficient).
  double d_sigma_tx_dp =
    -sin(q_phi) * cos(q_theta); // Triad Basis Gradient Coefficients (theta-x coefficient).
  double d_sigma_ty_dp =
    cos(q_phi) * cos(q_theta); // Triad Basis Gradient Coefficients (theta-y coefficient).
...
main(int argc, char **argv)
{
  ...
  struct gkyl_vlasov_kinetic_species neut = {
    ...
    .bcz = {.lower = {.type = GKYL_SPECIES_REFLECT}, .upper = {.type = GKYL_SPECIES_REFLECT}},

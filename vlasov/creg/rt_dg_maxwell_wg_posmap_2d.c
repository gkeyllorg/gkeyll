...
// Symmetric, spatially-varying position map in x: x(xi) = xi - (A*Lx/2pi) sin(2pi xi/Lx),
// A=0.4. The cell Jacobian dx/dxi = 1 - A cos(2pi xi/Lx) varies in space and is
// mirror-symmetric about x=Lx/2 (x(Lx-xi) = Lx - x(xi)), so cell i and Nx-1-i are
// mirror physical cells. Endpoints fixed. Used to check the reflecting (PEC) BCs
// with a spatially-varying Jacobian: a symmetric IC must stay symmetric.
void
mapc2p_pos_x(double t, const double* GKYL_RESTRICT xc, double* GKYL_RESTRICT xp, void* ctx)
{
  struct wg_2d_ctx *app = ctx;
  double Lx = app->Lx;
  double A = 0.4;
  double xi = xc[0];
  xp[0] = xi - (A * Lx / (2.0 * M_PI)) * sin(2.0 * M_PI * xi / Lx);
}
...
main(int argc, char **argv)
{
  ...
  struct gkyl_vm app_inp = {
    ...
    .upper = { ctx.Lx, ctx.Ly },
    .cells = { NX, NY },
    .mapc2p_pos[0] = { .mapc2p_pos_func = mapc2p_pos_x, .mapc2p_pos_ctx = &ctx },

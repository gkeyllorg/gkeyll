...
test_sr_vars(int vdim, int poly_order, bool use_tensor, bool use_nonuniform, bool use_gpu)
{
  ...
  gkyl_dg_vlasov_calc_hamil(
    &vel_grid, &vbasis, &vel_local, GKYL_MODEL_SR, vvm, gamma, gamma_inv, use_gpu
  );
  ...
  // type, and the agreement is to machine precision; gamma_inv likewise.
  {
    ...
    while (gkyl_range_iter_next(&viter)) {
      ...
      for (int i = 0; i < vbasis.num_basis; ++i) {
        ...
        TEST_MSG(
          "gamma %dv p%d %s %s cell=%d node=%d: %.15e vs %.15e", vdim, poly_order,
          use_tensor ? "tensor" : "ser", use_nonuniform ? "nonuni" : "uni", viter.idx[0], i,
          vbasis.eval_expand(zn, g_c), gex
        );
  ...
  struct gkyl_dg_calc_sr_vars *sr_vars = gkyl_dg_calc_sr_vars_new(
    &phase_grid, &vel_grid, &cbasis, &vbasis, &conf_local, &vel_local, vvm, use_gpu
  );
  ...
  while (gkyl_range_iter_next(&citer)) {
    ...
    TEST_MSG(
      "n_rest p%d %s %s cell=%d: %.15e vs n0=%.15e (rel %.2e)", poly_order,
      use_tensor ? "tensor" : "ser", use_nonuniform ? "nonuni" : "uni", citer.idx[0],
      nc[0] / sqrt(2.0), N0, nc[0] / sqrt(2.0) / N0 - 1.0
    );
  ...
  gkyl_dg_bin_op_mem *div_mem =
    use_gpu ? gkyl_dg_bin_op_mem_cu_dev_new(conf_local.volume, cbasis.num_basis) :
  ...
  while (gkyl_range_iter_next(&citer)) {
    ...
    for (int k = 0; k < cbasis.num_basis; ++k) {
      ...
      for (int d = 0; d < vdim; ++d) {
        expected += usq[d * cbasis.num_basis + k];
      }
      TEST_CHECK(gkyl_compare_double(gvsq[k], expected, 1e-12));
      TEST_MSG(
        "GammaV_sq cell=%d coeff=%d: %.15e vs 1+u_sq %.15e", citer.idx[0], k, gvsq[k], expected
      );
    }
    ...
    TEST_MSG(
      "GammaV cell=%d: %.15e vs %.15e (rel %.2e)", citer.idx[0], gv[0] / sqrt(2.0), GammaV0,
      gv[0] / sqrt(2.0) / GammaV0 - 1.0
    );
  }
  ...
  gkyl_dg_calc_sr_vars_pressure(
    sr_vars, &conf_local, &phase_local, gamma, gamma_inv, u_i, u_i_sq, GammaV, GammaV_sq, Jf,
    pressure
  );
  struct gkyl_array *pressure_ho =
    gkyl_array_new(GKYL_DOUBLE, cbasis.num_basis, conf_local_ext.volume);
  ...
  while (gkyl_range_iter_next(&citer)) {
    ...
    TEST_CHECK(
      gkyl_compare_double(pc[0] / sqrt(2.0) / (N0 * TEMP), 1.0, poly_order == 1 ? 5e-2 : 2e-3)
    );
    TEST_MSG(
      "pressure p%d %s %s cell=%d: P=%.15e vs n0*T=%.15e (rel %.2e)", poly_order,
      use_tensor ? "tensor" : "ser", use_nonuniform ? "nonuni" : "uni", citer.idx[0],
      pc[0] / sqrt(2.0), N0 * TEMP, pc[0] / sqrt(2.0) / (N0 * TEMP) - 1.0
    );
  }
  ...
  gkyl_array_release(gamma_inv_ho);
  gkyl_array_release(moms_ho);
  gkyl_array_release(moms);
  gkyl_array_release(Jf);
  gkyl_array_release(M0);
  gkyl_array_release(M1i);
  gkyl_array_release(n_rest);
  gkyl_array_release(n_ho);
  gkyl_array_release(u_i);
  gkyl_array_release(u_i_sq);
  gkyl_array_release(GammaV);
  gkyl_array_release(GammaV_sq);
  gkyl_array_release(u_i_sq_ho);
  gkyl_array_release(GammaV_ho);
  gkyl_array_release(GammaV_sq_ho);
  gkyl_array_release(pressure);
  gkyl_array_release(pressure_ho);
  gkyl_dg_bin_op_mem_release(div_mem);
  gkyl_dg_calc_sr_vars_release(sr_vars);
  gkyl_mom_calc_release(m0_calc);
  gkyl_mom_calc_release(m1_calc);
...
// map handles at 32^3).
static void
t_sr_1x1v_ser_p1_uni(void)
{
  test_sr_vars(1, 1, false, false, false);
}
static void
t_sr_1x1v_ser_p1_non(void)
{
  test_sr_vars(1, 1, false, true, false);
}
static void
t_sr_1x1v_ser_p2_uni(void)
{
  test_sr_vars(1, 2, false, false, false);
}
static void
t_sr_1x1v_ser_p2_non(void)
{
  test_sr_vars(1, 2, false, true, false);
}
static void
t_sr_1x1v_ten_p2_uni(void)
{
  test_sr_vars(1, 2, true, false, false);
}
static void
t_sr_1x1v_ten_p2_non(void)
{
  test_sr_vars(1, 2, true, true, false);
}
static void
t_sr_1x2v_ser_p1_uni(void)
{
  test_sr_vars(2, 1, false, false, false);
}
static void
t_sr_1x2v_ser_p1_non(void)
{
  test_sr_vars(2, 1, false, true, false);
}
static void
t_sr_1x2v_ser_p2_uni(void)
{
  test_sr_vars(2, 2, false, false, false);
}
static void
t_sr_1x2v_ser_p2_non(void)
{
  test_sr_vars(2, 2, false, true, false);
}
static void
t_sr_1x2v_ten_p2_uni(void)
{
  test_sr_vars(2, 2, true, false, false);
}
static void
t_sr_1x2v_ten_p2_non(void)
{
  test_sr_vars(2, 2, true, true, false);
}
static void
t_sr_1x3v_ser_p1_non(void)
{
  test_sr_vars(3, 1, false, true, false);
}
static void
t_sr_1x3v_ser_p2_non(void)
{
  test_sr_vars(3, 2, false, true, false);
}
static void
t_sr_1x3v_ten_p2_non(void)
{
  test_sr_vars(3, 2, true, true, false);
}
#ifdef GKYL_HAVE_CUDA
static void
t_sr_1x1v_ser_p1_uni_gpu(void)
{
  test_sr_vars(1, 1, false, false, true);
}
static void
t_sr_1x1v_ser_p1_non_gpu(void)
{
  test_sr_vars(1, 1, false, true, true);
}
static void
t_sr_1x1v_ser_p2_uni_gpu(void)
{
  test_sr_vars(1, 2, false, false, true);
}
static void
t_sr_1x1v_ser_p2_non_gpu(void)
{
  test_sr_vars(1, 2, false, true, true);
}
static void
t_sr_1x1v_ten_p2_uni_gpu(void)
{
  test_sr_vars(1, 2, true, false, true);
}
static void
t_sr_1x1v_ten_p2_non_gpu(void)
{
  test_sr_vars(1, 2, true, true, true);
}
static void
t_sr_1x2v_ser_p1_uni_gpu(void)
{
  test_sr_vars(2, 1, false, false, true);
}
static void
t_sr_1x2v_ser_p1_non_gpu(void)
{
  test_sr_vars(2, 1, false, true, true);
}
static void
t_sr_1x2v_ser_p2_uni_gpu(void)
{
  test_sr_vars(2, 2, false, false, true);
}
static void
t_sr_1x2v_ser_p2_non_gpu(void)
{
  test_sr_vars(2, 2, false, true, true);
}
static void
t_sr_1x2v_ten_p2_uni_gpu(void)
{
  test_sr_vars(2, 2, true, false, true);
}
static void
t_sr_1x2v_ten_p2_non_gpu(void)
{
  test_sr_vars(2, 2, true, true, true);
}
static void
t_sr_1x3v_ser_p1_non_gpu(void)
{
  test_sr_vars(3, 1, false, true, true);
}
static void
t_sr_1x3v_ser_p2_non_gpu(void)
{
  test_sr_vars(3, 2, false, true, true);
}
static void
t_sr_1x3v_ten_p2_non_gpu(void)
{
  test_sr_vars(3, 2, true, true, true);
}
#endif

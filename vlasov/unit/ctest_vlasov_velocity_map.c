...
static double
map_quartic(double v)
{
  return v + 0.01 * v * v * v * v;
}
...
static void
calc_nodal_vals_and_grads(
  map_t fn, double lower, double dx, int ncells, double *node_f, double *node_g
)
{
  ...
  node_g[ncells] =
    (3.0 * node_f[ncells] - 4.0 * node_f[ncells - 1] + node_f[ncells - 2]) / (2.0 * dx);
}
...
test_vmap(int vdim, int poly_order, bool use_gpu)
{
  ...
  struct gkyl_vlasov_velocity_map *vvm =
    gkyl_vlasov_velocity_map_new(&vgrid, &local_vel, &basis_vel, inp_vmap, false, use_gpu);
  ...
  for (int d = 0; d < vdim; ++d) {
    calc_nodal_vals_and_grads(test_maps[d], lower[d], vgrid.dx[d], cells[d], node_f[d], node_g[d]);
  }
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    for (int d = 0; d < vdim; ++d) {
      ...
      TEST_MSG(
        "vdim=%d dir=%d cell=%d: lower vertex value %.15e expected %.15e", vdim, d, iter.idx[d],
        b1.eval_expand(zlo, c_d), node_f[d][jl]
      );
      ...
      TEST_MSG(
        "vdim=%d dir=%d cell=%d: lower vertex deriv %.15e expected %.15e", vdim, d, iter.idx[d],
        grad_lo, node_g[d][jl]
      );
      ...
      for (int i = 0; i < poly_order + 1; ++i) {
        ...
        TEST_MSG(
          "vdim=%d dir=%d cell=%d quad=%d: jacob_vel %.15e expected %.15e", vdim, d, iter.idx[d], i,
          jacob_vel_d[d * (poly_order + 1) + i], expected
        );
        if (d == 0) {
          ...
          TEST_CHECK(
            gkyl_compare_double(jacob_vel_d[d * (poly_order + 1) + i], map_quad_prime(vc), 1e-12)
          );
        }
      }
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        TEST_MSG(
          "vdim=%d dir=%d cell=%d coeff=%d: vmap_pgkyl %.15e expected %.15e", vdim, d, iter.idx[d],
          i, pgkyl_dir[pidx[i]], pgkyl_scale[vdim - 1] * c_d[i]
        );
      }
    }
    ...
    while (gkyl_range_iter_next(&qiter)) {
      ...
      TEST_MSG(
        "vdim=%d cell=(%d): jacob_vel_gauss[%ld] %.15e expected %.15e", vdim, iter.idx[0], qloc,
        jacob_vel_gauss_d[qloc], expected
      );
    }
  }
  ...
  for (int d = 0; d < vdim; ++d) {
    for (int i = local_vel.lower[d]; i < local_vel.upper[d]; ++i) {
      ...
      for (int k = 0; k < vdim; ++k) {
        idxl[k] = idxr[k] = local_vel.lower[k];
      }
      idxl[d] = i;
      idxr[d] = i + 1;

      const double *cl =
        (const double *)gkyl_array_cfetch(vvm->vmap_host, gkyl_range_idx(&local_vel, idxl)) + 4 * d;
      const double *cr =
        (const double *)gkyl_array_cfetch(vvm->vmap_host, gkyl_range_idx(&local_vel, idxr)) + 4 * d;

      double zup[] = {1.0}, zlo[] = {-1.0};
      TEST_CHECK(gkyl_compare_double(b1.eval_expand(zup, cl), b1.eval_expand(zlo, cr), 1e-12));
      TEST_CHECK(
        gkyl_compare_double(b1.eval_grad_expand(0, zup, cl), b1.eval_grad_expand(0, zlo, cr), 1e-12)
      );
...
test_vmap_ser(int vdim, int poly_order, bool use_gpu)
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    for (int d = 0; d < vdim; ++d) {
      ...
      TEST_MSG(
        "vdim=%d dir=%d cell=%d: lower vertex value %.15e expected %.15e", vdim, d, iter.idx[d],
        b1.eval_expand(zlo, c_d), test_maps[d](vlo)
      );
      ...
      for (int i = 0; i < poly_order + 1; ++i) {
        ...
        TEST_MSG(
          "vdim=%d dir=%d cell=%d quad=%d: jacob_vel %.15e expected %.15e", vdim, d, iter.idx[d], i,
          jacob_vel_d[d * (poly_order + 1) + i], jac
        );
      }
      for (int i = 0; i < poly_order + 2; ++i) {
  ...
  for (int d = 0; d < vdim; ++d) {
    for (int i = local_vel.lower[d]; i < local_vel.upper[d]; ++i) {
      ...
      TEST_MSG(
        "vdim=%d dir=%d interface=%d: C^0 continuity %.15e vs %.15e", vdim, d, i,
        b1.eval_expand(zup, cl), b1.eval_expand(zlo, cr)
      );
    }
  }
...
test_vmap_identity(int vdim, int poly_order, bool use_gpu, bool ser_basis)
{
  ...
  TEST_CHECK(
    vvm->rep ==
    (ser_basis || poly_order == 1 ? GKYL_VLASOV_VMAP_C0_LINEAR : GKYL_VLASOV_VMAP_C1_CUBIC)
  );
...
static void
test_vmap_1v_p2(void)
{
  test_vmap(1, 2, false);
}
static void
test_vmap_1v_p3(void)
{
  test_vmap(1, 3, false);
}
static void
test_vmap_2v_p2(void)
{
  test_vmap(2, 2, false);
}
static void
test_vmap_3v_p2(void)
{
  test_vmap(3, 2, false);
}
static void
test_vmap_identity_1v_p2(void)
{
  test_vmap_identity(1, 2, false, false);
}
static void
test_vmap_identity_2v_p2(void)
{
  test_vmap_identity(2, 2, false, false);
}
static void
test_vmap_identity_3v_p2(void)
{
  test_vmap_identity(3, 2, false, false);
}

static void
test_vmap_ser_1v_p1(void)
{
  test_vmap_ser(1, 1, false);
}
static void
test_vmap_ser_1v_p2(void)
{
  test_vmap_ser(1, 2, false);
}
static void
test_vmap_ser_1v_p3(void)
{
  test_vmap_ser(1, 3, false);
}
static void
test_vmap_ser_2v_p1(void)
{
  test_vmap_ser(2, 1, false);
}
static void
test_vmap_ser_2v_p2(void)
{
  test_vmap_ser(2, 2, false);
}
static void
test_vmap_ser_3v_p1(void)
{
  test_vmap_ser(3, 1, false);
}
static void
test_vmap_ser_3v_p2(void)
{
  test_vmap_ser(3, 2, false);
}
static void
test_vmap_ser_identity_1v_p1(void)
{
  test_vmap_identity(1, 1, false, true);
}
static void
test_vmap_ser_identity_2v_p2(void)
{
  test_vmap_identity(2, 2, false, true);
}
static void
test_vmap_ser_identity_3v_p1(void)
{
  test_vmap_identity(3, 1, false, true);
}
...
test_vmap_gpu(int vdim, int poly_order, bool identity, bool ser_basis)
{
  ...
  struct gkyl_vlasov_velocity_map *vvm_ho =
    gkyl_vlasov_velocity_map_new(&vgrid, &local_vel, &basis_vel, inp_vmap, false, false);
  struct gkyl_vlasov_velocity_map *vvm =
    gkyl_vlasov_velocity_map_new(&vgrid, &local_vel, &basis_vel, inp_vmap, false, true);

  TEST_CHECK(gkyl_vlasov_velocity_map_is_cu_dev(vvm));
  TEST_CHECK(vvm->is_identity == identity);

  struct gkyl_array *dev_arrays[] = {
    vvm->vmap, vvm->jacob_vel, vvm->jacob_vel_surf, vvm->jacob_vel_gauss
  };
  struct gkyl_array *ho_arrays[] = {
    vvm_ho->vmap, vvm_ho->jacob_vel, vvm_ho->jacob_vel_surf, vvm_ho->jacob_vel_gauss
  };
  for (int n = 0; n < 4; ++n) {
...
static void
test_vmap_1v_p2_gpu(void)
{
  test_vmap_gpu(1, 2, false, false);
}
static void
test_vmap_1v_p3_gpu(void)
{
  test_vmap_gpu(1, 3, false, false);
}
static void
test_vmap_2v_p2_gpu(void)
{
  test_vmap_gpu(2, 2, false, false);
}
static void
test_vmap_3v_p2_gpu(void)
{
  test_vmap_gpu(3, 2, false, false);
}
static void
test_vmap_identity_2v_p2_gpu(void)
{
  test_vmap_gpu(2, 2, true, false);
}
static void
test_vmap_ser_1v_p1_gpu(void)
{
  test_vmap_gpu(1, 1, false, true);
}
static void
test_vmap_ser_2v_p2_gpu(void)
{
  test_vmap_gpu(2, 2, false, true);
}
static void
test_vmap_ser_3v_p1_gpu(void)
{
  test_vmap_gpu(3, 1, false, true);
}
static void
test_vmap_ser_identity_2v_p1_gpu(void)
{
  test_vmap_gpu(2, 1, true, true);
}
#endif

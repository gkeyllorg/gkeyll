...
test_triad_math_1v()
{
  ...
  kernel_conf_poisson_tensor_1v(
    h_ij_inv, triad_basis, cov_tangent_basis, triad_basis_gradient, conf_poisson_tensor
  );
...
void
eval_triad_basis_1v(double t, const double *xn, double *restrict fout, void *ctx)
{
  fout[0] = 1;
}

void
eval_cov_tangent_basis_1v(double t, const double *xn, double *restrict fout, void *ctx)
{
  fout[0] = 1;
}

void
eval_triad_basis_gradient_1v(double t, const double *xn, double *restrict fout, void *ctx)
{
  fout[0] = 0;
}
...
test_triad_1x1v_flat_conf(int poly_order)
{
  ...
  gkyl_vlasov_triad_geom_new(
    &confGrid, &confLocal, confBasis, &grid, &local, basis, inp_basis_vectors, conf_poisson_tensor
  );
...
void
eval_triad_basis_flat_2v(double t, const double *xn, double *restrict fout, void *ctx)
{
  fout[0] = 1.0;
  fout[1] = 0.0;
  fout[2] = 0.0;
...
void
eval_cov_tangent_basis_flat_2v(double t, const double *xn, double *restrict fout, void *ctx)
{
  fout[0] = 1.0;
  fout[1] = 0.0;
  fout[2] = 0.0;
...
void
eval_triad_basis_gradient_flat_2v(double t, const double *xn, double *restrict fout, void *ctx)
{
  fout[0] = 0.0;
  fout[1] = 0.0;
  fout[2] = 0.0;
...
void
eval_cov_tangent_basis_annulus_2v(double t, const double *xn, double *restrict fout, void *ctx)
{
  double q_r = xn[0];
  double q_theta = 0.0;
  fout[0] = cos(q_theta);
...
void
eval_triad_basis_annulus_2v(double t, const double *xn, double *restrict fout, void *ctx)
{
  double q_r = xn[0];
  double q_theta = 0.0;
  fout[0] = cos(q_theta);
...
void
eval_triad_basis_gradient_annulus_2v(double t, const double *xn, double *restrict fout, void *ctx)
{
  double q_r = xn[0];
  double q_theta = 0.0;
  fout[0] = 0.0;
...
test_triad_1x2v_annulus_conf(int poly_order)
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    double conf_pt_pnt1_vals[18] = {
      ...
      0.0,
      0.0,
      5.6865230654862211e+00,
      -3.3402132856134248e+00,
      1.1941468087349127e+00,
      0.0,
      0.0,
      0.0,
      5.6865230654862211e+00,
    ...
    double conf_pt_pnt2_vals[18] = {
      ...
      0.0,
      0.0,
      1.8807796203407585e+00,
      -3.3402132856134242e-01,
      5.0077124237270627e-02,
      0.0,
      0.0,
      0.0,
      1.8807796203407585e+00,
...
static void
fill_nodal_from_evalf(
  const struct gkyl_rect_grid *cgrid, const struct gkyl_range *crange,
  const struct gkyl_basis *cbasis, int vdim, evalf_t eval_cov_tangent_basis,
  evalf_t eval_triad_basis, evalf_t eval_triad_basis_gradient,
...
static void
test_triad_from_nodal(
  int cdim, int vdim, int poly_order, const double *confLower, const double *confUpper,
  const int *confCells, evalf_t eval_cov_tangent_basis, evalf_t eval_triad_basis,
  evalf_t eval_triad_basis_gradient
)
{
  int ndim = cdim + vdim;
  int num_pt_indices[3] = {1, 6, 18};
  ...
  gkyl_vlasov_triad_geom_new(
    &confGrid, &confLocal, confBasis, &grid, &local, basis, inp_basis_vectors, pt_ref
  );
  ...
  struct gkyl_array *triad_basis_gradient_nodal =
    mkarr(NC * vdim * vdim * vdim, confLocal_ext.volume);

  fill_nodal_from_evalf(
    &confGrid, &confLocal, &confBasis, vdim, eval_cov_tangent_basis, eval_triad_basis,
    eval_triad_basis_gradient, cov_tangent_basis_nodal, triad_basis_nodal,
    triad_basis_gradient_nodal
  );

  struct gkyl_array *pt_nodal = mkarr(NC * num_pt, confLocal_ext.volume);
  gkyl_vlasov_triad_geom_from_nodal(
    &confGrid, &confLocal, confBasis, &grid, &local, basis, cov_tangent_basis_nodal,
    triad_basis_nodal, triad_basis_gradient_nodal, pt_nodal
  );
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    for (int k = 0; k < NC * num_pt; ++k) {
      ...
      TEST_MSG(
        "cell (%d), coeff %d: from_nodal %1.16e vs from_basis %1.16e", iter.idx[0], k, nod_d[k],
        ref_d[k]
      );
    }
  }
...
void
eval_cov_tangent_basis_sph_3v(double t, const double *xn, double *restrict fout, void *ctx)
{
...
void
eval_triad_basis_sph_3v(double t, const double *xn, double *restrict fout, void *ctx)
{
...
void
eval_triad_basis_gradient_sph_3v(double t, const double *xn, double *restrict fout, void *ctx)
{
  double theta = sph_theta_c, phi = sph_phi_c;
  // gradient in r
  for (int k = 0; k < 9; ++k) {
...
static void
test_triad_from_nodal_interior(
  int cdim, int vdim, const double *confLower, const double *confUpper, const int *confCells,
...
)
{
  ...
  gkyl_vlasov_triad_geom_from_nodal_interior(
    &confGrid, &confLocal, confBasis, &grid, &local, basis, &nrange, ctb_n, tb_n, tbg_n, pt_out
  );
  ...
  while (gkyl_range_iter_next(&citer)) {
    ...
    for (int k = 0; k < NC * num_pt; ++k) {
      ...
      TEST_MSG(
        "cell (%d,%d,%d), coeff %d: got %1.16e expected %1.16e", citer.idx[0],
        cdim > 1 ? citer.idx[1] : 0, cdim > 2 ? citer.idx[2] : 0, k, out_d[k], pt_expected[k]
      );
    }
  }
...
void
eval_cov_tangent_basis_sph_3x3v(double t, const double *xn, double *restrict fout, void *ctx)
{
...
void
eval_triad_basis_sph_3x3v(double t, const double *xn, double *restrict fout, void *ctx)
{
...
void
eval_triad_basis_gradient_sph_3x3v(double t, const double *xn, double *restrict fout, void *ctx)
{
  double theta = 1.0 + 0.2 * xn[1], phi = 0.5 + 0.3 * xn[2];
  // gradient in r
  for (int k = 0; k < 9; ++k) {
...
test_triad_from_nodal_interior_1x2v_annulus_p1()
{
  ...
  int confCells[] = {4};
  test_triad_from_nodal_interior(
    1, 2, confLower, confUpper, confCells, eval_cov_tangent_basis_annulus_2v,
    eval_triad_basis_annulus_2v, eval_triad_basis_gradient_annulus_2v
  );
}
...
test_triad_from_nodal_interior_3x3v_spherical_p1()
{
  ...
  int confCells[] = {3, 2, 2};
  test_triad_from_nodal_interior(
    3, 3, confLower, confUpper, confCells, eval_cov_tangent_basis_sph_3x3v,
    eval_triad_basis_sph_3x3v, eval_triad_basis_gradient_sph_3x3v
  );
}
...
static void
fill_cyl_tangents(const double *xmu, double *tan)
{
...
static void
fill_cyl_triad(const double *xmu, double *tri)
{
  double a = xmu[1];
  tri[0] = cos(a);
  tri[1] = sin(a);
...
static void
fill_cyl_triad_grad(const double *xmu, double *grad)
{
  double a = xmu[1];
  for (int k = 0; k < 27; ++k) {
    grad[k] = 0.0;
...
static void
fill_cyl_bhat(const double *xmu, double *b)
{
  b[0] = 0.0;
  b[1] = 0.0;
  b[2] = 1.0;
...
static void
fill_flat_tangents(const double *xmu, double *tan)
{
  for (int k = 0; k < 9; ++k) {
    tan[k] = 0.0;
  }
...
static void
fill_flat_triad_grad(const double *xmu, double *grad)
{
  for (int k = 0; k < 27; ++k) {
    grad[k] = 0.0;
  }
...
static void
test_triad_from_tangents_interior(
  nodal_fill_t fill_tan, nodal_fill_t fill_tri, nodal_fill_t fill_grad, nodal_fill_t fill_bhat,
...
)
{
  ...
  gkyl_vlasov_triad_geom_from_tangents_interior(
    &confGrid, &confLocal, confBasis, &grid, &local, basis, &nrange, tan_n, bhat_n, true, pt_a
  );

  struct gkyl_array *pt_b = mkarr(NC * num_pt, confLocal_ext.volume);
  gkyl_vlasov_triad_geom_from_nodal_interior(
    &confGrid, &confLocal, confBasis, &grid, &local, basis, &nrange, tan_n, tri_n, grad_n, pt_b
  );
  ...
  while (gkyl_range_iter_next(&citer)) {
    ...
    for (int k = 0; k < NC * num_pt; ++k) {
      ...
      TEST_MSG(
        "cell (%d,%d,%d), coeff %d: from_tangents %1.16e vs analytic %1.16e", citer.idx[0],
        citer.idx[1], citer.idx[2], k, a_d[k], b_d[k]
      );
    }
  }
...
test_triad_from_tangents_interior_3x3v_flat()
{
  ...
  test_triad_from_tangents_interior(
    fill_flat_tangents, fill_flat_tangents, fill_flat_triad_grad, fill_cyl_bhat, confLower,
    confUpper, confCells, 1e-14
  );
}
...
test_triad_from_tangents_interior_3x3v_cylindrical()
{
  ...
  test_triad_from_tangents_interior(
    fill_cyl_tangents, fill_cyl_triad, fill_cyl_triad_grad, fill_cyl_bhat, confLower, confUpper,
    confCells, 1e-3
  );
}
...
test_triad_from_nodal_1x2v_annulus_p1()
{
  ...
  int confCells[] = {4};
  test_triad_from_nodal(
    1, 2, 1, confLower, confUpper, confCells, eval_cov_tangent_basis_annulus_2v,
    eval_triad_basis_annulus_2v, eval_triad_basis_gradient_annulus_2v
  );
}
...
test_triad_from_nodal_1x2v_annulus_p2()
{
  ...
  int confCells[] = {4};
  test_triad_from_nodal(
    1, 2, 2, confLower, confUpper, confCells, eval_cov_tangent_basis_annulus_2v,
    eval_triad_basis_annulus_2v, eval_triad_basis_gradient_annulus_2v
  );
}
...
test_triad_from_nodal_1x3v_spherical_p1()
{
  ...
  int confCells[] = {4};
  test_triad_from_nodal(
    1, 3, 1, confLower, confUpper, confCells, eval_cov_tangent_basis_sph_3v,
    eval_triad_basis_sph_3v, eval_triad_basis_gradient_sph_3v
  );
}
...
void
test_triad_1x1v_flat()
{
  test_triad_1x1v_flat_conf(2);
}
void
test_triad_1x2v_flat()
{
  test_triad_1x2v_flat_conf(2);
}
void
test_triad_1x2v_annulus()
{
  test_triad_1x2v_annulus_conf(2);
}
void
test_triad_2v()
{

...
static void
bench_case(
  enum gkyl_model_id model_id, int cdim, int vdim, int poly_order, const int *conf_cells,
  const int *vel_cells, double ab_rel_tol
)
{
  int pdim = cdim + vdim;
  ...
  for (int d = 0; d < cdim; ++d) {
    cells[d] = conf_cells[d];
    ghost[d] = 1;
    lower[d] = 0.0;
    upper[d] = 1.0;
  }
  for (int d = 0; d < vdim; ++d) {
    cells[cdim + d] = vel_cells[d];
    ghost[cdim + d] = 0;
    lower[cdim + d] = -1.0;
    upper[cdim + d] = 1.0;
    vellower[d] = -1.0;
  ...
  struct gkyl_array *qmem =
    gkyl_array_new(GKYL_DOUBLE, 8 * confBasis.num_basis, confRange_ext.volume);
  ...
  struct gkyl_vlasov_velocity_map *vel_map =
    gkyl_vlasov_velocity_map_new(&velGrid, &velRange, &velBasis, inp_vmap, false, false);
  ...
  struct gkyl_vlasov_position_map *pos_map = gkyl_vlasov_position_map_new(
    &confGrid, &confRange, &confRange_ext, &confBasis, inp_pmap, false
  );
  gkyl_dg_vlasov_calc_hamil(
    &velGrid, &velBasis, &velRange, GKYL_MODEL_DEFAULT, vel_map, hamil, gamma_inv, false
  );

  // Poisson tensor: synthetic (smooth, nonzero) so the nc kernels do real work.
  int num_pt_indices[3] = {1, 6, 18};
  struct gkyl_array *poisson_tensor_conf = gkyl_array_new(
    GKYL_DOUBLE, confBasis.num_basis * num_pt_indices[vdim - 1], confRange_ext.volume
  );
  ...
  struct gkyl_array *pot_tot =
    gkyl_array_new(GKYL_DOUBLE, confBasis.num_basis * 4, confRange_ext.volume);
  ...
  for (int fl = 0; fl < 2; ++fl) {
    ...
    up[fl].slvr =
      gkyl_hyper_dg_new(&phaseGrid, &basis, up[fl].eqn, pdim, up_dirs, zero_flux_flags, 1, false);
    ...
    if (is_triad) {
      gkyl_dg_vlasov_conf_flux_surf_advance(
        up[fl].calc_conf_flux, &confRange, &phaseRange, &phaseRange_ext, poisson_tensor_conf, hamil,
        fin, cflrate, conf_flux_surf[fl]
      );
    }
    gkyl_dg_vlasov_vel_flux_surf_advance(
      up[fl].calc_vel_flux, &confRange, &phaseRange, poisson_tensor_conf, hamil, qmem, pot_tot, rad,
      f_no_J, cflrate, vel_flux_surf[fl]
    );
  ...
  for (int fl = 0; fl < 2; ++fl) {
    t_cflux[fl] = t_flux[fl] = t_hyper[fl] = t_mom[fl] = 0.0;
  }
  struct timespec tm;
  for (int n = 0; n < nrep; ++n) {
    for (int fl = 0; fl < 2; ++fl) {
      if (is_triad) {
        tm = gkyl_wall_clock();
        gkyl_dg_vlasov_conf_flux_surf_advance(
          up[fl].calc_conf_flux, &confRange, &phaseRange, &phaseRange_ext, poisson_tensor_conf,
          hamil, fin, cflrate, conf_flux_surf[fl]
        );
        t_cflux[fl] += gkyl_time_diff_now_sec(tm);
      }
      tm = gkyl_wall_clock();
      gkyl_dg_vlasov_vel_flux_surf_advance(
        up[fl].calc_vel_flux, &confRange, &phaseRange, poisson_tensor_conf, hamil, qmem, pot_tot,
        rad, f_no_J, cflrate, vel_flux_surf[fl]
      );
  ...
  printf(
    "\n%s %dx%dv p%d (%ld phase cells):\n", is_triad ? "TRIAD" : "DEFAULT", cdim, vdim, poly_order,
    (long)phaseRange.volume
  );
  printf("  %-14s %12s %12s %9s\n", "updater", "dense ms", "sparse ms", "speedup");
  if (is_triad) {
    printf(
      "  %-14s %12.3f %12.3f %8.2fx\n", "conf_flux_surf", 1e3 * t_cflux[0], 1e3 * t_cflux[1],
      t_cflux[0] / t_cflux[1]
    );
  }
  printf(
    "  %-14s %12.3f %12.3f %8.2fx\n", "vel_flux_surf", 1e3 * t_flux[0], 1e3 * t_flux[1],
    t_flux[0] / t_flux[1]
  );
  printf(
    "  %-14s %12.3f %12.3f %8.2fx\n", "hyper_dg", 1e3 * t_hyper[0], 1e3 * t_hyper[1],
    t_hyper[0] / t_hyper[1]
  );
  printf(
    "  %-14s %12.3f %12.3f %8.2fx\n", "five_moments", 1e3 * t_mom[0], 1e3 * t_mom[1],
    t_mom[0] / t_mom[1]
  );
  if (is_triad) {
    printf(
      "  max |sparse - dense|: rhs %.2e, conf_flux %.2e, vel_flux %.2e, moments %.2e\n", d_rhs,
      d_cflux, d_flux, d_mom
    );
  } else {
  ...
  struct gkyl_vlasov_velocity_map *vel_map_cu =
    gkyl_vlasov_velocity_map_new(&velGrid, &velRange, &velBasis, inp_vmap, false, true);
  struct gkyl_vlasov_position_map *pos_map_cu =
    gkyl_vlasov_position_map_new(&confGrid, &confRange, &confRange_ext, &confBasis, inp_pmap, true);

  struct gkyl_array *fin_cu =
    gkyl_array_cu_dev_new(GKYL_DOUBLE, basis.num_basis, phaseRange_ext.volume);
  struct gkyl_array *f_no_J_cu =
    gkyl_array_cu_dev_new(GKYL_DOUBLE, basis.num_basis, phaseRange_ext.volume);
  struct gkyl_array *cflrate_cu = gkyl_array_cu_dev_new(GKYL_DOUBLE, 1, phaseRange_ext.volume);
  struct gkyl_array *qmem_cu =
    gkyl_array_cu_dev_new(GKYL_DOUBLE, 8 * confBasis.num_basis, confRange_ext.volume);
  struct gkyl_array *hamil_cu =
    gkyl_array_cu_dev_new(GKYL_DOUBLE, velBasis.num_basis, velRange.volume);
  struct gkyl_array *poisson_tensor_conf_cu = gkyl_array_cu_dev_new(
    GKYL_DOUBLE, confBasis.num_basis * num_pt_indices[vdim - 1], confRange_ext.volume
  );
  struct gkyl_array *pot_tot_cu =
    gkyl_array_cu_dev_new(GKYL_DOUBLE, confBasis.num_basis * 4, confRange_ext.volume);
  struct gkyl_array *rad_cu =
    gkyl_array_cu_dev_new(GKYL_DOUBLE, vdim * velBasis.num_basis, velRange.volume);
  ...
  for (int fl = 0; fl < 2; ++fl) {
    ...
    slvr_cu[fl] =
      gkyl_hyper_dg_new(&phaseGrid, &basis, eqn_cu[fl], pdim, up_dirs, zero_flux_flags, 1, true);
    ...
    if (is_triad) {
      ...
      gkyl_dg_vlasov_conf_flux_surf_advance(
        calc_conf_flux_cu[fl], &confRange, &phaseRange, &phaseRange_ext, poisson_tensor_conf_cu,
        hamil_cu, fin_cu, cflrate_cu, conf_flux_surf_cu[fl]
      );
    }
    gkyl_dg_vlasov_vel_flux_surf_advance(
      calc_vel_flux_cu[fl], &confRange, &phaseRange, poisson_tensor_conf_cu, hamil_cu, qmem_cu,
      pot_tot_cu, rad_cu, f_no_J_cu, cflrate_cu, vel_flux_surf_cu[fl]
    );
    ...
    if (is_triad) {
      gkyl_dg_vlasov_conf_flux_surf_advance(
        up[fl].calc_conf_flux, &confRange, &phaseRange, &phaseRange_ext, poisson_tensor_conf, hamil,
        fin, cflrate_ref, conf_flux_surf[fl]
      );
    }
    gkyl_dg_vlasov_vel_flux_surf_advance(
      up[fl].calc_vel_flux, &confRange, &phaseRange, poisson_tensor_conf, hamil, qmem, pot_tot, rad,
      f_no_J, cflrate_ref, vel_flux_surf[fl]
    );
  ...
  for (int fl = 0; fl < 2; ++fl) {
    ...
    printf(
      "  %-14s %-7s %10.3f %10.3f %8.2fx\n", "hyper_dg", fl ? "sparse" : "dense", 1e3 * t_hyper[fl],
      1e3 * t_hyper_gpu[fl], t_hyper[fl] / t_hyper_gpu[fl]
    );
    ...
    printf(
      "  %-14s %-7s %10.3f %10.3f %8.2fx\n", "overall", fl ? "sparse" : "dense", 1e3 * cpu_tot,
      1e3 * gpu_tot, cpu_tot / gpu_tot
    );
  }
  if (is_triad) {
    printf(
      "  max |gpu - cpu|: conf_flux %.2e/%.2e, vel_flux %.2e/%.2e, cflrate %.2e/%.2e, rhs %.2e/%.2e (dense/sparse)\n",
      d_gpu_cflux[0], d_gpu_cflux[1], d_gpu_vflux[0], d_gpu_vflux[1], d_gpu_cfl[0], d_gpu_cfl[1],
      d_gpu_rhs[0], d_gpu_rhs[1]
    );
  } else {
    printf(
      "  max |gpu - cpu|: vel_flux %.2e/%.2e, cflrate %.2e/%.2e, rhs %.2e/%.2e (dense/sparse)\n",
      d_gpu_vflux[0], d_gpu_vflux[1], d_gpu_cfl[0], d_gpu_cfl[1], d_gpu_rhs[0], d_gpu_rhs[1]
    );
  }

  for (int fl = 0; fl < 2; ++fl) {
    if (calc_conf_flux_cu[fl]) {
      gkyl_dg_vlasov_conf_flux_surf_release(calc_conf_flux_cu[fl]);
    }
    ...
    if (conf_flux_surf_cu[fl]) {
      gkyl_array_release(conf_flux_surf_cu[fl]);
    }
  }
  gkyl_array_release(rhs_ho);
  gkyl_array_release(vel_flux_ho);
  if (conf_flux_ho) {
    gkyl_array_release(conf_flux_ho);
  }
  ...
  for (int fl = 0; fl < 2; ++fl) {
    if (up[fl].calc_conf_flux) {
      gkyl_dg_vlasov_conf_flux_surf_release(up[fl].calc_conf_flux);
    }
    ...
    if (conf_flux_surf[fl]) {
      gkyl_array_release(conf_flux_surf[fl]);
    }
...
bench_1x2v_p1(void)
{
  bench_case(GKYL_MODEL_DEFAULT, 1, 2, 1, (int[]){32}, (int[]){16, 16}, 1e-11);
}
static void
bench_1x2v_p2(void)
{
  bench_case(GKYL_MODEL_DEFAULT, 1, 2, 2, (int[]){32}, (int[]){16, 16}, 1e-11);
}
static void
bench_1x3v_p1(void)
{
  bench_case(GKYL_MODEL_DEFAULT, 1, 3, 1, (int[]){16}, (int[]){12, 12, 12}, 1e-11);
}
static void
bench_1x3v_p2(void)
{
  bench_case(GKYL_MODEL_DEFAULT, 1, 3, 2, (int[]){8}, (int[]){8, 8, 8}, 1e-11);
}
static void
bench_2x3v_p1(void)
{
  bench_case(GKYL_MODEL_DEFAULT, 2, 3, 1, (int[]){8, 8}, (int[]){8, 8, 8}, 1e-11);
}
static void
bench_2x3v_p2(void)
{
  bench_case(GKYL_MODEL_DEFAULT, 2, 3, 2, (int[]){4, 4}, (int[]){6, 6, 6}, 1e-11);
}
static void
bench_3x3v_p1(void)
{
  bench_case(GKYL_MODEL_DEFAULT, 3, 3, 1, (int[]){4, 4, 4}, (int[]){8, 8, 8}, 1e-11);
}
// non-canonical bracket (triads; same separable H, Poisson-tensor forces)
static void
bench_triad_1x2v_p1(void)
{
  bench_case(GKYL_MODEL_TRIAD, 1, 2, 1, (int[]){32}, (int[]){16, 16}, 1e-11);
}
static void
bench_triad_1x2v_p2(void)
{
  bench_case(GKYL_MODEL_TRIAD, 1, 2, 2, (int[]){32}, (int[]){16, 16}, 1e-11);
}
static void
bench_triad_1x3v_p1(void)
{
  bench_case(GKYL_MODEL_TRIAD, 1, 3, 1, (int[]){16}, (int[]){12, 12, 12}, 1e-11);
}
...
static void
bench_triad_1x3v_p2(void)
{
  bench_case(GKYL_MODEL_TRIAD, 1, 3, 2, (int[]){8}, (int[]){8, 8, 8}, 1e-3);
}
...
static void
bench_triad_2x2v_p2(void)
{
  bench_case(GKYL_MODEL_TRIAD, 2, 2, 2, (int[]){8, 8}, (int[]){8, 8}, 1e-3);
}
static void
bench_triad_2x3v_p1(void)
{
  bench_case(GKYL_MODEL_TRIAD, 2, 3, 1, (int[]){8, 8}, (int[]){8, 8, 8}, 1e-11);
}
static void
bench_triad_2x3v_p2(void)
{
  bench_case(GKYL_MODEL_TRIAD, 2, 3, 2, (int[]){4, 4}, (int[]){6, 6, 6}, 1e-3);
}
static void
bench_triad_3x3v_p1(void)
{
  bench_case(GKYL_MODEL_TRIAD, 3, 3, 1, (int[]){4, 4, 4}, (int[]){8, 8, 8}, 1e-11);
}

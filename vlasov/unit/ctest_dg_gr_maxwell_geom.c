...
void
field_init_func(double t, const double *xn, double *restrict fout, void *ctx)
{
  ...
  double Dphi = -2.0 * massBH * B_0 /
                (r * r * sqrt(1.0 + 2.0 * massBH / r)); // Total electric field (phi-direction).

  double Br =
    -B_0 * cos(theta) / (sqrt(1.0 + 2.0 * massBH / r)); // Total magnetic field (r-direction).
  double Btheta = B_0 * sin(theta) /
                  (r * sqrt(1.0 + 2.0 * massBH / r)); // Total magnetic field (theta-direction).
...
static const double Dphi_ref[4][4] = {
  {-1.6354821637490904e+00, +3.1682899412226662e-01, +0.0000000000000000e+00,
...
static const double Br_ref[4][4] = {
  {-1.1141258010329544e+00, -3.9838967223318700e-02, +1.0646084038672876e-01,
   +3.8068321609655564e-03},
  {-4.6148601697763836e-01, -1.6501840534835825e-02, +2.5701920472327799e-01,
   +9.1905058326813213e-03},
  {+4.6148601697763814e-01, +1.6501840534835860e-02, +2.5701920472327799e-01,
   +9.1905058326813283e-03},
  {+1.1141258010329544e+00, +3.9838967223318700e-02, +1.0646084038672876e-01,
   +3.8068321609655564e-03}
};

static const double Btheta_ref[4][4] = {
  {+3.7263287721296423e-01, -2.9826465965693272e-02, +2.0753349447565991e-01,
   -1.6611499114132122e-02},
  {+8.9961534595364645e-01, -7.2007458652036282e-02, +8.5963188058500117e-02,
   -6.8807082244221884e-03},
  {+8.9961534595364645e-01, -7.2007458652036255e-02, -8.5963188058500034e-02,
   +6.8807082244221884e-03},
  {+3.7263287721296434e-01, -2.9826465965693265e-02, -2.0753349447565989e-01,
   +1.6611499114132122e-02}
};

static const double JDphi_ref[4][4] = {
  {-1.4915589107194009e+00, +0.0000000000000000e+00, -8.3070617727859664e-01,
   +0.0000000000000000e+00},
  {-3.6009417513372184e+00, -3.3306690738754696e-16, -3.4408976497590316e-01,
   +0.0000000000000000e+00},
  {-3.6009417513372184e+00, -2.2204460492503131e-16, +3.4408976497590282e-01,
   +0.0000000000000000e+00},
  {-1.4915589107194016e+00, +0.0000000000000000e+00, +8.3070617727859641e-01,
   -2.7755575615628914e-17}
};

static const double JBr_ref[4][4] = {
  {-1.0064417291126031e+00, -2.2936950127375039e-01, -4.9045618369336014e-01,
   -1.1177566171621522e-01},
  {-1.0064417291126033e+00, -2.2936950127375047e-01, +4.9045618369336008e-01,
   +1.1177566171621517e-01},
  {+1.0064417291126031e+00, +2.2936950127375044e-01, +4.9045618369336008e-01,
   +1.1177566171621517e-01},
  {+1.0064417291126035e+00, +2.2936950127375053e-01, -4.9045618369335997e-01,
   -1.1177566171621522e-01}
};

static const double JBtheta_ref[4][4] = {
  {+4.5544074017426023e-01, +5.2589766787906389e-02, +3.8720225028423161e-01,
   +4.4710264686486062e-02},
  {+2.0445592598257387e+00, +2.3608536780690687e-01, +3.8720225028423155e-01,
   +4.4710264686485979e-02},
  {+2.0445592598257387e+00, +2.3608536780690687e-01, -3.8720225028423133e-01,
   -4.4710264686486034e-02},
  {+4.5544074017426045e-01, +5.2589766787906451e-02, -3.8720225028423166e-01,
   -4.4710264686486104e-02}
};

static const char *em_comp_names[8] = {"Dr",     "Dtheta", "Dphi", "Br",
                                       "Btheta", "Bphi",   "phi",  "psi"};
...
test_ks_r_theta_2x_geom_p1()
{
  ...
  struct gkyl_dg_gr_maxwell_geom_ctx ctx = {.mass_bh = 1.0, .spin_bh = 0.0};
  ...
  struct gkyl_vlasov_position_map *pos_map = gkyl_vlasov_position_map_new(
    &confGrid, &confLocal, &confLocal_ext, &confBasis, inp_pmap, false
  );
  ...
  struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *lapse_proj =
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_new(
      &confGrid, &confBasis, 1, poly_order, gkyl_dg_gr_maxwell_preset_lapse(triad_preset_geom_type),
      &ctx
    );
  ...
  struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *shift_proj =
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_new(
      &confGrid, &confBasis, 3, poly_order, gkyl_dg_gr_maxwell_preset_shift(triad_preset_geom_type),
      &ctx
    );
  ...
  struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *h_ij_proj =
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_new(
      &confGrid, &confBasis, 6, poly_order, gkyl_dg_gr_maxwell_preset_h_ij(triad_preset_geom_type),
      &ctx
    );
  ...
  // triangular matrix of 6 unique elements)
  struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *h_ij_inv_proj =
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_new(
      &confGrid, &confBasis, 6, poly_order,
      gkyl_dg_gr_maxwell_preset_h_ij_inv(triad_preset_geom_type), &ctx
    );
  ...
  struct gkyl_dg_gr_maxwell_surf_and_vol_nodes *det_h_proj =
    gkyl_dg_gr_maxwell_surf_and_vol_nodes_new(
      &confGrid, &confBasis, 1, poly_order, gkyl_dg_gr_maxwell_preset_det_h(triad_preset_geom_type),
      &ctx
    );
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    const double *lapse_surf_x_d =
      gkyl_array_cfetch(lapse->nodal_arr_surf_x, gkyl_range_idx(&confLocal, iter.idx));
    ...
    const double *lapse_surf_y_d =
      gkyl_array_cfetch(lapse->nodal_arr_surf_y, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_surf_nodes; ++m) {
      double expected = 0;
    ...
    const double *lapse_vol_d =
      gkyl_array_cfetch(lapse->nodal_arr_vol, gkyl_range_idx(&confLocal, iter.idx));
    ...
    const double *shift_surf_x_d =
      gkyl_array_cfetch(shift->nodal_arr_surf_x, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_surf_nodes; ++m) {
      ...
      for (int n = 0; n < NC; ++n) {
        ...
        if (n == 0) {
          beta_lower_x_surf = (2.0 * Mass * r_val) / (r_val * r_val + 2.0 * Mass * r_val);
        }
        if (n == 1) {
          beta_lower_x_surf = 0.0;
        }
        if (n == 2) {
          beta_lower_x_surf = 0.0;
        }
    ...
    const double *shift_surf_y_d =
      gkyl_array_cfetch(shift->nodal_arr_surf_y, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_surf_nodes; ++m) {
      // Shift has 3 components
      double NC = 3;
      for (int n = 0; n < NC; ++n) {
        ...
        if (n == 0) {
          beta_lower_y_surf = (2.0 * Mass * r_val) / (r_val * r_val + 2.0 * Mass * r_val);
        }
        if (n == 1) {
          beta_lower_y_surf = 0.0;
        }
        if (n == 2) {
          beta_lower_y_surf = 0.0;
        }
    ...
    const double *shift_vol_d =
      gkyl_array_cfetch(shift->nodal_arr_vol, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_vol_nodes; ++m) {
      // Shift has 3 components
      double NC = 3;
      for (int n = 0; n < NC; ++n) {
        ...
        double beta_vol;
        if (n == 0) {
          beta_vol = (2.0 * Mass * r_val) / (r_val * r_val + 2.0 * Mass * r_val);
        }
        if (n == 1) {
          beta_vol = 0.0;
        }
    ...
    const double *h_ij_surf_x_d =
      gkyl_array_cfetch(h_ij->nodal_arr_surf_x, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_surf_nodes; ++m) {
      ...
      for (int n = 0; n < NC; ++n) {
        ...
        if (n == 0) {
          h_ij_lower_x_surf = (1.0 + 2.0 * Mass / r_val);
        }
        if (n == 1) {
          h_ij_lower_x_surf = 0.0;
        }
        if (n == 2) {
          h_ij_lower_x_surf = 0.0;
        }
        if (n == 3) {
          h_ij_lower_x_surf = r_val * r_val;
        }
        if (n == 4) {
          h_ij_lower_x_surf = 0.0;
        }
        if (n == 5) {
          h_ij_lower_x_surf = r_val * r_val * sin(theta_val) * sin(theta_val);
        }
    ...
    const double *h_ij_surf_y_d =
      gkyl_array_cfetch(h_ij->nodal_arr_surf_y, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_surf_nodes; ++m) {
      // h_ij has 3 components
      double NC = 6;
      for (int n = 0; n < NC; ++n) {
        ...
        if (n == 0) {
          h_ij_lower_y_surf = (1.0 + 2.0 * Mass / r_val);
        }
        if (n == 1) {
          h_ij_lower_y_surf = 0.0;
        }
        if (n == 2) {
          h_ij_lower_y_surf = 0.0;
        }
        if (n == 3) {
          h_ij_lower_y_surf = r_val * r_val;
        }
        if (n == 4) {
          h_ij_lower_y_surf = 0.0;
        }
        if (n == 5) {
          h_ij_lower_y_surf = r_val * r_val * sin(theta_val) * sin(theta_val);
        }
    ...
    const double *h_ij_vol_d =
      gkyl_array_cfetch(h_ij->nodal_arr_vol, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_vol_nodes; ++m) {
      // h_ij has 3 components
      double NC = 6;
      for (int n = 0; n < NC; ++n) {
        ...
        if (n == 3) {
          h_ij_vol = r_val * r_val;
        }
        if (n == 4) {
          h_ij_vol = 0.0;
        }
        if (n == 5) {
          h_ij_vol = r_val * r_val * sin(theta_val) * sin(theta_val);
        }
        expected = h_ij_vol;
    ...
    const double *h_ij_inv_surf_x_d =
      gkyl_array_cfetch(h_ij_inv->nodal_arr_surf_x, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_surf_nodes; ++m) {
      ...
      for (int n = 0; n < NC; ++n) {
        ...
        if (n == 0) {
          h_ij_inv_lower_x_surf = 1.0 / (1.0 + 2.0 * Mass / r_val);
        }
        if (n == 1) {
          h_ij_inv_lower_x_surf = 0.0;
        }
        if (n == 2) {
          h_ij_inv_lower_x_surf = 0.0;
        }
        if (n == 3) {
          h_ij_inv_lower_x_surf = 1.0 / (r_val * r_val);
        }
        if (n == 4) {
          h_ij_inv_lower_x_surf = 0.0;
        }
        if (n == 5) {
          h_ij_inv_lower_x_surf = 1.0 / (r_val * r_val * sin(theta_val) * sin(theta_val));
        }
    ...
    const double *h_ij_inv_surf_y_d =
      gkyl_array_cfetch(h_ij_inv->nodal_arr_surf_y, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_surf_nodes; ++m) {
      // h_ij_inv has 6 components
      double NC = 6;
      for (int n = 0; n < NC; ++n) {
        ...
        if (n == 0) {
          h_ij_inv_lower_y_surf = 1.0 / (1.0 + 2.0 * Mass / r_val);
        }
        if (n == 1) {
          h_ij_inv_lower_y_surf = 0.0;
        }
        if (n == 2) {
          h_ij_inv_lower_y_surf = 0.0;
        }
        if (n == 3) {
          h_ij_inv_lower_y_surf = 1.0 / (r_val * r_val);
        }
        if (n == 4) {
          h_ij_inv_lower_y_surf = 0.0;
        }
        if (n == 5) {
          h_ij_inv_lower_y_surf = 1.0 / (r_val * r_val * sin(theta_val) * sin(theta_val));
        }
    ...
    const double *h_ij_inv_vol_d =
      gkyl_array_cfetch(h_ij_inv->nodal_arr_vol, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_vol_nodes; ++m) {
      // h_ij_inv has 6 components
      double NC = 6;
      for (int n = 0; n < NC; ++n) {
        ...
        double h_ij_inv_vol;
        if (n == 0) {
          h_ij_inv_vol = 1.0 / (1.0 + 2.0 * Mass / r_val);
        }
        if (n == 1) {
          h_ij_inv_vol = 0.0;
        }
        ...
        if (n == 3) {
          h_ij_inv_vol = 1.0 / (r_val * r_val);
        }
        if (n == 4) {
          h_ij_inv_vol = 0.0;
        }
        if (n == 5) {
          h_ij_inv_vol = 1.0 / (r_val * r_val * sin(theta_val) * sin(theta_val));
        }
    ...
    const double *det_h_surf_x_d =
      gkyl_array_cfetch(det_h->nodal_arr_surf_x, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_surf_nodes; ++m) {
    ...
    const double *det_h_surf_y_d =
      gkyl_array_cfetch(det_h->nodal_arr_surf_y, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_surf_nodes; ++m) {
      // det_h has 1 component
      double NC = 1;
      for (int n = 0; n < NC; ++n) {
    ...
    const double *det_h_vol_d =
      gkyl_array_cfetch(det_h->nodal_arr_vol, gkyl_range_idx(&confLocal, iter.idx));
    for (int m = 0; m < num_vol_nodes; ++m) {
      // det_h has 1 component
      double NC = 1;
      for (int n = 0; n < NC; ++n) {
  ...
  gkyl_proj_on_basis *proj =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 8, field_init_func, &ctx);
  ...
  gkyl_dg_gr_maxwell_divide_Jc(
    &confBasis, &confLocal, det_h, field_with_J_con, field_no_J_con, false
  );
  ...
  for (int d = 0; d < cdim; ++d) {
    if (conf_skin_ghost.lower_skin[d].volume > bc_buff_sz) {
      bc_buff_sz = conf_skin_ghost.lower_skin[d].volume;
    }
    if (conf_skin_ghost.upper_skin[d].volume > bc_buff_sz) {
      bc_buff_sz = conf_skin_ghost.upper_skin[d].volume;
    }
  }
  ...
  gkyl_sub_range_init(
    &radial_ghost_lo, &confLocal_ext, radial_ghost_lo_lower, radial_ghost_lo_upper
  );
  gkyl_sub_range_init(
    &radial_ghost_up, &confLocal_ext, radial_ghost_up_lower, radial_ghost_up_upper
  );

  struct gkyl_array *field_with_J_radial_init =
    mkarr(8 * confBasis.num_basis, confLocal_ext.volume);
  ...
  gkyl_proj_on_basis *proj_radial_ghost =
    gkyl_proj_on_basis_new(&confGrid, &confBasis, poly_order + 1, 8, field_init_func, &ctx);
  ...
  gkyl_dg_gr_maxwell_divide_Jc(
    &confBasis, &radial_ghost_lo, det_h, field_with_J_radial_init, field_no_J_radial_init, false
  );
  gkyl_dg_gr_maxwell_divide_Jc(
    &confBasis, &radial_ghost_up, det_h, field_with_J_radial_init, field_no_J_radial_init, false
  );
  ...
  struct gkyl_array *bc_buffer_lo_fixed =
    mkarr(field_no_J_con->ncomp, conf_skin_ghost.lower_ghost[0].volume);
  struct gkyl_array *bc_buffer_up_fixed =
    mkarr(field_no_J_con->ncomp, conf_skin_ghost.upper_ghost[0].volume);
  struct gkyl_array *bc_buffer_lo_fixed_with_J =
    mkarr(field_no_J_con->ncomp, conf_skin_ghost.lower_ghost[0].volume);
  struct gkyl_array *bc_buffer_up_fixed_with_J =
    mkarr(field_no_J_con->ncomp, conf_skin_ghost.upper_ghost[0].volume);
  ...
  bc_lo[0] = gkyl_bc_basic_new(
    0, GKYL_LOWER_EDGE, GKYL_BC_FIXED_FUNC, &confBasis, &conf_skin_ghost.lower_skin[0],
    &conf_skin_ghost.lower_ghost[0], field_no_J_con->ncomp, cdim, false
  );
  bc_up[0] = gkyl_bc_basic_new(
    0, GKYL_UPPER_EDGE, GKYL_BC_FIXED_FUNC, &confBasis, &conf_skin_ghost.upper_skin[0],
    &conf_skin_ghost.upper_ghost[0], field_no_J_con->ncomp, cdim, false
  );
  bc_lo[1] = gkyl_bc_basic_new(
    1, GKYL_LOWER_EDGE, GKYL_BC_MAXWELL_THETA_POLE, &confBasis, &conf_skin_ghost.lower_skin[1],
    &conf_skin_ghost.lower_ghost[1], field_no_J_con->ncomp, cdim, false
  );
  bc_up[1] = gkyl_bc_basic_new(
    1, GKYL_UPPER_EDGE, GKYL_BC_MAXWELL_THETA_POLE, &confBasis, &conf_skin_ghost.upper_skin[1],
    &conf_skin_ghost.upper_ghost[1], field_no_J_con->ncomp, cdim, false
  );

  // For further analysis, copy the field to the boundary (in primative variables)
  struct gkyl_bc_basic *bc_lo_copy[GKYL_MAX_DIM] = {0};
  bc_lo_copy[0] = gkyl_bc_basic_new(
    0, GKYL_LOWER_EDGE, GKYL_BC_COPY, &confBasis, &conf_skin_ghost.lower_skin[0],
    &conf_skin_ghost.lower_ghost[0], field_no_J_con->ncomp, cdim, false
  );
  ...
  gkyl_array_copy_to_buffer(
    bc_buffer_lo_fixed_with_J->data, field_with_J_radial_init, &radial_ghost_lo
  );
  gkyl_array_copy_to_buffer(
    bc_buffer_up_fixed_with_J->data, field_with_J_radial_init, &radial_ghost_up
  );
  ...
  for (int ir = confLocal.lower[0]; ir <= confLocal.upper[0]; ++ir) {
    ...
    apply_expected_theta_pole_bc_cell(
      &confBasis, gkyl_array_cfetch(field_no_J_fixed, gkyl_range_idx(&confLocal_ext, idx_skin_lo)),
      expected_lo
    );
    apply_expected_theta_pole_bc_cell(
      &confBasis, gkyl_array_cfetch(field_no_J_fixed, gkyl_range_idx(&confLocal_ext, idx_skin_up)),
      expected_up
    );

    const double *actual_lo =
      gkyl_array_cfetch(field_no_J_fixed, gkyl_range_idx(&confLocal_ext, idx_ghost_lo));
    const double *actual_up =
      gkyl_array_cfetch(field_no_J_fixed, gkyl_range_idx(&confLocal_ext, idx_ghost_up));
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    for (int dir = 0; dir < cdim; ++dir) {
      ...
      if (dir == 0) {
        dg_gr_maxwell_alpha_quad_x_2x_ser_p1(
          meq, xcC, confGrid.dx, theta_pole, jacob_pos_l, jacob_pos_c, lapse_d, shift_d, h_ij_d,
          h_ij_inv_d, det_h_d, field_con_l, field_con_c, field_no_J_con_l, field_no_J_con_c,
          flux_l_x, flux_r_x, alpha_quad_x
        );

        double cflrate = lax_flux_x_2x_ser_p1(
          confGrid.dx, theta_pole, jacob_pos_l, jacob_pos_c, det_h_d, flux_l_x, flux_r_x,
          alpha_quad_x, field_con_l, field_con_c, field_no_J_con_l, field_no_J_con_c, flux
        );
      } else if (dir == 1) {
        dg_gr_maxwell_alpha_quad_y_2x_ser_p1(
          meq, xcC, confGrid.dx, theta_pole, jacob_pos_l, jacob_pos_c, lapse_d, shift_d, h_ij_d,
          h_ij_inv_d, det_h_d, field_con_l, field_con_c, field_no_J_con_l, field_no_J_con_c,
          flux_l_y, flux_r_y, alpha_quad_y
        );

        double cflrate = lax_flux_y_2x_ser_p1(
          confGrid.dx, theta_pole, jacob_pos_l, jacob_pos_c, det_h_d, flux_l_y, flux_r_y,
          alpha_quad_y, field_con_l, field_con_c, field_no_J_con_l, field_no_J_con_c, flux
        );
      }
      ...
      if (dir == 1) {
        ...
        Dx_con_l_n[0] = -(0.8660254037844386 * Dx_l[3]) + 0.8660254037844386 * Dx_l[2] -
                        0.5 * Dx_l[1] + 0.5 * Dx_l[0];
        Dy_con_l_n[0] = -(0.8660254037844386 * Dy_l[3]) + 0.8660254037844386 * Dy_l[2] -
                        0.5 * Dy_l[1] + 0.5 * Dy_l[0];
        Dz_con_l_n[0] = -(0.8660254037844386 * Dz_l[3]) + 0.8660254037844386 * Dz_l[2] -
                        0.5 * Dz_l[1] + 0.5 * Dz_l[0];
        Bx_con_l_n[0] = -(0.8660254037844386 * Bx_l[3]) + 0.8660254037844386 * Bx_l[2] -
                        0.5 * Bx_l[1] + 0.5 * Bx_l[0];
        By_con_l_n[0] = -(0.8660254037844386 * By_l[3]) + 0.8660254037844386 * By_l[2] -
                        0.5 * By_l[1] + 0.5 * By_l[0];
        Bz_con_l_n[0] = -(0.8660254037844386 * Bz_l[3]) + 0.8660254037844386 * Bz_l[2] -
                        0.5 * Bz_l[1] + 0.5 * Bz_l[0];

        Dx_con_l_n[1] = 0.8660254037844386 * Dx_l[3] + 0.8660254037844386 * Dx_l[2] +
                        0.5 * Dx_l[1] + 0.5 * Dx_l[0];
        Dy_con_l_n[1] = 0.8660254037844386 * Dy_l[3] + 0.8660254037844386 * Dy_l[2] +
                        0.5 * Dy_l[1] + 0.5 * Dy_l[0];
        Dz_con_l_n[1] = 0.8660254037844386 * Dz_l[3] + 0.8660254037844386 * Dz_l[2] +
                        0.5 * Dz_l[1] + 0.5 * Dz_l[0];
        Bx_con_l_n[1] = 0.8660254037844386 * Bx_l[3] + 0.8660254037844386 * Bx_l[2] +
                        0.5 * Bx_l[1] + 0.5 * Bx_l[0];
        By_con_l_n[1] = 0.8660254037844386 * By_l[3] + 0.8660254037844386 * By_l[2] +
                        0.5 * By_l[1] + 0.5 * By_l[0];
        Bz_con_l_n[1] = 0.8660254037844386 * Bz_l[3] + 0.8660254037844386 * Bz_l[2] +
                        0.5 * Bz_l[1] + 0.5 * Bz_l[0];
        ...
        Dx_con_r_n[0] = 0.8660254037844386 * Dx_r[3] - 0.8660254037844386 * Dx_r[2] -
                        0.5 * Dx_r[1] + 0.5 * Dx_r[0];
        Dy_con_r_n[0] = 0.8660254037844386 * Dy_r[3] - 0.8660254037844386 * Dy_r[2] -
                        0.5 * Dy_r[1] + 0.5 * Dy_r[0];
        Dz_con_r_n[0] = 0.8660254037844386 * Dz_r[3] - 0.8660254037844386 * Dz_r[2] -
                        0.5 * Dz_r[1] + 0.5 * Dz_r[0];
        Bx_con_r_n[0] = 0.8660254037844386 * Bx_r[3] - 0.8660254037844386 * Bx_r[2] -
                        0.5 * Bx_r[1] + 0.5 * Bx_r[0];
        By_con_r_n[0] = 0.8660254037844386 * By_r[3] - 0.8660254037844386 * By_r[2] -
                        0.5 * By_r[1] + 0.5 * By_r[0];
        Bz_con_r_n[0] = 0.8660254037844386 * Bz_r[3] - 0.8660254037844386 * Bz_r[2] -
                        0.5 * Bz_r[1] + 0.5 * Bz_r[0];

        Dx_con_r_n[1] = -(0.8660254037844386 * Dx_r[3]) - 0.8660254037844386 * Dx_r[2] +
                        0.5 * Dx_r[1] + 0.5 * Dx_r[0];
        Dy_con_r_n[1] = -(0.8660254037844386 * Dy_r[3]) - 0.8660254037844386 * Dy_r[2] +
                        0.5 * Dy_r[1] + 0.5 * Dy_r[0];
        Dz_con_r_n[1] = -(0.8660254037844386 * Dz_r[3]) - 0.8660254037844386 * Dz_r[2] +
                        0.5 * Dz_r[1] + 0.5 * Dz_r[0];
        Bx_con_r_n[1] = -(0.8660254037844386 * Bx_r[3]) - 0.8660254037844386 * Bx_r[2] +
                        0.5 * Bx_r[1] + 0.5 * Bx_r[0];
        By_con_r_n[1] = -(0.8660254037844386 * By_r[3]) - 0.8660254037844386 * By_r[2] +
                        0.5 * By_r[1] + 0.5 * By_r[0];
        Bz_con_r_n[1] = -(0.8660254037844386 * Bz_r[3]) - 0.8660254037844386 * Bz_r[2] +
                        0.5 * Bz_r[1] + 0.5 * Bz_r[0];
        ...
        Dx_con_copy_l_n[0] = -(0.8660254037844386 * Dx_l[3]) + 0.8660254037844386 * Dx_l[2] -
                             0.5 * Dx_l[1] + 0.5 * Dx_l[0];
        Dy_con_copy_l_n[0] = -(0.8660254037844386 * Dy_l[3]) + 0.8660254037844386 * Dy_l[2] -
                             0.5 * Dy_l[1] + 0.5 * Dy_l[0];
        Dz_con_copy_l_n[0] = -(0.8660254037844386 * Dz_l[3]) + 0.8660254037844386 * Dz_l[2] -
                             0.5 * Dz_l[1] + 0.5 * Dz_l[0];
        Bx_con_copy_l_n[0] = -(0.8660254037844386 * Bx_l[3]) + 0.8660254037844386 * Bx_l[2] -
                             0.5 * Bx_l[1] + 0.5 * Bx_l[0];
        By_con_copy_l_n[0] = -(0.8660254037844386 * By_l[3]) + 0.8660254037844386 * By_l[2] -
                             0.5 * By_l[1] + 0.5 * By_l[0];
        Bz_con_copy_l_n[0] = -(0.8660254037844386 * Bz_l[3]) + 0.8660254037844386 * Bz_l[2] -
                             0.5 * Bz_l[1] + 0.5 * Bz_l[0];

        Dx_con_copy_l_n[1] = 0.8660254037844386 * Dx_l[3] + 0.8660254037844386 * Dx_l[2] +
                             0.5 * Dx_l[1] + 0.5 * Dx_l[0];
        Dy_con_copy_l_n[1] = 0.8660254037844386 * Dy_l[3] + 0.8660254037844386 * Dy_l[2] +
                             0.5 * Dy_l[1] + 0.5 * Dy_l[0];
        Dz_con_copy_l_n[1] = 0.8660254037844386 * Dz_l[3] + 0.8660254037844386 * Dz_l[2] +
                             0.5 * Dz_l[1] + 0.5 * Dz_l[0];
        Bx_con_copy_l_n[1] = 0.8660254037844386 * Bx_l[3] + 0.8660254037844386 * Bx_l[2] +
                             0.5 * Bx_l[1] + 0.5 * Bx_l[0];
        By_con_copy_l_n[1] = 0.8660254037844386 * By_l[3] + 0.8660254037844386 * By_l[2] +
                             0.5 * By_l[1] + 0.5 * By_l[0];
        Bz_con_copy_l_n[1] = 0.8660254037844386 * Bz_l[3] + 0.8660254037844386 * Bz_l[2] +
                             0.5 * Bz_l[1] + 0.5 * Bz_l[0];
        ...
        Dx_con_copy_r_n[0] = 0.8660254037844386 * Dx_r[3] - 0.8660254037844386 * Dx_r[2] -
                             0.5 * Dx_r[1] + 0.5 * Dx_r[0];
        Dy_con_copy_r_n[0] = 0.8660254037844386 * Dy_r[3] - 0.8660254037844386 * Dy_r[2] -
                             0.5 * Dy_r[1] + 0.5 * Dy_r[0];
        Dz_con_copy_r_n[0] = 0.8660254037844386 * Dz_r[3] - 0.8660254037844386 * Dz_r[2] -
                             0.5 * Dz_r[1] + 0.5 * Dz_r[0];
        Bx_con_copy_r_n[0] = 0.8660254037844386 * Bx_r[3] - 0.8660254037844386 * Bx_r[2] -
                             0.5 * Bx_r[1] + 0.5 * Bx_r[0];
        By_con_copy_r_n[0] = 0.8660254037844386 * By_r[3] - 0.8660254037844386 * By_r[2] -
                             0.5 * By_r[1] + 0.5 * By_r[0];
        Bz_con_copy_r_n[0] = 0.8660254037844386 * Bz_r[3] - 0.8660254037844386 * Bz_r[2] -
                             0.5 * Bz_r[1] + 0.5 * Bz_r[0];

        Dx_con_copy_r_n[1] = -(0.8660254037844386 * Dx_r[3]) - 0.8660254037844386 * Dx_r[2] +
                             0.5 * Dx_r[1] + 0.5 * Dx_r[0];
        Dy_con_copy_r_n[1] = -(0.8660254037844386 * Dy_r[3]) - 0.8660254037844386 * Dy_r[2] +
                             0.5 * Dy_r[1] + 0.5 * Dy_r[0];
        Dz_con_copy_r_n[1] = -(0.8660254037844386 * Dz_r[3]) - 0.8660254037844386 * Dz_r[2] +
                             0.5 * Dz_r[1] + 0.5 * Dz_r[0];
        Bx_con_copy_r_n[1] = -(0.8660254037844386 * Bx_r[3]) - 0.8660254037844386 * Bx_r[2] +
                             0.5 * Bx_r[1] + 0.5 * Bx_r[0];
        By_con_copy_r_n[1] = -(0.8660254037844386 * By_r[3]) - 0.8660254037844386 * By_r[2] +
                             0.5 * By_r[1] + 0.5 * By_r[0];
        Bz_con_copy_r_n[1] = -(0.8660254037844386 * Bz_r[3]) - 0.8660254037844386 * Bz_r[2] +
                             0.5 * Bz_r[1] + 0.5 * Bz_r[0];
        ...
        JDx_con_l_n[0] = -(0.8660254037844386 * JDx_l[3]) + 0.8660254037844386 * JDx_l[2] -
                         0.5 * JDx_l[1] + 0.5 * JDx_l[0];
        JDy_con_l_n[0] = -(0.8660254037844386 * JDy_l[3]) + 0.8660254037844386 * JDy_l[2] -
                         0.5 * JDy_l[1] + 0.5 * JDy_l[0];
        JDz_con_l_n[0] = -(0.8660254037844386 * JDz_l[3]) + 0.8660254037844386 * JDz_l[2] -
                         0.5 * JDz_l[1] + 0.5 * JDz_l[0];
        JBx_con_l_n[0] = -(0.8660254037844386 * JBx_l[3]) + 0.8660254037844386 * JBx_l[2] -
                         0.5 * JBx_l[1] + 0.5 * JBx_l[0];
        JBy_con_l_n[0] = -(0.8660254037844386 * JBy_l[3]) + 0.8660254037844386 * JBy_l[2] -
                         0.5 * JBy_l[1] + 0.5 * JBy_l[0];
        JBz_con_l_n[0] = -(0.8660254037844386 * JBz_l[3]) + 0.8660254037844386 * JBz_l[2] -
                         0.5 * JBz_l[1] + 0.5 * JBz_l[0];

        JDx_con_l_n[1] = 0.8660254037844386 * JDx_l[3] + 0.8660254037844386 * JDx_l[2] +
                         0.5 * JDx_l[1] + 0.5 * JDx_l[0];
        JDy_con_l_n[1] = 0.8660254037844386 * JDy_l[3] + 0.8660254037844386 * JDy_l[2] +
                         0.5 * JDy_l[1] + 0.5 * JDy_l[0];
        JDz_con_l_n[1] = 0.8660254037844386 * JDz_l[3] + 0.8660254037844386 * JDz_l[2] +
                         0.5 * JDz_l[1] + 0.5 * JDz_l[0];
        JBx_con_l_n[1] = 0.8660254037844386 * JBx_l[3] + 0.8660254037844386 * JBx_l[2] +
                         0.5 * JBx_l[1] + 0.5 * JBx_l[0];
        JBy_con_l_n[1] = 0.8660254037844386 * JBy_l[3] + 0.8660254037844386 * JBy_l[2] +
                         0.5 * JBy_l[1] + 0.5 * JBy_l[0];
        JBz_con_l_n[1] = 0.8660254037844386 * JBz_l[3] + 0.8660254037844386 * JBz_l[2] +
                         0.5 * JBz_l[1] + 0.5 * JBz_l[0];
        ...
        JDx_con_r_n[0] = 0.8660254037844386 * JDx_r[3] - 0.8660254037844386 * JDx_r[2] -
                         0.5 * JDx_r[1] + 0.5 * JDx_r[0];
        JDy_con_r_n[0] = 0.8660254037844386 * JDy_r[3] - 0.8660254037844386 * JDy_r[2] -
                         0.5 * JDy_r[1] + 0.5 * JDy_r[0];
        JDz_con_r_n[0] = 0.8660254037844386 * JDz_r[3] - 0.8660254037844386 * JDz_r[2] -
                         0.5 * JDz_r[1] + 0.5 * JDz_r[0];
        JBx_con_r_n[0] = 0.8660254037844386 * JBx_r[3] - 0.8660254037844386 * JBx_r[2] -
                         0.5 * JBx_r[1] + 0.5 * JBx_r[0];
        JBy_con_r_n[0] = 0.8660254037844386 * JBy_r[3] - 0.8660254037844386 * JBy_r[2] -
                         0.5 * JBy_r[1] + 0.5 * JBy_r[0];
        JBz_con_r_n[0] = 0.8660254037844386 * JBz_r[3] - 0.8660254037844386 * JBz_r[2] -
                         0.5 * JBz_r[1] + 0.5 * JBz_r[0];

        JDx_con_r_n[1] = -(0.8660254037844386 * JDx_r[3]) - 0.8660254037844386 * JDx_r[2] +
                         0.5 * JDx_r[1] + 0.5 * JDx_r[0];
        JDy_con_r_n[1] = -(0.8660254037844386 * JDy_r[3]) - 0.8660254037844386 * JDy_r[2] +
                         0.5 * JDy_r[1] + 0.5 * JDy_r[0];
        JDz_con_r_n[1] = -(0.8660254037844386 * JDz_r[3]) - 0.8660254037844386 * JDz_r[2] +
                         0.5 * JDz_r[1] + 0.5 * JDz_r[0];
        JBx_con_r_n[1] = -(0.8660254037844386 * JBx_r[3]) - 0.8660254037844386 * JBx_r[2] +
                         0.5 * JBx_r[1] + 0.5 * JBx_r[0];
        JBy_con_r_n[1] = -(0.8660254037844386 * JBy_r[3]) - 0.8660254037844386 * JBy_r[2] +
                         0.5 * JBy_r[1] + 0.5 * JBy_r[0];
        JBz_con_r_n[1] = -(0.8660254037844386 * JBz_r[3]) - 0.8660254037844386 * JBz_r[2] +
                         0.5 * JBz_r[1] + 0.5 * JBz_r[0];
      ...
      if (dir == 0) {
        ...
        Dx_con_l_n[0] = -(0.8660254037844386 * Dx_l[3]) - 0.5 * Dx_l[2] +
                        0.8660254037844386 * Dx_l[1] + 0.5 * Dx_l[0];
        Dy_con_l_n[0] = -(0.8660254037844386 * Dy_l[3]) - 0.5 * Dy_l[2] +
                        0.8660254037844386 * Dy_l[1] + 0.5 * Dy_l[0];
        Dz_con_l_n[0] = -(0.8660254037844386 * Dz_l[3]) - 0.5 * Dz_l[2] +
                        0.8660254037844386 * Dz_l[1] + 0.5 * Dz_l[0];
        Bx_con_l_n[0] = -(0.8660254037844386 * Bx_l[3]) - 0.5 * Bx_l[2] +
                        0.8660254037844386 * Bx_l[1] + 0.5 * Bx_l[0];
        By_con_l_n[0] = -(0.8660254037844386 * By_l[3]) - 0.5 * By_l[2] +
                        0.8660254037844386 * By_l[1] + 0.5 * By_l[0];
        Bz_con_l_n[0] = -(0.8660254037844386 * Bz_l[3]) - 0.5 * Bz_l[2] +
                        0.8660254037844386 * Bz_l[1] + 0.5 * Bz_l[0];

        Dx_con_l_n[1] = 0.8660254037844386 * Dx_l[3] + 0.5 * Dx_l[2] +
                        0.8660254037844386 * Dx_l[1] + 0.5 * Dx_l[0];
        Dy_con_l_n[1] = 0.8660254037844386 * Dy_l[3] + 0.5 * Dy_l[2] +
                        0.8660254037844386 * Dy_l[1] + 0.5 * Dy_l[0];
        Dz_con_l_n[1] = 0.8660254037844386 * Dz_l[3] + 0.5 * Dz_l[2] +
                        0.8660254037844386 * Dz_l[1] + 0.5 * Dz_l[0];
        Bx_con_l_n[1] = 0.8660254037844386 * Bx_l[3] + 0.5 * Bx_l[2] +
                        0.8660254037844386 * Bx_l[1] + 0.5 * Bx_l[0];
        By_con_l_n[1] = 0.8660254037844386 * By_l[3] + 0.5 * By_l[2] +
                        0.8660254037844386 * By_l[1] + 0.5 * By_l[0];
        Bz_con_l_n[1] = 0.8660254037844386 * Bz_l[3] + 0.5 * Bz_l[2] +
                        0.8660254037844386 * Bz_l[1] + 0.5 * Bz_l[0];
        ...
        Dx_con_r_n[0] = 0.8660254037844386 * Dx_r[3] - 0.5 * Dx_r[2] -
                        0.8660254037844386 * Dx_r[1] + 0.5 * Dx_r[0];
        Dy_con_r_n[0] = 0.8660254037844386 * Dy_r[3] - 0.5 * Dy_r[2] -
                        0.8660254037844386 * Dy_r[1] + 0.5 * Dy_r[0];
        Dz_con_r_n[0] = 0.8660254037844386 * Dz_r[3] - 0.5 * Dz_r[2] -
                        0.8660254037844386 * Dz_r[1] + 0.5 * Dz_r[0];
        Bx_con_r_n[0] = 0.8660254037844386 * Bx_r[3] - 0.5 * Bx_r[2] -
                        0.8660254037844386 * Bx_r[1] + 0.5 * Bx_r[0];
        By_con_r_n[0] = 0.8660254037844386 * By_r[3] - 0.5 * By_r[2] -
                        0.8660254037844386 * By_r[1] + 0.5 * By_r[0];
        Bz_con_r_n[0] = 0.8660254037844386 * Bz_r[3] - 0.5 * Bz_r[2] -
                        0.8660254037844386 * Bz_r[1] + 0.5 * Bz_r[0];

        Dx_con_r_n[1] = -(0.8660254037844386 * Dx_r[3]) + 0.5 * Dx_r[2] -
                        0.8660254037844386 * Dx_r[1] + 0.5 * Dx_r[0];
        Dy_con_r_n[1] = -(0.8660254037844386 * Dy_r[3]) + 0.5 * Dy_r[2] -
                        0.8660254037844386 * Dy_r[1] + 0.5 * Dy_r[0];
        Dz_con_r_n[1] = -(0.8660254037844386 * Dz_r[3]) + 0.5 * Dz_r[2] -
                        0.8660254037844386 * Dz_r[1] + 0.5 * Dz_r[0];
        Bx_con_r_n[1] = -(0.8660254037844386 * Bx_r[3]) + 0.5 * Bx_r[2] -
                        0.8660254037844386 * Bx_r[1] + 0.5 * Bx_r[0];
        By_con_r_n[1] = -(0.8660254037844386 * By_r[3]) + 0.5 * By_r[2] -
                        0.8660254037844386 * By_r[1] + 0.5 * By_r[0];
        Bz_con_r_n[1] = -(0.8660254037844386 * Bz_r[3]) + 0.5 * Bz_r[2] -
                        0.8660254037844386 * Bz_r[1] + 0.5 * Bz_r[0];
        ...
        Dx_con_copy_l_n[0] = -(0.8660254037844386 * Dx_l[3]) - 0.5 * Dx_l[2] +
                             0.8660254037844386 * Dx_l[1] + 0.5 * Dx_l[0];
        Dy_con_copy_l_n[0] = -(0.8660254037844386 * Dy_l[3]) - 0.5 * Dy_l[2] +
                             0.8660254037844386 * Dy_l[1] + 0.5 * Dy_l[0];
        Dz_con_copy_l_n[0] = -(0.8660254037844386 * Dz_l[3]) - 0.5 * Dz_l[2] +
                             0.8660254037844386 * Dz_l[1] + 0.5 * Dz_l[0];
        Bx_con_copy_l_n[0] = -(0.8660254037844386 * Bx_l[3]) - 0.5 * Bx_l[2] +
                             0.8660254037844386 * Bx_l[1] + 0.5 * Bx_l[0];
        By_con_copy_l_n[0] = -(0.8660254037844386 * By_l[3]) - 0.5 * By_l[2] +
                             0.8660254037844386 * By_l[1] + 0.5 * By_l[0];
        Bz_con_copy_l_n[0] = -(0.8660254037844386 * Bz_l[3]) - 0.5 * Bz_l[2] +
                             0.8660254037844386 * Bz_l[1] + 0.5 * Bz_l[0];

        Dx_con_copy_l_n[1] = 0.8660254037844386 * Dx_l[3] + 0.5 * Dx_l[2] +
                             0.8660254037844386 * Dx_l[1] + 0.5 * Dx_l[0];
        Dy_con_copy_l_n[1] = 0.8660254037844386 * Dy_l[3] + 0.5 * Dy_l[2] +
                             0.8660254037844386 * Dy_l[1] + 0.5 * Dy_l[0];
        Dz_con_copy_l_n[1] = 0.8660254037844386 * Dz_l[3] + 0.5 * Dz_l[2] +
                             0.8660254037844386 * Dz_l[1] + 0.5 * Dz_l[0];
        Bx_con_copy_l_n[1] = 0.8660254037844386 * Bx_l[3] + 0.5 * Bx_l[2] +
                             0.8660254037844386 * Bx_l[1] + 0.5 * Bx_l[0];
        By_con_copy_l_n[1] = 0.8660254037844386 * By_l[3] + 0.5 * By_l[2] +
                             0.8660254037844386 * By_l[1] + 0.5 * By_l[0];
        Bz_con_copy_l_n[1] = 0.8660254037844386 * Bz_l[3] + 0.5 * Bz_l[2] +
                             0.8660254037844386 * Bz_l[1] + 0.5 * Bz_l[0];
        ...
        Dx_con_copy_r_n[0] = 0.8660254037844386 * Dx_r[3] - 0.5 * Dx_r[2] -
                             0.8660254037844386 * Dx_r[1] + 0.5 * Dx_r[0];
        Dy_con_copy_r_n[0] = 0.8660254037844386 * Dy_r[3] - 0.5 * Dy_r[2] -
                             0.8660254037844386 * Dy_r[1] + 0.5 * Dy_r[0];
        Dz_con_copy_r_n[0] = 0.8660254037844386 * Dz_r[3] - 0.5 * Dz_r[2] -
                             0.8660254037844386 * Dz_r[1] + 0.5 * Dz_r[0];
        Bx_con_copy_r_n[0] = 0.8660254037844386 * Bx_r[3] - 0.5 * Bx_r[2] -
                             0.8660254037844386 * Bx_r[1] + 0.5 * Bx_r[0];
        By_con_copy_r_n[0] = 0.8660254037844386 * By_r[3] - 0.5 * By_r[2] -
                             0.8660254037844386 * By_r[1] + 0.5 * By_r[0];
        Bz_con_copy_r_n[0] = 0.8660254037844386 * Bz_r[3] - 0.5 * Bz_r[2] -
                             0.8660254037844386 * Bz_r[1] + 0.5 * Bz_r[0];

        Dx_con_copy_r_n[1] = -(0.8660254037844386 * Dx_r[3]) + 0.5 * Dx_r[2] -
                             0.8660254037844386 * Dx_r[1] + 0.5 * Dx_r[0];
        Dy_con_copy_r_n[1] = -(0.8660254037844386 * Dy_r[3]) + 0.5 * Dy_r[2] -
                             0.8660254037844386 * Dy_r[1] + 0.5 * Dy_r[0];
        Dz_con_copy_r_n[1] = -(0.8660254037844386 * Dz_r[3]) + 0.5 * Dz_r[2] -
                             0.8660254037844386 * Dz_r[1] + 0.5 * Dz_r[0];
        Bx_con_copy_r_n[1] = -(0.8660254037844386 * Bx_r[3]) + 0.5 * Bx_r[2] -
                             0.8660254037844386 * Bx_r[1] + 0.5 * Bx_r[0];
        By_con_copy_r_n[1] = -(0.8660254037844386 * By_r[3]) + 0.5 * By_r[2] -
                             0.8660254037844386 * By_r[1] + 0.5 * By_r[0];
        Bz_con_copy_r_n[1] = -(0.8660254037844386 * Bz_r[3]) + 0.5 * Bz_r[2] -
                             0.8660254037844386 * Bz_r[1] + 0.5 * Bz_r[0];
        ...
        JDx_con_l_n[0] = -(0.8660254037844386 * JDx_l[3]) - 0.5 * JDx_l[2] +
                         0.8660254037844386 * JDx_l[1] + 0.5 * JDx_l[0];
        JDy_con_l_n[0] = -(0.8660254037844386 * JDy_l[3]) - 0.5 * JDy_l[2] +
                         0.8660254037844386 * JDy_l[1] + 0.5 * JDy_l[0];
        JDz_con_l_n[0] = -(0.8660254037844386 * JDz_l[3]) - 0.5 * JDz_l[2] +
                         0.8660254037844386 * JDz_l[1] + 0.5 * JDz_l[0];
        JBx_con_l_n[0] = -(0.8660254037844386 * JBx_l[3]) - 0.5 * JBx_l[2] +
                         0.8660254037844386 * JBx_l[1] + 0.5 * JBx_l[0];
        JBy_con_l_n[0] = -(0.8660254037844386 * JBy_l[3]) - 0.5 * JBy_l[2] +
                         0.8660254037844386 * JBy_l[1] + 0.5 * JBy_l[0];
        JBz_con_l_n[0] = -(0.8660254037844386 * JBz_l[3]) - 0.5 * JBz_l[2] +
                         0.8660254037844386 * JBz_l[1] + 0.5 * JBz_l[0];

        JDx_con_l_n[1] = 0.8660254037844386 * JDx_l[3] + 0.5 * JDx_l[2] +
                         0.8660254037844386 * JDx_l[1] + 0.5 * JDx_l[0];
        JDy_con_l_n[1] = 0.8660254037844386 * JDy_l[3] + 0.5 * JDy_l[2] +
                         0.8660254037844386 * JDy_l[1] + 0.5 * JDy_l[0];
        JDz_con_l_n[1] = 0.8660254037844386 * JDz_l[3] + 0.5 * JDz_l[2] +
                         0.8660254037844386 * JDz_l[1] + 0.5 * JDz_l[0];
        JBx_con_l_n[1] = 0.8660254037844386 * JBx_l[3] + 0.5 * JBx_l[2] +
                         0.8660254037844386 * JBx_l[1] + 0.5 * JBx_l[0];
        JBy_con_l_n[1] = 0.8660254037844386 * JBy_l[3] + 0.5 * JBy_l[2] +
                         0.8660254037844386 * JBy_l[1] + 0.5 * JBy_l[0];
        JBz_con_l_n[1] = 0.8660254037844386 * JBz_l[3] + 0.5 * JBz_l[2] +
                         0.8660254037844386 * JBz_l[1] + 0.5 * JBz_l[0];
        ...
        JDx_con_r_n[0] = 0.8660254037844386 * JDx_r[3] - 0.5 * JDx_r[2] -
                         0.8660254037844386 * JDx_r[1] + 0.5 * JDx_r[0];
        JDy_con_r_n[0] = 0.8660254037844386 * JDy_r[3] - 0.5 * JDy_r[2] -
                         0.8660254037844386 * JDy_r[1] + 0.5 * JDy_r[0];
        JDz_con_r_n[0] = 0.8660254037844386 * JDz_r[3] - 0.5 * JDz_r[2] -
                         0.8660254037844386 * JDz_r[1] + 0.5 * JDz_r[0];
        JBx_con_r_n[0] = 0.8660254037844386 * JBx_r[3] - 0.5 * JBx_r[2] -
                         0.8660254037844386 * JBx_r[1] + 0.5 * JBx_r[0];
        JBy_con_r_n[0] = 0.8660254037844386 * JBy_r[3] - 0.5 * JBy_r[2] -
                         0.8660254037844386 * JBy_r[1] + 0.5 * JBy_r[0];
        JBz_con_r_n[0] = 0.8660254037844386 * JBz_r[3] - 0.5 * JBz_r[2] -
                         0.8660254037844386 * JBz_r[1] + 0.5 * JBz_r[0];

        JDx_con_r_n[1] = -(0.8660254037844386 * JDx_r[3]) + 0.5 * JDx_r[2] -
                         0.8660254037844386 * JDx_r[1] + 0.5 * JDx_r[0];
        JDy_con_r_n[1] = -(0.8660254037844386 * JDy_r[3]) + 0.5 * JDy_r[2] -
                         0.8660254037844386 * JDy_r[1] + 0.5 * JDy_r[0];
        JDz_con_r_n[1] = -(0.8660254037844386 * JDz_r[3]) + 0.5 * JDz_r[2] -
                         0.8660254037844386 * JDz_r[1] + 0.5 * JDz_r[0];
        JBx_con_r_n[1] = -(0.8660254037844386 * JBx_r[3]) + 0.5 * JBx_r[2] -
                         0.8660254037844386 * JBx_r[1] + 0.5 * JBx_r[0];
        JBy_con_r_n[1] = -(0.8660254037844386 * JBy_r[3]) + 0.5 * JBy_r[2] -
                         0.8660254037844386 * JBy_r[1] + 0.5 * JBy_r[0];
        JBz_con_r_n[1] = -(0.8660254037844386 * JBz_r[3]) + 0.5 * JBz_r[2] -
                         0.8660254037844386 * JBz_r[1] + 0.5 * JBz_r[0];
...
void
test_ks_r_theta_2x_p1()
{
  test_ks_r_theta_2x_geom_p1();
}

TEST_LIST = {{"test_ks_r_theta_2x_p1", test_ks_r_theta_2x_p1}, {NULL, NULL}};

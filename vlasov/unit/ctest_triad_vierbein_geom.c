...
test_triad_math_1v()
{
  ...
  kernel_conf_poisson_tensor_vierbein_1v(vierbein_inv, vierbein_gradient, conf_poisson_tensor);
...
void
eval_vierbein_1v(double t, const double *xn, double *restrict fout, void *ctx)
{
  fout[0] = 1.0;
}

void
eval_vierbein_gradient_1v(double t, const double *xn, double *restrict fout, void *ctx)
{
  fout[0] = 0.0;
}
...
void
eval_vierbein_2v(double t, const double *xn, double *restrict fout, void *ctx)
{
  fout[0] = 1.0;
  fout[1] = 0.0;
  fout[2] = 0.0;
...
void
eval_vierbein_gradient_2v(double t, const double *xn, double *restrict fout, void *ctx)
{
  fout[0] = 0.0;
  fout[1] = 0.0;
  fout[2] = 0.0;
...
void
eval_vierbein_annulus_2v(double t, const double *xn, double *restrict fout, void *ctx)
{
  double q_r = xn[0];
  double q_theta = 0.0;
  fout[0] = 1.0;
...
void
eval_vierbein_gradient_annulus_2v(double t, const double *xn, double *restrict fout, void *ctx)
{
  double q_r = xn[0];
  double q_theta = 0.0;
  fout[0] = 0.0;
...
void
eval_vierbein_obl_sph_3v(double t, const double *xn, double *restrict fout, void *ctx)
{
  // Parameters
  double a = 0.2;
  double M = 0.7;
  ...
  // Metric spatial covariant components
  double h_rr = (1.0 + 2.0 * M * r / rho_sq);
  double h_pp = sin(theta) * sin(theta) *
                (rho_sq + a * a * (1.0 + 2.0 * M * r / rho_sq) * sin(theta) * sin(theta));
  double h_thth = rho_sq;
...
void
eval_vierbein_gradient_obl_sph_3v(double t, const double *xn, double *restrict fout, void *ctx)
{
  double a = 0.2;
  double M = 0.7;
  ...
  // Gradient w.r.t. r: d(e_i^a)/dr
  fout[0] = (M * (a * a * cos(theta) * cos(theta) - r * r)) /
            (pow(rho_sq, 1.5) * sqrt(rho_sq + 2.0 * M * r));
  fout[1] = 0.0;
  fout[2] = 0.0;
  fout[3] = 0.0;
  fout[4] = r / sqrt(rho_sq);
  fout[5] = 0.0;
  fout[6] = -(M * a * sin(theta) * sin(theta) * (a * a * cos(theta) * cos(theta) - r * r)) /
            (pow(rho_sq, 1.5) * sqrt(rho_sq + 2.0 * M * r));
  fout[7] = 0.0;
  ...
  fout[14] = 0.0;
  fout[15] =
    -(2.0 * a * cos(theta) * sin(theta) *
      (a * a * a * a * cos(theta) * cos(theta) * cos(theta) * cos(theta) + 2.0 * M * r * r * r +
       r * r * r * r + 2.0 * a * a * r * r * cos(theta) * cos(theta) +
       2.0 * M * a * a * r * cos(theta) * cos(theta) + M * a * a * r * sin(theta) * sin(theta))) /
    (pow(rho_sq, 1.5) * sqrt(rho_sq + 2.0 * M * r));
  fout[16] = 0.0;
  fout[17] =
    (sin(4.0 * theta) * a * a + 2.0 * sin(2.0 * theta) * r * r) /
    (2.0 * sqrt(1.0 - cos(2.0 * theta)) * sqrt(a * a * cos(2.0 * theta) + a * a + 2.0 * r * r));
...
void
test_triad_2x2v_rphi_ks()
{
  test_triad_2x2v_rphi_ks_pnt(2);
}
void
test_triad_3v_obl_sph()
{
  test_triad_3v_obl_sph_conf(2);
}
void
test_triad_3v_obl_sph_pnt()
{
  test_triad_3v_obl_sph_conf_pnt(2);
}

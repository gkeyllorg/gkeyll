...
hybrid_m0_error(int cdim, enum gkyl_basis_type ctype, bool use_gpu)
{
  ...
  struct gkyl_vlasov_velocity_map *vmap = gkyl_vlasov_velocity_map_new(&vgrid, &vlocal, &vbasis, vmap_inp, false, use_gpu);
  struct gkyl_array *hamil = use_gpu ? gkyl_array_cu_dev_new(GKYL_DOUBLE, vbasis.num_basis, vlocal_ext.volume)
    : gkyl_array_new(GKYL_DOUBLE, vbasis.num_basis, vlocal_ext.volume);
  struct gkyl_array *hinv = use_gpu ? gkyl_array_cu_dev_new(GKYL_DOUBLE, vbasis.num_basis, vlocal_ext.volume)
    : gkyl_array_new(GKYL_DOUBLE, vbasis.num_basis, vlocal_ext.volume);
  gkyl_dg_vlasov_calc_hamil(&vgrid, &vbasis, &vlocal, GKYL_MODEL_DEFAULT, vmap, hamil, hinv, use_gpu);
  ...
  for (int k=0; k<pbasis.num_basis; ++k) fd[k] = lcg_rand(&seed);
  ...
  int nq = 1;
  for (int d=0; d<pdim; ++d) nq *= 3;
  for (int q=0; q<nq; ++q) {
    double eta[6], w = 1.0;
    int qq = q;
    for (int d=0; d<pdim; ++d) { int i = qq%3; qq /= 3; eta[d] = xq[i]; w *= wq[i]; }
    double pb[216];
    pbasis.eval(eta, pb);
    double fv = 0.0;
    for (int k=0; k<pbasis.num_basis; ++k) fv += fd[k]*pb[k];
    double cb[8];
    cbasis_ser.eval(eta, cb);
    for (int j=0; j<cbasis_ser.num_basis; ++j) ref[j] += w*fv*cb[j];
  }

  struct gkyl_mom_vlasov_inp inp = {
    .conf_basis = &cbasis, .phase_basis = &pbasis, .vel_range = &vlocal, .vel_map = vmap,
    .hamil_range = &vlocal, .hamil = hamil, .model_id = GKYL_MODEL_TRIAD,
    .hamil_id = GKYL_HAMIL_VEL_SPARSE, .mom_type = GKYL_F_MOMENT_M0, .use_gpu = use_gpu,
  };
  struct gkyl_mom_type *mt = gkyl_mom_vlasov_inew(&inp);
  gkyl_mom_calc *mc = gkyl_mom_calc_new(&pgrid, mt, use_gpu);
  struct gkyl_array *m0 = use_gpu ? gkyl_array_cu_dev_new(GKYL_DOUBLE, cbasis.num_basis, clocal_ext.volume)
  ...
  double err = 0.0;
  for (int j=0; j<cbasis.num_basis; ++j) err = fmax(err, fabs(m[j]-ref[j]));

  if (use_gpu) { gkyl_array_release(f); gkyl_array_release(m0_ho); }
  gkyl_array_release(f_ho); gkyl_array_release(m0);
  gkyl_mom_calc_release(mc); gkyl_mom_type_release(mt);
  gkyl_array_release(hamil); gkyl_array_release(hinv);
...
test_hybrid_m0(int cdim, enum gkyl_basis_type ctype, bool use_gpu)
{
  ...
  TEST_MSG("%dx3v %s-typed conf basis: max|M0 - quadrature| = %g", cdim,

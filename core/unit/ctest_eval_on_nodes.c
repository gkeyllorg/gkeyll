...
void test_2x2v_hyb(int poly_order, int test_func_op)
{
  ...
  // The hybrid basis places its nodes at Gauss-Lobatto points (2 per
  // configuration direction, 3 per velocity direction), so the nodal
  // projection must (a) interpolate the function exactly at every node and
  // (b) yield a globally CONTINUOUS representation: the node sets of two
  // cells coincide on their shared face and uniquely determine the face
  // trace, so the left and right polynomials agree on the whole face.
  evalf_t fn = test_func_op==0 ? evalFunc_4x_quad : evalFunc_4x_trig;

  double *nodes = gkyl_malloc(sizeof(double)*basis.num_basis*ndim);
  basis.node_list(nodes);

  // (a) nodal interpolation at every node of every cell.
  double xc[GKYL_MAX_DIM], xp[GKYL_MAX_DIM], fval[1];
  struct gkyl_range_iter iter;
  gkyl_range_iter_init(&iter, &arr_range);
  while (gkyl_range_iter_next(&iter)) {
    long loc = gkyl_range_idx(&arr_range, iter.idx);
    const double *fc = gkyl_array_cfetch(distf, loc);
    gkyl_rect_grid_cell_center(&grid, iter.idx, xc);
    for (int n=0; n<basis.num_basis; ++n) {
      const double *eta = &nodes[n*ndim];
      for (int d=0; d<ndim; ++d) xp[d] = xc[d] + 0.5*grid.dx[d]*eta[d];
      fn(0.0, xp, fval, NULL);
      double fp = basis.eval_expand(eta, fc);
      TEST_CHECK( gkyl_compare(fval[0], fp, 1e-10) );
      TEST_MSG("cell (%d,%d,%d,%d) node %d: f=%.15e proj=%.15e",
        iter.idx[0], iter.idx[1], iter.idx[2], iter.idx[3], n, fval[0], fp);
    }
  }
  gkyl_free(nodes);

  // (b) continuity across every interior face, sampled at points that do NOT
  // coincide with basis nodes (a stronger check than node agreement).
  double tpts[3] = { -0.7853981633974483, 0.1234567890123456, 0.9130102149414117 };
  int idxr[GKYL_MAX_DIM];
  gkyl_range_iter_init(&iter, &arr_range);
  while (gkyl_range_iter_next(&iter)) {
    long locL = gkyl_range_idx(&arr_range, iter.idx);
    const double *fL = gkyl_array_cfetch(distf, locL);
    for (int dir=0; dir<ndim; ++dir) {
      if (iter.idx[dir] == arr_range.upper[dir]) continue;
      gkyl_copy_int_arr(ndim, iter.idx, idxr);
      idxr[dir] += 1;
      const double *fR = gkyl_array_cfetch(distf, gkyl_range_idx(&arr_range, idxr));
      // tensor sample of transverse points on the shared face
      int npt = 1;
      for (int d=0; d<ndim; ++d) if (d != dir) npt *= 3;
      for (int s=0; s<npt; ++s) {
        double etaL[GKYL_MAX_DIM], etaR[GKYL_MAX_DIM];
        int q = s;
        for (int d=0; d<ndim; ++d) {
          if (d == dir) { etaL[d] = 1.0; etaR[d] = -1.0; }
          else { etaL[d] = etaR[d] = tpts[q%3]; q /= 3; }
        }
        double vL = basis.eval_expand(etaL, fL);
        double vR = basis.eval_expand(etaR, fR);
        TEST_CHECK( gkyl_compare(vL, vR, 1e-12) );
        TEST_MSG("dir %d cell (%d,%d,%d,%d) sample %d: left=%.15e right=%.15e",
          dir, iter.idx[0], iter.idx[1], iter.idx[2], iter.idx[3], s, vL, vR);
      }
    }
  }

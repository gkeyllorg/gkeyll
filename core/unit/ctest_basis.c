...
test_hyb_combo(int cdim, int vdim)
{
  ...
  double ord1[2][3] = {
    {-1.0 / sqrt(3.0), 1.0 / sqrt(3.0), 0.0}, {-sqrt(3.0 / 5.0), 0.0, sqrt(3.0 / 5.0)}
  };
  double wgt1[2][3] = {{1.0, 1.0, 0.0}, {5.0 / 9.0, 8.0 / 9.0, 5.0 / 9.0}};
  ...
  for (int n = 0; n < nb; ++n) {
    fnodal[n] = basis.eval_expand(&nodes[n * ndim], f);
  }
  basis.nodal_to_modal(fnodal, fmodal);
  for (int i = 0; i < nb; ++i) {
    TEST_CHECK(gkyl_compare(f[i], fmodal[i], 1e-11));
  }
  gkyl_free(nodes);
  ...
  for (int n = 0; n < nb; ++n) {
    basis.modal_to_quad_nodal(f, fquad, n);
  }
  for (int i = 0; i < nb; ++i) {
    basis.quad_nodal_to_modal(fquad, f2, i);
  }
  for (int i = 0; i < nb; ++i) {
    TEST_CHECK(gkyl_compare(f[i], f2[i], 1e-11));
  }

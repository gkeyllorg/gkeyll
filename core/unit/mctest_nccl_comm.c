...
void nccl_n2_allgather_1d_host()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 2) {
...
void nccl_n4_allgather_2d_host()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 4) {
...
void nccl_n4_sync_2d_no_corner()
{
  nccl_n4_sync_2d(false);
}
void nccl_n4_sync_2d_use_corner()
{
  nccl_n4_sync_2d(true);
}

void nccl_n4_sync_1x1v()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 4) {
...
static void nccl_n4_create_comm_from_ranks_1()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 4) {
...
static void nccl_n4_create_comm_from_ranks_2()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 4) {

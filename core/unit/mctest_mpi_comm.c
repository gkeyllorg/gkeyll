...
static void mpi_n2_allgather_1d()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 2) {
...
static void mpi_n4_allgather_2d()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 4) {
...
static void mpi_n2_allgather_1d_host()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 2) {
...
static void mpi_n4_allgather_2d_host()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 4) {
...
void mpi_n4_sync_2d_no_corner()
{
  mpi_n4_sync_2d(false);
}
void mpi_n4_sync_2d_use_corner()
{
  mpi_n4_sync_2d(true);
}

static void mpi_n4_sync_1x1v()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 4) {
...
static void mpi_n1_per_sync_2d()
{
  int per_dirs_0[] = {0};
  int per_dirs_1[] = {1};
  int per_dirs_01[] = {0, 1};
...
static void mpi_n1_per_sync_corner_2d(void)
{
  mpi_per_sync_corner_2d(1, (int[]){1, 1});
}
static void mpi_n2_per_sync_corner_2d(void)
{
  mpi_per_sync_corner_2d(2, (int[]){2, 1});
}
static void mpi_n4_per_sync_corner_2d(void)
{
  mpi_per_sync_corner_2d(4, (int[]){2, 2});
}

static void mpi_n2_per_sync_2d()
{
  int cuts_21[] = {2, 1};
  int cuts_12[] = {1, 2};
  int per_dirs_0[] = {0};
...
static void mpi_per_sync_corner_3d(int nrank, int cuts[])
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    if (!TEST_CHECK(d[0] == 1.5)) {
      TEST_MSG(
        "3D periodic sync failed on rank %d (%d,%d,%d). Expected %g. Got %g\n", rank, iter.idx[0],
        iter.idx[1], iter.idx[2], 1.5, d[0]
      );
    }
  }
...
static void mpi_n1_per_sync_corner_3d(void)
{
  mpi_per_sync_corner_3d(1, (int[]){1, 1, 1});
}
static void mpi_n2_per_sync_corner_3d(void)
{
  mpi_per_sync_corner_3d(2, (int[]){2, 1, 1});
}
static void mpi_n4_per_sync_corner_3d(void)
{
  mpi_per_sync_corner_3d(4, (int[]){2, 1, 2});
}
static void mpi_n8_per_sync_corner_3d(void)
{
  mpi_per_sync_corner_3d(8, (int[]){2, 2, 2});
}
static void mpi_n27_per_sync_corner_3d(void)
{
...
static void mpi_n4_create_comm_from_ranks_1()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 4) {
...
static void mpi_n4_create_comm_from_ranks_2()
{
  int m_sz;
  MPI_Comm_size(MPI_COMM_WORLD, &m_sz);
  if (m_sz != 4) {

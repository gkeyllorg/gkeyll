...
void
eval_const(double t, const double *xn, double *fout, void *ctx)
{
  fout[0] = 3.0;
}

void
eval_linear(double t, const double *xn, double *fout, void *ctx)
{
...
void
eval_mode(double t, const double *xn, double *fout, void *ctx)
{
...
void
eval_bump(double t, const double *xn, double *fout, void *ctx)
{
  // Off-center bump along the filtered direction.
  struct profile_ctx *pctx = ctx;
  fout[0] =
    (0.1 + exp(-pow((xn[FILTER_DIR] - pctx->x0) / pctx->w, 2))) * transverse_mod(xn, pctx->ndim);
}
...
filter_env_new(struct filter_env *env, bool use_gpu, int ndim, const int *cells, int poly_order)
{
  ...
  env->fin_ho = use_gpu ? mkarr(false, env->fin->ncomp, env->fin->size) :
                          gkyl_array_acquire(env->fin);
  env->fout_ho = use_gpu ? mkarr(false, env->fout->ncomp, env->fout->size) :
                           gkyl_array_acquire(env->fout);
}
...
static void
filter_apply(
  struct filter_env *env, const struct gkyl_range *sub, int M, double cutoff_wavelength,
  evalf_t func, void *func_ctx
)
{
  // Project func and filter it over sub, leaving both fields on the host.
  gkyl_proj_on_basis *proj =
    gkyl_proj_on_basis_new(&env->grid, &env->basis, env->basis.poly_order + 1, 1, func, func_ctx);
  ...
  struct gkyl_dg_lowpass_filter *lpf = gkyl_dg_lowpass_filter_new(
    FILTER_DIR, M, cutoff_wavelength, &env->basis, &env->grid, sub, env->use_gpu
  );
...
static void
filter_gain_check(
  struct filter_env *env, const struct gkyl_range *sub, int edge_skip, double gain, double tol,
  const char *what
)
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    for (int c = 0; c < env->basis.num_basis; c++) {
      ...
      TEST_MSG(
        "%s, cell %d coeff %d: expected %.13e, got %.13e", what, iter.idx[FILTER_DIR], c,
        gain * in_c[c], out_c[c]
      );
    }
  }

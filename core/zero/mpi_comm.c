...
)
{
  ...
  int ret = MPI_Allgather(
    gkyl_mem_buff_data(mpi->allgather_buff_local.buff), nelem, MPI_CHAR,
...
)
{
  ...
  for (int n = 0; n < mpi->neigh->num_neigh; ++n) {
    ...
    if (recv_vol > 0) {
      ...
      MPI_Irecv(
        gkyl_mem_buff_data(mpi->recv[nridx].buff), recv_vol, MPI_CHAR, nid, tag, mpi->mcomm,
  ...
  for (int n = 0; n < mpi->neigh->num_neigh; ++n) {
    ...
    if (send_vol > 0) {
      ...
      MPI_Isend(
        gkyl_mem_buff_data(mpi->send[nsidx].buff), send_vol, MPI_CHAR, nid, tag, mpi->mcomm,

...
static ncclDataType_t g2_nccl_datatype[] =
  {[GKYL_INT] = ncclInt, [GKYL_INT_64] = ncclInt64, [GKYL_FLOAT] = ncclFloat, [GKYL_DOUBLE] = ncclDouble
};

// Mapping of Gkeyll ops to ncclRedOp_t.
static ncclRedOp_t g2_nccl_op[] = {[GKYL_MIN] = ncclMin, [GKYL_MAX] = ncclMax, [GKYL_SUM] = ncclSum};
...
)
{
  ...
  gkyl_array_copy_to_buffer(gkyl_mem_buff_data(nccl->allgather_buff_local.buff), array_local, local);
...
static int array_allgather_host(
  struct gkyl_comm *comm, const struct gkyl_range *local, const struct gkyl_range *global,
  const struct gkyl_array *array_local, struct gkyl_array *array_global
)
{
...
)
{
  ...
  ncclResult_t status = ncclBroadcast(
    asend->data, arecv->data, nelem, g2_nccl_datatype[asend->type], root, nccl->ncomm,
    nccl->custream
  );
  wait_for_nccl_collective(nccl, status);
...
)
{
  ...
  if (has_nccl_ops) {
    ...
    for (int r = 0; r < nridx; ++r) {
      ...
      checkNCCL(ncclRecv(
        gkyl_mem_buff_data(nccl->recv[r].buff), recv_vol, ncclChar, recv_nids[r], nccl->ncomm,
        nccl->custream
      ));
    }
    ...
    for (int s = 0; s < nsidx; ++s) {
      ...
      checkNCCL(ncclSend(
        gkyl_mem_buff_data(nccl->send[s].buff), send_vol, ncclChar, send_nids[s], nccl->ncomm,
        nccl->custream
      ));
    }

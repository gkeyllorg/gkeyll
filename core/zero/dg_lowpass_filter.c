...
)
{
  ...
  if (up->use_gpu) {
    ...
    gkyl_cu_memcpy(
      up->weights_cu, up->weights, (2 * half_width + 1) * sizeof(double), GKYL_CU_MEMCPY_H2D
    );
    gkyl_cu_memcpy(
      up->sign_plain_cu, up->sign_plain, up->num_basis * sizeof(double), GKYL_CU_MEMCPY_H2D
    );
    gkyl_cu_memcpy(
      up->sign_mirror_cu, up->sign_mirror, up->num_basis * sizeof(double), GKYL_CU_MEMCPY_H2D
    );
  }
#endif
...
void
gkyl_dg_lowpass_filter_advance(
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    for (int k = -M; k < M + 1; k++) {
      ...
      if (idx_k < up->range.lower[dir]) {
        if (!up->reflect_lo) {
          continue; // Interior edge: drop donor, renormalize.
        }
        idx_do[dir] =
          dg_lpf_mirror_idx(idx_k, up->range.lower[dir], up->range.upper[dir], &mirrored);

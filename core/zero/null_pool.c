...
static void null_pool_free(const struct gkyl_ref_count *ref)
{
  struct gkyl_job_pool *np = container_of(ref, struct gkyl_job_pool, ref_count);
  gkyl_free(np);
}

static bool null_pool_add_work(const struct gkyl_job_pool *jp, jp_work_func func, void *ctx)
{
  func(ctx); // run job immediately
  return true;
}

static void null_pool_wait(const struct gkyl_job_pool *jp)
{
}

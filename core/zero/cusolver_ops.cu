...
)
{
  ...
  cusolverSpXcsrperm_bufferSizeHost(
    prob->cusolverSpH, prob->mrow, prob->ncol, prob->nnz, prob->A, h_csrRowIndB, h_csrColIndB,
  ...
  cusolverSpDcsrluBufferInfoHost(
    prob->cusolverSpH, prob->mrow, prob->nnz, prob->A, h_csrValB, h_csrRowIndB, h_csrColIndB,
    prob->infolu, &prob->size_internal, &size_lu
  );
  ...
  cusolverSpDcsrluFactorHost(
    prob->cusolverSpH, prob->mrow, prob->nnz, prob->A, h_csrValB, h_csrRowIndB, h_csrColIndB,
    prob->infolu, prob->pivot_threshold, buffer_cpu
  );
  ...
  cusolverSpDcsrluExtractHost(
    prob->cusolverSpH, h_Plu, h_Qlu, prob->A, h_csrValL, h_csrRowIndL, h_csrColIndL, prob->A,
  ...
  if ((prob->nprob == 1) && (prob->nrhs == 1)) {
    cusolverRfSetupHost(
      prob->mrow, prob->nnz, csrrowptrA, csrcolindA, csrvalA, nnzL, h_csrRowIndL, h_csrColIndL,
      h_csrValL, nnzU, h_csrRowIndU, h_csrColIndU, h_csrValU, h_P, h_Q, prob->cusolverRfH
    );
  } else {
    for (size_t k = 0; k < prob->nrhs; k++) {
    ...
    cusolverRfBatchSetupHost(
      prob->nrhs, prob->mrow, prob->nnz, csrrowptrA, csrcolindA, csrvalApointers, nnzL,
      h_csrRowIndL, h_csrColIndL, h_csrValL, nnzU, h_csrRowIndU, h_csrColIndU, h_csrValU, h_P, h_Q,
      prob->cusolverRfH
    );
  }
  ...
  if (prob->nrhs == 1) {
    cusolverRfResetValues(
      prob->mrow, prob->nnz, prob->csrrowptrA_cu, prob->csrcolindA_cu, prob->csrvalA_cu, prob->d_P,
      prob->d_Q, prob->cusolverRfH
    );
  } else {
    cusolverRfBatchResetValues(
      prob->nrhs, prob->mrow, prob->nnz, prob->csrrowptrA_cu, prob->csrcolindA_cu,
  ...
  if (prob->nrhs == 1) {
    prob->d_T = (double *)gkyl_cu_malloc(
      sizeof(double) * prob->mrow
    ); // Working space in cusolverRfSolve, |d_T| = n * nrhs.
  } else {
    prob->d_T = (double *)gkyl_cu_malloc(
      sizeof(double) * prob->mrow * prob->nrhs * 2
    ); // Working space in cusolverRfSolve, |d_T| = 2*n*nrhs*batchSize.
  }
...
void gkyl_culinsolver_solve(struct gkyl_culinsolver_prob *prob)
{
  ...
  if (prob->nrhs == 1) {
    ...
    cusolverRfBatchSolve(
      prob->cusolverRfH, prob->d_P, prob->d_Q, 1, prob->d_T, prob->mrow, prob->rhspointers_cu,
      prob->mrow
    );
  }
}

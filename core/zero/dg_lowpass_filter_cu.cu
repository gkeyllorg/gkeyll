...
gkyl_dg_lowpass_filter_advance_cu_ker(
  int dir, int M, int num_basis, bool reflect_lo, bool reflect_up,
...
void
gkyl_dg_lowpass_filter_advance_cu(
...
)
{
  ...
  gkyl_dg_lowpass_filter_advance_cu_ker<<<nblocks, nthreads>>>(
    up->dir, up->half_width, up->num_basis, up->reflect_lo, up->reflect_up, up->weights_cu,
    up->sign_plain_cu, up->sign_mirror_cu, up->range, fdo->on_dev, ftar->on_dev
  );
}

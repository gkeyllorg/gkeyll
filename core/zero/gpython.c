...
/* ---- file I/O ----------------------------------------------------------- */
static void
grid_out(const struct gkyl_rect_grid *grid, int *ndim, double *lower, double *upper, int *cells)
{
  *ndim = grid->ndim;
...
)
{
  ...
  enum gkyl_array_rio_status status = gkyl_grid_sub_array_header_read(&grid, &hdr, fname);
...
gpython_array *
gpython_read_field(const char *fname, int *ndim, double *lower, double *upper, int *cells)
{
  struct gkyl_rect_grid grid;
...
{
  return CARR(out)->ncomp == CARR(a1)->ncomp && CARR(out)->size == CARR(a1)->size &&
...
)
{
  ...
  gkyl_dg_bin_op_mem *mem = gkyl_dg_bin_op_mem_new(CARR(a1)->size, CBAS(b)->num_basis);
  for (int f = 0; f < nf; ++f) {
...
)
{
  ...
  if ((size_t)crange.volume != CARR(cop)->size || (size_t)prange.volume != CARR(pop)->size) {
    return 1;
  }

  gkyl_dg_mul_conf_phase_op_range(
    CBAS(cbasis), CBAS(pbasis), ARR(pout), CARR(cop), CARR(pop), &crange, &prange
  );
  return 0;
}
...
)
{
  ...
  for (int f = 0; f < nf; ++f) {
    gkyl_dg_differentiate_op_local(CBAS(b), dir, diff_order, dx, f, ARR(out), f, CARR(in));
  }
  return 0;
}
...
)
{
  static const enum gkyl_array_integrate_op ops[] = {
    GKYL_ARRAY_INTEGRATE_OP_NONE, GKYL_ARRAY_INTEGRATE_OP_ABS, GKYL_ARRAY_INTEGRATE_OP_SQ
  };
...
)
{
  ...
  struct gkyl_array_average *up = gkyl_array_average_new(
    &grid, CBAS(b), CBAS(b_avg), &range, &range_avg, &range_avg, weight ? CARR(weight) : NULL,
    avg_dim, false
  );
...
)
{
  ...
  struct gkyl_dg_eval_at_coord_proj *up =
    gkyl_dg_eval_at_coord_proj_new(cdim_do, CBAS(b), num_eval, eval_dirs, false);
  ...
  gkyl_dg_eval_at_coord_proj_target_basis(
    up, &cdim_tar, &ndim_tar_full, &btype_tar, &poly_order_tar, &num_basis_tar
  );
...
)
{
  ...
  struct gkyl_proj_powsqrt_on_basis *up = gkyl_proj_powsqrt_on_basis_new(CBAS(b), num_quad, false);
...
)
{
  ...
  return (int)gkyl_grid_sub_array_write(&grid, &range, meta_sz ? &md : NULL, CARR(a), fname);
}
